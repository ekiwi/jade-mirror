...
/**
 * This is an agent that plays at the same time the part of a seller of CDs and a buyer of CDs. More
 * in details the conversation between the "seller" and the "buyer" will go on as follows: -
 * "Seller" informs "buyer" that he owns a CD ("Synchronicity"). - "Buyer" asks for the price of
 * that CD - "Seller" informs "buyer" about the price - "Buyer" requests "seller" to sell him the CD
 * specifying his credit card - "Seller" performs the action (this step is not actually implemented
 * as it would imply interacting with a delivery system like UPS and an electronic payment system)
 * and notifies "buyer"
 */
