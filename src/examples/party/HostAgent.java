 ...
 * compliance with the FIPA specifications. Jade is Copyright (C) 2000 CSELT S.p.A. This file
 * copyright (c) 2001 Hewlett-Packard Corp.
 * 
...
/*****************************************************************************
 * Source code information ----------------------- Original author Ian Dickinson, HP Labs Bristol
 * Author email Ian_Dickinson@hp.com Package Created 1 Oct 2001 Filename $RCSfile$ Revision
 * $Revision$ Release status Experimental. $State$
 *
...
/**
 * <p>
 * Agent representing the host for a party, to which a user-controlled number of guests is invited.
 * The sequence is as follows: the user selects a number guests to attend the party from 0 to 1000,
 * using the slider on the UI. When the party starts, the host creates N guest agents, each of which
 * registers with the DF, and sends the host a message to say that they have arrived. When all the
 * guests have arrived, the party starts. The host selects one guest at random, and tells them a
 * rumour. The host then selects two other guests at random, and introduces them to each other. The
 * party then proceeds as follows: each guest that is introduced to someone asks the host to
 * introduce them to another guest (at random). If a guest has someone introduce themselves, and the
 * guest knows the rumour, they tell the other guest. When a guest hears the rumour for the first
 * time, they notify the host. When all the guests have heard the rumour, the party ends and the
 * guests leave.
 * </p>
 * <p>
...
public class HostAgent extends Agent {
  ...
  /**
   * Invite a number of guests, as determined by the given parameter. Clears old state variables,
   * then creates N guest agents. A list of the agents is maintained, so that the host can tell them
   * all to leave at the end of the party.
   *
  ...
  protected void inviteGuests(int nGuests) {
    ...
    PlatformController container = getContainerController(); // get a container controller for
                                                             // creating new agents
    ...
    try {
      for (int i = 0; i < nGuests; i++) {
        ...
        AgentController guest =
            container.createNewAgent(localName, "examples.party.GuestAgent", null);
        guest.start();
  ...
  /**
   * Start the conversation in the party. Tell a random guest a rumour, and select two random guests
  ...
  /**
   * Introduce guest0 to a random other guest. Also updates the introduction count on the UI, and
   * the avg no of introductions per rumour.
   */
  ...
  /**
   * Increment the number of guests that have heard the rumour, and update the UI. If all guests
   * have heard the rumour, end the party.
   */
  protected void incrementRumourCount() {
    m_rumourCount++;
    SwingUtilities.invokeLater(new Runnable() {
      public void run() {
        ((HostUIFrame) m_frame).prog_rumourCount
            .setValue(Math.round(100 * m_rumourCount / m_guestCount));
      }
    });
    updateRumourAvg();
