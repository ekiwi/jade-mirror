...
/**
 * This agent is able to handle the engagement of people by requesting an engager agent to do that.
 ...
 * </ul>
 * Then it cyclically gets from the user the details of a person to engage and handles the
 * engagement of that person in collaboration with the initially indicated engager agent.
 * 
 * <b>Note:</b> While entering input data, all fields composed of more than one word must be
 * enclosed in "". E.g. the name Giovanni Caire must be entered as "Giovanni Caire".
 * 
...
public class RequesterAgent extends Agent {
  ...
  // AGENT BEHAVIOURS
  /**
   * Main behaviour for the Requester Agent. First the details of a person to engage are requested
   * to the user. Then a check is performed to verify that the indicated person is not already
   * working for the indicated company Finally, according to the above check, the engagement is
  ...
  class HandleEngagementBehaviour extends SequentialBehaviour {
    ...
    public void onStart() {
      ...
      try {
        ...
        // Create an ACL message to query the engager agent if the above fact is true or
        // false
  ...
  /**
   * This behaviour embeds the check that the indicated person is not already working for the
   * indicated company. This is done following a FIPA-Query interaction protocol
   */
    ...
    protected void handleInform(ACLMessage msg) {
      try {
        ...
        if (cs.getTypeName().equals(EmploymentOntology.WORKS_FOR)) {
          ...
          // Get person and company details and create an object representing the
          // engagement action
  ...
  /**
   * This behaviour embeds the request to engage the indicated person in the indicated company. This
   * is done following a FIPA-Request interaction protocol
   */
