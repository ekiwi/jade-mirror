...
public class EngagerAgent extends Agent {
  ...
  // AGENT BEHAVIOURS
  /**
   * This behaviour handles all queries about people working for a company following the FIPA-Query
   * protocol
   */
  class HandleEnganementQueriesBehaviour extends SimpleAchieveREResponder {
    ...
    public ACLMessage prepareResponse(ACLMessage msg) {
      ...
      try {
        ...
        System.err.println(myAgent.getLocalName() + " Fill/extract content unsucceeded. Reason:"
            + fe.getMessage());
      } catch (OntologyException oe) {
        System.err.println(
            myAgent.getLocalName() + " getRoleName() unsucceeded. Reason:" + oe.getMessage());
      }
  ...
  /**
   * This behaviour handles a single engagement action that has been requested following the
  ...
  class HandleEngageBehaviour extends SimpleAchieveREResponder {
    ...
    /**
     * This method implements the <code>FipaRequestResponderBehaviour.Factory</code> interface. It
     * will be called within a <code>FipaRequestResponderBehaviour</code> when an engagement action
     * is requested to instantiate a new <code>HandleEngageBehaviour</code> handling the requested
     * action
     * 
     * @param msg The ACL message by means of which the engagement action has been requested
     */
    /**
  ...
  private Company representedCompany; // The company on behalf of which this agent is able to engage
                                      // people
  ...
  protected void setup() {
    System.out.println(
        "This is the EngagerAgent representing the company " + representedCompany.getName());
