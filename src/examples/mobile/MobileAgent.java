...
/**
 * This is an example of mobile agent. This class contains the resources used by the agent
 * behaviours: the counter, the flag cntEnabled, and the list of visited locations. At the setup it
 * creates a gui and adds behaviours to: get the list of available locations from AMS, serve the
 * incoming messages, and to increment the counter. In particular, notice the usage of the two
 * methods <code>beforeMove()</code> and <code>afterMove()</code> to execute some
 * application-specific tasks just before and just after the agent migration takes effect.
 * 
 * Because this agent has a GUI, it extends the class GuiAgent that, in turn, extends the class
 * Agent. Being the GUI a different thread, the communication between the agent and its GUI is based
 * on event passing.
 * 
...
public class MobileAgent extends GuiAgent {
  ...
  /**
   * This method is executed just before moving the agent to another location. It is automatically
   * called by the JADE framework. It disposes the GUI and prints a bye message on the standard
   * output.
   */
  ...
  /**
   * This method is executed as soon as the agent arrives to the new destination. It creates a new
   * GUI and sets the list of visited locations and the list of available locations (via the
