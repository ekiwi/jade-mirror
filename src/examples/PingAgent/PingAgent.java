...
/**
 * This agent implements a simple Ping Agent that registers itself with the DF and then waits for
 * ACLMessages. If a REQUEST message is received containing the string "ping" within the content
      ...
      if (msg != null) {
        ...
        if (msg.getPerformative() == ACLMessage.REQUEST) {
          ...
          if ((content != null) && (content.indexOf("ping") != -1)) {
            ...
            myLogger.log(Logger.INFO, "Agent " + getLocalName() + " - Unexpected request ["
                + content + "] received from " + msg.getSender().getLocalName());
          ...
          reply.setContent(
              "( (Unexpected-act " + ACLMessage.getPerformative(msg.getPerformative()) + ") )");
        }
        send(reply);
      } else {
        block();
