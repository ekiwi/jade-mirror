    ...
    if (IAmTheCreator) {
      ...
      try {
        ...
        System.out.println(getLocalName() + " CREATED AND STARTED NEW THANKSAGENT:" + t2AgentName
            + " ON CONTAINER " + ac.getContainerName());
      } catch (Exception e2) {
        e2.printStackTrace();
      }
      ...
      send(msg);
      System.out.println(
          getLocalName() + " SENT GREETINGS MESSAGE  TO " + t1AgentName + " AND " + t2AgentName);
    } /*
       * IF YOU COMMENTED OUT THIS ELSE CLAUSE, THEN YOU WOULD GENERATE AN INTERESTING INFINITE LOOP
       * WITH INFINTE AGENTS AND AGENT CONTAINERS BEING CREATED else { IAmTheCreator = true;
      ...
      public void action() {
        ...
        if (msg != null) {
          if (GREETINGS.equalsIgnoreCase(msg.getContent())) {
            ...
            System.out.println(myAgent.getLocalName() + " RECEIVED GREETINGS MESSAGE FROM "
            ...
            // if an ANSWER to a greetings message is arrived
            // then send a THANKS message
            System.out.println(myAgent.getLocalName() + " RECEIVED ANSWER MESSAGE FROM "
                + msg.getSender().getLocalName());
            ...
            System.out.println(myAgent.getLocalName() + " RECEIVED THANKS MESSAGE FROM "
                + msg.getSender().getLocalName());
          } else {
