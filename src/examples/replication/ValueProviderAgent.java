...
public class ValueProviderAgent extends Agent implements AgentReplicationHelper.Listener {
  ...
  protected void setup() {
    try {
      // Makes this agent become the master replica of a newly defined replicated
      // agent
      AgentReplicationHelper helper =
          (AgentReplicationHelper) getHelper(AgentReplicationHelper.SERVICE_NAME);
      AID virtualAid =
          helper.makeVirtual(getLocalName() + "_V", AgentReplicationHelper.HOT_REPLICATION);
      ...
      addBehaviour(new OntologyServer(this, ValueManagementOntology.getInstance(),
          ACLMessage.REQUEST, this));

      // Show the GUI that allows the user to set the value and to create other
      // replicas
  ...
  public void afterClone() {
    ...
    // Just after cloning restore transient field such as registered ontologies and
    // language codecs
  ...
  // This is invoked whenever the user selects a new value by moving the slider in
  // the GUI
  ...
  void createReplica(String replicaName, String where) {
    ...
    try {
      ...
      System.out
          .println("Agent " + getLocalName() + " - Error creating replica on container " + where);
      e.printStackTrace();
    }
  }
  ...
  public void serveGetValueRequest(GetValue gv, ACLMessage request) {
    ...
    System.out.println("Agent " + getLocalName() + " - Serving GetValue request. Current value is "
        + currentValue);
  ...
  public void replicaAdded(AID replicaAid, Location where) {
    System.out.println("Agent " + getLocalName() + " - New replica " + replicaAid.getLocalName()
        + " successfully added in " + where.getName());
  }

  @Override
  public void replicaRemoved(AID replicaAid, Location where) {
    System.out.println(
        "Agent " + getLocalName() + " - Replica " + replicaAid.getLocalName() + " removed");
  }

  @Override
  public void replicaCreationFailed(AID replicaAid, Location where) {
    System.out.println("Agent " + getLocalName() + " - Creation of new replica "
        + replicaAid.getLocalName() + " in " + where.getName() + " failed");
  }
