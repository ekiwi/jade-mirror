...
public class ValueReaderAgent extends Agent {
  ...
  protected void setup() {
    ...
    try {
      DFAgentDescription[] res =
          DFService.searchUntilFound(this, this.getDefaultDF(), template, null, 10000);
      if (res.length > 0) {
        provider = res[0].getName();
        System.out.println("Agent " + getLocalName() + " - ValueProvider agent "
            + provider.getLocalName() + " found");

        // Add the behaviour that periodically read the value from the value provider
        // agent
      ...
      System.out.println(
          "Agent " + getLocalName() + " - Error searching the DF for a ValueProvider agent");
      fe.printStackTrace();
      doDelete();
    }
  }
  ...
  /**
   * Inner class PeriodicValueReader This is a behaviour that periodically retrieve the value from
   * the value provider agent and prints it in the standard output
   */
  private class PeriodicValueReader extends TickerBehaviour {
    ...
    protected void onTick() {
      ...
      try {
        ...
        System.out.println(
            "Agent " + myAgent.getLocalName() + " - Retrieved value = " + inform.getContent());
      } catch (Exception e) {
        System.out.println("Agent " + myAgent.getLocalName()
            + " - Error retrieving value from value provider agent");
        e.printStackTrace();
      }
    }
  }
