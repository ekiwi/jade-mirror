...
/**
 * This agent makes the following task: 1. searches in the DF for an ObjectReaderAgent; 2. sends an
 * ACLMessage with a content encoded in Base64 to the ObjectReaderAgent. 3. sends the same message
 * by using the BitEfficient ACLCodec first, and the XML ACLCodec then 4. sends an ACLMessage with a
 * content encoded as a String 5. sends the same message by using again BitfficientACLCodec and
 * XMLACLCodec
 * 
 * Notice that JADE actually encodes ACLMessages only and only if the receiver of the message
 * belongs to a remote platform. Notice that the ObjectReaderAgent does not need any special
 * provision to read bit-efficient or XML-coded messages because all the burden is put into the
 * platform that most provide the proper decoding capabilities. See also the bitefficient tutorial
 * and the XMLACL tutorial that describe how to launch the platform with this codec capabilities.
 * 
...
public class ObjectWriterAgent extends Agent {
  ...
  protected void setup() {
    ...
    try {
      while (true) {
        System.out
            .println(getLocalName() + " waiting for an ObjectReaderAgent registering with the DF");
      ...
      fe.printStackTrace();
      System.err.println(
          getLocalName() + " search with DF is not succeeded because of " + fe.getMessage());
      doDelete();
    }
