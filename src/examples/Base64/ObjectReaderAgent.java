...
/**
 * This agent makes the following task: 1. registers itself with the df as a reader; 2. waits a
 * message from its companion, the ObjectWriterAgent; 3. reads the content of the message; if the
 * language was set to "JavaSerialization", then the agent knows a-priori that it is encoded in
 * Base64 and contains a Java object. Notice that this is a private a-priori agreement between the
 * Writer and the Reader and it does not comply to any standard.
 * <p>
 * Becase this agent implements a single sequential task, it does not use any Behaviour.
 *
...
public class ObjectReaderAgent extends Agent {
  ...
  protected void setup() {
    ...
    try {
      ...
      System.err
          .println(getLocalName() + " registration with DF unsucceeded. Reason: " + e.getMessage());
      doDelete();
    }
    ...
    while (true) {
      try {
        ...
        if ("JavaSerialization".equals(msg.getLanguage())) {
          ...
          System.out.println(
              getLocalName() + " read Java Object " + p.getClass().getName() + p.toString());
        } else
  ...
  public void takeDown() {
    try {
      ...
      System.err.println(
          getLocalName() + " deregistration with DF unsucceeded. Reason: " + e.getMessage());
    }
  }
