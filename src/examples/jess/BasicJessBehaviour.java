...
// with 'System.err'
// * Updated getAIDListFromCache to take a ValueVector as parameter. It also requires Context as a
////////////////////////////////////////////////////////////// parameter to
...
//
// * Check for two cases, i.e. if send has a RU.VARIABLE as its first parameter (send ?m) and
////////////////////////////////////////////////////////////// secondly if
// send has an RU.FUNCALL as its first parameter (send (assert (ACLMessage ...))).
// 1) The first case is straight forward. Get the first parameter of the ValueVector, extract the
////////////////////////////////////////////////////////////// Fact Id and find the fact
// by looking up the Fact Id. Pass the resulting ValueVector to JessFact2ACL(Context context,
////////////////////////////////////////////////////////////// jess.ValueVector vv).
// 2) The second case is a little more tricky. Get the first parameter of the ValueVector, extract
////////////////////////////////////////////////////////////// the function call
// (assert). Get the first parameter again and extract the ACLMessage. Jess variables must be
////////////////////////////////////////////////////////////// resolved with calls to
 ...
 */
/**
 * This is the basic class that implements a behaviour of JADE that allows to embed a Jess engine
 * inside the agent code.
 * <p>
 * <a href="http://herzberg.ca.sandia.gov/jess">Jess</a> supports the development of rule-based
 * expert systems.
 * <p>
 * This JADE implementation has been tested with version 6.0 of JESS.
 * <p>
 * The programmer can override this class. In particular, its methods <code>ACL2JessString</code>
 * and <code>JessFact2ACL</code> to convert between ACLMessage JADE structure and Jess facts. Also
 * the method <code>ACLJessTemplate</code> might need to be overide in order to change the
 * deftemplate of the ACLMessage in Jess.
 * <p>
 * When this behaviour is added to the list of agent behaviours, it creates a Jess engine and
 ...
 * <li>calling the <code>ACL2JessString</code> method that returns the fact to be asserted in Jess,
 ...
 * <li>use the userfunction <code>send</code> to send ACLMessages. The parameter of
 * <code>send</code> must be a fact-id of type ACLMessage or an ACLMessage itself; There are two
 * styles of usage:
 * <p>
 ...
 * (sender ?s))))</code>
 * <li>remember to load all the Jess Packages you need because, by default, Jess just loads the
 * built-in functions
 * </ul>
 * <p>
 * Look at the sample file JadeAgent.clp that is shipped with this example.
 * <p>
 * WARNING: FIPA2000 standard has specified an AgentIdentifier to have a template composed of
 * several slots, where the only mandatory one is the agent name, i.e. the globally unique
 * identifier of the agent (GUID). In order to reduce the porting of the JESS user code, this basic
 * behaviour automatically takes care of replacing the GUIDs with full-fledged AgentIdentifiers.
 */
public class BasicJessBehaviour extends CyclicBehaviour {
  ...
  public BasicJessBehaviour(Agent agent, String jessFile) {
    ...
    // The jess.MiscFunctions is no more used since JESS 6.0 (see e-mail of Csaba
    // Tenkes
    // try {
  ...
  /**
   * Creates a <code>BasicJessBehaviour</code> instance that limits the reasoning time of Jess
   * before looking again for arrival of messages.
   *
   * @param agent the agent that adds the behaviour
   * @param jessFile the name of the Jess file to be executed
   * @param maxJessPasses the maximum number of passes that every run of Jess can execute before
   *        giving again the control to this behaviour; put <code>0</code> if you do not ever want
   *        to stop Jess.
   */
  ...
  /**
   * replace a char in a String with a String It is used to convert all the quotation marks in
   * backslash quote before asserting the content of a message in Jess.
   * 
  ...
  /**
   * makeasserts a fact representing an ACLMessage in Jess. It is called after the arrival of a
   * message.
   */
  ...
  /**
   * Remove the first and the last character of the string (if it is a quotation mark) and convert
   * all backslash quote in quote It is used to convert a Jess content into an ACL message content.
   */
  ...
  /**
   * Insert the first and the last character of the string as a quotation mark Replace all the quote
   * characters into backslash quote. It is used to convert an ACL message content into a Jess
   * content.
   */
  ...
  /**
   * This method searches in the local cache for the full AID of the passed agentName. If not found
   * it creates a new AID where only the guid is set.
   **/
  ...
  /**
   * put a new AID in the local cache. If one exists already with the same agentName, it is
   * overwritten
   */
  ...
  /**
   * @return the ACLMessage representing the passed Jess Fact. This message will be then sent by the
   *         caller.
   */
  public ACLMessage JessFact2ACL(Context context, jess.ValueVector vv) throws jess.JessException {
  ...
  /**
   * @return the String representing the facts (even more than one fact is allowed, but this method
   *         just returns one fact) to be asserted in Jess as a consequence of the receipt of the
   *         passed ACL Message. The messate content is quoted before asserting the Jess Fact. It is
   *         unquoted by the JessFact2ACL function.
   */
  ...
  /**
   * This class implements the Jess userfunction to send ACLMessages directly from Jess. It can be
   * used by Jess by using the name <code>send</code>.
   */
  public class JessSend implements Userfunction {
    ...
    public Value call(ValueVector vv, Context context) throws JessException {
      ...
      if (vv.get(1).type() == RU.VARIABLE) {
        ...
        vv = context.getEngine().findFactByID(vv.get(1).factValue(context).getFactId()); // JESS6.0
      }
      //////////////////////////////////
