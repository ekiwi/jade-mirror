...
/**
 * This example shows how to implement nested protocols. In this case in particular we want to
 * implement a broker agent that forwards incoming requests to perform actions to another agent. To
 * implement that we use an <code>AchieveREResponder</code> ("Achieve Rational effect") and we
 * register an <code>AchieveREInitiator</code> in the state where the requested action has to be
 * performed.
 * 
    ...
    if (args != null && args.length > 0) {
      ...
      arer.registerPrepareResultNotification(new AchieveREInitiator(this, null) {
        ...
        protected Vector prepareRequests(ACLMessage request) {
          ...
          System.out.println(
              "Agent " + getLocalName() + ": Forward the request to " + responder.getName());
