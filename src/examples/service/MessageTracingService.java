...
/**
 * This example shows how to implement a JADE Kernel Service that provides a Filter to process
 * OUTGOING commands. In particular in this case we intercept SEND_MESSAGE commands of the Messaging
 * Service to log all messages exchanged by agents.<br>
...
public class MessageTracingService extends BaseService {
  ...
  /**
   * Inner class TracingFilter. This is the Filter intercepting SEND_MESSAGE outgoing commands end
   * tracing included messages
   */
  private class TracingFilter extends Filter {
    ...
    protected boolean accept(VerticalCommand cmd) {
      if (cmd.getName().equals(MessagingSlice.SEND_MESSAGE)) {
        ...
        System.out.println(
            sender.getLocalName() + " --> " + ACLMessage.getPerformative(msg.getPerformative())
