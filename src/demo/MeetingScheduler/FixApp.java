...
public class FixApp extends JFrame {
  ...
  public FixApp() {
    ...
    appointmentScrollPane =
        new JScrollPane(textArea1, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
    ...
    invitedScrollPane =
        new JScrollPane(listInvitedPersons, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
            ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
    ...
    knownScrollPane =
        new JScrollPane(listKnownPersons, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
            ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
   ...
   */
  /**
   * public void setVisible(boolean b) { if(b) setLocation(50, 50); super.setVisible(b); }
   **/
  ...
  /**
   * public void addNotify() { // Record the size of the window prior to calling parents addNotify.
   * Dimension d = getSize();
   * 
   * super.addNotify();
   * 
   ...
   * 
   * // Adjust components according to the insets setSize(insets().left + insets().right + d.width,
   * insets().top + insets().bottom + d.height); Component components[] = getComponents(); for (int
   * i = 0; i < components.length; i++) { Point p = components[i].getLocation();
   * p.translate(insets().left, insets().top); components[i].setLocation(p); } fComponentsAdjusted =
   * true; }
   * 
