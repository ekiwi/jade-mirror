...
public class myFipaContractNetResponderBehaviour extends ContractNetResponder {
  ...
  protected void handleOutOfSequence(ACLMessage cfp, ACLMessage propose,
    ...
    System.err.println(
        myAgent.getLocalName() + ":myFipaContractNetResponder:received out of sequence message"
            + outOfSequenceMsg.toString());
  }

  protected ACLMessage prepareResultNotification(ACLMessage cfp, ACLMessage propose, ACLMessage msg)
  ...
  protected void handleRejectProposal(ACLMessage cfp, ACLMessage propose, ACLMessage msg) {
    System.err.println(myAgent.getLocalName()
        + ":FipaContractNetResponder: the proposal has been rejected with this message"
        + msg.toString());
  }
