...
public class MeetingSchedulerAgent extends GuiAgent {
  ...
  private Hashtable appointments = new Hashtable(); // list of appointments: (String)date ->
                                                    // Appointment
  ...
  /**
   * This method is executed by the Agent thread when the GUI posts a new Event
   **/
    ...
    try {
      ...
      if (l != null)
        for (int i = 0; i < l.length; i++) {
          ...
          Person prs = new Person(((ServiceDescription) dfd.getAllServices().next()).getOwnership(),
              dfd.getName(), dfname);
          addKnownPerson(prs);
        }
  ...
  /**
   * This method returns an Enumeration of String. Each String is the name of a DF with which the
  ...
  /**
   * This function fills the Appointment with all the dates when this user is available
   * 
  ...
  void removeMyAppointment(Appointment app) {
    ...
    if (a == null) {
      mf.showErrorMessage("Someone has requested to cancel an appointment for "
          + app.getFixedDate().toString() + " but there was no appointment actually");
      if (logger.isLoggable(Logger.WARNING))
        logger.log(Logger.WARNING, "Someone has requested to cancel an appointment for "
            + app.getFixedDate().toString() + " but there was no appointment actually");
    } else {
