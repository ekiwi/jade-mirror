...
/**
 * A profile implementation enhanced to support boot's argument parsing. This class serves as the
 * bridge between boot properties and profile properties. It defines a collection of property keys
 * which correspond to boot argument names. These are used to access the boot properties from the
 * argument properties. The class Profile defines a similar collection of keys which are used to
...
public class BootProfileImpl extends ProfileImpl {
  ...
  /**
   * Return the properties collection which resulted from the arguments. This collection is used to
   * create/modify the underlying profile's properties.
   * 
  ...
  /**
   * Copy a collection of argument properties into the existing argument properties and then into
   * the profile properties. When moving between the argument properties and profile properties
   * different keys are required.
   * 
   * @param source A collection of argument properties. The keys to this collection are from the
   *        XXX_KEY strings defined in this class.
   */
  public void setArgProperties(ExtendedProperties source) {
    ...
    /*
     * PER IL DF value = argProp.getProperty(df.KBDF_MAX_RESULTS); if (value != null) {
     * profileProp.setProperty(df.KBDF_MAX_RESULTS, value); } value =
     * argProp.getProperty(df.DBDF_DRIVER); if (value != null) {
     * profileProp.setProperty(df.DBDF_DRIVER, value); } value = argProp.getProperty(df.DBDF_URL);
     * if (value != null) { profileProp.setProperty(df.DBDF_URL, value); }
     */
    ...
    if (value != null) {
      ...
      for (Enumeration e = helper.getCommandLineAgentSpecifiers(agentVector); e
          .hasMoreElements();) {
    ...
    // Note All the jade.util.leap.ArrayList structures will only print their type
    // unless a
  ...
  protected boolean fetchAndVerifyBoolean(String aKey) throws PropertiesException {
    ...
    if (value != null) {
      ...
      throw new PropertiesException(
          "The value of the attribute " + aKey + " must be either true or false.");
    }
    return false;
  }
  ...
  /**
   * Parse a String reading for a set of <code>parameter(arg)</code> each delimited by a
   * <code>;</code> and no space in between.
   * <p>
   * For instance <code>jade.mtp.iiop(50);http.mtp.http(8080)</code> is a valid string, while
   * <code>jade.mtp.iiop(50 80);http.mtp.http(8080)</code> is not valid For each object specifier, a
   * new java object <code>Specifier</code> is added to the passed <code>out</code> List parameter.
   */
