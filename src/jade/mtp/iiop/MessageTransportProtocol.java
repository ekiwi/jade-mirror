...
/**
 * Implementation of <code><b>fipa.mts.mtp.iiop.std</b></code> specification for delivering ACL
 * messages over the OMG IIOP transport protocol.
 * 
...
public class MessageTransportProtocol implements MTP {
  ...
  public TransportAddress activate(InChannel.Dispatcher disp, Profile p) throws MTPException {
    ...
    /*
     * //Open log file String fileName = "iiop"+iiop.getHost()+iiop.getPort()+".log"; try{ logFile =
     * new PrintWriter(new FileWriter(fileName,true)); }catch(java.io.IOException e){e
     * .printStackTrace();}
     */
    return iiop;
  ...
  public void deliver(String addr, Envelope env, byte[] payload) throws MTPException {
    try {
      ...
      String IDLpayloadEncoding =
          (env.getPayloadEncoding() != null) ? env.getPayloadEncoding() : "";
       ...
       * ReceivedObject[] received = env.getStamps(); FIPA.ReceivedObject[] IDLreceived = new
       * FIPA.ReceivedObject[received.length]; for(int i = 0; i < received.length; i++)
      ...
      FIPA.Envelope IDLenv = new FIPA.Envelope(IDLto, IDLfrom, IDLcomments, IDLaclRepresentation,
          IDLpayloadLength, IDLpayloadEncoding, IDLdate, IDLencrypted, IDLintendedReceiver,
          IDLreceived, IDLtransportBehaviour, IDLuserDefinedProperties);
  ...
  private FIPA.AgentID marshalAID(AID id) {
    ...
    FIPA.AgentID result =
        new FIPA.AgentID(name, addresses, new AgentID[numOfResolvers], userDefinedProperties);
    for (int i = 0; i < numOfResolvers; i++) {
      result.resolvers[i] = marshalAID(resolvers[i]); // Recursively marshal all resolvers, which
                                                      // are, in turn,
                                                      // AIDs.
    }
  ...
  private FIPA.DateTime marshalDateTime(Date d) {
    ...
    FIPA.DateTime result =
        new FIPA.DateTime(year, month, day, hour, minutes, seconds, milliseconds, typeDesignator);
    return result;
  }
  ...
  /*
   * public static synchronized void log(String str) { logFile.println(str); logFile.flush(); }
   */
...
</code> Notice that, in the third case, BIG_ENDIAN is assumed by default. In the first and second
 * case, instead, the endianess information is contained within the IOR definition.
 **/
class IIOPAddress implements TransportAddress {
  ...
  private boolean isUnreservedURIChar(byte b) {
    ...
    if ((b == ASCII_MINUS) || (b == ASCII_UNDERSCORE) || (b == ASCII_DOT) || (b == ASCII_BANG)
        || (b == ASCII_TILDE) || (b == ASCII_STAR) || (b == ASCII_QUOTE)
        || (b == ASCII_OPEN_BRACKET) || (b == ASCII_CLOSED_BRACKET))
      return true;
  ...
  private static class LittleEndianCodec extends CDRCodec {
    ...
    public int readLong() {
      setReadAlignment(4);
      int result = readBuffer[readIndex++] + (readBuffer[readIndex++] << 8)
          + (readBuffer[readIndex++] << 16) + (readBuffer[readIndex++] << 24);
      return result;
    }
