...
/**
 * Singleton class for obtaining sockets. HTTP MTP calls methods use this class every time that a
 * socket is needed.
 * 
...
public class HTTPSocketFactory {
  ...
  public void configure(Profile profile, HTTPAddress hta) throws Exception {
    ...
    if (hta.getProto().equals("https")) {
      _usingHttps = true;
      try {
        String trustManagerClass = profile.getParameter(PREFIX + "trustManagerClass",
            "jade.mtp.http.https.NoAuthentication");

        String keyManagerClass = profile.getParameter(PREFIX + "keyManagerClass",
        ...
        if (profile.getParameter(PREFIX + "needClientAuth", "no").equals("yes"))
    ...
    /*
     * #DOTNET_INCLUDE_BEGIN _socketFactory = SocketFactory.getDefault(); _serverSocketFactory =
     * ServerSocketFactory.getDefault(); #DOTNET_INCLUDE_END
     */
  ...
  public Socket createSocket(String host, int port) throws IOException {
    ...
    /*
     * #DOTNET_INCLUDE_BEGIN s = _socketFactory.createSocket(host, port); #DOTNET_INCLUDE_END
     */

    return s;
  }

  public Socket createSocket(String host, int port, InetAddress dest, int outport)
      throws IOException {
    Socket s;
    ...
    /*
     * #DOTNET_INCLUDE_BEGIN s = _socketFactory.createSocket(host, port, dest, outport);
     * #DOTNET_INCLUDE_END
     */
  ...
  public ServerSocket createServerSocket(String interfaceAddress, int port) throws IOException {
    InetAddress ifAddr =
        interfaceAddress == null || interfaceAddress.equals(Profile.LOCALHOST_CONSTANT) ? null
