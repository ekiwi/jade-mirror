...
public class MessageTransportProtocol implements MTP {
   ...
   * <LI><B>numKeepAlive</B>: Maximum number of keep-alive connections. Default value is 10. Set to
   * 0 to disable keep-alive coonections (possible performance impact).</LI>
   * <LI><B>proxyHost</B>: Proxy host name or IP-address. No default value.</LI>
   * <LI><B>proxyPort</B>: Default value is 1080.</LI>
   * <LI><B>outPort</B>: Fix port to be used by HTTP client (for firewall configuration). Default
   * value is freely chosen by Java</LI>
   * <LI><B>parser</B>: XML SAX2 parser implementation to use. Default value is JDK 1.4 default
   * parser.</LI>
   * <LI><B>policy</B>: "conservative" (default value) or "aggressive". (see documentation for
   * details).</LI>
   * <LI><B>timeout</B>: Timeout for keep-alive connections. Default value is 1 min. 0 means
   * infinite.</LI>
   * </UL>
  ...
  private TransportAddress activateServer(InChannel.Dispatcher disp, TransportAddress ta, Profile p)
    ...
    try {
      if (ta != null) {
        ...
        try {
           ...
           * HTTPAddress(InetAddress.getLocalHost().getHostName(),port, false); #DOTNET_INCLUDE_END
           */
    ...
    try {
      ...
      HTTPServer srv =
          new HTTPServer(interfaceAddress, port, disp, numKA, saxClass, timeout, changePortIfBusy);
      ...
      if (actualPort != port) {
        ...
        // #DOTNET_EXCLUDE_BEGIN
        hta = new HTTPAddress(InetAddress.getLocalHost().getCanonicalHostName(), actualPort,
            useHttps);
        // #DOTNET_EXCLUDE_END
        /*
         * #DOTNET_INCLUDE_BEGIN hta = new
  ...
  public void deliver(String addr, Envelope env, byte[] payload) throws MTPException {
    ...
    try {
      ...
      if (useProxy) {
        url = new HTTPAddress(proxyHost, proxyPort, false); // false=>do_not_use HTTPS with the
                                                            // proxy
      } else {
        url = host;
      }
