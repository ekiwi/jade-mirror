...
public class MicroBoot {
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      if (props.getProperty(MicroRuntime.JVM_KEY) == null) {
        ...
        // #PJAVA_EXCLUDE_END
        /*
         * #PJAVA_INCLUDE_BEGIN props.setProperty(MicroRuntime.JVM_KEY, MicroRuntime.PJAVA);
         * #PJAVA_INCLUDE_END
         */
      }
      ...
      logger.log(Logger.SEVERE,
          "Error reading command line configuration properties. " + iae.getMessage());
      iae.printStackTrace();
      printUsage();
      System.exit(-1);
    } catch (IOException ioe) {
      logger.log(Logger.SEVERE,
          "Error reading configuration properties from file " + propsFile + ".", ioe);
      printUsage();
      System.exit(-1);
    }
  }
  ...
  private static void printUsage() {
    ...
    logger.log(Logger.ALL,
        "    -conf <file-name>. Read configuration properties from the specified file name");
    logger.log(Logger.ALL,
        "    -host <host-name>. The name/address of the host where the BackEnd has to be created");
    logger.log(Logger.ALL,
        "    -port <port-number>. The port of the J2SE container active on \"host\"");
