...
/**
 * Utility class that allow using an <code>ACLMessage</code> object as an ontological agent action.
 * 
...
public class OntoACLMessage extends ACLMessage implements AgentAction {
  ...
  /**
   * Create an ontological ACL message that wraps an existing <code>ACLMessage</code>.
   * 
   * @param msg the <code>ACLMessage</code>to be wrapped. If <code>msg</code> is already an
   *        ontological ACL message no new object is created and <code>msg</code> is returned with
   *        the sender and receivers properly wrapped if necessary.
   */
