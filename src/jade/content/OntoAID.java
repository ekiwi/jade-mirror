...
/**
 * Utility class that allow using an <code>AID</code> object as an ontological concept.
 * 
...
public class OntoAID extends AID implements Concept {
  ...
  /**
   * Create an ontological Agent identifier that wraps an existing <code>AID</code>.
   * 
   * @param id the <code>AID</code>to be wrapped. If <code>id</code> is already an ontological agent
   *        identifier no new object is created and <code>id</code> is returned with the resolvers
