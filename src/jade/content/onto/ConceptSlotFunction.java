 ...
 * For instance, if an ontology defines a concept <code>Person</code> with a slot <code>name</code>
 * and a slot <code>age</code>, it is possible to create expressions such as<br>
 * (= (age (Person :name John)) 41) <br>
 * (> (age (Person :name John)) (age (Person :name Bill)))<br>
 * (iota ?x (= (age (Person :name John)) ?x)) In order to exploit this feature it is necessary to
 * instruct an ontology to use concept slots as functions by means of the
...
public class ConceptSlotFunction extends AbsConceptSlotFunction {
  ...
  public AbsObject getAbsObject(String name) {
    if (ConceptSlotFunctionSchema.CONCEPT_SLOT_FUNCTION_CONCEPT.equals(name)) {
      try {
        ...
        throw new RuntimeException("Error externalizing Concept " + concept.getClass().getName(),
            oe);
      }
    } else {
      return null;
  ...
  public Object apply(Concept c) throws OntologyException {
    if (concept.getClass().isAssignableFrom(c.getClass())) {
      ...
      throw new OntologyException("Concept " + c.getClass()
          + " is not compatible with internal concept " + concept.getClass());
    }
  }
