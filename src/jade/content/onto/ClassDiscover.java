  ...
  static List<Class> getClassesForPackage(String pkgname) throws ClassNotFoundException {
    ...
    try {
      ...
      while (resources.hasMoreElements()) {
        ...
        if (res.getProtocol().equalsIgnoreCase("jar")) {
          ...
          for (JarEntry e : Collections.list(jar.entries())) {
            ...
            if (e.getName().startsWith(pkgname.replace('.', '/')) && e.getName().endsWith(".class")
                && !e.getName().contains("$")) {
              String className =
      ...
      throw new ClassNotFoundException(
          pkgname + " does not appear to be " + "a valid package (Null pointer exception)");
    } catch (UnsupportedEncodingException encex) {
      throw new ClassNotFoundException(
          pkgname + " does not appear to be " + "a valid package (Unsupported encoding)");
    } catch (IOException ioex) {
      throw new ClassNotFoundException(
          "IOException was thrown when trying " + "to get all resources for " + pkgname);
    }
    ...
    for (File directory : directories) {
      if (directory.exists()) {
        ...
        throw new ClassNotFoundException(
            pkgname + " (" + directory.getPath() + ") does not appear to be a valid package");
      }
    }
    return classes;
  }

  static List<Class> getClassessOfInterface(String thePackage, Class<Concept> theInterface)
