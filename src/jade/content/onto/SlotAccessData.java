...
class SlotAccessData implements Serializable {
  ...
  SlotAccessData(Class type, Method getter, Method setter, boolean mandatory, Class aggregateClass,
      int cardMin, int cardMax, Object defaultValue, String regex, String[] permittedValues,
      String documentation, boolean manageAsSerializable) {
    this.type = type;
    this.getter = getter;
    this.setter = setter;
  ...
  static boolean isAggregate(Class clazz) {
    return (clazz.isArray() && clazz != byte[].class)
        ...
        || jade.util.leap.Collection.class.isAssignableFrom(clazz);
  }
