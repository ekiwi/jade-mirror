 ...
 * Framework is a framework to develop multi-agent systems in compliance with the FIPA
 * specifications. Copyright (C) 2000 CSELT S.p.A.
 *
...
/**
 * An application-specific ontology describes the elements that agents can use within content of
 * messages. It defines a vocabulary and relationships between the elements in such a vocabulary.
 * The relationships can be:
 * <ul>
 * <li>structural, e.g., the predicate <code>fatherOf</code> accepts two parameters, a father and a
 * set of children;
 * <li>semantic, e.g., a concept of class <code>Man</code> is also of class <code>Person</code>.
 * </ul>
 ...
 * </ul>
 * Element schemas are objects describing the structure of concepts, actions, and predicates that
 * are allowed in messages. For example, <code>People</code> ontology contains an element schema
 * called <code>Person</code>. This schema states that a <code>Person</code> is characterized by a
 * <code>name</code> and by an <code>address</code>: <code>
 ...
 * </code> where <code>PERSON<code>, <code>NAME</code> and <code>ADDRESS</code> are string
 * constants. When you register your schema with the ontology, such constants become part of the
 * vocabulary of the ontology.<br>
 * Schemas that describe concepts support inheritance. You can define the concept <code>Man</code>
 * as a refinement of the concept <code>Person</code>: <code>
 * ConceptSchema manSchema = new ConceptSchema(MAN);
 * manSchema.addSuperSchema(personSchema);
 * </code> Each element schema can be associated with a Java class to map elements of the ontology
 * that comply with a schema with Java objects of that class. The following is a class that might be
 * associated with the <code>Person</code> schema: <code>
 ...
 *       }
 * }
 * </code> When sending/receiving messages you can represent your content in terms of objects
 * belonging to classes that the ontology associates with schemas.<br>
 * As the previous example suggests, you cannot use objects of class <code>Person</code> when asking
 * for the value of some attribute, e.g., when asking for the value of <code>address</code>.
 * Basically, the problem is that you cannot 'assign' a variable to an attribute of an object, i.e.
 * you cannot write something like: <code>person.setName(new Variable("X"))</code>.<br>
 * In order to solve this problem, you can describe your content in terms of <i>abstract
 * descriptors</i>. An abstract descriptor is an object that reifies an element of the ontology. The
 * following is the creation of an abstract descriptor for a concept of type <code>Man</code>:
 * <code>
 * AbsConcept absMan = new AbsConcept(MAN);
 * absMan.setSlot(NAME,    "John");
 * absMan.setSlot(ADDRESS, absAddress);
 * </code> where <code>absAddress</code> is the abstract descriptor for John's address: <code>
 ...
 * <ul>
 * <li>register schemas with associated (i) a mandatory term of the vocabulary e.g.
 * <code>NAME</code> and (ii) an optional Java class, e.g. <code>Person</code>;
 * <li>retrieve the registered information through various keys.
 * </ul>
 * The framework already provides the <code>BasicOntology</code> ontology that provides all basic
 * elements, i.e. primitive data types, aggregate types, etc. Application-specific ontologies should
 * be implemented extending it.
 * 
...
public class Ontology implements Serializable {
  ...
  /**
   * Construct an Ontology object with a given <code>name</code> that extends a given ontology. The
   * <code>ReflectiveIntrospector</code> is used by default to convert between Java objects and
  ...
  public Ontology(String name, Ontology base) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Class " + DEFAULT_INTROSPECTOR_CLASS + "for default Introspector not found");
    }
  }

  /**
   * Construct an Ontology object with a given <code>name</code> that uses a given Introspector to
  ...
  /**
   * Construct an Ontology object with a given <code>name</code> that extends a given ontology and
   * that uses a given Introspector to convert between Java objects and abstract descriptors.
   * 
   * @param name The identifier of the ontology.
   * @param base The base ontology.
   * @param introspector The introspector.
  ...
  /**
   * Construct an Ontology object with a given <code>name</code> that extends a given set of
   * ontologies and that uses a given Introspector to convert between Java objects and abstract
   * descriptors.
   * 
   * @param name The identifier of the ontology.
   * @param base The base ontology.
  ...
  /**
   * Adds a schema to the ontology and associates it to the class <code>javaClass</code>
   * 
    ...
    if (javaClass != null) {
      ...
      if (!absObjectClass.isAssignableFrom(javaClass)) {
        ...
        if (!javaClass.isInstance(schema.newInstance())) {
          throw new OntologyException("Java class " + javaClass.getName()
              + " can't represent instances of schema " + schema);
        }
      }
    }
  }
  ...
  /**
   * Retrieves the schema of element <code>name</code> in this ontology. The search is extended to
   * the base ontologies if the schema is not found.
   * 
        ...
        if (base[i] == null) {
          if (logger.isLoggable(Logger.FINE))
            logger.log(Logger.FINE,
                "Base ontology # " + i + " for ontology " + getName() + " is null");
        } else {
  ...
  /**
   * Retrieves the schema associated to a given class in this ontology. The search is extended to
   * the base ontologies if the schema is not found.
   * 
   * @param clazz the class whose associated schema must be retrieved.
   * @return the schema associated to the given class or <code>null</code> if the schema is not
   *         found.
   * @throws OntologyException
   */
  public ObjectSchema getSchema(Class clazz) throws OntologyException {
    ...
    if (ret == null) {
      if (logger.isLoggable(Logger.FINE))
        logger.log(Logger.FINE,
            "Ontology " + getName() + ". Schema for class " + clazz + " not found");
      for (int i = 0; i < base.length; ++i) {
        if (base[i] == null) {
          if (logger.isLoggable(Logger.FINE))
            logger.log(Logger.FINE,
  ...
  /**
   * Retrieves the concrete class associated with element <code>name</code> in this ontology. The
   * search is extended to the base ontologies
   * 
   * @param name the name of the schema.
   * @return the Java class or null if no schema called <code>name</code> is found or if no class is
   *         associated to that schema.
  ...
  /**
   * Retrieves the ontology actually containing the definition of a given schema. This can be the
   * ontology itself or one of its super-ontologies.
   * 
   * @param lcName The lower-case version of the name of the schema whose defining ontology must be
   *        retrieved
   * @return The ontology actually containing the definition of schema <code>lcName</code> or null
   *         if such schema is not defined neither in this ontology nor in one of its
   *         super-ontologies
   */
   ...
   * 
   * @param abs the abstract descriptor.
   * @param lcType the type of the abstract descriptor to be translated aconverted into lower case.
   *        This is passed as parameters to avoid making the conversion to lower case for each base
   *        ontology.
   * @param globalOnto The ontology this ontology is part of (i.e. the ontology that extends this
   *        ontology).
   * @return the object
   * @throws UnknownSchemaException If no schema for the abs descriptor to be translated is defined
   *         in this ontology.
   * @throws UngroundedException if the abstract descriptor contains a variable
   * @throws OntologyException if some mismatch with the schema is found * ontology. In this case
   *         UnknownSchema
   */
    ...
    if (schema != null) {
      ...
      try {
        ...
        throw new OntologyException(
            "Class " + javaClass + " does not have an accessible constructor", iae);
      }
    }
  ...
  private Object internalizeSpecialType(AbsObject abs, ObjectSchema schema, Class javaClass,
      Ontology globalOnto) throws OntologyException {
    if (introspector == null) {
      ...
      for (int i = 0; i < names.length; ++i) {
        ...
        if (absSlotValue != null) {
          ...
          if (absSlotValue.getAbsType() == AbsObject.ABS_AGGREGATE) {
            // Manage as aggregate
            slotValue = introspector.internalizeAggregate(slotName, (AbsAggregate) absSlotValue,
                schema, globalOnto);
          } else {
   ...
   *        ontology).
   * @return the abstract descriptor.
   * @throws UnknownSchemaException If no schema for the object to be translated is defined in this
   *         ontology.
   * @throws OntologyException if some mismatch with the schema is found
   */
  protected AbsObject fromObject(Object obj, Ontology globalOnto)
    ...
    if (logger.isLoggable(Logger.FINE))
      logger.log(Logger.FINE,
          "Ontology " + getName() + ". Translating object of class " + javaClass);

    // Retrieve the schema
    ObjectSchema schema = (ObjectSchema) schemas.get(javaClass);
    if (schema != null) {
      if (logger.isLoggable(Logger.FINE))
        logger.log(Logger.FINE, "Ontology " + getName() + ". Schema for class " + javaClass
            + " found locally: " + schema);
  ...
  private AbsObject externalizeSpecialType(Object obj, ObjectSchema schema, Class javaClass,
      Ontology globalOnto) throws OntologyException {
    if (introspector == null) {
      throw new NotASpecialType();
    }
    ...
    if (introspector != null) {
      ...
      for (int i = 0; i < names.length; ++i) {
        ...
        if (slotValue != null) {
          ...
          try {
            absSlotValue =
                introspector.externalizeAggregate(slotName, slotValue, schema, globalOnto);
  ...
  /**
   * Set the value of slot <code>slotName</code> as <code>slotValue</code> to object
   * <code>obj</code>
   */
  ...
  private Object getSlotValue(String slotName, Object obj, ObjectSchema schema)
      throws OntologyException {
    if (schema.isOwnSlot(slotName)) {
      // The slot is defined in "schema" --> use the Introspector of the ontology
      // actually defining "schema"
  ...
  /**
   * Check whether a given object is a valid term. If it is an Aggregate (i.e. a <code>List</code>)
   * it also check the elements.
   * 
  ...
  public static void checkIsTerm(Object obj) throws OntologyException {
    ...
    /*
     * if (obj instanceof String || obj instanceof Boolean || obj instanceof Integer || obj
     * instanceof Long || //#MIDP_EXCLUDE_BEGIN obj instanceof Float || obj instanceof Double ||
     * //#MIDP_EXCLUDE_END obj instanceof Date || obj instanceof Term) { return; } if (obj
     * instanceof List) { Iterator it = ((List) obj).iterator(); while (it.hasNext()) {
     * checkIsTerm(it.next()); } return; }
     * 
     * // If we reach this point the object is not a term throw new
     * OntologyException("Object "+obj+" of class "+obj.getClass().getName() +" is not a term");
     */
  }
  ...
  // #J2ME_EXCLUDE_BEGIN
  /**
   * Retrieve the names of the concepts defined in this ontology only (excluding extended
   * ontologies). It should be noticed that an agent-action is itself a concept and therefore the
   * returned list also includes names of agent-actions defined in this ontology. <br>
  ...
  /**
   * Retrieve the names of all concepts defined in this ontology (including extended ontologies). It
   * should be noticed that an agent-action is itself a concept and therefore the returned list also
   * includes names of agent-actions defined in this ontology. <br>
   * <b>NOT available in J2ME</b> <br>
   * 
  ...
  /**
   * Retrieve the names of the agent actions defined in this ontology only (excluding extended
   * ontologies). <br>
   * <b>NOT available in J2ME</b> <br>
   * 
   * @return the names of the agent actions defined in this ontology only (excluding extended
   *         ontologies)
   */
  ...
  /**
   * Retrieve the names of all agent actions defined in this ontology (including extended
   * ontologies). <br>
   * <b>NOT available in J2ME</b> <br>
   * 
  ...
  /**
   * Retrieve the names of the predicates defined in this ontology only (excluding extended
   * ontologies). <br>
   * <b>NOT available in J2ME</b> <br>
   * 
   * @return the names of the predicates defined in this ontology only (excluding extended
   *         ontologies)
   */
  ...
  private Set getElementNames(Class c) {
    ...
    for (int i = 0; i < base.length; ++i) {
      ...
      if ((o != null) && (o.getClass() != BasicOntology.class)
          && (o.getClass() != SerializableOntology.class)) {
  ...
  // #MIDP_EXCLUDE_BEGIN
  /**
   * Create a ConceptSlotFunction for a given slot of a given Concept. The ConceptSlotFunction class
   * allows treating the slots of an ontological concept as functions. For instance, if an ontology
   * defines a concept <code>Person</code> with a slot <code>name</code> and a slot
   ...
   * @param c The concept a ConceptSlotFunction must be created for. This concept must have a slot
   *        called <code>slotName</code>
   ...
   * @since JADE 3.7
   */
  public ConceptSlotFunction createConceptSlotFunction(String slotName, Concept c)
      throws OntologyException {
    ...
    if (schema != null) {
      if (conceptSlots != null) {
        if (schema.containsSlot(slotName)) {
          ...
          throw new OntologyException("Schema " + schema.getTypeName() + " for class "
              + c.getClass() + " does not contain a slot called " + slotName);
        }
      } else {
        throw new OntologyException(
  ...
  /**
   * Instruct this ontology to support usage of concept slots as functions. This method must be
   * invoked after all schemas have been completely defined and added to this ontology.
   * 
   * @see ConceptSlotFunction
  ...
  public static AbsObject externalizeSlotValue(Object obj, Introspector introspector,
  ...
  public static Object internalizeSlotValue(AbsObject abs, Introspector introspector,
      Ontology referenceOnto) throws OntologyException {
  ...
  // #J2ME_EXCLUDE_BEGIN
  private static void addReferencedSchemas(ObjectSchema schema, List<ObjectSchema> schemas)
      throws OntologyException {
    ObjectSchema[] superSchemas = schema.getSuperSchemas();
  ...
  public static List<ObjectSchema> getReferencedSchemas(ObjectSchema rootSchema)
      throws OntologyException {
