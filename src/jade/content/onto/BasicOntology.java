 ...
 * basic ontological elements required for minimal agent interaction. Users should always extend
 * this ontology when defining their ontologies.
 * 
...
public class BasicOntology extends Ontology implements SL0Vocabulary {
  ...
  private void initialize() {
    ...
    try {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN add(new PrimitiveSchema(STRING)); add(new PrimitiveSchema(DATE));
       * add(new PrimitiveSchema(BYTE_SEQUENCE)); add(new ConceptSchema(AID)); add(new
       * AgentActionSchema(ACLMSG)); add(new PredicateSchema(TRUE_PROPOSITION)); add(new
       * PredicateSchema(FALSE_PROPOSITION)); add(new AgentActionSchema(ACTION)); add(new
       * PredicateSchema(DONE)); add(new PredicateSchema(RESULT)); add(new PredicateSchema(EQUALS));
       * #MIDP_INCLUDE_END
       */
      ...
      msgSchema.add(ACLMSG_CONVERSATION_ID, (PrimitiveSchema) getSchema(STRING),
      ...
      msgSchema.add(ACLMSG_BYTE_SEQUENCE_CONTENT, (PrimitiveSchema) getSchema(BYTE_SEQUENCE),
          ObjectSchema.OPTIONAL);
      ...
      doneSchema.add(DONE_CONDITION, (PredicateSchema) PredicateSchema.getBaseSchema(),
          ObjectSchema.OPTIONAL);
  ...
  private void fillPrimitiveSchemas() throws OntologyException {
    // This map is only needed to make the getSchema(Class) method work properly
    // also in the case of java primitives
  ...
  protected Object toObject(AbsObject abs, String lcType, Ontology referenceOnto)
    ...
    try {
      ...
      if (abs.getAbsType() == AbsObject.ABS_CONCEPT_SLOT_FUNCTION) {
        AbsObject absConcept =
            abs.getAbsObject(ConceptSlotFunctionSchema.CONCEPT_SLOT_FUNCTION_CONCEPT);
        return referenceOnto.createConceptSlotFunction(abs.getTypeName(),
            (Concept) referenceOnto.toObject(absConcept));
      }
      // #MIDP_EXCLUDE_END
      // AID
      ...
      if (CaseInsensitiveString.equalsIgnoreCase(abs.getTypeName(),
          BasicOntology.TRUE_PROPOSITION)) {
        TrueProposition t = new TrueProposition();
        return t;
      }
      // FALSE_PROPOSITION
      if (CaseInsensitiveString.equalsIgnoreCase(abs.getTypeName(),
          BasicOntology.FALSE_PROPOSITION)) {
      ...
      if (CaseInsensitiveString.equalsIgnoreCase(abs.getTypeName(), BasicOntology.DONE)) {
        Done d = new Done();
        d.setAction(
            (AgentAction) referenceOnto.toObject(abs.getAbsObject(BasicOntology.DONE_ACTION)));
        AbsPredicate condition = (AbsPredicate) abs.getAbsObject(BasicOntology.DONE_CONDITION);
        if (condition != null) {
          d.setCondition(
              (Predicate) referenceOnto.toObject(abs.getAbsObject(BasicOntology.DONE_CONDITION)));
        }
        return d;
      }
      // RESULT
      if (CaseInsensitiveString.equalsIgnoreCase(abs.getTypeName(), BasicOntology.RESULT)) {
        Result r = new Result();
        r.setAction(
            (AgentAction) referenceOnto.toObject(abs.getAbsObject(BasicOntology.RESULT_ACTION)));
  ...
  protected AbsObject fromObject(Object obj, Ontology referenceOnto) throws OntologyException {
    try {
      ...
      if (obj instanceof Done) {
        ...
        absDone.set(BasicOntology.DONE_ACTION,
            (AbsAgentAction) referenceOnto.fromObject(((Done) obj).getAction()));
        if (((Done) obj).getCondition() != null) {
          absDone.set(BasicOntology.DONE_CONDITION,
              (AbsPredicate) referenceOnto.fromObject(((Done) obj).getCondition()));
        }
        return absDone;
      }
      ...
      if (obj instanceof Result) {
        ...
        absResult.set(BasicOntology.RESULT_ACTION,
            (AbsAgentAction) referenceOnto.fromObject(((Result) obj).getAction()));
        absResult.set(BasicOntology.RESULT_VALUE,
            (AbsTerm) referenceOnto.fromObject(((Result) obj).getValue()));
        return absResult;
      }

      if (obj instanceof Equals) {
        AbsPredicate absEquals = new AbsPredicate(BasicOntology.EQUALS);
        absEquals.set(BasicOntology.EQUALS_LEFT,
            (AbsTerm) referenceOnto.fromObject(((Equals) obj).getLeft()));
        absEquals.set(BasicOntology.EQUALS_RIGHT,
            (AbsTerm) referenceOnto.fromObject(((Equals) obj).getRight()));
        return absEquals;
      }
  ...
  /**
   * Redefine the <code>getSchema()</code> method to take into account ACL performatives.
   * 
  ...
  /**
   * Redefine the <code>getSchema()</code> method to take into account java primitives.
   * 
  ...
  public ObjectSchema getSchema(Class clazz) throws OntologyException {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN ObjectSchema schema = super.getSchema(clazz); #MIDP_INCLUDE_END
     */
    return schema;
  }

  /**
   * Note that we don't want to keep 22 different schemas for the 22 FIPA communicative acts --> We
   * generate the schemas for communicative acts on the fly as needed.
   */
  ...
  public static Object adjustPrimitiveValue(Object srcValue, Class destClass) {
    ...
    if (srcValue != null) {
      ...
      // Note that we deal with Integer, int, Long, long... classes only --> we can
      // compare the classes using == and != instead of using instanceof
        ...
        if (destClass == Integer.class || destClass == int.class) {
          ...
          try {
            ...
            try {
              ...
              java.text.SimpleDateFormat W3CISO8601DateFormat =
