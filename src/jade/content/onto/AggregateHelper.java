/*****************************************************************
 * WADE - Workflow and Agent Development Environment is a framework to develop multi-agent systems
 * able to execute tasks defined according to the workflow metaphor. Copyright (C) 2008 Telecom
 * Italia S.p.A.
 * 
...
public class AggregateHelper {
      ...
      if (srcClass != destClass) {
        ...
        if (destClass.isArray()) {
          ...
          if (java.util.Collection.class.isAssignableFrom(srcClass)) {
            ...
            destValue = collectionToArray(javaCollection.iterator(), destClass.getComponentType(),
                javaCollection.size());
          }
          ...
          // Source is a jade collection
            ...
            destValue = collectionToArray(jadeCollection.iterator(), destClass.getComponentType(),
                jadeCollection.size());
          }
        }
  ...
  static java.util.Collection createConcreteJavaCollection(Class clazz)
    ...
    if ((modifiers & ACC_ABSTRACT) == 0 && (modifiers & ACC_INTERFACE) == 0) {
      ...
      // class is either abstract or an interface, we have to somehow choose a
      // concrete collection :-(
      if (java.util.List.class.isAssignableFrom(clazz)) {
  ...
  static jade.util.leap.Collection createConcreteJadeCollection(Class clazz)
      throws InstantiationException, IllegalAccessException {
    int modifiers = clazz.getModifiers();
