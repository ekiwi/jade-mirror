...
class BeanIntrospector implements Introspector {
  ...
  private Object invokeGetterMethod(Method method, Object obj) throws OntologyException {
    ...
    try {
      ...
      if (result != null && java.util.Calendar.class.isAssignableFrom(result.getClass())) {
        // ontologically, Calendar is translated into a Date => convert the Calendar
        // into a Date
      ...
      throw new OntologyException(
          "Error invoking getter method " + method.getName() + " on object " + obj, e);
    }
    return result;
  }

  private void invokeSetterMethod(Method method, Object obj, Object value, Class clazz)
      throws OntologyException {
    try {
      if (java.util.Calendar.class.isAssignableFrom(clazz)) {
        // ontologically, Calendar is translated into a Date => convert the date back
        // into a Calendar
      ...
      throw new OntologyException("Error invoking setter method " + method.getName() + " on object "
          + obj + " with parameter " + value, e);
    }
  }
  ...
  public Object getSlotValue(String slotName, Object obj, ObjectSchema schema)
    ...
    if (slotAccessData == null) {
      throw new OntologyException(
          "cannot retrieve a getter for slot " + slotName + ", class " + obj.getClass());
    }
  ...
  public void setSlotValue(String slotName, Object slotValue, Object obj, ObjectSchema schema)
    ...
    if (slotAccessData == null) {
      throw new OntologyException(
          "cannot retrieve a setter for slot " + slotName + ", class " + obj.getClass());
    }

    invokeSetterMethod(slotAccessData.setter, obj, slotValue, slotAccessData.type);
  }

  public AbsAggregate externalizeAggregate(String slotName, Object slotValue, ObjectSchema schema,
      ...
      if (iter.hasNext() || schema.isMandatory(slotName)) {
        ...
        try {
          while (iter.hasNext()) {
            ...
            // Do not call Ontology.fromObject directly since each element in the aggregate
            // may be an aggregate itself
            absAggregate.add((AbsTerm) Ontology.externalizeSlotValue(object, this, referenceOnto));
          }
  ...
  public Object internalizeAggregate(String slotName, AbsAggregate absAggregate,
      ObjectSchema schema, Ontology referenceOnto) throws OntologyException {
    SlotAccessData slotAccessData = accessors.get(new SlotKey(schema.getTypeName(), slotName));
    if (slotAccessData == null) {
      Class containerClazz = referenceOnto.getClassForElement(schema.getTypeName());
      throw new OntologyException(
          "cannot retrieve a setter for slot " + slotName + ", class " + containerClazz);
    }

    Class elementClazz = slotAccessData.aggregateClass;
    Class aggregateClass = slotAccessData.type;

    if (aggregateClass == null || aggregateClass == Object.class || aggregateClass == Term.class
      ...
      if (aggregateClass.isArray()) {
        ...
        while (iterator.hasNext()) {
          Array.set(result, index,
              Ontology.internalizeSlotValue((AbsTerm) iterator.next(), this, referenceOnto));
          index++;
        }
      } else if (java.util.Collection.class.isAssignableFrom(aggregateClass)) {
        java.util.Collection javaCollection =
            AggregateHelper.createConcreteJavaCollection(aggregateClass);
        if (javaCollection == null) {
          throw new OntologyException(
              "cannot create a concrete collection for class " + aggregateClass.getName());
        }
        result = javaCollection;
        while (iterator.hasNext()) {
          javaCollection
              .add(Ontology.internalizeSlotValue((AbsTerm) iterator.next(), this, referenceOnto));
        }
      } else if (jade.util.leap.Collection.class.isAssignableFrom(aggregateClass)) {
        jade.util.leap.Collection jadeCollection =
            AggregateHelper.createConcreteJadeCollection(aggregateClass);
        ...
        throw new OntologyException(
            "don't know how to handle aggregate slot of class " + aggregateClass.getName());
      }
    } catch (InstantiationException ie) {
      throw new OntologyException(
          "cannot instantiate aggregate slot of non-concrete class " + aggregateClass.getName(),
          ie);
    } catch (IllegalAccessException iae) {
      throw new OntologyException(
          "cannot instantiate aggregate slot through unaccessible default constructor of class "
  ...
  public AbsObject externalizeSpecialType(Object obj, ObjectSchema schema, Class javaClass,
      Ontology referenceOnto) throws OntologyException {
    if (!javaClass.isEnum()) {
  ...
  public Object internalizeSpecialType(AbsObject abs, ObjectSchema schema, Class javaClass,
      Ontology referenceOnto) throws OntologyException {
    if (!javaClass.isEnum()) {
      throw new NotASpecialType();
    }
