...
public class CFReflectiveIntrospector extends ReflectiveIntrospector {
  ...
  public AbsAggregate externalizeAggregate(String slotName, Object obj, ObjectSchema schema,
    ...
    if (!c.isEmpty() || schema == null || schema.isMandatory(slotName)) {
      // Note 1: schema is null when we are externalizing a first-level object and not
      // the value of a slot
      // Note 2: we ignore the aggregateType specified in the slot schema and we use
      // SET for java.util.Set and SEQUENCE for java.util.List
  ...
  private AbsAggregate externaliseCollection(Collection c, Ontology referenceOnto,
  ...
  private Collection internaliseCollection(AbsAggregate absAggregate, Ontology referenceOnto)
