...
/**
 * Backward Compatible reflective introspector. This Introspector uses Java Reflection to translate
 * java objects to/from abstract descriptors as the <code>ReflectiveIntrospector</code> does, but it
 * assumes the accessors methods for aggregate slots to be in the "old JADE style" i.e. <i> For
 * every aggregate <b>slot</b> named <code>XXX</code>, with elements of type <code>T</code>, the
 * Java class must have two accessible methods, with the following signature:</i>
 * <ul>
...
public class BCReflectiveIntrospector extends ReflectiveIntrospector {
  ...
  protected boolean isAggregateObject(Object obj) {
    ...
    // #J2ME_EXCLUDE_END
    /*
     * #J2ME_INCLUDE_BEGIN return obj instanceof Iterator; #J2ME_INCLUDE_END
     */
  }

  public Object getSlotValue(String slotName, Object obj, ObjectSchema schema)
    ...
    if (slotSchema != null) {
      ...
      throw new OntologyException(
          "No slot named " + slotName + " found in schema " + schema.getTypeName());
    }
  }
  ...
  public void setSlotValue(String slotName, Object slotValue, Object obj, ObjectSchema schema)
    ...
    if (slotSchema != null) {
      String methodName;
      // Note that here checking if absSlotValue is an AbsAggregate would be wrong as
      // we have add methods only if the schema of the slot is AggregateSchema
      if (slotSchema instanceof AggregateSchema) {
  ...
  private void invokeAddMethod(Method method, Object obj, Object value) throws OntologyException {
    try {
      ...
      throw new OntologyException("Can't apply recursively method " + method.getName()
          + " to object " + obj + " as value " + value + " is not a List", cce);
    }
  }

  /**
   * Check the structure of a java class associated to an ontological element to ensure that
   * translations to/from abstract descriptors and java objects (instances of that class) can be
   * accomplished by this introspector.
   * 
   ...
   * @throws OntologyException if the java class does not have the correct structure
   */
        ...
        if (!(slotSchema instanceof PrimitiveSchema)) {
          ...
          if (slotClass != null) {
            if (!slotGetSetClass.isAssignableFrom(slotClass)) {
              throw new OntologyException("Wrong class for schema: " + schema.getTypeName()
                  + ". Slot " + sName + ": expected class=" + slotClass + ", Get/Set method class="
                  + slotGetSetClass);
            }
          }
        } else {
          ...
          if (type.equals(BasicOntology.STRING)) {
            if (!slotGetSetClass.isAssignableFrom(String.class)) {
              throw new OntologyException("Wrong class for schema: " + schema.getTypeName()
                  + ". Slot " + sName + ": expected class=" + String.class
                  + ", Get/Set method class=" + slotGetSetClass);
            }
          } else if (type.equals(BasicOntology.INTEGER)) {
            if ((!slotGetSetClass.equals(Integer.TYPE)) && (!slotGetSetClass.equals(Integer.class))
                && (!slotGetSetClass.equals(Long.TYPE)) && (!slotGetSetClass.equals(Long.class))) {
              throw new OntologyException(
                  "Wrong class for schema: " + schema.getTypeName() + ". Slot " + sName
                      + ": expected class=INTEGER, Get/Set method class=" + slotGetSetClass);
            }
          }
        }
      } catch (Exception e) {
        throw new OntologyException("Wrong class for schema: " + schema.getTypeName() + ". Slot "
            + sName + ": unexpected error. " + e.getMessage());
      }
    }
  }
  ...
  private Class checkGetAndSet(String name, Class c) throws OntologyException {
    ...
    if (getParams.length > 0)
      throw new OntologyException(
          "Wrong class: method " + getMethod.getName() + "() must take no arguments.");
    ...
    if ((setParams.length != 1) || (!setParams[0].equals(result)))
      throw new OntologyException("Wrong class: method " + setMethod.getName()
          + "() must take a single argument of type " + result.getName() + ".");
    Class setReturn = setMethod.getReturnType();
    if (!setReturn.equals(Void.TYPE))
      throw new OntologyException(
          "Wrong class: method " + setMethod.getName() + "() must return void.");
  ...
  private Class checkGetAndSet2(String name, Class c) throws OntologyException {
    ...
    if (!(getReturnType(getMethod)).isAssignableFrom(jade.util.leap.Iterator.class))
      throw new OntologyException("Wrong class: method " + getMethod.getName()
          + "() must return a jade.util.leap.Iterator." + getReturnType(getMethod).toString());

    // check 'add' method
    if (getArgumentLength(addMethod) != 1)
      throw new OntologyException(
          "Wrong class: method " + addMethod.getName() + "() must take one argument.");
    if (!getArgumentType(addMethod, 0).equals(result))
      throw new OntologyException(
          "Wrong class: method " + addMethod.getName() + "() has the wrong argument type.");
    if (!getReturnType(addMethod).equals(Void.TYPE))
      throw new OntologyException(
          "Wrong class: method " + addMethod.getName() + "() must return a void.");
  ...
  public AbsAggregate externalizeAggregate(String slotName, Object obj, ObjectSchema schema,
    ...
    // #J2ME_EXCLUDE_END
    /*
     * #J2ME_INCLUDE_BEGIN Iterator it = (Iterator) obj; #J2ME_INCLUDE_END
     */
    if (it.hasNext() || schema.isMandatory(slotName)) {
      ...
      try {
        while (it.hasNext())
          absAggregateValue
              .add((AbsTerm) Ontology.externalizeSlotValue(it.next(), this, referenceOnto));
      } catch (ClassCastException cce) {
  ...
  public Object internalizeAggregate(String slotName, AbsAggregate abs, ObjectSchema schema,
      Ontology referenceOnto) throws OntologyException {
