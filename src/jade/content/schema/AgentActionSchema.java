...
/**
 * The class to be used to define schemas of agent actions in an ontology. Note that an
 * AgentActionSchema should also be a ConceptSchema, but this inheritance relation is cut as Java
 * does not support multiple inheritance. As a consequence in practice it will not be possible to
 * define e.g. a ConceptSchema with a slot whose value must be instances of a certain type of
 * agent-action even if in theory this should be possible as a ConceptSchema can have slots of type
 * term and an agent-action is a concept and therefore a term.
 * 
...
public class AgentActionSchema extends ConceptSchema {
   ...
   * s is TermSchema.getBaseSchema() and this schema is an instance of ConceptSchema. Moreover, as
   * AgentActionSchema extends GenericActionSchema, but should also extend ConceptSchema (this is
   * not possible in practice as Java does not support multiple inheritance), this method returns
   * true also in the case that s is equals to, or is an ancestor of, ConceptSchema.getBaseSchema()
   * (i.e. TermSchema.getBaseSchema() descends from s)
   */
  ...
  /**
   * Define that the result produced by the execution of an action described by this schema has a
   * structure conforming to a given term schema.
   * 
   * @param resultSchema the schema of the result
   */
  public void setResult(TermSchema resultSchema) {
    // We treat the result as if it were a slot to inherit all mechanisms to deal
    // with super-schemas and facets
    add(RESULT_SLOT_NAME, resultSchema, OPTIONAL);
  }

  /**
   * Define that the result produced by the execution of an action described by this schema is an
   * aggregate of n (with n between cardMin and cardMax) elements each one having a structure
   * conforming to a given term schema.
   * 
