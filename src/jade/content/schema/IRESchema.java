 ...
 * Note that an IRESchema should also be a TermSchema, but this inheritance relation is cut as Java
 * does not support multiple inheritance. As a consequence in practice it will not be possible to
 * define e.g. a ConceptSchema with a slot whose value must be instances of a certain type of IRE
 * even if in theory this should be possible as a ConceptSchema can have slots of type term and an
 * IRE is a term.
 * 
...
public class IRESchema extends TermSchema {
  ...
  /**
   * Creates a <code>IRESchema</code> with a given type-name. All ire-s have a variable and a
   * proposition.
   * 
   * @param typeName The name of this <code>IRESchema</code> (e.g. IOTA, ANY, ALL).
   */
  ...
  /**
   * An IRE can be put whereever a term of whatever type is required --> An IRESchema is compatible
   * with s if s descends from TermSchema.getBaseSchema()
   */
   ...
   * IRESchema extends ContentElementSchema, but should also extend TermSchema (this is not possible
   * in practice as Java does not support multiple inheritance), this method returns true also in
   * the case that s is equals to, or is an ancestor of, TermSchema.getBaseSchema() (i.e.
