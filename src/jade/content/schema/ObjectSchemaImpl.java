...
class ObjectSchemaImpl extends ObjectSchema {
  ...
  // Note that the list of facets for a given slot cannot be included in the slot
  // descriptor.
  // In fact facets are associated to the a slot in a given schema, not to the
  // slot itself.
  ...
  // When validating an instance of B only facet f1 must be applied, while when
  // validating
  ...
  /**
   * Construct a schema that vinculates an entity to be a generic object (i.e. no constraints at
   * all)
   */
  ...
  protected void add(String name, ObjectSchema slotSchema, int optionality) {
    ...
    if (slots.put(ciName, new SlotDescriptor(name, slotSchema, optionality)) == null) {
      // We treat Action results as if they were slots. However we don't want the
      // special
  ...
  /**
   * Add a slot with cardinality between <code>cardMin</code> and <code>cardMax</code> to this
   * schema. Adding such a slot corresponds to add a slot of type Aggregate and then to add proper
   * facets (constraints) to check that the type of the elements in the aggregate are compatible
   * with <code>elementsSchema</code> and that the aggregate contains at least <code>cardMin</code>
   * elements and at most <code>cardMax</code> elements. By default the Aggregate is of type
   * <code>BasicOntology.SEQUENCE</code>.
   * 
   * @param name The name of the slot.
  ...
  /**
   * Add a slot with cardinality between <code>cardMin</code> and <code>cardMax</code> to this
   * schema and allow specifying the type of Aggregate to be used for this slot.
   * 
   * @param name The name of the slot.
   * @param elementsSchema The schema for the elements of this slot.
   * @param cardMin This slot must get at least <code>cardMin</code> values
  ...
  /**
   * Add a slot with optionality and cardinality between <code>cardMin</code> and
   * <code>cardMax</code> to this schema and allow specifying the type of Aggregate to be used for
   * this slot.
   * 
   * @param name The name of the slot.
   * @param elementsSchema The schema for the elements of this slot.
   ...
   */
  protected void add(String name, ObjectSchema elementsSchema, int cardMin, int cardMax,
      String aggType, int optionality) {
    try {
      // If the aggregate type is not yet present in the BasicOntology, add it
      // (without elements type specification)
      ...
      // If elements are typed, we need an ad-hoc schema to hold the elements type
      // specification
  ...
  /**
   * Add a super schema to this schema, i.e. this schema will inherit all characteristics from the
   * super schema
   * 
   ...
   * 
   * @param slotName the name of the slot the <code>Facet</code> must be added to.
   * @param f the <code>Facet</code> to be added.
   * @throws OntologyException if slotName does not identify a valid slot in this schema
   */
  ...
  /**
   * Returns the names of all the slots in this <code>Schema</code> (including slots defined in
   * super schemas).
   *
  ...
  /**
   * Returns the names of the slots defined in this <code>Schema</code> (excluding slots defined in
   * super schemas).
   *
   ...
   * @throws OntologyException If no slot with this name is present in this schema.
   */
  ...
  /**
   * Indicate whether a given <code>String</code> is the name of a slot defined in this
   * <code>Schema</code>
   *
   * @param name The <code>String</code> to test.
   * @return <code>true</code> if <code>name</code> is the name of a slot defined in this
   *         <code>Schema</code>.
   */
  ...
  /**
   * Indicate whether a given <code>String</code> is the name of a slot actually defined in this
   * <code>Schema</code> (excluding super-schemas)
   *
   * @param name The <code>String</code> to test.
   * @return <code>true</code> if <code>name</code> is the name of a slot actually defined in this
   *         <code>Schema</code> (excluding super-schemas).
   */
  ...
  /**
   * Creates an Abstract descriptor to hold an object compliant to this <code>Schema</code>.
   */
  ...
  /**
   * For each slot - get the corresponding attribute value from the abstract descriptor abs - Check
   * that it is not null if the slot is mandatory - Check that its schema is compatible with the
   * schema of the slot - Check that it is a correct abstract descriptor by validating it against
   * its schema.
   */
   ...
   * @return true if the slot is defined in this schema (or in one of its super schemas). false
   *         otherwise
   */
  private boolean validate(CaseInsensitiveString slotName, AbsObject value, Ontology onto)
      throws OntologyException {
    // DEBUG
    if (logger.isLoggable(Logger.FINE))
      logger.log(Logger.FINE, "Validating " + (value != null ? value.toString() : "null")
          + " as a value for slot " + slotName);
      ...
      if (value == null) {
        ...
        if (dsc.optionality == MANDATORY) {
          throw new OntologyException(
              "Missing value for mandatory slot " + slotName + ". Schema is " + this);
        }
        ...
        if (s == null) {
          throw new OntologyException("No schema found for type " + value.getTypeName()
              + ". Ontology is " + onto.getName());
        }
        if (!s.isCompatibleWith(dsc.schema)) {
          throw new OntologyException("Schema " + s + " for element " + value
              + " is not compatible with schema " + dsc.schema + " for slot " + slotName);
        }
        // DEBUG
        if (logger.isLoggable(Logger.CONFIG))
          logger.log(Logger.CONFIG, "Schema " + s + " for type " + value
              + " is compatible with schema " + dsc.schema + " for slot " + slotName);
  ...
  /**
   * Check if this schema is compatible with a given schema s. This is the case if 1) This schema is
   * equals to s 2) s is one of the super-schemas of this schema 3) This schema descends from s i.e.
   * - s is the base schema for the XXXSchema class this schema is an instance of (e.g. s is
   * ConceptSchema.getBaseSchema() and this schema is an instance of ConceptSchema) - s is the base
   * schema for a super-class of the XXXSchema class this schema is an instance of (e.g. s is
   * TermSchema.getBaseSchema() and this schema is an instance of ConceptSchema)
   */
  ...
  /**
   * Get the facets defined upon a slot. More in details this method returns all facets defined in
   * this schema plus all facets defined in super-schemas up to the schema actually declaring the
   * given slot.
   * 
  ...
  public boolean isAssignableFrom(ObjectSchema s) {
    ...
    // the source schema must have a super-schema with the same type name of the
    ...
    try {
      // All slots of the destination schema that are present in the source schema too
      // must be assignable.
      // Slots of the destination schema not present in the source schema must be
      // OPTIONAL.
