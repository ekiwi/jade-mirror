...
public class AbsHelper {
  /**
   * Converts a <code>List</code> into a <code>AbsAggregate</code> using the specified ontology.
   * 
   ...
   */
  public static AbsAggregate externaliseList(List obj, Ontology onto, String AggregateType)
      throws OntologyException {
  ...
  /**
   * Converts a <code>Set</code> into a <code>AbsAggregate</code> using the specified ontology.
   * 
   ...
   * @throws OntologyException
   */
  public static AbsAggregate externaliseSet(Set obj, Ontology onto, String AggregateType)
      throws OntologyException {
    AbsAggregate ret = new AbsAggregate(AggregateType);
  ...
  /**
   * Converts an <code>Iterator</code> into a <code>AbsAggregate</code> using the specified
   * ontology.
   * 
   ...
   */
  // #J2ME_EXCLUDE_BEGIN
  public static AbsAggregate externaliseIterator(java.util.Iterator obj, Ontology onto,
      String AggregateType) throws OntologyException {
    // #J2ME_EXCLUDE_END
    /*
     * #J2ME_INCLUDE_BEGIN public static AbsAggregate externaliseIterator(Iterator obj, Ontology
     * onto, String AggregateType) throws OntologyException { #J2ME_INCLUDE_END
     */
    AbsAggregate ret = new AbsAggregate(AggregateType);
  ...
  /**
   * Converts an <code>AID</code> into an <code>AbsConcept</code> representing an AID
   * 
  ...
  /**
   * Converts a <code>ContentElementList</code> into an <code>AbsContentElementList</code> using the
   * specified ontology.
   * 
  ...
  /**
   * Converts an <code>ACLMessage</code> into an <code>AbsAgentAction</code> using the specified
   * ontology.
   * 
   ...
   * @throws OntologyException
   */
  public static AbsAgentAction externaliseACLMessage(ACLMessage obj, Ontology onto)
      throws OntologyException {
    try {
  ...
  /**
   * Converts an <code>AbsAggregate</code> into a List using the specified ontology.
   * 
   * @param onto the ontology
   * @return the List
   * @throws OntologyException
   */
  public static List internaliseList(AbsAggregate aggregate, Ontology onto)
      throws OntologyException {
  ...
  /**
   * Converts an <code>AbsAggregate</code> into a Set using the specified ontology.
   * 
   * @param onto the ontology
   * @return the Set
   * @throws OntologyException
  ...
  public static java.util.Collection internaliseJavaCollection(AbsAggregate aggregate,
    ...
    try {
      ...
      throw new OntologyException(
          "Cannot instantiate java collection of class " + aggregate.getTypeName(), e);
    }
    for (int i = 0; i < aggregate.size(); i++) {
      Object element = onto.toObject(aggregate.get(i));
      // Check if the element is a Term, a primitive an AID or a List
   ...
   * 
   * @return the OntoAID
   * @throws OntologyException if <code>aid</code> does not represent a valid AID
   */
  ...
  /**
   * Converts to an <code>AbsContentElementList</code> into a ContentElementList using the specified
   * ontology.
   * 
   * @param onto the ontology
  ...
  /**
   * Converts to an <code>AbsAgentAction</code> representing an ACLMessage into an OntoACLMessage
   ...
   * @throws OntologyException
   */
  public static OntoACLMessage internaliseACLMessage(AbsAgentAction absMsg, Ontology onto)
      throws OntologyException {
   ...
   */
  public static void setAttribute(AbsObject abs, String attrName, AbsObject attrValue)
      throws OntologyException {
    ...
    if (abs.getAbsType() == AbsObject.ABS_CONCEPT) {
      ...
      if (attrValue instanceof AbsVariable
          && CaseInsensitiveString.equalsIgnoreCase(attrName, IRESchema.VARIABLE)) {
        ((AbsIRE) abs).setVariable((AbsVariable) attrValue);
        return;
      } else if (attrValue instanceof AbsPredicate
          && CaseInsensitiveString.equalsIgnoreCase(attrName, IRESchema.PROPOSITION)) {
        ((AbsIRE) abs).setProposition((AbsPredicate) attrValue);
        return;
      }
    } else if (abs.getAbsType() == AbsObject.ABS_VARIABLE) {
      if (attrValue instanceof AbsPrimitive
          && CaseInsensitiveString.equalsIgnoreCase(attrName, VariableSchema.NAME)) {
        ((AbsVariable) abs).setName(((AbsPrimitive) attrValue).getString());
        return;
      } else if (attrValue instanceof AbsPrimitive
          && CaseInsensitiveString.equalsIgnoreCase(attrName, VariableSchema.VALUE_TYPE)) {
    ...
    throw new OntologyException(
        "Type incompatibility: value of attribute " + attrName + " of " + abs + " is " + attrValue);
  }
   ...
   * Prefix is identified from last separator string Eg. A.B.C -> C (if separator is .)
   * 
  ...
  /**
   * Recursively add prefix to abs-type-name, except for BasicOntology types Eg. C -> A.B.C (if
   * prefix is A.B.)
   * 
  ...
  /**
   * Return true if the abs-object is a template. A template is an abs with all slots AbsVariable or
   * AbsAggregate empty
   * 
  ...
  /**
   * Apply default value to template Replace node with default value only if the value in template
   * is null or AbsVariable or default value is null and template is a true template
   * 
   ...
   * @throws OntologyException
   */
  public static AbsObject applyDefaultValues(AbsObject absTemplate, AbsObject absDefault)
      throws OntologyException {

    // Aggregate not empty but all filled -> ignore default
    if (absTemplate instanceof AbsAggregate && absTemplate.getCount() != 0
        && absTemplate.isGrounded()) {
      return absTemplate;
    }
    ...
    if (!(absDefault instanceof AbsConcept)) {
      throw new OntologyException("Default abs structure (" + absDefault.getAbsType()
          + ") not compatible with template (" + absTemplate.getAbsType() + ")");
    }
    ...
    for (String slotName : absTemplate.getNames()) {
      ...
      if (absDefaultValue != null) {
        AbsHelper.setAttribute(absTemplate, slotName,
            applyDefaultValues(slotTemplateValue, absDefaultValue));
      }
    }
    return absTemplate;
  }
   ...
   * @throws Exception
   */
  public static AbsObject createAbsTemplate(ObjectSchema schema, String prefix)
      throws OntologyException {
    return createAbsTemplate(schema, prefix, VarIndexWrapper.ZERO);
  }

  private static AbsObject createAbsTemplate(ObjectSchema schema, String prefix,
      VarIndexWrapper viw) throws OntologyException {
