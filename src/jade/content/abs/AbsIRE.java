...
/**
 * An Abstract descriptor that can hold an Identifying Referential Expression (IRE). Note that an
 * IRE is both a content element (as in the case of a QUERY-REF communicative act) and a Term (as in
 * the case of (== (X) (iota ?x P(?x))
 * 
...
public class AbsIRE extends AbsObjectImpl implements AbsContentElement, AbsTerm {
   ...
   * 
   * @param typeName The name of the type of the IRE held by this abstract descriptor.
   */
  ...
  /**
   * Redefine the <code>isGrounded()</code> method in order to always return <code>false</code>.
   * Infact an IRE always includes a variable.
   */
