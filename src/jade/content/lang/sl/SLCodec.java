...
/**
 * The codec class for the <b><i>FIPA-SL</i>n</b> languages. This class implements the
 * <code>Codec</code> interface and allows converting back and forth between strings and frames,
 * according to the SL grammar. By default the class implements full SL grammar, otherwise the
 * proper value must be used in the constructor.
 * 
 ...
 */
/*
 * #MIDP_INCLUDE_BEGIN public class SLCodec extends SimpleSLCodec { #MIDP_INCLUDE_END
 */
// #MIDP_EXCLUDE_BEGIN
public class SLCodec extends StringCodec {
  ...
  /**
   * This variable is true, when meta symbols are allowed (metas are a semantics-specific extension
   * to the SL Grammar)
   **/
  ...
  /**
   * Create an SLCodec for the full SL-language (FIPA-SL) specifying whether or not java primitive
   * types (long, int, float, double) must be preserved. This is achieved by encoding long values as
   * <numeric-value>L and float values as <numeric-valueF. It should be noticed that this encoding
   * is NOT FIPA SL standard
   * 
   ...
   * 
   * @param slType specify 0 for FIPA-SL0, 1 for FIPA-SL1, 2 for FIPA-SL2, any other value can be
   *        used for full FIPA-SL
   */
  ...
  /**
   * Create an SLCodec for the given profile of SL-language specifying whether or not java primitive
   * types (long, int, float, double) must be preserved.
   * 
   * @param slType specify 0 for FIPA-SL0, 1 for FIPA-SL1, 2 for FIPA-SL2, any other value can be
   *        used for full FIPA-SL
   * @param preserveJavaTypes Indicates whether or not java primitive types must be preserved
   ...
   * @throws CodecException
   */
  public synchronized String encode(Ontology ontology, AbsContentElement content)
      throws CodecException {
    try {
      domainOnto = ontology;
  ...
  /**
   * Encode a string, taking care of quoting separated words and escaping strings, if necessary. And
  ...
  private void encodeAndAppend(AbsPredicate val) throws CodecException {
    ...
    if (val.getCount() > 0) { // predicate with arguments
      ...
      if (slOnto.isUnaryLogicalOp(propositionSymbol)) {
        ...
        buffer.append(' ');
        try {
          encodeAndAppend((AbsVariable) val.getAbsObject(slotNames[0])); // FIXME. The hypothesis is
                                                                         // that the
                                                                         // first slot is the
                                                                         // variable
          buffer.append(' ');
        ...
        buffer.append(' ');
        try {
          encodeAndAppend((AbsTerm) val.getAbsObject(slotNames[0])); // FIXME check it is an action
                                                                     // expression
   ...
   * @throws CodecException
   */
  public synchronized AbsContentElement decode(Ontology ontology, String content)
      throws CodecException {
    try {
  ...
  /**
   * Decodes the content to an abstract description, where the content is known to be a Well-formed
   * Formula
   * 
   * @param ontology the ontology.
   ...
   * @since JADE 3.4
   */
  public synchronized AbsPredicate decodeFormula(Ontology ontology, String formula)
      throws CodecException {
    try {
      if (preserveJavaTypes) {
   ...
   * @since JADE 3.4
   */
  public synchronized String encodeFormula(Ontology ontology, AbsPredicate formula)
      throws CodecException {
    try {
      domainOnto = ontology;
      buffer = new StringBuffer();
  ...
  public static void main(String[] args) {
    ...
    while (true) {
      try {
        System.out.println("insert an SL "
            + (contentType == 'F' ? "Well-Formed Formula"
                : (contentType == 'T' ? "Term" : "Content Expression"))
            + " to parse (all the expression on a single line!): ");
  ...
  /**
   * @return the ontology containing the schemas of the operator defined in this language
   */
  ...
  /**
   * Encode the slots of an abstract descriptor by order, i.e. without writing the slot names. Also
   * take into account that, in order to ensure a correct parsing, empty slots can only occur at the
   * end. Append this encoded string to buffer.
   */
  ...
  /**
   * Encode the slots of an abstract descriptor by name, i.e. writing for each non-empty slot the
   * slot name followed by the slot value. Append this encoded string to buffer.
   */
   ...
   * is preparing to return it to the caller. <br>
