...
/**
 * MIDP implementation of the SLCodec. Actually the MIDP version of the SLCodec just extends
 * SimpleSLCodec
 * 
...
public class SimpleSLCodec extends StringCodec {
  ...
  private void stringifyComplex(AbsObject val, Ontology onto, StringBuffer str)
      throws CodecException {
    str.append("(");
   ...
   * end.
   */
  private void encodeSlotsByOrder(AbsObject val, String[] slotNames, Ontology onto,
      StringBuffer str) throws CodecException {
   ...
   * slot name followed by the slot value.
   */
  private void encodeSlotsByName(AbsObject val, String[] slotNames, Ontology onto, StringBuffer str)
      throws CodecException {
    for (int i = 0; i < slotNames.length; i++) {
      AbsObject s = val.getAbsObject(slotNames[i]);
      if (s != null) {
  ...
  private void stringifyAggregate(AbsAggregate val, Ontology onto, StringBuffer str)
      throws CodecException {
    str.append("(");
    str.append(val.getTypeName());
  ...
  private void stringifyPrimitive(AbsPrimitive val, StringBuffer str) throws CodecException {
    ...
    if (type.equals(BasicOntology.STRING)) {
      ...
      if (CaseInsensitiveString.equalsIgnoreCase("true", s)
          || CaseInsensitiveString.equalsIgnoreCase("false", s)) {
  ...
  private void fillSlotsByOrder(AbsObject abs, ObjectSchema s, SimpleSLTokenizer p, Ontology o)
      throws CodecException {
  ...
  private void fillSlotsByName(AbsConcept abs, SimpleSLTokenizer p, Ontology o)
      throws CodecException {
    while (!p.isClosedBracket()) {
  ...
  private void fillAggregate(AbsAggregate abs, SimpleSLTokenizer p, Ontology o)
      throws CodecException {
    int i = 0;
    while (!p.isClosedBracket()) {
      try {
