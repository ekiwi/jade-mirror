...
public class SimpleSLTokenizer {
  ...
  // We need a dedicated method since just doing nextToken().equals("(") would
  // fail in case
  ...
  /**
   * Check that the next character (after eventual spaces) is 'c' and advance the pointer to the
    ...
    try {
      ...
      if (content.charAt(current++) != c) {
        throw new Codec.CodecException(
            "Parse error: position " + (current - 1) + ", found " + content.charAt(current - 1)
                + " while " + c + " was expected [" + content.substring(0, current) + "]");
      }
    } catch (IndexOutOfBoundsException ioobe) {
      throw new Codec.CodecException(msg + current);
    }
  ...
  /**
   * Return the next SL element (i.e. a word or a generic sequence of char enclosed into "") and
   * advance the pointer to the character just after. If the element starts with ':' this is
   * automatically removed. If the element is a sequence of char enclosed into "" the enclosing "
  ...
  // FIXME We might improve performance if we merged isAWord and quoteString into
  // a single method
  // infact they both have to loop over the chars of the String.
  /**
   * Test if the given string is a legal SL word using the FIPA XC00008D spec. In addition to FIPA's
  ...
  /**
   * Take a java String and quote it to form a legal FIPA SL0 string. Add quotation marks to the
