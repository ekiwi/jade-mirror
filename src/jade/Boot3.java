...
/**
 * Tests: java jade.Boot (with/without rmiregistry started): run a main-container java jade.Boot
 * -host pippo.cselt.it java jade.Boot -host fbellif.cselt.it java jade.Boot -host fbellif.cselt.it
 * -port 1200 java jade.Boot -help java jade.Boot -gui
 **/
package jade;
...
public class Boot3 {
   ...
   * 
   * @param args The command line arguments. These use the form key:value or -key (shorthand for
   *        key:true).
   */
   ...
   * </pre>
   * 
   * If the arguments contain either import:x or agents:x we will assume that the arguments are
   * already in the new format and leave them alone. For "import:" we test if what follows is a file
   * name and in the event it isn't we assume that it was if there are any other "-x" options
   * following.
   * <p>
   * You can't mix the old form with the new as this would make the distinction between foo:bar as
   * meaning a property named foo with a value bar or an agent named foo implmented by class bar
   * impossible.
   * <p>
   * 
  ...
  public void usage(PrintStream out) {
    ...
    out.println(
        "  -container\t\tIf specified, a new Agent Container is added to an existing platform");
    ...
    out.println(
        "  -conf <file name>\tStarts JADE using the configuration properties read in the specified file.");
    out.println("  -dump\t\t\tIf specified, lists boot's current properties.");
    out.println(
        "  -version\t\tIf specified, current JADE version number and build date is printed.");
    out.println(
    ...
    out.println(
        "  -name <platform name>\tThe symbolic platform name specified only for the main container.");
    out.println("  -owner <username:password>\tThe owner of a container or platform.");
    out.println(
        "  -nomobility\t\tIf specified, disables the mobility and cloning support for the container.");
    ...
    out.println("");
    out.print(
        "An agent specifier is composed of an agent name and an agent class, separated by \"");
    out.println(NAME2CLASS_SEPARATOR + "\"");
    out.println("");
    out.println(
        "Take care that the specified agent name represents only the local name of the agent.");
    out.println(
        "Its guid (globally unique identifier) is instead assigned by the AMS after concatenating");
    ...
    out.println(
        "  java jade.Boot -container -host zork.zot.za -port 1100 peter:heAgent paula:sheAgent");
    out.println("");
  ...
  /**
   * This method verifies the configuration properties and eventually correct them. It checks if the
   * port number is a number greater than 0 otherwise it throws a BootException, and if the -nomtp
   * has been set, then delete some other mtp wrongly set. If the user wants to start a platform the
   * host must be the local host so if a different name is speficied it will be corrected and an
  ...
  protected void check() throws BootException {
    try {
      Integer.parseInt(
          profile.getParameter(Profile.MAIN_PORT, Integer.toString(BootProfileImpl.DEFAULT_PORT)));
    } catch (NumberFormatException nfe) {
    ...
    if (localHost != null && !Profile.isLocalHost(localHost)) {
      throw new BootException("Error: Not possible to launch JADE a remote host ("
          + properties.getProperty(Profile.LOCAL_HOST)
