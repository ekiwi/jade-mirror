...
/**
 * This class implements the Fipa-Propose interaction protocol with an API similar and homogeneous
 * to <code>AchieveREInitiator</code>. <br>
 * This implementation works both for 1:1 and 1:N conversation and, of course, implements the role
 * of the initiator of the protocol.
 * <p>
 * The following is a brief description of the protocol. The programmer should however refer to the
 * <a href=http://www.fipa.org/specs/fipa00036/SC00036H.html>FIPA Spec</a> for a complete
 * description.
 * <p>
 * The initiator sends a <code>PROPOSE</code> message to the Participant indicating that it will
 * perform some action if the Participant agrees. The implementation of the callback method
 * <code>prepareInitiations</code> must return the vector of messages to be sent (eventually a
 * single message with multiple receivers).
 * <p>
 * The responders can then reply by sending a <code>ACCEPT-PROPOSAL</code> message. Alternatively,
 * responders may send a <code>REJECT-PROPOSAL</code>, to refuse the proposal or, eventually, a
 * <code>NOT-UNDERSTOOD</code> to communicate communication problems. This category of reply
 * messages has been here identified as a response and can be handled via the
 ...
 * does not comply with the FIPA protocol, than the method <code>handleOutOfSequence</code> would be
 * called.
 * <p>
 ...
 * guide </a> that describes the usage of this class.
 *
...
public class ProposeInitiator extends FSMBehaviour {
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the vector of PROPOSE ACLMessage objects
  ...
  protected String[] toBeReset = new String[] {HANDLE_ACCEPT_PROPOSAL, HANDLE_REJECT_PROPOSAL,
      HANDLE_NOT_UNDERSTOOD, HANDLE_OUT_OF_SEQ};
   ...
   * @param initiation The message that must be used to initiate the protocol. Notice that the
   *        default implementation of the <code>prepareInitiations()</code> method returns an array
   ...
   * @param store The <code>DataStore</code> that will be used by this <code>ProposeInitiator</code>
   */
  public ProposeInitiator(Agent a, ACLMessage initiation, DataStore store) {
    ...
    registerTransition(SEND_INITIATIONS, DUMMY_FINAL, 0); // Exit the protocol if no initiation
                                                          // message is sent
  ...
  /**
   * Create and initialize the Sessions and sends the initiation messages. This method is called
   ...
   * state of this protocol. This default implementation just returns the ACLMessage object (a
   * PROPOSE) passed in the constructor. Programmers might prefer to override this method in order
   * to return a vector of PROPOSE objects for 1:N conversations or also to prepare the messages
   * during the execution of the behaviour.
   * 
  ...
  /**
   * This method is called every time an <code>accept-proposal</code> message is received, which is
  ...
  /**
   * This method is called every time an <code>reject-proposal</code> message is received, which is
   * not out-of-sequence according to the protocol rules. This default implementation does nothing;
   * programmers might wish to override the method in case they need to react to this event.
   * 
  ...
  /**
   * This method is called every time a message is received, which is out-of-sequence according to
   * the protocol rules. This default implementation does nothing; programmers might wish to
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the
   * HANDLE_ACCEPT_PROPOSAL state. This behaviour would override the homonymous method. This method
   * also set the data store of the registered <code>Behaviour</code> to the DataStore of this
   * current behaviour. The registered behaviour can retrieve the <code>inform</code> ACLMessage
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the
   * HANDLE_REJECT_PROPOSAL state. This behaviour would override the homonymous method. This method
   * also set the data store of the registered <code>Behaviour</code> to the DataStore of this
   * current behaviour. The registered behaviour can retrieve the <code>inform</code> ACLMessage
   * object received from the datastore at the <code>REPLY_KEY</code> key.
   * 
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the
   * HANDLE_NOT_UNDERSTOOD state. This behaviour would override the homonymous method. This method
   * also set the data store of the registered <code>Behaviour</code> to the DataStore of this
   * current behaviour. The registered behaviour can retrieve the <code>not-understood</code>
   * ACLMessage object received from the datastore at the <code>REPLY_KEY</code> key.
   * 
   * @param b the Behaviour that will handle this state
   */
   ...
   * messages. By response message we intend here all the <code>accept-proposal,
   * reject-proposal, not-understood</code> received messages, which are not not out-of-sequence
   * according to the protocol rules. This default implementation does nothing; programmers might
   * wish to override the method in case they need to react to this event by analysing all the
  ...
  /**
   * Override the onStart() method to initialize the vectors that will keep all the replies in the
   * data store.
   */
  public void onStart() {
   ...
   *        <code>:conversation-id</code> slot, its value is used, else a new conversation
  ...
  protected void adjustReplyTemplate(ACLMessage msg) {
    ...
    if (myAgent.getAID().equals(r)) {
      replyTemplate = MessageTemplate.and(replyTemplate,
