 ...
 * <code>AchieveREResponder</code>. that does not allow to register Behaviour for the Prepare
 * Response and Prepare Result Notification states of the protocol
 *
...
public class SimpleAchieveREResponder extends SimpleBehaviour
   ...
   *         <code>agree, refuse, not-understood, inform</code>. <b>Remind</b> to use the method
   *         createReply of the class ACLMessage in order to create a good reply message
  ...
  /**
   * This method is called after the response has been sent and only when one of the folliwing two
   * cases arise: the response was an <code>agree</code> message OR no response message was sent.
   * This default implementation return null which has the effect of sending no result notification.
   ...
   *         <code>inform, failure</code>. <b>Remind</b> to use the method createReply of the class
   *         ACLMessage in order to create a good reply message
   * @see jade.lang.acl.ACLMessage#createReply()
   ...
   * 
   * @return <code>true</code> if this behaviour has completed its task, <code>false</code>
   *         otherwise.
   */
  public boolean done() {
    return finished;
