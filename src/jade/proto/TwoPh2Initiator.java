...
public class TwoPh2Initiator extends Initiator {
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the vector of ACCEPT_PROPOSAL or
   * REJECT_PROPOSAL messages that have to be sent.
   */
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the vector of ACCEPT_PROPOSAL or
   * REJECT_PROPOSAL messages for which a response has not been received yet.
   */
   ...
   * @param conversationId <code>Conversation-id</code> slot used for all the duration of phase2's
   *        protocol.
   * @param inputKey Data store key where behaviour can get accept-proposal or reject-proposal
   *        messages prepared in the previous phase.
   */
  ...
  public TwoPh2Initiator(Agent a, ACLMessage acceptance, DataStore store) {
     ...
     * a receiver of one of accept/reject-proposal messages sent.
     */
    b = new OneShotBehaviour(myAgent) {
      int ret = -1;
    ...
    /*
     * HANDLE_OLD_RESPONSE state activate if arrived a failure message coming from phase 0 (timeout
     * expired), a disconfirm or inform message coming from phase 1 (timeout expired).
     */
    b = new OneShotBehaviour(myAgent) {
      public void action() {
   ...
   * vector of ACCEPT_PROPOSAL or REJECT_PROPOSAL objects for 1:N conversations.
   * 
   ...
   *         ignored and regenerated automatically by this class. Instead user can specify the
   *         <code>reply-by</code> slot representing phase2 timeout.
   */
   ...
   * <code>inform</code> message is received, which is not out-of-sequence according to the protocol
  ...
  /**
   * This method is called when all the responses have been collected. By response message we intend
   * here all the <code>inform</code> (phase 2), <code>failure</code> (phase 0),
  ...
  /**
   * This method allows to register a user-defined <code>Behaviour</code> in the PREPARE_ACCEPTANCES
   ...
   * to be sent into the datastore at the <code>ALL_ACCEPTANCES_KEY</code> key.
   * 
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the HANDLE_OLD_RESPONSE
   * state. This behaviour would override the homonymous method. This method also set the data store
   * of the registered <code>Behaviour</code> to the DataStore of this current behaviour. The
   * registered behaviour can retrieve the <code>failure, disconfirm or inform</code> ACLMessage
   ...
   * current behaviour. The registered behaviour can retrieve the vector of ACLMessage received from
  ...
  protected String[] getToBeReset() {
    if (toBeReset == null) {
      toBeReset = new String[] {HANDLE_INFORM, HANDLE_OLD_RESPONSE, HANDLE_NOT_UNDERSTOOD,
  ...
  /**
   * Returns vector of accept/reject-proposal stored in the data store at key <code>inputKey</code>
  ...
  /**
   * This method sets for all prepared accept/reject-proposal <code>conversation-id</code> slot
   * (with value passed in the constructor), <code>protocol</code> slot and <code>reply-with</code>
   * slot with a unique value constructed by concatenating receiver's agent name and phase number
  ...
  private void oldResponse(ACLMessage reply) {
    ...
    int perf = reply.getPerformative();
    if (perf == ACLMessage.FAILURE || perf == ACLMessage.NOT_UNDERSTOOD
        || perf == ACLMessage.DISCONFIRM) {
  ...
  protected ProtocolSession getSession(ACLMessage msg, int sessionIndex) {
    ...
    return new Session(
        "R" + hashCode() + "_" + Integer.toString(sessionIndex) + "_" + TwoPhConstants.PH2);
  }
