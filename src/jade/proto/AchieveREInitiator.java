...
/**
 * This is a single homogeneous and effective implementation of all the FIPA-Request-like
 * interaction protocols defined by FIPA, that is all those protocols where the initiator sends a
 * single message (i.e. it performs a single communicative act) within the scope of an interaction
 * protocol in order to verify if the RE (Rational Effect) of the communicative act has been
 ...
 * verify if the expected rational effect of a single communicative act has been achieved.
 * <p>
 * The structure of these protocols is equal. The initiator sends a message (in general it performs
 * a communicative act).
 * <p>
 * The responder can then reply by sending a <code>not-understood</code>, or a <code>refuse</code>
 * to achieve the rational effect of the communicative act, or also an <code>agree</code> message to
 * communicate the agreement to perform (possibly in the future) the communicative act. This first
 * category of reply messages has been here identified as a response.
 * <p>
 * The responder performs the action and, finally, must respond with an <code>inform</code> of the
 * result of the action (eventually just that the action has been done) or with a
 * <code>failure</code> if anything went wrong. This second category of reply messages has been here
 * identified as a result notification.
 * <p>
 * Notice that we have extended the protocol to make optional the transmission of the agree message.
 ...
 * <p>
 * <b>Known bugs:</b> <i> The handler <code>handleAllResponses</code> is not called if the <code>
 * agree</code> message is skipped and the <code>inform</code> message is received instead. <br>
...
public class AchieveREInitiator extends Initiator {
   ...
   *        implementation of the <code>prepareRequest()</code> method returns an array composed of
   *        only this message. The values of the slot <code>reply-with</code> is ignored and a
   *        different value is assigned automatically by this class for each receiver.
   ...
   * state of this protocol. This default implementation just returns the ACLMessage object passed
   * in the constructor. Programmers might prefer to override this method in order to return a
   * vector of objects for 1:N conversations or also to prepare the messages during the execution of
   * the behaviour.
   * 
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the PREPARE_REQUESTS
   * state. This behaviour would override the homonymous method. This method also set the data store
   * of the registered <code>Behaviour</code> to the DataStore of this current behaviour. It is
   * responsibility of the registered behaviour to put the Vector of ACLMessage objects to be sent
   * into the datastore at the <code>ALL_REQUESTS_KEY</code> key. The values of the slot
   * <code>reply-with</code> is ignored and a different value is assigned automatically by this
   * class for each receiver.
   * 
   ...
   * registered <code>Behaviour</code> to the DataStore of this current behaviour. The registered
   * behaviour can retrieve the <code>agree</code> ACLMessage object received from the datastore at
   * the <code>REPLY_KEY</code> key.
   * 
   * @param b the Behaviour that will handle this state
   */
