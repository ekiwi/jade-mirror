...
public class TwoPh0Initiator extends Initiator {
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the vector of CFP messages that have to be
   * sent.
   */
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the vector of PROPOSE messages that have
   * been received as response.
   */
  public final String ALL_PROPOSES_KEY = "__all-proposes" + hashCode();
  /**
   * key to retrieve from the DataStore of the behaviour the vector of CFP messages for which a
   ...
   *        message only.
   * @param outputKey Data store key where the behaviour will store the Vector of messages to be
   *        sent to initiate the successive phase.
   */
   ...
   * to be sent into the datastore at the <code>ALL_CFPS_KEY</code> key.
   * 
   ...
   * current behaviour. The registered behaviour can retrieve the vector of ACLMessage proposes,
   * failures, pending and responses from the datastore at <code>ALL_PROPOSES_KEY</code>,
   * <code>ALL_FAILURES_KEY</code>, <code>ALL_PH0_PENDINGS_KEY</code> and <code>output</code> field.
   * 
   * @param b the Behaviour that will handle this state
   */
  ...
  protected String[] getToBeReset() {
    if (toBeReset == null) {
      toBeReset =
          new String[] {HANDLE_PROPOSE, HANDLE_NOT_UNDERSTOOD, HANDLE_FAILURE, HANDLE_OUT_OF_SEQ};
    }
    return toBeReset;
  }
  ...
  /**
   * This method sets for all prepared cfps <code>conversation-id</code> slot (with value passed in
   * the constructor), <code>protocol</code> slot and <code>reply-with</code> slot with a unique
   * value constructed by concatenating receiver's agent name and phase number (i.e. 0). After that
   * it sends all cfps.
   * 
  ...
  protected ProtocolSession getSession(ACLMessage msg, int sessionIndex) {
    ...
    return new Session(
        "R" + hashCode() + "_" + Integer.toString(sessionIndex) + "_" + TwoPhConstants.PH0);
  }
