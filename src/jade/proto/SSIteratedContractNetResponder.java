...
public class SSIteratedContractNetResponder extends SSContractNetResponder {
  ...
  /**
   * Construct a SSIteratedContractNetResponder that is activated by the reception of a given
   * initiation CFP message and uses a given DataStore.
   */
  public SSIteratedContractNetResponder(Agent a, ACLMessage cfp, DataStore store) {
    ...
    registerTransition(CHECK_IN_SEQ, HANDLE_CFP, ACLMessage.CFP,
        new String[] {HANDLE_CFP, SEND_REPLY, RECEIVE_NEXT});
  }
  ...
  protected void beforeReply(ACLMessage reply) {
    ...
    if (lastReceivedMsg != null
        && lastReceivedMsg.getPerformative() == ACLMessage.ACCEPT_PROPOSAL) {
      // We are sending the reply to the ACCEPT_PROPOSAL --> Jump out and terminate
      // just after sending this reply
