...
public class SSIteratedAchieveREResponder extends SSResponder {
  ...
  /**
   * Construct a SSIteratedAchieveREResponder that is activated by the reception of a given
  ...
  /**
   * Construct a SSIteratedAchieveREResponder that is activated by the reception of a given
   * initiation REQUEST message and uses a given DataStore.
   */
  public SSIteratedAchieveREResponder(Agent a, ACLMessage request, DataStore store) {
    // 4th parameter is false since in this protocol we treat the initiation message
    // exactly as all subsequent incoming messages
    ...
    registerTransition(CHECK_IN_SEQ, HANDLE_REQUEST, initiationPerformative,
        new String[] {HANDLE_REQUEST, SEND_REPLY, RECEIVE_NEXT, CHECK_IN_SEQ});
  ...
  /**
   * This method is called to handle the initial REQUEST message and then again whenever a REQUEST
   * message is received. This default implementation does nothing and returns null. Programmers
   * have to override it to react to this event.
   * 
   * @param request the REQUEST message to handle.
   * @return the reply message to be sent back to the initiator. Returning a message defferent from
   *         INFORM (or returning null) terminates the protocol. An optional AGREE message can be
   *         sent back to the initiator by calling the <code>sendAgree()</code> method.
   * @exception RefuseException if the REQUEST is refused. Throwing a RefuseException has the same
   *            effect as returning a REFUSE message, but automatically manages the
   *            <code>:content</code> slot.
   * @exception FailureException if there is an error serving the REQUEST. Throwing a
   *            FailureException has the same effect as returning a FAILURE message, but
   *            automatically manages the <code>:content</code> slot.
   * @exception NotUnderstoodException if the REQUEST content is not undrerstood. Throwing a
  ...
  /**
   * This method is called when a CANCEL message is received from the initiator. This default
   * implementation does nothing. Programmers may override it to react to this event.
   * 
   * @param cancel the received CANCEL message or null if no further REQUEST message is received
   *        from the initiator within the timeout specified in the <code>:reply-by</code> slot of
   *        the last INFORM message.
   */
  protected void handleCancel(ACLMessage cancel) {}

  /**
   * This method allows to register a user defined <code>Behaviour</code> in the HANDLE_REQUEST
   * state. This behaviour would override the homonymous method. This method also sets the data
   * store of the registered <code>Behaviour</code> to the DataStore of this current behaviour. <br>
   * The registered behaviour can retrieve the received <code>REQUEST</code> message from the
   * datastore at the <code>REQUEST_KEY</code> key. <br>
   ...
   * from INFORM (or null) terminates the protocol. An optional AGREE message can be sent back to
   * the initiator by calling the <code>sendAgree()</code> method.
   * 
   ...
   * state. This behaviour would override the homonymous method. This method also sets the data
   * store of the registered <code>Behaviour</code> to the DataStore of this current behaviour. <br>
   * The registered behaviour can retrieve the <code>CANCEL</code> message received from the
   * datastore at the <code>CANCEL_KEY</code> key. <br>
   * 
   * @param b the Behaviour that will handle this state
   */
  ...
  /**
   * Utility method to send an optional AGREE message back to the initiator ensuring that all
   * protocol fields are properly set.
   */
  ...
  /**
   * Close the ongoing session, as soon as the next INFORM will be sent back to the initiator
   * without the need for an explicit CANCEL message. The initiator will be able to detect that the
   * session has been closed by calling the <code>isSessionTerminated()</code> method of the
  ...
  protected boolean checkInSequence(ACLMessage received) {
    return received.getPerformative() == initiationPerformative
        || received.getPerformative() == ACLMessage.CANCEL;
  }
