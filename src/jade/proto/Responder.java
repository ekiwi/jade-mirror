...
/**
 * Common base class for all classes implementing the Responder role in interaction protocols where
 * the responder is expected to receive more than one message from the initiator and reply to them.
 * 
...
abstract class Responder extends FSMBehaviour {
  ...
  private static class CfpReceiver extends MsgReceiver {
    ...
    public CfpReceiver(Agent myAgent, MessageTemplate mt, long deadline, DataStore s,
        Object msgKey) {
  ...
  private static class NextReceiver extends MsgReceiver {
    ...
    public NextReceiver(Agent myAgent, MessageTemplate mt, long deadline, DataStore s,
        Object msgKey) {
      super(myAgent, mt, deadline, s, msgKey);
    }
   ...
   * @param mt is the MessageTemplate that must be used to match the initiation message. Take care
   ...
   * protocol after a reply has been sent. This default implementation does nothing.
   */
