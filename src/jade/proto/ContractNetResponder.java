...
/**
 * Behaviour class for <code>fipa-contract-net</code> <em>Responder</em> role. This behaviour
 * implements the <code>fipa-contract-net</code> interaction protocol from the point of view of a
 * responder to a call for proposal (<code>cfp</code>) message.
 * <p>
 * The API of this class is similar and homogeneous to the <code>AchieveREResponder</code>.
 * <p>
 * Read also the introduction to <a href="ContractNetInitiator.html">ContractNetInitiator</a> for a
 ...
 * instance the <code>PROPOSE</code> reply message. Any other type of returned communicative act is
 * sent and then closes the protocol.
 * <p>
 * Then, if the initiator accepted the proposal, i.e. if an <code>ACCEPT-PROPOSAL</code> message was
 * received, the callback method <code>prepareResultNotification</code> would be executed that must
 * return the message with the result notification, i.e. <code>INFORM</code> or
 * <code>FAILURE</code>. <br>
 * In alternative, if the initiator rejected the proposal, i.e. if an <code>REJECT-PROPOSAL</code>
 * message was received, the callback method <code>handleRejectProposal</code> would be executed and
...
public class ContractNetResponder extends SSContractNetResponder {
  ...
  /**
   * @deprecated Use either <code>ACCEPT_PROPOSAL_KEY</code> or <code>REJECT_PROPOSAL_KEY</code>
   ...
   *        that if mt is null every message is consumed by this protocol. The best practice is to
   *        have a MessageTemplate that matches the protocol slot; the static method
   *        <code>createMessageTemplate</code> might be usefull.
   ...
   * protocol and the performative) to be passed to the constructor of this behaviour.
   * 
  ...
  public static MessageTemplate createMessageTemplate(String iprotocol) {
    if (CaseInsensitiveString
        .equalsIgnoreCase(FIPANames.InteractionProtocol.FIPA_ITERATED_CONTRACT_NET, iprotocol)) {
      return MessageTemplate.and(
          MessageTemplate.MatchProtocol(FIPANames.InteractionProtocol.FIPA_ITERATED_CONTRACT_NET),
          MessageTemplate.MatchPerformative(ACLMessage.CFP));
    } else if (CaseInsensitiveString
        .equalsIgnoreCase(FIPANames.InteractionProtocol.FIPA_CONTRACT_NET, iprotocol)) {
      return MessageTemplate.and(
