...
/**
 * This behaviour is designed to be used together with the Single-Session responder protocol
 * classes. More in details it is aimed at dealing with protocol initiation messages and dispatching
 * them to responders. The latter are created by means of the <code>createResponder()</code>
 * abstract method that developers must implement.
 * 
...
public abstract class SSResponderDispatcher extends CyclicBehaviour {
  ...
  public SSResponderDispatcher(Agent a, MessageTemplate tpl) {
    ...
    template = MessageTemplate.and(tpl, activeConversations.getMessageTemplate());
  }
