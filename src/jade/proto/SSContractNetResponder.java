...
public class SSContractNetResponder extends SSResponder {
  ...
  /**
   * Key to retrieve from the DataStore of the behaviour the last received ACCEPT_PROPOSAL
   * ACLMessage
   */
  public final String ACCEPT_PROPOSAL_KEY = RECEIVED_KEY;
  /**
   * Key to retrieve from the DataStore of the behaviour the last received REJECT_PROPOSAL
   * ACLMessage
   */
  ...
  /**
   * Construct a SSContractNetResponder that is activated by the reception of a given initiation CFP
   * message.
   */
  ...
  /**
   * Construct a SSContractNetResponder that is activated by the reception of a given initiation CFP
   * message and uses a given DataStore.
   */
  ...
  /**
   * This method is called to handle the initial CFP message. This default implementation does
   * nothing and returns null. Programmers have to override it to react to this event.
   * 
   * @param cfp the initial CFP message to handle.
   * @return the reply message to be sent back to the initiator. Returning a message different than
   *         PROPOSE (or returning null) terminates the protocol.
   * @exception RefuseException if the CFP is refused. Throwing a RefuseException has the same
   *            effect as returning a REFUSE message, but automatically manages the
   *            <code>:content</code> slot.
   * @exception FailureException if there is an error serving the CFP. Throwing a FailureException
   *            has the same effect as returning a FAILURE message, but automatically manages the
   *            <code>:content</code> slot.
   * @exception NotUnderstoodException if the CFP content is not understood. Throwing a
   *            NotUnderstoodException has the same effect as returning a NOT_UNDERSTOOD message,
  ...
  /**
   * This method is called when an ACCEPT_PROPOSAL message is received from the initiator. This
   * default implementation does nothing and returns null. Programmers have to override it to react
   * to this event.
   * 
   ...
   * @exception FailureException if there is an error serving the ACCEPT_PROPOSAL. Throwing a
   *            FailureException has the same effect as returning a FAILURE message, but
  ...
  /**
   * This method is called when a REJECT_PROPOSAL message is received from the initiator. This
   * default implementation does nothing. Programmers may override it to react to this event.
   * 
   * @param cfp the initial CFP message.
   * @param propose the PROPOSE message sent back as reply to the initial CFP message.
   * @param reject the received REJECT_PROPOSAL message or null if no acceptance message is received
   *        from the initiator within the timeout specified in the <code>:reply-by</code> slot of
   *        the PROPOSE message.
   */
  protected void handleRejectProposal(ACLMessage cfp, ACLMessage propose, ACLMessage reject) {}

  /**
   * This method is called whenever a message is received that does not comply to the protocol
   * rules. This default implementation does nothing. Programmers may override it in case they need
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the HANDLE_CFP state.
   * This behaviour would override the homonymous method. This method also sets the data store of
   * the registered <code>Behaviour</code> to the DataStore of this current behaviour. <br>
   * The registered behaviour can retrieve the initial <code>CFP</code> message from the datastore
   * at the <code>CFP_KEY</code> key. <br>
   * It is responsibility of the registered behaviour to put the reply to be sent back to the
   * initiator into the datastore at the <code>REPLY_KEY</code> key. Putting a message defferent
   * from PROPOSE (or putting no message) terminates the protocol.
   * 
   ...
   * The registered behaviour can retrieve the received <code>ACCEPT_PROPOSAL</code> message from
   * the datastore at the <code>ACCEPT_PROPOSAL_KEY</code> key, the initial <code>CFP</code> message
   * at the <code>CFP_KEY</code> and the previously sent <code>PROPOSE</code> message at the
   * <code>PROPOSE_KEY</code>. <br>
   * It is responsibility of the registered behaviour to put the reply to be sent back to the
   * initiator into the datastore at the <code>REPLY_KEY</code> key.
   * 
   * @param b the Behaviour that will handle this state
   */
   ...
   * also sets the data store of the registered <code>Behaviour</code> to the DataStore of this
   * current behaviour. <br>
   * The registered behaviour can retrieve the received <code>REJECT_PROPOSAL</code> message from
   * the datastore at the <code>REJECT_PROPOSAL_KEY</code> key, the initial <code>CFP</code> message
   * at the <code>CFP_KEY</code> and the previously sent <code>PROPOSE</code> message at the
