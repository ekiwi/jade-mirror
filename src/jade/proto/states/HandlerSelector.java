...
/**
 * This class implements a selector of handler (i.e. <code>jade.core.behaviours.Behaviour</code>) A
 * number of handlers can be registered with this <code>HandlerSelector</code>, each handler bound
 * to a different key. The abstract method <code>getSelectionKey<code> is then called that must
 * return the key to select one of the registered handlers. The selected handler is finally
...
public abstract class HandlerSelector extends FSMBehaviour {
  ...
  // States exit values
  /**
   * Value returned by <code>onEnd</code> method if an handler was found mapped to the key
   **/
  public static final int SELECTION_OK = 1;
  /**
   * Value returned by <code>onEnd</code> method if no handler was found mapped to the key
   **/
   ...
   * @param accessKey is the key to get the proper object from the DataStore, this is the object
   *        that will be later passed as argument to the method <code>getSelectionKey</code>
  ...
  /**
   * Subclasses must provide a concrete implementation for this method. It must return the key to
   * which the handler was bound.
   * 
   * @param selectionVar the object retrieved from the datastore at the <code>accessKey</code>
