...
/**
 * This behaviour is a simple implementation of a message receiver. It puts into the given key of
 * the given datastore the received message according to the given message template and timeout. All
 * these data must be passed in the constructor. If the timeout expires before any message arrives,
...
public class MsgReceiver extends SimpleBehaviour {
   ...
   * @param mt the MessageTemplate of the message to be received, if null the first received message
   *        is returned by this behaviour
   * @param deadline a timeout for waiting until a message arrives. It must be expressed as an
   *        absolute time, as it would be returned by <code>System.currentTimeMillisec()</code>
   * @param s the dataStore for this bheaviour
   * @param msgKey the key where the beahviour must put the received message into the DataStore.
   **/
  ...
  /**
   * @return the performative if a message arrived, <code>TIMEOUT_EXPIRED</code> if the timeout
   *         expired or <code>INTERRUPTED</code> if this <code>MsgReceiver</code> was interrupted
  ...
  /**
   * This is invoked when a message matching the specified template is received or the timeout has
   * expired (the <code>msg</code> parameter is null in this case). Users may redefine this method
   ...
   * 
   * @param mt The template to match ACL messages against during the receive operation.
   * @param deadline The relative timeout of the receive operation. If the <code>INFINITE</code>
   *        constant is used, then no deadline is set and the operation will wait until a matching
   *        ACL message arrives.
   * @param s The datastore where the received ACL message is to be put.
   * @param msgKey The key to use to put the received message into the selected datastore.
   */
