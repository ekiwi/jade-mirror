...
abstract class Initiator extends FSMBehaviour {
   ...
   * @param s The <code>DataStore</code> that will be used by this <code>Initiator</code>
   */
  ...
  /**
   * Check the global status of the sessions after the reception of the last reply or the expiration
   * of the timeout
   */
  protected abstract int checkSessions(ACLMessage reply);

  /**
   * Return the states that must be reset before they are visited again. Note that resetting a state
   * before visiting it again is required only if - The onStart() method is redefined - The state
  ...
  // #APIDOC_EXCLUDE_BEGIN
  /**
   * Attach a behaviour to the <code>Prepare-initiations</code> protocol state.
   * 
   * @param b The behaviour object to be executed in the <code>Prepare-initiations</code> state.
   */
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the HANDLE_FAILURE
   ...
   * registered behaviour can retrieve the <code>failure</code> ACLMessage object received from the
  ...
  protected String createConvId(Vector msgs) {
    ...
    if (msgs.size() > 0) {
      ...
      if ((msg == null) || (msg.getConversationId() == null)) {
        convId = "C" + hashCode() + "_" + myAgent.getLocalName() + "_" + System.currentTimeMillis()
            + "_" + getCnt();
      } else {
