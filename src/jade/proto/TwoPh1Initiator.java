...
public class TwoPh1Initiator extends Initiator {
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the Vector of QUERY_IF messages that have
   * to be sent.
   */
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the Vector of all messages that have been
   * received as response.
   */
  public final String ALL_RESPONSES_KEY = "__all-responses" + hashCode();
  /**
   * key to retrieve from the DataStore of the behaviour the Vector of CONFIRM messages that have
   * been received as response.
   */
  public final String ALL_CONFIRMS_KEY = "__all-confirms" + hashCode();
  /**
   * key to retrieve from the DataStore of the behaviour the Vector of DISCONFIRM messages that have
   * been received as response.
   */
  public final String ALL_DISCONFIRMS_KEY = "__all-disconfirms" + hashCode();
  /**
   * key to retrieve from the DataStore of the behaviour the Vector of INFORM messages that have
   * been received as response.
   */
  public final String ALL_INFORMS_KEY = "__all-informs" + hashCode();
  /**
   * key to retrieve from the DataStore of the behaviour the Vector of QUERY_IF messages for which a
   ...
   * @param conversationId <code>Conversation-id</code> slot used for all the duration of phase1's
   *        protocol.
   * @param inputKey Data store key where behaviour can get queryIf messages prepared in the
   *        previous phase.
   * @param outputKey Data store key where the behaviour prepares a vector of messages which will be
   *        send by a <code>TwoPh2Initiator</code> behaviour. If phase 1 ends with all confirm or
   *        inform than messages prepared are <code>accept-proposal</code>, otherwise they are
  ...
  public TwoPh1Initiator(Agent a, ACLMessage queryIf, String outputKey, DataStore store) {
    ...
    /*
     * HANDLE_CONFIRM state activated if arrived a confirm message compliant with conversationId and
     * a receiver of one of queryIf messages sent.
     */
    ...
    /*
     * HANDLE_DISCONFIRM state activated if arrived a disconfirm message compliant with
    ...
    /*
     * HANDLE_INFORM state activated if arrived an inform message compliant with conversationId and
     * a receiver of one of queryIf messages sent.
     */
    b = new OneShotBehaviour(myAgent) {
      public void action() {
    ...
    /*
     * HANDLE_ALL_RESPONSES state activated when timeout is expired or all the answers have been
     * received.
     */
    b = new OneShotBehaviour(myAgent) {
      public void action() {
        ...
        handleAllResponses(responses, confirms, disconfirms, informs, pendings, nextPhMsgs);
      }
    };
    b.setDataStore(getDataStore());
   ...
   * QUERY_IF) passed in the constructor. Programmers might prefer to override this method in order
   ...
   * @return a Vector of ACLMessage objects. The value of the <code>reply-with</code> slot is
   *         ignored and regenerated automatically by this class. Instead user can specify
  ...
  /**
   * This method is called every time a <code>confirm</code> message is received, which is not
  ...
  /**
   * This method is called every time a <code>disconfirm</code> message is received, which is not
   * out-of-sequence according to the protocol rules. This default implementation does nothing;
   * programmers might wish to override the method in case they need to react to this event.
   * 
   ...
   * expired. The used timeout is the minimum value of the slot <code>reply-By</code> of all the
   * sent messages. By response message we intend here all the
   * <code>disconfirm, confirm, inform</code> received messages, which are not out-of-sequence
   ...
   * @param responses prepared responses for next phase: <code>accept-proposal</code> or
   *        <code>reject-proposal</code>
   */
  ...
  /**
   * This method allows to register a user-defined <code>Behaviour</code> in the PREPARE_QUERYIFS
   ...
   * responsibility of the registered behaviour to put the <code>Vector</code> of ACLMessage objects
   * to be sent into the datastore at the <code>ALL_QUERYIFS_KEY</code> key.
   * 
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the HANDLE_CONFIRM
   * state. This behaviour would override the homonymous method. This method also set the data store
   * of the registered <code>Behaviour</code> to the DataStore of this current behaviour. The
   * registered behaviour can retrieve the <code>confirm</code> ACLMessage object received from the
  ...
  /**
   * This method allows to register a user defined <code>Behaviour</code> in the HANDLE_DISCONFIRM
   * state. This behaviour would override the homonymous method. This method also set the data store
   * of the registered <code>Behaviour</code> to the DataStore of this current behaviour. The
   * registered behaviour can retrieve the <code>disconfirm</code> ACLMessage object received from
   * the datastore at the <code>REPLY_KEY</code> key.
   * 
   * @param b the Behaviour that will handle this state
   */
   ...
   * current behaviour. The registered behaviour can retrieve the vector of ACLMessage confirms,
   * disconfirms, informs, pending and responses from the datastore at
   * <code>ALL_CONFIRMS_KEY</code>, <code>ALL_DISCONFIRMS_KEY</code>, <code>ALL_INFORMS_KEY</code>,
   * <code>ALL_PH1_PENDINGS_KEY</code> and <code>output</code> field.
   * 
   * @param b the Behaviour that will handle this state
   */
  ...
  protected String[] getToBeReset() {
    if (toBeReset == null) {
      toBeReset = new String[] {HANDLE_CONFIRM, HANDLE_DISCONFIRM, HANDLE_INFORM,
  ...
  /**
   * This method sets for all prepared queryIfs <code>conversation-id</code> slot (with value passed
   * in the constructor), <code>protocol</code> slot and <code>reply-with</code> slot with a unique
   * value constructed by concatenating receiver's agent name and phase number (i.e. 1). After that
   * it sends all cfps.
   * 
  ...
  /**
   * Check whether a reply is in-sequence and than update the appropriate Session and removes
  ...
  private void fillNextPhInitiations(Vector nextPhMsgs, Vector confirms, Vector informs,
      Vector pendings) {
    if ((confirms.size() + informs.size()) == totSessions) {
      ...
      // At least one responder disconfirmed, failed or didn't reply --> Fill the
      // vector
  ...
  protected ProtocolSession getSession(ACLMessage msg, int sessionIndex) {
    ...
    return new Session(
        "R" + hashCode() + "_" + Integer.toString(sessionIndex) + "_" + TwoPhConstants.PH1);
  }
