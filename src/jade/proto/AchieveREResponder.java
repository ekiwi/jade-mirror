...
public class AchieveREResponder extends FSMBehaviour implements FIPANames.InteractionProtocol {
  ...
  /**
   * key to retrieve from the DataStore of the behaviour the ACLMessage object sent as a result
  ...
  public static MessageTemplate createMessageTemplate(String iprotocol) {
    ...
    if (CaseInsensitiveString.equalsIgnoreCase(FIPA_REQUEST, iprotocol))
      return MessageTemplate.and(MessageTemplate.MatchProtocol(FIPA_REQUEST),
          MessageTemplate.MatchPerformative(ACLMessage.REQUEST));
    else if (CaseInsensitiveString.equalsIgnoreCase(FIPA_QUERY, iprotocol))
      return MessageTemplate.and(MessageTemplate.MatchProtocol(FIPA_QUERY),
   ...
   * FIPARequestResponder will react to and reset the protocol.
   */
  ...
  /**
   * This method is called when the protocol initiation message (matching the MessageTemplate
   * specified in the constructor) is received. This default implementation returns null which has
   * the effect of sending no reponse. Programmers should override this method in case they need to
   * react to this event.
   * 
  ...
  /**
   * This method is called after the execution of the handleRequest() method if no response was sent
   * or the response was an <code>AGREE</code> message. This default implementation returns null
   * which has the effect of sending no result notification. Programmers should override the method
   ...
   * state. This behaviour would override the homonymous method. This method also set the DataStore
   * of the registered <code>Behaviour</code> to the DataStore of this AchieveREResponder. It is
   ...
   * PREPARE_RESULT_NOTIFICATION state. This behaviour would override the homonymous method. This
   * method also set the DataStore of the registered <code>Behaviour</code> to the DataStore of this
   * AchieveREResponder. It is responsibility of the registered behaviour to put the result
   * notification message to be sent into the DataStore at the <code>RESULT_NOTIFICATION_KEY</code>
   * key.
   * 
   * @param b the Behaviour that will handle this state
   */
