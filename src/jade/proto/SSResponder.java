...
/**
 * Common base class for all classes implementing the Single Session version of the Responder role
 * in interaction protocols where the responder is expected to receive more than one message from
 * the initiator and reply to each of them.
 * 
 * @author Giovanni Caire - TILAB
 */
abstract class SSResponder extends FSMBehaviour {
  /**
   * Key to retrieve from the DataStore of the behaviour the initiation ACLMessage that triggered
  ...
  /**
   * Key to set into the DataStore of the behaviour the new ACLMessage to be sent back to the
   * initiator as a reply.
   */
  ...
  public SSResponder(Agent a, ACLMessage initiation, DataStore store, boolean useInitiationKey) {
    ...
    registerDefaultTransition(HANDLE_OUT_OF_SEQUENCE, RECEIVE_NEXT,
        new String[] {HANDLE_OUT_OF_SEQUENCE});
  ...
  /**
   * This method can be redefined by protocol specific implementations to customize a reply that is
   * going to be sent back to the initiator. This default implementation does nothing.
   */
  protected void beforeReply(ACLMessage reply) {}

  /**
   * This method can be redefined by protocol specific implementations to update the status of the
   * protocol just after a reply has been sent. This default implementation does nothing.
   */
  protected void afterReply(ACLMessage reply) {}

  /**
   * This method can be redefined by protocol specific implementations to take proper actions after
   * the completion of the current protocol session.
   */
  ...
  private static class NextReplySender extends ReplySender {
    ...
    private MessageTemplate createNextMsgTemplate(ACLMessage reply) {
      return MessageTemplate.and(MessageTemplate.MatchConversationId(reply.getConversationId()),
