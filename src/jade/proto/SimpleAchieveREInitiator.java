...
/**
 * This is simple implementation of the AchieveREInitiator. This implementation in particular is 1:1
 * and does not allow the possibility to add Handler.
 * 
 ...
 * FIPA-query, FIPA-Request-When, FIPA-recruiting, FIPA-brokering, FIPA-subscribe, that allows the
 * initiator to verify if the expected rational effect of a single communicative act has been
 * achieved.
 * <p>
 ...
 * agree</code> message is skipped and the <code>inform</code> message is received instead.
 *
...
public class SimpleAchieveREInitiator extends SimpleBehaviour {
   ...
   * @param msg The message that must be used to initiate the protocol. Notice that in this simple
   *        implementation, the <code>prepareMessage</code> method returns a single message.
        ...
        if (request == null) {
          ...
          if (r.equals(myAgent.getAID())) {
            ...
            mt = MessageTemplate.and(mt,
                MessageTemplate.not(MessageTemplate.MatchCustom(request, true)));
          }
  ...
  /**
   * This method must return the ACLMessage to be sent. This default implementation just return the
   * ACLMessage object passed in the constructor. Programmer might override the method in order to
   * return a different ACLMessage. Note that for this simple version of protocol, the message will
   ...
   * expired. By response message we intend here all the <code>agree, not-understood,
  ...
  /**
   * This method resets this behaviour so that it restarts from the initial state of the protocol
   * with a null message.
   */
  public void reset() {
    reset(null);
  ...
  /**
   * This method resets this behaviour so that it restarts the protocol with another request
   * message.
   * 
