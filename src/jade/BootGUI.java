...
public class BootGUI extends JDialog {
  ...
  public BootGUI(Boot3 theBooter) {
    ...
    for (Enumeration it = propertiesVector.elements(); it.hasMoreElements();) {
        ...
        if (param.equals("Run")) {
          ...
          try {
            ...
            if (propertyFileName != null) {
              ...
              ExtendedProperties p = readPropertiesFromFile(propertyFileName);
      ...
      for (int i = 0; (i < size) && !found; i++) {
        ...
        if (name.equalsIgnoreCase(label.getText())) {
          ...
          if (type.equalsIgnoreCase(PropertyType.COMBO_TYPE)) {
            ...
            // JComboBox
            JComboBox box = (JComboBox) singlePanel.getComponent(1);

            out.setProperty(name.toLowerCase(), box.getSelectedItem().toString());
          }
            ...
            out.setProperty(name.toLowerCase(), (new Boolean(box.isSelected())).toString());
          } else {
   ...
   */
  boolean compareProperties(ExtendedProperties p1, ExtendedProperties p2) {
  ...
  /**
   * Update the gui when a new file is opened. For every property it sets in the vector of property
   * the value read in the file or if it is absent the default value.
   */
  ...
  void loadPropertiesFromFile(String fileName) throws FileNotFoundException, IOException {
    ...
    // for every property set the value read in the file and set the command line
    // value to null.
   ...
   */
  ExtendedProperties readPropertiesFromFile(String fileName)
  ...
  Vector createPropertyVector(ExtendedProperties theProperties) {
    ...
    pv.add(new PropertyType(BootProfileImpl.LOGIN_KEY, PropertyType.COMBO_TYPE, loginEnum,
        theProperties.getProperty(BootProfileImpl.LOGIN_KEY), "User Authentication context",
        false));

    pv.add(new PropertyType(BootProfileImpl.MAIN_HOST, PropertyType.STRING_TYPE,
        theProperties.getProperty(BootProfileImpl.MAIN_HOST), "Host Name of the main-container",
        false));

    pv.add(new PropertyType(BootProfileImpl.GUI_KEY, PropertyType.BOOLEAN_TYPE,
        new Boolean(theProperties.getBooleanProperty(BootProfileImpl.GUI_KEY, false)).toString(),
        "Select to launch the RMA Gui", false));

    pv.add(new PropertyType(BootProfileImpl.MAIN_PORT, PropertyType.STRING_TYPE,
        new Integer(
            theProperties.getIntProperty(BootProfileImpl.MAIN_PORT, BootProfileImpl.DEFAULT_PORT))
                .toString(),
        "Port Number of the main-container", false));

    pv.add(new PropertyType(BootProfileImpl.NAME_KEY, PropertyType.STRING_TYPE,
        theProperties.getProperty(BootProfileImpl.NAME_KEY), "The symbolic plaform name", false));

    pv.add(new PropertyType(BootProfileImpl.CONTAINER_KEY, PropertyType.BOOLEAN_TYPE,
        new Boolean(theProperties.getBooleanProperty(BootProfileImpl.CONTAINER_KEY, false))
            .toString(),
        "Select to launch an agent-container", false));

    pv.add(new PropertyType(BootProfileImpl.MTP_KEY, PropertyType.STRING_TYPE,
        theProperties.getProperty(BootProfileImpl.MTP_KEY), "List of MTPs to activate", false));

    pv.add(new PropertyType(BootProfileImpl.NOMTP_KEY, PropertyType.BOOLEAN_TYPE,
        new Boolean(theProperties.getBooleanProperty(BootProfileImpl.NOMTP_KEY, false)).toString(),
        "Disable all external MTPs on this container", false));

    pv.add(new PropertyType(BootProfileImpl.ACLCODEC_KEY, PropertyType.STRING_TYPE,
        theProperties.getProperty(BootProfileImpl.ACLCODEC_KEY), "List of ACLCodec to install",
        false));

    pv.add(new PropertyType(BootProfileImpl.AGENTS, PropertyType.STRING_TYPE,
        theProperties.getProperty(BootProfileImpl.AGENTS), "Agents to launch", false));
    pv.add(new PropertyType(BootProfileImpl.NOMOBILITY_KEY, PropertyType.BOOLEAN_TYPE,
        new Boolean(theProperties.getBooleanProperty(BootProfileImpl.NOMOBILITY_KEY, false))
            .toString(),
        "Disable Mobility", false));
    return pv;
  }
