 ...
 * compliance with the FIPA specifications. Copyright (C) 2000 CSELT S.p.A. GNU Lesser General
 * Public License This library is free software; you can redistribute it and/or modify it under the
...
public class RMIIMTPManager implements IMTPManager {
  ...
  /**
   * This constant is the name of the property whose value contains the TCP port where a local
   * Service Manager is to be exported (when using JADE fault-tolerant deployment).
   */
  public static final String LOCAL_SERVICE_MANAGER_PORT = "smport";

  /**
   * This constant is the name of the property whose value contains the TCP port where the local RMI
   * Node is to be exported.
   */
  public static final String LOCAL_NODE_PORT = "nodeport";

  /**
   * This constant is the name of the property whose boolean value indicates whether or not RMI logs
   * have to be enabled (default false)
   */
  ...
  public void initialize(Profile p) throws IMTPException {
    ...
    // If this is a backup main, initialize the RMI address of the original
    // PlatformManager
    ...
    if (myLogger.isLoggable(Logger.CONFIG)) {
      myLogger.log(Logger.CONFIG,
          "IMTP parameters: main-host=" + mainHost + " main-port=" + mainPort + " local-host="
              + localHost + " local-port=" + localPort + " sm-port=" + localSvcMgrPort
              + " node-port=" + localNodePort);
    }

    // Configure Java runtime system to put the selected local-host address in RMI
    // messages (e.g. a fully qualified name instead of a local name)
    ...
    // Create the local node and (if needed) mark it as hosting a local Service
    // Manager
    try {
      localNode = new NodeAdapter(PlatformManager.NO_NAME,
          myProfile.getBooleanProperty(Profile.MAIN, true), localNodePort, this);
  ...
  /**
   * Get the RMIRegistry. If a registry is already active on this host and the given portNumber,
   * then that registry is returned, otherwise a new registry is created and returned.
   * 
  ...
  private Registry getRmiRegistry(String host, int portNumber) throws RemoteException {
    ...
    try {
      ...
      rmiRegistry.list();
      myLogger.log(Logger.CONFIG,
          "Local RMI Registry on port " + portNumber + " already exists. Use it");
    } catch (Exception exc) {
  ...
  public void exportPlatformManager(PlatformManager mgr) throws IMTPException {
    try {
      ...
      if (originalPMAddr != null) {
        try {
          ...
          myLogger.log(Logger.INFO,
              "Connecting to master Main Container at address " + originalPMAddr);
          ...
          if ((t != null) && (t instanceof ConnectException)) {
            ...
            myLogger.log(Logger.INFO, "No master Main Container found at address " + originalPMAddr
                + ". Take the leadership");
            originalPMAddr = null;
      ...
      System.out.println(
          "  Make sure the specified host name or IP address belongs to the local machine.");
      System.out
      ...
      System.out
          .println("  Use the '-container' option, then use '-host' and '-port' to specify the ");
      ...
      throw new IMTPException(
          "Communication failure while starting JADE Runtime System. Check if the RMIRegistry CLASSPATH includes the RMI Stub classes of JADE.",
          re);
  ...
  public Service.Slice createSliceProxy(String serviceName, Class itf, Node where)
  ...
  public List getLocalAddresses() throws IMTPException {
    try {
      ...
      TransportAddress addr = new RMIAddress(InetAddress.getLocalHost().getHostName(),
          String.valueOf(localPort), null, null);
      l.add(addr);
      return l;
    } catch (Exception e) {
  ...
  /**
   * Inner class PlatformManagerAdapter. An adapter, implementing the PlatformManager interface, to
