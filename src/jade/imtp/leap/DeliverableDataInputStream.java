 ...
 * environment and functionalities that JADE provides. Copyright (C) 2001 Siemens AG.
 * 
...
/**
 * This class implements a data input stream deserializing Deliverables from a given byte array
 * according to the LEAP surrogate serialization mechanism. In order to function correctly, the byte
 * array must contain serialized Deliverables, primitive types or J2SE class types, that are
 * considered (by this stream class) as primitive types.
 *
...
class DeliverableDataInputStream extends DataInputStream {
  ...
  /**
   * Constructs a data input stream that is deserializing Deliverables from the given byte array
   * according to the LEAP surrogate serialization mechanism.
   * 
   * @param serialized_byte_array the byte array this data input stream deserializes Deliverables
   *        from
   */
  ...
  /**
   * Reads an object whose class is not known from the context from this data input stream.
   * 
   * @return the object that has been read.
   * @exception LEAPSerializationException if an error occurs during deserialization or the object
   *            is an instance of a class that cannot be deserialized.
   */
   ...
   * @exception LEAPSerializationException if an error occurs during deserialization.
   */
   ...
   */
  /*
   * public Long readLongObject() throws LEAPSerializationException { try { boolean presenceFlag =
   * readBoolean();
   * 
   * if (presenceFlag) { return new Long(readLong()); } else { return null; } } catch (IOException
   * e) { throw new LEAPSerializationException("Error deserializing Long"); } }
   */
  ...
  /**
   * Note that when delivering messages to agents we never deal with ACLMessage objects since they
   * are ALWAYS encoded/decoded by the Messaging filters. However there may be services that use
   * ACLMessage objects as parameters in their HCommands
   */
  ...
  private GenericMessage deserializeGenericMessage()
  ...
  // #J2ME_EXCLUDE_BEGIN
  private MultipleGenericMessage deserializeMultipleGenericMessage()
      throws IOException, LEAPSerializationException {
  ...
  public ContainerID[] deserializeContainerIDArray()
      throws IOException, LEAPSerializationException {
  ...
  private ServiceDescriptor deserializeServiceDescriptor() throws LEAPSerializationException {
    ...
    try {
      ...
      throw new LEAPSerializationException("Cannot deserialize ServiceDescriptor: class "
          + className + " for service " + name + " not found");
    } catch (Throwable t) {
  ...
  private SliceProxy deserializeSliceProxy() throws LEAPSerializationException {
    ...
    try {
      ...
      throw new LEAPSerializationException(
          "Cannot deserialize SliceProxy: class " + className + " not found");
    } catch (Throwable t) {
