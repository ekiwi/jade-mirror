...
/**
 * helper class that holds the ByteBuffers, SSLEngine and other Objects that deal with the non
 * static part of the ssl/nio handshaking/input/output. The contained SSLEngine is hidden to apps,
 * because this helper takes on the responsibility of dealing with concurrency issues
 * 
 * @author eduard
 */
public final class SSLEngineHelper implements BufferTransformer {
  ...
  public SSLEngineHelper(String host, int port, NIOJICPConnection connection) throws ICPException {
    ...
    if (!SSLHelper.needAuth()) {
      // if we don't do authentication we restrict our ssl connection to use specific
      // cipher suites
  ...
  public synchronized void close() throws IOException {
    /*
     * try to nicely terminate ssl connection 1 closeoutbound 2 send ssl close message 3 wait for
     * client to also send close message 4 close inbound 5 don't let all this frustrate the channel
     * closing
     */
    if (ssle != null) {
  ...
  private SSLEngineResult unwrapData(ByteBuffer socketData) throws SSLException {
    ...
    do {
      try {
        // Unwrap (decode) the next SSL block. Unwrapped application data (if any) are
        // put into the unwrapData buffer
        result = ssle.unwrap(socketData, unwrapData);
        if (log.isLoggable(Level.FINE))
          log.fine("Decoded " + result.bytesConsumed() + " bytes; Produced "
              + result.bytesProduced() + " application-data bytes [" + getRemoteHost() + "]");
  ...
  private void checkStatusAfterHandshakeTasks(SSLEngineResult.HandshakeStatus handshakeStatus)
      throws SSLException, IOException {
    if (handshakeStatus.equals(HandshakeStatus.FINISHED)) {
      log.warning("Unexpected FINISHED SSL handshake status after execution of handshake tasks ["
          + getRemoteHost() + "]");
    } else if (handshakeStatus.equals(HandshakeStatus.NEED_TASK)) {
      log.warning("Unexpected NEED_TASK SSL handshake status after execution of handshake tasks ["
          + getRemoteHost() + "]");
      ...
      log.warning(
          "Unexpected NOT_HANDSHAKING SSL handshake status after execution of handshake tasks ["
              + getRemoteHost() + "]");
    }
  }
  ...
  private synchronized int decrypt(ByteBuffer socketData) throws SSLException, IOException {
    if (log.isLoggable(Level.FINE)) {
      log.fine("Decrypt incoming data: remaining = " + socketData.remaining() + ", position = "
          + socketData.position() + ", limit = " + socketData.limit() + " [" + getRemoteHost()
          + "]");
    }
    ...
    if (status.equals(Status.OK)) {
      if (handshakeStatus.equals(HandshakeStatus.NOT_HANDSHAKING)) {
        ...
        // The SSLEngine requires one or more "tasks" to be executed before the
        ...
        // This should never happen since we cannot exit the unwrapData() method with
        // status == OK and handshakeStatus == NEED_UNWRAP
        throw new SSLException(
            "Unexpected NEED_UNWRAP SSL handshake status! [" + getRemoteHost() + "]");
      } else if (handshakeStatus.equals(HandshakeStatus.NEED_WRAP)) {
      ...
      // There is not enough data in the socketData buffer to unwrap a meaningful SSL
      // block -->
      // Wait for next data from the socket
      if (log.isLoggable(Level.FINE)) {
        log.fine("Not enough data to decode a meaningful SSL block. " + socketData.remaining()
            + " unprocessed bytes. [" + getRemoteHost() + "]");
      }
      // Avoid entering an infinite recursion trying to continuously decode bytes
      // still present in socketData (that are
      // not sufficient to unwrap a meaningful SSL block)
      return n;
    } else if (status.equals(Status.BUFFER_OVERFLOW)) {
      // The unwrapData buffer is too small to hold all unwrapped data --> Repeat with
      // a larger buffer
      if (log.isLoggable(Level.FINE)) {
      ...
      unwrapData.flip();
      unwrapData = NIOHelper.enlargeAndFillBuffer(unwrapData,
          BEManagementService.getBufferIncreaseSize(), "unwrapData");
   ...
   * {@link NIOJICPConnection#writeToChannel(java.nio.ByteBuffer) }, possibly recursive when
   * required by handshaking
   * 
  ...
  public synchronized ByteBuffer preprocessBufferToWrite(ByteBuffer dataToSend) throws IOException {
    ...
    if (ssle == null) {
      throw new IllegalStateException(
          "SSLEngine previously closed. The connection must be reestablished.");
    }
    ...
    while (dataToSend.hasRemaining()) {
      ...
      if (res.getStatus().equals(Status.BUFFER_OVERFLOW)) {
        wrapData.flip();
        wrapData = NIOHelper.enlargeAndFillBuffer(wrapData,
            BEManagementService.getBufferIncreaseSize(), "wrapData");
      }
    }
    wrapData.flip();
    return wrapData;
  ...
  public synchronized ByteBuffer postprocessBufferRead(ByteBuffer socketData)
      throws PacketIncompleteException, IOException {
