...
public class NIOHTTPBEDispatcher implements NIOMediator, Dispatcher, BEConnectionManager {
  ...
  public void init(JICPMediatorManager mgr, String id, Properties props) throws ICPException {
    ...
    myLogger.log(Logger.INFO,
        "Created NIOHTTPBEDispatcher V1.0. ID = " + myID + "\n- Max-disconnection-time = "
            + maxDisconnectionTime + "\n- Keep-alive-time = " + keepAliveTime);
    myLogger.log(Logger.CONFIG,
        myID + " - Next command for FE will have SID = " + nextOutgoingCommandSid);
  ...
  private final BackEndSkel startBackEndContainer(Properties props) throws ICPException {
    try {
      ...
      myLogger.log(Logger.CONFIG,
          myID + " - BackEndContainer " + myID + " successfully joined the platform.");
   ...
   * CONNECT_MEDIATOR request is received. In both cases the Front-end is connecting --> Set the
   * front-end status to CONNECTING, but don't use this connection: to allow us sending commands to
   * the front-end an initial dummy response will be received soon.
   */
  public synchronized boolean handleIncomingConnection(Connection c, JICPPacket pkt,
      ...
      if (pkt.getType() == JICPProtocol.COMMAND_TYPE) {
        ...
        if ((pkt.getInfo() & JICPProtocol.TERMINATED_INFO) != 0) {
          ...
          if (sid == lastIncomingCommandSid && lastResponse != null) {
            ...
            response =
                new JICPPacket(JICPProtocol.RESPONSE_TYPE, JICPProtocol.DEFAULT_INFO, rspData);
  ...
  /**
   * Termination initiated by the BackEndContainer (i.e. by the platform). When the BackEndContainer
   * exits
   */
  ...
  public synchronized byte[] dispatch(byte[] payload, boolean flush, int oldSessionId)
    ...
    if (status == ACTIVE) {
      if (frontEndStatus == CONNECTED) {
        ...
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE,
              myID + " - Delivering outgoing command to front-end. SID = " + sid);
        }
        try {
          c.writePacket(cmd);
          close(c);
          ...
          JICPPacket response =
              getResponse(RESPONSE_TIMEOUT + RESPONSE_TIMEOUT_INCREMENT * (cmd.getLength() / 1024));
          if (myLogger.isLoggable(Logger.FINE)) {
    ...
    if (status == ACTIVE) {
      if (frontEndStatus == CONNECTED) {
        try {
          ...
          myLogger.log(Logger.WARNING,
              myID + " - Error delivering keep-alive packet to the front-end", ioe);
          setFrontEndDisconnected();
        } catch (ICPException icpe) {
          // Note that in this case setFrontEndDisconnected() is already called within
          // getResponse() or getOutgoingCommandsConnection()
  ...
  private void handleResponse(Connection c, JICPPacket response, String from) {
    if (frontEndStatus != CONNECTED) {
      ...
      if (isKeepAliveResponse(response)) {
        ...
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE,
              myID + " - Response received. SID = " + response.getSessionID());
        }
      }
      synchronized (responseToLastOutgoingCommandLock) {
