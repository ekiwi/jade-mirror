...
/**
 * This class implements the FrontEndDispatcher related BackEnd dispatcher managable by an
 * asynchronous JICPMediatorManager
 * 
...
public class BackEndDispatcher implements NIOMediator, BEConnectionManager, Dispatcher {
  ...
  public void init(JICPMediatorManager mgr, String id, Properties props) throws ICPException {
    ...
    try {
      responseTimeoutOffset =
          Long.parseLong(props.getProperty(JICPProtocol.RESPONSE_TIMEOUT_OFFSET_KEY));
    } catch (Exception e) {
      // Keep default
    }

    // Response timeout multiplicative factor
    responseTimeoutMultiplicativeFactor =
        JICPProtocol.DEFAULT_RESPONSE_TIMEOUT_MULTIPLICATIVE_FACTOR;
    try {
     ...
     * (Integer.parseInt(props.getProperty("outcnt")) -1); if (lastSid < 0) { lastSid = 0x0f; } }
     * catch (Exception e) { // Keep default } myLogger.log(Logger.INFO,
  ...
  /**
   * Notify this NIOMediator that an error occurred on one of the Connections it is using. This
  ...
  /**
   * Passes to this mediator a JICPPacket received by the JICPMediatorManager this mediator is
   * attached to. In a NIOMediator this should never be called.
   */
    ...
    switch (type) {
      ...
      case JICPProtocol.ERROR_TYPE:
        myLogger.log(Logger.INFO,
            "BE " + myID + " - RESPONSE/ERROR received: " + pkt.getSessionID());
  ...
  /**
   * Request the FE to refresh the connection. This default implementation does nothing. Subclasses
   * may redefine this method to exploit some application specific out-of-band channel
   */
      ...
      try {
        if (active && isConnected()) {
          ...
          try {
            ...
            if (myLogger.isLoggable(Logger.FINE)) {
              myLogger.log(Logger.FINE, "[Thread=" + Thread.currentThread().getName() + "] BE "
                  + myID + " - Sending command to FE " + pkt.getSessionID());
            }
            writePacket(myConnection, pkt);
            if (myLogger.isLoggable(Logger.FINE)) {
              myLogger.log(Logger.FINE, "[Thread=" + Thread.currentThread().getName() + "] BE "
                  + myID + " - Waiting for response from FE " + pkt.getSessionID());
            }

            long responseTimeout = responseTimeoutOffset
                + (long) (responseTimeoutMultiplicativeFactor * pkt.getLength());
            pkt = waitForResponse(inpCnt, responseTimeout);
            if (pkt != null) {
              if (myLogger.isLoggable(Logger.FINE)) {
                myLogger.log(Logger.FINE, "[Thread=" + Thread.currentThread().getName() + "] BE "
                    + myID + " - Response received from FE " + pkt.getSessionID());
              }
              ...
              myLogger.log(Logger.WARNING, "[Thread=" + Thread.currentThread().getName() + "] BE "
                  + myID + ": Response timeout expired");
              handleConnectionError(myConnection, null);
  ...
  /**
   * Inner class OutputManager This class manages the reception of commands and keep-alive packets
   * from the FrontEnd.
   */
  protected class OutputManager {
    ...
    final JICPPacket handleCommand(JICPPacket cmd) throws ICPException {
      ...
      if (sid == lastSid && lastResponse != null) {
        myLogger.log(Logger.WARNING, myID + ": Duplicated packet from FE: pkt-type=" + cmd.getType()
            + " info=" + cmd.getInfo() + " SID=" + sid);
        reply = lastResponse;
      } else {
        if (myLogger.isLoggable(Logger.FINE)) {
