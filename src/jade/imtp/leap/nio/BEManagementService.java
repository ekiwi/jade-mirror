...
/**
 * This service handles BEContainer creation requests and manages IO operations for data exchanged
 * between the created BEContainers and their FEContainers in an asynchronous way using java.nio
 * <br>
 * <br>
 * Having this functionality implemented as a service allows propagating (through the ServiceHelper)
 * BackEnd related events (e.g. disconnections and reconnections) at the agent level. <br>
 * <br>
 * This service accepts the following configuration parameters:<br>
 * <code>jade_imtp_leap_nio_BEManagementService_servers</code>: list of of IOEventServer ids
 * separated by ';'<br>
 * Actually this service is a collection of IOEventServer objects. Each IOEventServer opens and
 * manages a server socket, accepts BackEnd creation/connection requests and passes incoming data to
 * BEDispatchers independently from the others. If the above parameter is not specified a single
 * IOEventServer will be started and its ID will be
 * <code>jade_imtp_leap_nio_BEManagementService</code>. All other parameters are related to a single
 * IOEventServer and must be specified in the form<br>
 * serverid_parametername<br>
 * They are:
 * <ul>
 * <li><code>serverid_local-host</code>: Specifies the local network interface for the server socket
 * managed by this server (defaults to localhost).</li>
 * <li><code>serverid_local-port</code>: Specifies the local port for the server socket managed by
 * this server (defaults to 2099)</li>
 * <li><code>serverid_protocol</code>: Specifies the protocol used by this server in the form of
 * <code>jade.imtp.leap.JICP.ProtocolManager<code> 
class
</li>
<li>
<code>serverid_additional-services</code>: Specifies additional services that have to be activated
 * in each BackEnd Container started by the indicated server</li>
 * <li><code>serverid_leap-property-file</code>: Specifies the leap-property file to be used by this
 * server.</li>
 * <li><ode>serverid_poolsize</code>: Specifies the number of threads used by this server to manage
 * IO events.</li>
 * <ul>
 * 
...
public class BEManagementService extends BaseService {
  ...
  /**
   * this property can be used to set how many bytes will be used when nio buffer need to be
   * increased
   */
    ...
    if (acceptOption != null) {
      ...
      for (int i = 0; i < acceptedProtocols.size(); ++i) {
        ...
        if (proto.equalsIgnoreCase(MicroRuntime.SOCKET_PROTOCOL)
          ...
          myLogger.log(Logger.WARNING, "Unsupported protocol " + proto
              + ". Permitted values are socket, ssl, http and https!!!!");
        }
      }
  ...
  /**
   * This method is called by the JADE runtime just after this service has been installed. It
   * activates the IO event servers specified in the Profile and the Ticker thread.
   */
  ...
  private void initializeSAM() {
    try {
      ...
      if (sam != null) {
        ...
        // sec
        samHelper.addCounterValueProvider("Processing_Time_GT1Sec_Count",
            new CounterValueProvider() {
        ...
        samHelper.addCounterValueProvider("Processing_Time_GT10Sec_Count",
            new CounterValueProvider() {
              public long getValue() {
        ...
        // Error counters
        samHelper.addCounterValueProvider("Incoming_Packet_Reading_Error_Count",
            new CounterValueProvider() {
              public long getValue() {
        ...
        samHelper.addCounterValueProvider("Incoming_Packet_Serving_Error_Count",
            new CounterValueProvider() {
              public long getValue() {
  ...
  /**
   * This is called by the JADE Runtime when this service is deinstalled. It stops the Ticker thread
   * and all active IO event servers.
   */
  ...
  /**
   * Inner class IOEventServer. This class asynchronously manages a server socket and all IO Events
   * that happen on it and on all sockets opened through it. The BEManagementService is basically a
   * collection of these servers.
   */
  private class IOEventServer
      implements PDPContextManager, PDPContextManager.Listener, JICPMediatorManager {
    ...
    public void init(String id, Profile p) {
       ...
       *
       * Perhaps a NIOJICPServer should do the Selector handling, now that is controlled by
       * instances of a inner class LoopManager. Than a NIOJICPPeer controls this server and
       * provides the ConnectionFactory
       *
      ...
      try {
        ...
        myLogger.log(Logger.CONFIG, myLogPrefix + "Can't read LEAP property file " + fileName
            + ". Keep default. [" + e + "]");
      ...
      if (pdpContextManagerClass != null) {
        try {
          ...
          myPDPContextManager =
              (PDPContextManager) Class.forName(pdpContextManagerClass).newInstance();
    ...
    public synchronized void shutdown() {
      ...
      try {
        ...
        myLogger.log(Logger.WARNING,
            myLogPrefix + "Interrupted while waiting for LoopManager to termnate", ie);
      }
    ...
    /**
     * Serve an IO event conveied by a SelectionKey (a JICPPacket or an IO exception). This method
     * is executed by one of the threads in the server Thread pool.
     */
    public void servePacket(KeyManager mgr, JICPPacket pkt) {
      ...
      try {
        switch (type) {
          ...
          case JICPProtocol.GET_ADDRESS_TYPE: {
            ...
            myLogger.log(Logger.INFO, myLogPrefix + "GET_ADDRESS request received from " + address
                + ":" + port + ". Port-request=" + (pkt.getData() != null));
          ...
          case JICPProtocol.GET_CONFIG_OPTIONS_TYPE: {
            ...
            myLogger.log(Logger.INFO, myLogPrefix
                + "GET_CONFIGURATION_OPTIONS request received from " + address + ":" + port);
          ...
          case JICPProtocol.CREATE_MEDIATOR_TYPE: {
            ...
            if (mediator == null) {
              ...
              try {
                ...
                myLogger.log(Logger.WARNING,
                    myLogPrefix
                        + "Security error! CREATE_MEDIATOR request from non authorized client ["
          ...
          case JICPProtocol.CONNECT_MEDIATOR_TYPE: {
            ...
            if (mediator == null) {
              myLogger.log(Logger.INFO, myLogPrefix + "CONNECT_MEDIATOR request received from "
                  + address + ":" + port + ". ID=" + recipientID);
               ...
               * myPDPContextManager.getPDPContextInfo(address, "OWNER???"); if (pdpContextInfo !=
               * null) { String msisdn = pdpContextInfo.getProperty(PDPContextManager.MSISDN); if
               * (msisdn == null || !msisdn.equals(recipientID)) { myLogger.log(Logger.WARNING,
               * myLogPrefix+"Security attack! CONNECT_MEDIATOR request with mediator-id != MSISDN. Address is: "
               * +address); reply = new JICPPacket(JICPProtocol.NOT_AUTHORIZED_ERROR, null); break;
               * } } else { myLogger.log(Logger.WARNING,
              ...
              if (mediator != null && mediator.getID() != null) {
                configureBlocking(pkt, key);
                closeConnection =
                    !mediator.handleIncomingConnection(connection, pkt, address, port);
                if (!closeConnection) {
                  // The mediator wants to keep this connection open --> associate
                  // it to the current key
                  mgr.setMediator(mediator);
                ...
                myLogger.log(Logger.WARNING,
                    myLogPrefix + "Mediator " + recipientID + " not found");
          ...
          default: {
            ...
            if (mediator != null) {
              if (myLogger.isLoggable(Logger.FINEST)) {
                myLogger.log(Logger.FINEST, myLogPrefix + "Passing packet of type " + type
                    + " to mediator " + mediator.getID());
              }
              reply = mediator.handleJICPPacket(connection, pkt, address, port);
            } else {
              myLogger.log(Logger.WARNING,
                  myLogPrefix + "No mediator for incoming packet of type " + type);
              if (type == JICPProtocol.COMMAND_TYPE) {
                reply = new JICPPacket(JICPProtocol.NOT_FOUND_ERROR, null);
              }
            }
        ...
        myLogger.log(Logger.WARNING,
            myLogPrefix + stringify(mediator) + "Error handling incoming packet. ", e);
      ...
      if (reply != null) {
        try {
          ...
          incomingPacketServingErrorCounter++;
          myLogger.log(Logger.WARNING,
              myLogPrefix + stringify(mediator) + "Communication error writing return packet to "
                  + address + ":" + port + " [" + e + "]",
              e);
      ...
      try {
        if (closeConnection) {
          try {
            // Close connection
            if (myLogger.isLoggable(Logger.FINEST)) {
              myLogger.log(Logger.FINEST, myLogPrefix + stringify(mediator)
                  + "Closing connection with " + address + ":" + port);
            }
            connection.close();
    ...
    public void serveException(KeyManager mgr, Exception e) {
      ...
      if (!connection.isClosed()) {
        ...
        if (mediator != null) {
          ...
          InetAddress address = s.getInetAddress();
          int port = s.getPort();
          myLogger.log(Logger.WARNING, myLogPrefix + "Exception reading incoming packet from "
              + address + ":" + port + " [" + e + "]");
        }
      }
    ...
    /**
     * Called by a Mediator to notify that it is no longer active. This is often called within the
     * tick() method. In this case directly removing the deregistering mediator from the mediators
     * table would cause a ConcurrentModificationException --> We just add the deregistering
     * mediator to a queue of mediators to be removed. The actual remotion will occur at the next
     * tick in a synchronized way.
     */
        ...
        if (!lm.isStuck()) {
          if (lm.getReadElapsedTime(currentTime) > 60000) {
            ...
            if (lmThread.isInterrupted()) {
              ...
              myLogger.log(Logger.WARNING,
                  "LM-" + lm.myIndex
                      + " did not recover after last interrupt --> Mark it as STUCK. "
    ...
    ///////////////////////////////////////
    /**
     * If no PDPContextManager is specified in the properties file the IOEventServer itself is used
     * and the default behaviour is to issue an INCOMING_CONNECTION outgoing command
     */
      ...
      try {
        ...
        if (ret != null) {
          if (ret instanceof Properties) {
            ...
            myLogger.log(Logger.FINER, myLogPrefix + "PDPContextProperties for address " + addr
                + " owner " + owner + " = " + ret);
            ...
            myLogger.log(Logger.WARNING, myLogPrefix
                + "Error retrieving PDPContextPropert for address " + addr + " owner " + owner,
                (Throwable) ret);
    ...
    protected NIOMediator startMediator(String id, Properties p) throws Exception {
      ...
      myLogger.log(Logger.INFO,
          myLogPrefix + "Initializing mediator " + id + " with properties " + p);
      m.init(this, id, p);
      return m;
    }
  ...
  /**
   * Inner class KeyManager Keep a SelectionKey together with the information associated to it such
   * as the connection wrapping the key channel and the mediator using that connection (if any).
   */
  private class KeyManager {
    ...
    /**
     * Read some data from the connection associated to the managed key and let the IOEventServer
     * serve it
     */
    public final void read() {
      ...
      try {
        ...
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE, "Incomplete JICPPacket (" + pie.getMessage()
              + ") from connection " + connection + ". Wait for more data..");
        }
      } catch (Exception e) {
  ...
  private class LoopManager implements Runnable {
      ...
      while (state == ACTIVE_STATE) {
        ...
        try {
          ...
          if (state == ACTIVE_STATE) {
            myLogger.log(Logger.SEVERE, myServer.getLogPrefix() + "Error selecting next IO event. ",
                e);
            // Abort
            state = ERROR_STATE;
          }
    ...
    /**
     * JDK 1.6 in Linux has a BUG so that Selector.select() in certain cases do not block causing an
     * infinite loop. The workaround is to recreate the Selector. Actually we recreate the whole
     * LoopManager
     */
    ...
    private final void handleAcceptOp(SelectionKey key, String prefix) {
      try {
        ...
        lm.register(sc);
        myLogger.log(Logger.INFO, prefix + "Incoming socket " + sc + " assigned to LM-" + lm.myIndex
            + " (" + lm.size() + ")");
      } catch (JADESecurityException jse) {
        myLogger.log(Logger.WARNING,
            prefix + "Connection attempt from malicious address " + jse.getMessage());
      } catch (NotFoundException nfe) {
        myLogger.log(Logger.SEVERE,
            prefix + "NO LoopManager available to handle incoming socket!!!!");
      } catch (Exception e) {
    ...
    private synchronized final void handlePendingChannels(String prefix) {
      if (pendingChannelPresent) {
        for (int i = 0; i < pendingChannels.size(); ++i) {
          ...
          if (myLogger.isLoggable(Logger.FINE)) {
            myLogger.log(Logger.FINE, prefix + "Registering Selector " + mySelector + " on channel "
                + sc + " for READ operations");
          }
          try {
            sc.register(mySelector, SelectionKey.OP_READ);
          } catch (Exception e) {
            myLogger.log(Logger.WARNING,
                prefix + "Error registering socket channel for asynchronous IO. ", e);
          }
        }
  ...
  /**
   * Inner class BEManagementHelperImpl. Implementation class for the BEManagementHelper.
   */
  ...
  /**
   * Check that the address of the initiator of a new connection is not in the list of addresses
