...
/**
 * Base interface for all mediators that can be managed by asynchronous JICPMediatorManagers such as
 * the <code>BEManagementService</code>.
 * 
...
public interface NIOMediator extends JICPMediator {
   ...
   */
  JICPPacket handleJICPPacket(Connection c, JICPPacket p, InetAddress addr, int port)
      throws ICPException;

  /**
   * Notify this NIOMediator that an error occurred on one of the Connections it was using. This
   * information is important since, unlike normal mediators, a NIOMediator may not read packets
   * from connections on its own (the JICPMediatorManager does that in general).
   */
