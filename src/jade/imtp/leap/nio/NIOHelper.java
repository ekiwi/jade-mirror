...
public class NIOHelper {
  ...
  public static void logBuffer(ByteBuffer b, String name, Level l) {
    if (log.isLoggable(l)) {
      log.log(l, "bufferinfo {0}: pos {1}, rem {2}, lim {3}, cap {4}",
          new Object[] {name, b.position(), b.remaining(), b.limit(), b.capacity()});
    }
  }

  /**
   * copy data from src to dst, as much as fits in dst. src's position() will be moved when data are
   * copied.
   * 
   * @param src copy from
   * @param dst copy to
  ...
  public static int copyAsMuchAsFits(ByteBuffer dst, ByteBuffer src) {
    if (dst.hasRemaining() && src.hasRemaining()) {
      ...
      if (log.isLoggable(Level.FINE)) {
        log.fine("bytes copied to dst " + (dst.position() - pos) + ", bytes left in src "
            + src.remaining());
        logBuffer(src, "src");
        logBuffer(dst, "dst");
      }
