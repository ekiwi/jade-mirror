...
public class NIOJICPConnection extends Connection {
  ...
  /**
   * Read a JICPPacket from the connection. The method is synchronized since we reuse the same
   * Buffer object for reading the packet header. It should be noted that the packet data may not be
   * completely available when the embedded channel is ready for a READ operation. In that case a
   * PacketIncompleteException is thrown to indicate that successive calls to this method must occur
  ...
  public synchronized JICPPacket readPacket() throws IOException {
    ...
    if (unmanagedJicpData == null) {
      // No JICP data to be processed from previous round --> Read new data from the
      // network
      read();
      jicpData = transformAfterRead(socketData);
      // NOTE: Transformers are expected to transform ALL (not just the first one)
      // units they
      // are able to manage --> At this point we have:
      // - socketData empty!
      // - unprocessedData inside transformers (see the BufferTransformerInfo inner
      // class)
      // possibly containing some bytes representing a portion of a transformation
      // unit (more
      ...
      // 3) Exactly 1 JICPPacket or the remaining part of the payload in case at
      // previous
      // round we fell in case 2 or 4.b
      // 4) Case 3 plus 0 or more complete JICPPacket plus, optionally, the first part
      // of a JICPPacket (case 4.b)
      // In case 1 and 2 the method throws PacketIncompleteException.
      // In case 3 the method returns the reconstructed JICPPacket.
      // In case 4 the method returns the first reconstructed packet and store
      // remaining JICP
      // data. When that occurs the moreDataAvailable() method returns true to
      // indicate that
      // the readPacket() method must be called again before going back to wait for
      // network data.
    } else {
    ...
    try {
      if (jicpData.hasRemaining()) {
        ...
        if (!headerReceived) {
          ...
          if ((info & JICPProtocol.DATA_PRESENT_INFO) != 0) {
            ...
            if (payloadLength > JICPPacket.MAX_SIZE) {
              throw new IOException(
                  "Packet size greater than maximum allowed size. " + payloadLength);
            }

            resizePayloadBuffer(payloadLength);

            // jicpData likely already contains some payload bytes --> copy them into the
            // payload buffer
            ...
            if (payloadBuf.hasRemaining()) {
              ...
              throw new PacketIncompleteException(
                  "Missing " + payloadBuf.remaining() + " payload bytes");
            } else {
          ...
          // We are in the middle of reading the payload of a packet (the previous call to
          // readPacket() resulted in a PacketIncompleteException)
          NIOHelper.copyAsMuchAsFits(payloadBuf, jicpData);
          if (payloadBuf.hasRemaining()) {
            // Payload not completely received. Wait for next round
            throw new PacketIncompleteException(
        ...
        throw new PacketIncompleteException(
            socketData.limit() + " bytes read from the network. No JICP data transformed");
      }
    } finally {
      if (unmanagedJicpData != null && !unmanagedJicpData.hasRemaining()) {
        // If we just fully processed some previous-round-unmanaged data, reset the
        // unmanagedJicpData buffer
  ...
  private void read() throws IOException {
    ...
    while (!socketData.hasRemaining()) {
      // We read exactly how many bytes how socketData can contain. VERY likely there
      // are
      // more bytes to read from the channel --> Enlarge socketData and read again
      socketData.flip();
      socketData = NIOHelper.enlargeAndFillBuffer(socketData,
          BEManagementService.getBufferIncreaseSize(), "socketData");
      try {
        readFromChannel(socketData);
  ...
  private ByteBuffer transformAfterRead(ByteBuffer incomingData) throws IOException {
    ...
    for (ListIterator<BufferTransformerInfo> it = transformers.listIterator(transformers.size()); it
        .hasPrevious();) {
      BufferTransformerInfo info = it.previous();
      BufferTransformer btf = info.getTransformer();

      // In case there were unprocessed data at previous round, append them before the
      // data to be processed at this round
      transformationInput = info.attachUnprocessedData(transformationInput);

      if (log.isLoggable(Level.FINER))
        log.finer("--------- Passing " + transformationInput.remaining() + " bytes to Transformer "
            + btf.getClass().getName());
      transformationOutput = btf.postprocessBufferRead(transformationInput);
      if (log.isLoggable(Level.FINER))
        log.finer("--------- Transformer " + btf.getClass().getName() + " did not transform "
            + transformationInput.remaining() + " bytes");

      // In case the transformer did not process all input data, store unprocessed
      // data for next round
      info.storeUnprocessedData(transformationInput);

      // Output of transformer N becomes input of transformer N-1 (transformers are
      // scanned in reverse order when managing incoming data)
      transformationInput = transformationOutput;

      if (!transformationInput.hasRemaining() && it.hasPrevious()) {
        // No bytes for next transformation --> no need to continue scanning
        // transformers
        break;
      }
    }
   ...
   * {@link #preprocessBufferToWrite(java.nio.ByteBuffer) }. When the buffer returned by
   * {@link #preprocessBufferToWrite(java.nio.ByteBuffer) }, no write will be performed.
   * 
  ...
  public synchronized int writePacket(JICPPacket pkt) throws IOException {
    ...
    if (toSend.hasRemaining() && transformers.size() > 0) {
      ...
      // for subclasses the subsequent transformers must transform all data from the
    ...
    if (log.isLoggable(Level.FINE)) {
      log.fine("writePacket: total bytes written " + totalWrited + ", total needed to write: "
          + totalToWrite);
    }
  ...
  // In some cases we may receive some data (often a socket closed by peer
  // indication) while
  // closing the channel locally. Trying to read such data results in an
  // Exception. To avoid printing
  ...
  /**
   * Inner class BufferTransformerInfo This class keeps together a BufferTransformer and a
   * ByteBuffer holding data already received but not yet processed by that transformer. Such data
   * will be used (together with newly received data) at next transformation attempt
   */
  private class BufferTransformerInfo {
    ...
    public void storeUnprocessedData(ByteBuffer transformationInput) {
      if (transformationInput.hasRemaining()) {
        // System.out.println("######## Storing "+transformationInput.remaining()+"
        // bytes for next round");
    ...
    public ByteBuffer attachUnprocessedData(ByteBuffer transformationInput) {
      ...
      if (unprocessedData != null && unprocessedData.hasRemaining()) {
        // System.out.println("######## Attaching "+unprocessedData.remaining()+"
        // unprocessed bytes");
        actualTransformationInput = NIOHelper.enlargeAndFillBuffer(unprocessedData,
            transformationInput.remaining(), "unprocessedData");
