...
/**
 * This class implements the BIFEDispatcher related BackEnd dispatcher managable by an asynchronous
 * JICPMediatorManager
 * 
...
public class NIOBEDispatcher implements NIOMediator, BEConnectionManager, Dispatcher {
  ...
  protected class InputManager {
    ...
    /**
     * Dispatch a JICP command to the FE and get back a reply. This method must NOT be executed in
     * mutual exclusion with setConnection() and resetConnection() since it performs a blocking read
     * operation --> It can't just be declared synchronized.
     */
    ...
    /*
     * Asynch-reply final synchronized void handleResponse(Connection c, JICPPacket reply) throws
     * ICPException { checkConnection(c); currentReply = reply; notifyAll(); }
     * 
     * private synchronized JICPPacket waitForReply(long timeout) throws ICPException, IOException {
     * try { if (currentReply == null) { wait(timeout); if (currentReply == null) { if
     * (isConnected()) { if (connectionRefreshed) { throw new ICPException("Connection refreshed");
     * } else { throw new IOException("Response timeout expired"); } } else { throw new
     * ICPException("Connection reset"); } } } JICPPacket tmp = currentReply; currentReply = null;
     * return tmp; } catch (InterruptedException ie) { throw new ICPException("Interrupted"); } }
     */
   ...
   * from the FrontEnd. This class also manages the maxDisconnectionTime, i.e. the remote FrontEnd
   * is considered dead if it cannot re-establish the OUT connection within the
   * maxDisconnectionTime.
   */
