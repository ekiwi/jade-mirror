...
public class NIOHTTPSConnection extends NIOHTTPConnection {
  ...
  /**
   * need to set a HTTPHelper here, that can do wrapping of JICPPacket in HTTPPacket and unwrapping
   * the other way
   * 
   * @param channel
   * @throws ICPException
  ...
  void init(SocketChannel channel) throws ICPException {
    super.init(channel);
    helper = new SSLEngineHelper(channel.socket().getInetAddress().getHostAddress(),
