 ...
 * environment and functionalities that JADE provides. Copyright (C) 2001 Motorola.
 * 
...
/**
 * This class is the JICP data packet representation along with methods for reading from and writing
 * to dataXXputStreams.
 * 
...
public class JICPPacket {
  ...
  /**
   * An optional field indicating the actual recipient for this JICPPacket. - A JICPServer receiving
   * a JICPPacket from a remote container interprets this field as the ID of a local Mediator. - A
   * Mediator receiving a JICPPacket from its mediated container interprets this field as the
  ...
  /**
   * constructs a JICPPacket of type JICPProtocol.ERROR_TYPE and sets the data to the string
  ...
  public byte[] getData() {
    ...
    // return (info & JICPProtocol.COMPRESSED_INFO) != 0 ?
    // JICPCompressor.decompress(data) : data;
    // }
    // else {
    return data;
    // }
  ...
  /**
   * Writes the packet into the provided <code>OutputStream</code>. The packet is serialized in an
   * internal representation, so the data should be retrieved and deserialized with the
   * <code>readFrom()</code> static method below. The output stream is flushed but not opened nor
   * closed by this method.
   * 
   * @param out The <code>OutputStream</code> to write the data in
   * @exception May send a large bunch of exceptions, mainly in the IO package.
   */
  ...
  /**
   * This static method reads from a given <code>DataInputStream</code> and returns the JICPPacket
   * that it reads. The input stream is not opened nor closed by this method.
   * 
  ...
  public static JICPPacket readFrom(InputStream in) throws IOException {
    ...
    if ((p.info & JICPProtocol.DATA_PRESENT_INFO) != 0) {
      ...
      // Logger.println("JICPPacket read. Type:"+p.type+" Info:"+p.info+"
      // RID:"+p.recipientID+" Data-length:"+(p.data != null ? p.data.length : 0));
    }
