...
public class NATUtils {
  ...
  public InetSocketAddress[] getNATMapping(final String localAddress, final int localPort,
    ...
    try {
      ...
      JICPPacket pkt =
          new JICPPacket(JICPProtocol.GET_ADDRESS_TYPE, JICPProtocol.DEFAULT_INFO, new byte[] {0});
      con.writePacket(pkt);
      InetSocketAddress internalEndPoint =
          new InetSocketAddress(con.getLocalHost(), con.getLocalPort());
      ...
      if (rsp.getType() != JICPProtocol.ERROR_TYPE) {
        ...
        if (data != null) {
          ...
          if (ss.length == 2) {
            try {
              InetSocketAddress externalEndPoint =
  ...
  public static void main(String[] args) {
    ...
    // }
    // };
    // JICPPacket pkt = new JICPPacket(JICPProtocol.GET_ADDRESS_TYPE, JICPProtocol.DEFAULT_INFO, new
    // byte[]{0});
    // con.writePacket(pkt);
    // pkt = con.readPacket();
