...
public class FrontEndDispatcher
    implements FEConnectionManager, Dispatcher, TimerListener, Runnable {
  ...
  /*
   * private void manageRemoteConfig() throws IMTPException { String remoteConfigHost =
   * myProperties.getProperty(MicroRuntime.REMOTE_CONFIG_HOST_KEY); String remoteConfigPort =
   * myProperties.getProperty(MicroRuntime.REMOTE_CONFIG_PORT_KEY); if (remoteConfigHost != null &&
   * remoteConfigPort != null) { // Remote configuration options specified: Retrieve connectivity
   * related // configurations from the indicated host and port JICPConnection c = getConnection(new
   * JICPAddress(remoteConfigHost, remoteConfigPort, myMediatorID, "")); JICPPacket pkt = new
   * JICPPacket(JICPProtocol.GET_CONFIG_OPTIONS_TYPE, JICPProtocol.DEFAULT_INFO, null);
   * writePacket(pkt, c); pkt = c.readPacket(); c.close(); ... String replyMsg = new
   * String(pkt.getData()); if (pkt.getType() != JICPProtocol.ERROR_TYPE) { // BackEnd creation
   * successful BackEndStub.parseCreateMediatorResponse(replyMsg, myProperties);
   * 
   * } }
   */
  ...
  private void initServerTimeOffset(Connection con) {
    JICPPacket pkt =
        new JICPPacket(JICPProtocol.GET_SERVER_TIME_TYPE, JICPProtocol.DEFAULT_INFO, null);
    ...
    try {
      ...
      if (rsp.getType() != JICPProtocol.ERROR_TYPE) {
        ...
        myLogger.log(Logger.INFO, "Server time initialized: value=" + serverTime + ", offset="
            + offset + ", uncertanty= +/- " + uncertanty);
      } else {
  ...
  public synchronized byte[] dispatch(byte[] payload, boolean flush, int oldSessionId)
    ...
    if (connectionDropped) {
      myLogger.log(Logger.INFO,
          myMediatorID + " - Dispatching with connection dropped. Reconnecting...");
      undrop();
    }
    ...
    if (myConnection != null) {
      ...
      try {
        ...
        myLogger.log(Logger.WARNING,
            myMediatorID + " - Error writing command. SID = " + pkt.getSessionID(), ioe);
        handleDisconnection();
  ...
  /**
   * Inner class ConnectionReader. This class is responsible for reading incoming packets (incoming
   * commands and responses to outgoing commands)
   */
    ...
    private JICPPacket handleIncomingPacket(JICPPacket pkt) {
      switch (pkt.getType()) {
        case JICPProtocol.COMMAND_TYPE:
          myLogger.log(getLevel(Logger.FINE), myMediatorID + " - CR-" + myId
              + " COMMAND received from BE, SID=" + pkt.getSessionID());
          updateTimers();
          serveCommand(pkt);
          myLogger.log(getLevel(Logger.FINEST), myMediatorID + " - CR-" + myId
              + " Incoming command passed to asynchronous command server");
          break;
        ...
        case JICPProtocol.ERROR_TYPE:
          myLogger.log(getLevel(Logger.FINER), myMediatorID + " - CR-" + myId
              + " RESPONSE/ERROR received from BE. " + pkt.getSessionID());
          notifyOutgoingResponseReceived(pkt);
          break;
        default:
          myLogger.log(Logger.WARNING,
              myMediatorID + " - Unexpected incoming packet type: " + pkt.getType());
      }
      return null;
    }
  ...
  private synchronized void writePacket(JICPPacket pkt, Connection c) throws IOException {
    ...
    /*
     * else { updateKeepAlive(); if (pkt.getType() != JICPProtocol.KEEP_ALIVE_TYPE && pkt.getType()
     * != JICPProtocol.DROP_DOWN_TYPE) { updateConnectionDropDown(); } }
     */
  }
      ...
      try {
        while (lastOutgoingResponse == null) {
          ...
          if (lastOutgoingResponse != null
            ...
            myLogger.log(Logger.WARNING,
                "Wrong sessionID in response from BE: type=" + lastOutgoingResponse.getType()
                    + " info=" + lastOutgoingResponse.getInfo() + " SID="
                    + lastOutgoingResponse.getSessionID() + " while " + sessionID
                    + " was expected.");
            // Go back waiting
            lastOutgoingResponse = null;
            continue;
    ...
    while (active) {
      ...
      if ((System.currentTimeMillis() - startTime) > maxDisconnectionTime) {
        handleReconnectionError(
            "Max disconnection time expired (" + System.currentTimeMillis() + ")");
        return;
      }
  ...
  private void connect(int attemptCnt) throws IOException, IMTPException, ICPException {
    myLogger.log(Logger.INFO, myMediatorID + " - Connecting to " + mediatorTA.getHost() + ":"
        + mediatorTA.getPort() + " " + attemptCnt);
  ...
  /**
   * Clear the keep-alive and drop-down timers. Mutual exclusion with updateKeepAlive(),
  ...
  /**
   * Refresh the keep-alive timer. Mutual exclusion with clearTimers(), updateConnectionDropDown()
   * and doTimeOut()
   */
  ...
  /**
   * Refresh the connection drop-down timer. Mutual exclusion with clearTimers(), updateKeepAlive()
   * and doTimeOut()
   */
  ...
  /**
   * Send a KEEP_ALIVE packet to the BE. Mutual exclusion with dispatch() and dropDownConnection().
   */
  ...
  /**
   * Send a DROP_DOWN packet to the BE and close the connection. Mutual exclusion with dispatch()
   * and sendKeepAlive().
   */
  ...
  /**
   * Inner class IncomingCommandServer Serving incoming commands asynchronously is necessary to
   * support commands whose serving process involves issuing one or more outgoing commands. If such
   * commands were served directly by the ConnectionReader thread, in facts, there would be no
   * chance to get the response to triggered outgoing commands.
   */
  private class IncomingCommandServer extends Thread {
    ...
    public void run() {
      ...
      while (active) {
        myLogger.log(getLevel(Logger.FINEST),
            myMediatorID + " - CS Waiting for next command to serve");
        acquireCurrentCommand();

        byte sid = currentCommand.getSessionID();
        myLogger.log(getLevel(Logger.FINE),
            myMediatorID + " - CS Start serving command, SID=" + sid);
        if (sid == lastSid) {
        ...
        try {
          ...
          myLogger.log(Logger.WARNING,
              myMediatorID + " - Communication error sending back response. " + e);
        }
