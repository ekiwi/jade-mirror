 ...
 * Copyright (C) 2001 Broadcom Eireann Research. Copyright (C) 2001 Motorola.
 * 
...
public class JICPPeer implements ICP, ProtocolManager {
  ...
  public TransportAddress activate(ICP.Listener l, String peerID, Profile p) throws ICPException {
    ...
    TransportAddress localTA = getProtocol().buildAddress(server.getLocalHost(),
        String.valueOf(server.getLocalPort()), null, null);
  ...
  public byte[] deliverCommand(TransportAddress ta, byte[] payload, boolean requireFreshConnection)
      throws ICPException {
    byte[] respPayload =
        client.send(ta, JICPProtocol.COMMAND_TYPE, payload, requireFreshConnection);
  ...
  /**
   * Pings the specified transport address in order to obtain the local hostname or IP address.
   * 
   * @param pingURL The <code>URL</code> to ping (usually the main container).
   * @return The local IP address of the local container as a <code>String</code>.
   * 
   * @throws ICPException
   */
  String getAddress(String pingURL) throws ICPException {
    ...
    try {
      ...
      throw new ICPException(
          "JICP GET_ADDRESS error. Cannot retrieve local hostname: " + icpe.getMessage());
    }
  ...
  protected ServerSocket getServerSocket(String host, int port, boolean changePortIfBusy)
      throws ICPException {
    try {
      return new ServerSocket(port, 50, (host != null ? InetAddress.getByName(host) : null));
    } catch (SocketException be) {
      // HACK! We should do this only in case of a BindException. However some
      // implementations
      // of the JVM (particularly that for Windows 7 64 bits) seem to have a bug and
      // throw a
      ...
      if (changePortIfBusy) {
        ...
        throw new ICPException("Cannot bind server socket to "
            + (host != null ? "host " + host : "localhost") + " port " + port);
      }
