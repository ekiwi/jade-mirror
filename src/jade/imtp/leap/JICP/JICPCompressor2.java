 ...
 * Copyright (C) 2001 Broadcom Eireann Research. Copyright (C) 2001 Siemens AG.
 *
...
class JICPCompressor2 {
  ...
  public static byte[] compress(byte[] data) {
    ...
    try {
      ...
      for (int i = 0; i != numWords; i = next(i)) // looking through the words; the first word is
                                                  // for sure not
                                                  // compressed
      {
        if (!dataEnded) {
        ...
        int j;
        for (j = (startFlag && (i < 126)) ? (0) : (back(i, 126)); j != i; j = next(j)) // trying to
                                                                                       // find the
                                                                                       // occurance
                                                                                       // of the
                                                                                       // current
                                                                                       ...
                                                                                       // words
        {
          ...
          while ((words[i] + charCounter < dataSize - 1) && // if the position with which we are
                                                            // comparing
          // is not yet achieved the end of the array
              (words[j] + charCounter < words[i]) && // and the search position not yet achieved
              // the current position
              (data[words[i] + charCounter] == data[words[j] + charCounter])) // and the characters
                                                                              // are
                                                                              // the same
          {
            charCounter++;
          ...
          if (countWords > 0) // the correspondance was found
          {
            if ((countWords == 1) && (words[next(i)] - words[i] <= 2)) {
              ...
              stream.write(-back(i, j)); // offset (backward) of the position where to find this
                                         // word
      ...
      /**************************
       * System.out.print("Uncompressed: "); System.out.print(dataSize); System.out.print("
       * Compressed: "); System.out.println(result.length); System.out.flush(); //
       *************************/
      // System.out.println("" + dataSize + "->" + result.length + " = " +
      // (result.length*100)/dataSize+"%");
      return result;
    } catch (Exception e) {
  ...
  public static byte[] decompress(byte[] data) {
    if (data == null)
      return null;
    int dataSize = data.length;
    int outputSize = ((data[0] >= 0) ? (data[0]) : (256 + data[0])) * 256
        + ((data[1] >= 0) ? (data[1]) : (256 + data[1]));
    ...
    try {
      ...
      while (i < dataSize) {
        if ((data[i] < 0) && (data[i] > -127)) {
          int wordsCount =
              (((i + 1 < dataSize) && (data[i + 1] < 0) && (data[i + 1] > -127)) ? (-data[i + 1])
                  : 1);
          int symCount = words[sum(back(numWords, (-data[i])), wordsCount)]
              - words[back(numWords, (-data[i]))];
          int k;
          for (k = 0; k < symCount; k++) {
            output[j + k] = output[words[back(numWords, (-data[i]))] + k];
          }
          for (k = 0; k < wordsCount; k++)
            words[sum(numWords, k)] =
                words[sum(back(numWords, (-data[i])), k)] - words[back(numWords, (-data[i]))] + j;
          j += symCount;
          ...
          while ((i < dataSize) && (!is_separator(data[i]))) // limit ourself to the case where all
                                                             // the
                                                             // symbols have codes < 128
          {
            output[j] = data[i];
            i++;
            j++;
          }
          while ((i < dataSize) && (is_separator(data[i])) && ((data[i] >= 0) || (data[i] == -127))) // copying
                                                                                                     // the
                                                                                                     // separators
          {
            if (data[i] == -127)
