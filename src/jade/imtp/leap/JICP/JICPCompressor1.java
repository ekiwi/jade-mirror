...
public class JICPCompressor1 {
   ...
   * only a magic byte (WORD_MAGIC) and the index of the word in the wordIndexes[] array is stored
   * as a byte. the magic byte is encoded as (WORD_MAGIC, 255). this limits the number of words that
   * are possible inside the wordIndexes[] array to 254 (=MAX_WORD constant).
   *
   */
  ...
  private byte[] compressHelper(byte[] uba) {
    ...
    /*
     * // do the remaining bytes for (int j = beginIndex; j<ba.length; ++j) { int ch =
     * (int)((char)ba[j] & 255); rleWrite(baos, ch); if (ch == WORD_MAGIC) { rleWrite(baos, 255); }
   ...
   * compressed array, decode it and look for words. if a magic byte (WORD_MAGIC) is detected, it
   * will be decoded to a "real" WORD_MAGIC if the following byte is 255 (WORD_MAGIC, 255 -
   * sequence). if the following byte is not 255, it is treated as an index in the wordIndexes[]
   * array. the index stored inside there is used as the beginning of a word. this word will be
   * copied to the end of the decoded stream. the end of the word is detected by a separator (see
   * isSep()).
   */
