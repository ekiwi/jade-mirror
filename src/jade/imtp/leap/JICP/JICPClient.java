...
class JICPClient {
   ...
   * @throws ICPException
   */
  public byte[] send(TransportAddress ta, byte dataType, byte[] data,
      boolean requireFreshConnection) throws ICPException {
    ...
    while (true) {
      try {
        ...
        DeliveryTracing.setTracingInfo("Network-waiting-time",
        ...
        if (!cw.isReused()) {
          throw new ICPException(
              "I/O error sending/receiving data to " + ta.getHost() + ":" + ta.getPort(), ioe);
        }
        ...
        throw new ICPException(
            "Problems in communication with " + ta.getHost() + ":" + ta.getPort(), e);
      } finally {
        if (done) {
          if (cw.isOneShot()) {
            // Note that a connection can be marked as one-shot due to the fact that the
            // server is not able to keep it open -->
            // In this case the connection was inserted in the pool at creation time and
            // must now be removed
            pool.remove(cw);
          }
        } else {
        ...
        try {
          ...
          try {
            log.log(Logger.WARNING,
                "Cannot set read-timeout on reused connection to " + c.getRemoteHost());
          } catch (Exception e1) {
