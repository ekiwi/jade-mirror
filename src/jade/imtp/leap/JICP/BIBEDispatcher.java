 ...
 */
public class BIBEDispatcher extends Thread
    implements BEConnectionManager, Dispatcher, JICPMediator {
  ...
  public void init(JICPMediatorManager mgr, String id, Properties props) throws ICPException {
    ...
    /*
     * lastSid try { lastSid = (byte) (Integer.parseInt(props.getProperty("outcnt")) -1); if
     * (lastSid < 0) { lastSid = 0x0f; } } catch (Exception e) { // Keep default }
     */
  ...
  /**
   * This is called by the JICPServer when a JICP packet addressing this mediator as recipient-ID is
   * received. In the case of the BIBEDispatcher this should never happen.
   */
  ...
  /**
   * Inner class InpConnectionHolder. Wrapper for the connection used to deliver commands to the
   * FrontEnd
   */
  ...
  /**
   * Inner class OutConnectionHolder Wrapper for the connection used to receive commands from the
   * FrontEnd
   */
