...
// #J2ME_EXCLUDE_END
{
  ...
  public static final String ACCEPT_LOCAL_HOST_ONLY =
      "jade_imtp_leap_JICP_JICPServer_acceptlocalhostonly";
   ...
   */
  public JICPServer(Profile p, JICPPeer myPeer, ICP.Listener l, ConnectionFactory f, int max)
      throws ICPException {
  ...
  /**
   * JICPServer thread entry point. Accept incoming connections and for each of them start a
   * ConnectionHandler that handles it.
   */
  public void run() {
  ...
  /**
   * Inner class ConnectionHandler. Handle a connection accepted by this JICPServer
   */
    ...
    public void run() {
      ...
      try {
            ...
            case JICPProtocol.RESPONSE_TYPE:
              ...
              if (recipientID != null) {
                ...
                if (m != null) {
                  if (myLogger.isLoggable(Logger.FINEST))
                    myLogger.log(Logger.FINEST,
                        "Passing incoming packet to mediator " + recipientID);
            ...
            case JICPProtocol.GET_ADDRESS_TYPE:
              ...
              if (myLogger.isLoggable(Logger.INFO))
                myLogger.log(Logger.INFO,
                    "Received a GET_ADDRESS request from " + addr + ":" + port);
              ...
              if (acceptMediators) {
                ...
                if (myPDPContextManager != null) {
                  try {
                    ...
                    // properties
                    Properties pdpContextInfo =
                        myPDPContextManager.getPDPContextInfo(addr, p.getProperty(Profile.OWNER));
        ...
        switch (status) {
          ...
          case REQUEST_SERVED:
            if (myLogger.isLoggable(Logger.SEVERE))
              myLogger.log(Logger.SEVERE, "Communication error writing return packet to " + addr
                  + ":" + port + " [" + e.toString() + "]");
            break;
          case RESPONSE_SENT:
            ...
            if (e instanceof EOFException) {
              if (myLogger.isLoggable(Logger.FINE))
                myLogger.log(Logger.FINE,
                    "Client " + addr + ":" + port + " has closed the connection.");
            } else {
              if (myLogger.isLoggable(Logger.FINE))
                myLogger.log(Logger.FINE,
