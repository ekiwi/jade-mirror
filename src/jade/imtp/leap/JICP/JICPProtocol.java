...
public class JICPProtocol extends TransportProtocol {
  ...
  // In case it is not possible to get the local host address, it is possible to
  // specify a remote URL where to find it by means of the GET_ADDRESS JICP
  // command
  public static final String REMOTE_URL_KEY = "remote-url";
  public static final String UNREACHABLE_KEY = "unreachable";
  // How many attempts to create the BackEnd in case the CREATE_MEDIATOR fails due
  // to network problems
  ...
  // The property to be specified among FE startup properties to activate server
  // time synchronization
  // If set to true this property is also passed in the CREATE_MEDIATOR request to
  // tell the server that a GET_SERVER_TIME will be sent immediately after
  // mediator creation completion
  public static final String GET_SERVER_TIME_KEY = "get-server-time";
  // The property keeping the server time offset set by the JADE runtime after
  // startup
  ...
  public TransportAddress stringToAddr(String s) throws ICPException {
    ...
    if (!NAME.equals(protocol)) {
      throw new ICPException(
          "Unexpected protocol \"" + protocol + "\" when \"" + NAME + "\" was expected.");
    }
