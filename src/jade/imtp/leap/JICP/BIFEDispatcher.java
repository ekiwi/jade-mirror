...
public class BIFEDispatcher implements FEConnectionManager, Dispatcher, TimerListener, Runnable {
    ...
    try {
      ...
      if (myLogger.isLoggable(Logger.CONFIG)) {
        myLogger.log(Logger.CONFIG,
            "Remote URL=" + JICPProtocol.getInstance().addrToString(mediatorTA));
      }
      ...
      try {
        ...
        // Use default
        props.setProperty(JICPProtocol.MAX_DISCONNECTION_TIME_KEY,
            String.valueOf(maxDisconnectionTime));
      }
      if (myLogger.isLoggable(Logger.CONFIG)) {
      ...
      try {
        if (connectionDropped) {
          ...
          terminationPacket = new JICPPacket(JICPProtocol.CONNECT_MEDIATOR_TYPE,
              JICPProtocol.TERMINATED_INFO, mediatorTA.getFile(), new byte[] {OUT});
        } else {
          terminationPacket =
              new JICPPacket(JICPProtocol.COMMAND_TYPE, JICPProtocol.TERMINATED_INFO, null);
        }
  ...
  private JICPConnection createBackEnd() throws IMTPException {
    ...
    JICPPacket pkt = new JICPPacket(JICPProtocol.CREATE_MEDIATOR_TYPE, JICPProtocol.DEFAULT_INFO,
        null, sb.toString().getBytes());
    ...
    for (int i = -1; i < backEndAddresses.length; i++) {
      ...
      try {
        myLogger.log(Logger.INFO,
            "Creating BackEnd on jicp://" + mediatorTA.getHost() + ":" + mediatorTA.getPort());
   ...
   */
  public synchronized byte[] dispatch(byte[] payload, boolean flush, int oldSessionId)
      throws ICPException {
    if (connectionDropped) {
  ...
  /**
   * Inner class InputManager. This class is responsible for serving incoming commands
   */
        ...
        while (isConnected()) {
          ...
          if (pkt.getType() == JICPProtocol.RESPONSE_TYPE) {
            myLogger.log(Logger.WARNING,
                "Unexpected response packet received on INP connection. Ignore it");
            continue;
          }
          status = 1;
          ...
          if (sid == lastSid) {
            ...
            if (myLogger.isLoggable(Logger.WARNING)) {
              myLogger.log(Logger.WARNING, "Duplicated packet from BE: pkt-type=" + pkt.getType()
                  + " info=" + pkt.getInfo() + " SID=" + sid);
            }
            pkt = lastResponse;
          } else {
            if (pkt.getType() == JICPProtocol.KEEP_ALIVE_TYPE) {
              ...
              if (myLogger.isLoggable(Logger.FINE)) {
                myLogger.log(Logger.FINE,
                    "Incoming command received " + sid + " pkt-type=" + pkt.getType());
              }
  ...
  /**
   * Close the current outConnection (if any) and starts a new thread that asynchronously tries to
   * restore it.
   */
  ...
  /*
   * TO BE REMOVED private void connect(byte type) { int cnt = 0; long startTime =
   * System.currentTimeMillis(); while (active) { try { if (myLogger.isLoggable(Logger.INFO)) {
   * myLogger.log(Logger.INFO,
   * "Connecting to "+mediatorTA.getHost()+":"+mediatorTA.getPort()+" "+type+"("+ cnt+")"); } int t
   * = (type == OUT ? RESPONSE_TIMEOUT : -1); Connection c = openConnection(mediatorTA, t);
   * JICPPacket pkt = new JICPPacket(JICPProtocol.CONNECT_MEDIATOR_TYPE, JICPProtocol.DEFAULT_INFO,
   * mediatorTA.getFile(), new byte[]{type}); writePacket(pkt, c); pkt = c.readPacket(); if
   * (pkt.getType() == JICPProtocol.ERROR_TYPE) { String errorMsg = new String(pkt.getData());
   * myLogger.log(Logger.WARNING, "JICP Error "+type+". "+errorMsg); c.close(); if
   * (errorMsg.equals(JICPProtocol.NOT_FOUND_ERROR)) { // The JICPMediatorManager didn't find my
   * Mediator anymore. Either // there was a fault our max disconnection time expired. // Try to
   * recreate the BackEnd if (type == OUT) { // BackEnd recreation is attempted only when restoring
   * the // OUT connection since the BackEnd uses the connection that // creates it to receive
   * outgoing commands. Moreover this ensures // that (if the BackEnd is created on a different
   * host) we do not // end up with the INP and OUT connections pointing to different // hosts. //
   * Finally, if BE re-creation fails, we behave as if there was an // IOException when trying to
   * reconnect. try { handleBENotFound(); // In some cases the INP connection may be re-established
   * by another // thread just after the BackEnd has been re-created but the // outConnection has
   * not been set yet. In these cases the resynch // process may fail since it finds the
   * outConnection null. // The synchronized block avoids this. synchronized (this) { c =
   * createBackEnd(); handleReconnection(c, type); } } catch (IMTPException imtpe) { // Behave as if
   * there was an IOException --> go back sleeping throw new IOException("BE-recreation failed"); }
   * } else { // In case the outConnection still appears to be OK, refresh it. refreshOut(); // Then
   * behave as if there was an IOException --> go back sleeping throw new IOException(); } } else {
   * // There was a JICP error. Abort handleError(); } } else { // The local-host address may have
   * changed props.setProperty(JICPProtocol.LOCAL_HOST_KEY, new String(pkt.getData())); if
   * (myLogger.isLoggable(Logger.INFO)) { myLogger.log(Logger.INFO, "Connect OK "+type); }
   * handleReconnection(c, type); } return; } catch (IOException ioe) { myLogger.log(Logger.WARNING,
   * "Connect failed "+type+". "+ioe); cnt++; if (type == OUT) { // Max disconnection time
   * expiration is detected only when // restoring the OUT connection. In this way we avoid having
   * // one connection restored while the other is declared dead. if ((System.currentTimeMillis() -
   * startTime) > maxDisconnectionTime) { handleError(); return; } }
   * 
   * // Wait a bit before trying again try { Thread.sleep(retryTime); } catch (Exception e) {} } } }
   */
    ...
    while (active) {
      try {
        synchronized (this) { // Mutual exclusion with BE re-creation
          myLogger.log(Logger.INFO, "Connecting to " + mediatorTA.getHost() + ":"
              + mediatorTA.getPort() + " " + cnt + " (INP)");
          Connection c = openConnection(mediatorTA, -1);
          JICPPacket pkt = new JICPPacket(JICPProtocol.CONNECT_MEDIATOR_TYPE,
              JICPProtocol.DEFAULT_INFO, mediatorTA.getFile(), new byte[] {INP});
          writePacket(pkt, c);
          pkt = c.readPacket();
          if (pkt.getType() == JICPProtocol.ERROR_TYPE) {
  ...
  private void connectOut() {
    ...
    while (active) {
      try {
        if (backEndExists) {
          myLogger.log(Logger.INFO, "Connecting to " + mediatorTA.getHost() + ":"
              + mediatorTA.getPort() + " " + cnt + " (OUT)");
          Connection c = openConnection(mediatorTA, RESPONSE_TIMEOUT);
          JICPPacket pkt = new JICPPacket(JICPProtocol.CONNECT_MEDIATOR_TYPE,
              JICPProtocol.DEFAULT_INFO, mediatorTA.getFile(), new byte[] {OUT});
          writePacket(pkt, c);
          pkt = c.readPacket();
          if (pkt.getType() == JICPProtocol.ERROR_TYPE) {
            String errorMsg = new String(pkt.getData());
  ...
  /*
   * TO BE REMOVED protected synchronized void handleReconnection(Connection c, byte type) { boolean
   * transition = false; if (type == INP) { myInputManager.setConnection(c); if (outConnection !=
   * null) { transition = true; } } else if (type == OUT) { if (connectionDropped) { // If we have
   * just reconnected after a connection drop-down, // refresh the INP connection too.
   * connectionDropped = false; refreshInp(); }
   * 
   * outConnection = c; refreshingOutput = false; // The Output connection is available again --> //
   * Activate postponed commands flushing waitingForFlush = myStub.flush(); if
   * (myInputManager.isConnected()) { transition = true; } } if (transition && myConnectionListener
   * != null) { myConnectionListener.handleConnectionEvent(ConnectionListener.RECONNECTED, null); }
   * }
   */
  ...
  /**
   * Send a KEEP_ALIVE packet to the BE. This is executed within a synchronized block --> Mutual
   * exclusion with dispatch() is guaranteed.
   */
  protected void sendKeepAlive() {
    if (outConnection != null) {
      JICPPacket pkt =
          new JICPPacket(JICPProtocol.KEEP_ALIVE_TYPE, JICPProtocol.DEFAULT_INFO, null);
      try {
  ...
  /**
   * Send a DROP_DOWN packet to the BE. The latter will also close the INP connection. This is
   * executed within a synchronized block --> Mutual exclusion with dispatch() is guaranteed.
   */
      ...
      try {
        ...
        if (rsp.getType() != JICPProtocol.ERROR_TYPE) {
          ...
          try {
            ...
            myLogger.log(Logger.WARNING,
                "Exception in connection drop-down closing the OUT connection. " + ioe);
          }
  ...
  /*
   * [WATCHDOG] private Object watchDogLock = new Object(); private Thread watchDogThread = null;
   * private boolean done = false;
   * 
   * private void startWatchDog(final Connection c) { synchronized (watchDogLock) { // If a watch
   * dog is already active, don't start another one. if (watchDogThread == null) {
   * myLogger.log(Logger.INFO, "Starting WatchDog thread."); done = false; watchDogThread = new
   * Thread() { public void run() { synchronized (watchDogLock) { try { if (!done) {
   * watchDogLock.wait(2*RESPONSE_TIMEOUT); if (!done) { // Timeout expired
   * myLogger.log(Logger.WARNING, "WatchDog: timer expired."); try { c.close();
   * myLogger.log(Logger.INFO, "WatchDog: connection closed."); } catch (IOException ioe) {
   * myLogger.log(Logger.WARNING, "WatchDog: IOException closing connection."); } } } } catch
   * (Exception e) { myLogger.log(Logger.WARNING, "WatchDog: Unexpected Exception "+e); }
   * watchDogThread = null; myLogger.log(Logger.INFO, "WatchDog: terminated."); } } };
   * watchDogThread.start(); } } }
   * 
   * private void stopWatchDog() { myLogger.log(Logger.INFO, "Stopping WatchDog thread.");
   * synchronized (watchDogLock) { done = true; watchDogLock.notifyAll(); } } // [WATCHDOG]
   */
