...
/**
 * *************************************************************** The LEAP libraries, when combined
 * with certain JADE platform components, provide a run-time environment for enabling FIPA agents to
 * execute on lightweight devices running Java. LEAP and JADE teams have jointly designed the API
 * for ease of integration and hence to take advantage of these dual developments and extensions so
 * that users only see one development platform and a single homogeneous set of APIs. Enabling
 * deployment to a wide range of devices whilst still having access to the full development
 * environment and functionalities that JADE provides. Copyright (C) 2001 Telecom Italia LAB S.p.A.
...
/**
 * This class provides the implementation of a command dispatcher. The command dispatcher misses
 * support for multiple remote objects, multiple ICPs and command routing.
 * 
 * <p>
 * The command dispatcher is based on an implementation written by Michael Watzke and Giovanni Caire
 * (TILAB), 09/11/2000.
 * </p>
 * 
...
class CommandDispatcher implements StubHelper, ICP.Listener {
  ...
  /**
   * The default name for new instances of the class <tt>CommandDispatcher</tt> that have not get an
   * unique name by their container yet.
   */
  ...
  /**
   * The unique name of this command dispatcher used to avoid loops in the forwarding mechanism.
   */
  protected String name;

  /**
   * The transport address of the default router. Commands that cannot be dispatched directly will
   * be sent to this address.
   */
  protected TransportAddress routerTA = null;

  /**
   * This hashtable maps the IDs of the objects remotized by this command dispatcher to the
   * skeletons for these objects. It is used when a command is received from a remote JVM.
   */
  protected Map skeletons = new HashMap();

  /**
   * This hashtable maps the objects remotized by this command dispatcher to their IDs. It is used
   * when a stub of a remotized object must be built to be sent to a remote JVM.
   */
  protected Map ids = new HashMap();

  /**
   * A counter that is used for determining IDs for remotized objects. Everytime a new object is
   * registered by the command dispatcher it gets the value of this field as ID and the field is
   * increased.
   */
  protected int nextID;

  /**
   * The pool of ICP objects used by this command dispatcher to actually send/receive data over the
   * network. It is a table that associates a <tt>String</tt> representing a protocol (e.g. "http")
   * to a list of ICPs supporting that protocol.
   */
  protected Map icps = new HashMap();

  /**
   * The transport addresses the ICPs managed by this command dispatcher are listening for commands
   * on.
   */
  ...
  /**
   * The stub for the platform service manager. This stub will be shared by all nodes within this
  ...
  /**
   * Returns a reference to the singleton instance of the CommandDispatcher for the indicated
   * platform. Such instance is created if necessary. If no platform name is specified or the
   ...
   */
  public synchronized static final CommandDispatcher getDispatcher(String name)
    ...
    if (enableMultiplePlatforms) {
      if (name != null) {
        ...
        throw new IMTPException(
            "No platform name specified and enable-multiple-platforms mode activated");
      }
    } else {
  ...
  /**
   * A sole constructor. To get a command dispatcher the constructor should not be called directly
   * but the static <tt>create</tt> and <tt>getDispatcher</tt> methods should be used. Thereby the
   * existence of a singleton instance of the command dispatcher will be guaranteed.
   */
  ...
  private void setPlatformManager(PlatformManager pm) throws IMTPException {
    ...
    if (platformName != null && !platformName.equals("*")) { // * is the wild-card for automatic
                                                             // main detection
      // PlatformName already set --> Check that it is consistent with the actual name
      // of the platform
      if (!platformName.equals(actualPlatformName)) {
        throw new IMTPException(
            "Wrong platform name " + platformName + ". It should be " + actualPlatformName);
      }
    } else {
  ...
  /**
   * Sets the transport address of the default router used for the forwarding mechanism.
   * 
  ...
  /**
   * This method dispatches the specified command to the first address (among those specified) to
   * which dispatching succeeds.
   * 
   * @param destTAs a list of transport addresses where the command dispatcher should try to
   ...
   * @throws UnreachableException if none of the destination addresses is reachable.
   */
  public Command dispatchCommand(List destTAs, Command command)
    ...
    if (response == null) {
      try {
        ...
        throw new DispatcherException(
            "Error serializing command " + command + " [" + lse.getMessage() + "]");
      }
    }
  ...
  /**
   * Dispatches the specified serialized command to one of the specified transport addresses (the
   * first where dispatching succeeds) directly or through the router.
   * 
   * @param destTAs a list of transport addresses where the command dispatcher should try to
   *        dispatch the command.
   * @param commandPayload the serialized command that is to be dispatched.
   * @param origin a <tt>String</tt> object describing the origin of the command to be dispatched.
   * @return a response command from the receiving container.
   * @throws DispatcherException if an error occurs during dispatching.
   * @throws UnreachableException if none of the destination addresses is reachable.
   */
  private Command dispatchSerializedCommand(List destTAs, byte[] commandPayload,
      boolean requireFreshConnection, String origin)
      throws DispatcherException, UnreachableException {
    ...
    try {
      ...
      if (routerTA != null) {
        responsePayload =
            dispatchThroughRouter(destTAs, commandPayload, requireFreshConnection, origin);
      } else {
        throw ue;
      }
    }
  ...
  /**
   * Dispatches the specified serialized command to one of the specified transport addresses (the
   * first where dispatching succeeds) directly.
   * 
   * @param destTAs a list of transport addresses where the command dispatcher should try to
   *        dispatch the command.
   * @param commandPayload the serialized command that is to be dispatched.
   ...
   * @throws UnreachableException if none of the destination addresses is reachable.
   */
  private byte[] dispatchDirectly(List destTAs, byte[] commandPayload,
      boolean requireFreshConnection) throws UnreachableException {
  ...
  /**
   * Dispatches the specified serialized command to one of the specified transport addresses (the
   * first where dispatching succeeds) through the router.
   * 
   * @param destTAs a list of transport addresses where the command dispatcher should try to
   *        dispatch the command.
   * @param commandPayload the serialized command that is to be dispatched.
   ...
   * @throws UnreachableException if none of the destination addresses is reachable.
   */
  private byte[] dispatchThroughRouter(List destTAs, byte[] commandPayload,
      boolean requireFreshConnection, String origin)
      throws DispatcherException, UnreachableException {
    ...
    try {
      ...
      throw new DispatcherException(
          "error serializing FORWARD command [" + lse.getMessage() + "].");
    }
  }

  /**
   * Checks whether some exceptions to be handled by the command dispatcher occurred on the remote
   * site. If this is the case the command dispatcher throws the corresponding exception locally.
   * 
   * @param response the resonse comman from the receiving container.
   * @throws DispatcherException if an error occurs on the remote site during dispatching.
  ...
  protected void checkRemoteExceptions(Command response)
    ...
    if (response.getCode() == Command.ERROR) {
      ...
      if (exception.equals("jade.imtp.leap.DispatcherException")) {
        throw new DispatcherException(
            "DispatcherException in remote site. " + response.getParamAt(1));
   ...
   * @param args the arguments required by the ICP for the activation. These arguments are ICP
   *        specific.
   */
  public void addICP(ICP peer, String peerID, Profile p) {
    try {
      ...
      if (myLogger.isLoggable(Logger.FINE)) {
        myLogger.log(Logger.FINE, "ICP " + peerID + " of class " + peer.getClass().getName()
            + " activated. Address is: " + url);
      }
      addresses.add(ta);
      urls.add(url);
   ...
   * @throws RuntimeException if the specified object is not remotized by this command dispatcher.
   */
  ...
  /**
   * Converts an URL into a transport address using the transport protocol supported by the ICPs
   * currently installed in the command dispatcher. If there is no ICP installed to the command
   * dispatcher or their transport protocols are not able to convert the specified URL a
   * <tt>DispatcherException</tt> is thrown.
   * 
   * @param url a <tt>String</tt> object specifying the URL to convert.
   * @return the converted URL.
   * @throws DispatcherException if there is no ICP installed to the command dispatcher or the
   *         transport protocols of the ICPs are not able to convert the specified URL.
   */
  ...
  public synchronized void registerSkeleton(Skeleton skeleton, Object remotizedObject)
    ...
    if (myLogger.isLoggable(Logger.FINE)) {
      myLogger.log(Logger.FINE, "Registering skeleton " + skeleton + " for remotized object "
          + remotizedObject + ". ID is " + id);
    }
  ...
  /**
   * Selects a suitable peer and sends the specified serialized command to the specified transport
   * address.
   * 
  ...
  /**
   * Handles a received (still serialized) command object, i.e. deserialize it and launch processing
   * of the command.
   * 
    ...
    try {
      ...
      if (command.getCode() == Command.FORWARD) {
        ...
        if (s != null) {
          ...
          response =
              buildExceptionResponse(new DispatcherException("No skeleton for object-id " + id));
        }
      }
