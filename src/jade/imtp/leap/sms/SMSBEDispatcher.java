...
/**
 * BackEnd side dispatcher class that extends <code>NIOBEDispatcher</code> and uses SMS to perform
 * OUT-of-bound notifications to the FrontEnd when the connection (currently dropped) must be
 * re-established.
 * 
 * @see SMSFEDispatcher
  ...
  protected void handleDropDown(Connection c, JICPPacket pkt, InetAddress addr, int port) {
    ...
    try {
      ...
      myLogger.log(Logger.CONFIG,
          "OUT-of-bound notification param: msisdn=" + msisdn + " port=" + smsPort);
    } catch (Exception e) {
      myLogger.log(Logger.SEVERE,
          "Cannot get FE port for OUT-of-bound notifications via SMS!!!!!!! " + e);
      e.printStackTrace();
    }
  }
