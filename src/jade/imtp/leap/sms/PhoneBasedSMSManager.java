        ...
        public void run() {
          try {
            ...
            myLogger.log(Logger.INFO,
                "PB-SMSManager waiting for the helper phone to connect on port " + localPort
                    + " ...");
            ...
            myLogger.log(Logger.SEVERE,
                "PB-SMSManager: Problems accepting connection from the helper phone.");
            e.printStackTrace();
            shutDown();
          }
        }
  ...
  protected void send(String msisdn, int port, byte type, byte[] data) {
    ...
    if (status == CONNECTED) {
      myLogger.log(Logger.FINEST,
          "PB-SMSManager sending SMS request to the helper phone. Recipient: " + recipientID);
      ...
      try {
        ...
        myLogger.log(Logger.WARNING,
            "PB-SMSManager: Error sending command to the helper phone. " + ioe);
      }
    } else {
      myLogger.log(Logger.WARNING,
          "PB-SMSManager: Helper phone not connected. Can't send SMS to " + recipientID);
    }
  }
}
