...
public abstract class SMSManager {
  ...
  /**
   * Send an SMS of a given type (BINARY or TEXT) to a given msisdn on a given port. Note that SMS
   * are not reliable --> Being sure that the SMS has been sent does not guarantee that the
   * destination msisdn receives it --> This is why this method does not throw any exception
   */
