...
public class HTTPBEDispatcher implements BEConnectionManager, Dispatcher, JICPMediator {
  ...
  public void init(JICPMediatorManager mgr, String id, Properties props) throws ICPException {
    ...
    // FIXME: Properly manage SID of next outgoing command and last incoming command
    // in case of BE re-creation
    ...
    try {
      ...
      if (myLogger.isLoggable(Logger.CONFIG)) {
        myLogger.log(Logger.CONFIG,
            "BackEndContainer " + myID + " successfully joined the platform.");
      }
   ...
   */
  public JICPPacket handleJICPPacket(JICPPacket pkt, InetAddress addr, int port)
      throws ICPException {
    ...
    if (pkt.getType() == JICPProtocol.COMMAND_TYPE) {
      ...
      if ((pkt.getInfo() & JICPProtocol.TERMINATED_INFO) != 0) {
        ...
        myLogger.log(Logger.INFO,
            "Peer termination notification received. Peer address is " + from);
        handlePeerExited();
        return null;
      } else {
        // NORMAL COMMAND
  ...
  /**
   * Handle an incoming connection. This is called by the MediatorManager when a CREATE or
   * CONNECT_MEDIATOR request is received. The HTTPBEDispatcher reacts to this call by resetting the
   * current situation
   */
  public boolean handleIncomingConnection(Connection c, JICPPacket pkt, InetAddress addr,
      int port) {
  ...
  //////////////////////////////////////////
  /**
   * This is called by the Stub using this Dispatcher to dispatch a serialized command to the
   * FrontEnd. Mutual exclusion with itself to ensure one command at a time is dispatched.
   */
  ...
  /**
   * Clean up this HTTPBEDispatcher. The shutdown process can be activated in the following cases:
   * 1) The local container is requested to exit --> The exit commad is forwarded to the FrontEnd
   * 1.a) Forwarding OK. The FrontEnd sends back a response with the TERMINATED_INFO set. When this
   * response is received the shutdown() method is called (see handleJICPPacket()). 1.b) Forwarding
   * failed. The BackEndContainer ignores the exception and directly calls the shutdown() method.
   * 
   * Note that in the case the FrontEnd spontaneously exits and in the case the max disconnection
   * time expires the kill() method is called --> see case 1.
   */
  ...
  /**
   * Inner class OutgoingsHandler. This class manages outgoing commands i.e. commands that must be
   * sent to the FrontEnd.
   * 
   * NOTE that, since HTTPBEDispatcher.dispatch() is synchronized only one thread at a time can
   * execute the deliverCommand() method. This also ensures that only one thread at a time can
   * execute the dispatchResponse() method. As a consequence it's impossible that at a certain point
   * in time there is both a thread waiting for a command and a thread waiting for a response.
   */
  private class OutgoingsHandler implements TimerListener {
    ...
    /**
     * Schedule a command for delivery, wait for the response from the FrontEnd and return it.
     * 
     * @exception ICPException if 1) the frontEndStatus is not REACHABLE, 2) the response timeout
     *            expires (the frontEndStatus is set to UNREACHABLE) or 3) the OutgoingsHandler is
     *            reset (the frontEndStatus is set to CONNECTING). Called by
     *            HTTPBEDispatcher#dispatch()
     */
    public synchronized JICPPacket deliverCommand(JICPPacket cmd, boolean flush, int oldSessionId)
        throws ICPException {
      if (frontEndStatus == REACHABLE) {
        ...
        currentCommand = cmd;
        commandReady = true;
        // Notify the thread that dispatched the response to the previous command (see
        // dispatchResponse())
        notifyAll();
        ...
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE,
              "Response to scheduled command received " + currentResponse.getSessionID());
        }
        responseReady = false;
    ...
    /**
     * Dispatch a response received from the FrontEnd to the issuer of the command this response
     * refers to. If no one is waiting for this response (the frontEndStatus must be different from
     * REACHABLE), set the frontEndStatus to REACHABLE. Then wait for the next command to transfer
     * to the FrontEnd.
     * 
     * @return the next outgoing command to be transferred to the FrontEnd or null if the
     *         OutgoingsHandler is reset. Called by HTTPBEDispatcher#handleJICPPacket()
     */
    public synchronized JICPPacket dispatchResponse(JICPPacket rsp, String from) {
      ...
      if ((rsp.getInfo() & JICPProtocol.OK_INFO) != 0) {
        ...
        if (responseWaiter != null) {
          ...
          // No one was waiting for this response. It must be the initial dummy response
          // or a
          // response that arrives after the timeout has expired.
          if (frontEndStatus == CONNECTING) {
            if (myLogger.isLoggable(Logger.INFO)) {
              myLogger.log(Logger.INFO,
                  "Initial dummy response received " + rsp.getSessionID() + from);
            }
            notifyInitialResponseReceived();
          } else {
            if (myLogger.isLoggable(Logger.WARNING)) {
              myLogger.log(Logger.WARNING,
                  "Unexpected response received (likely an out of time respose) "
                      + rsp.getSessionID() + from);
            }
          }
        }
      }
      ...
      if (frontEndStatus != REACHABLE) {
        // Certainly the front-end is reachable now. If it was unreachable reset the
        // max-disconnection timer and
    ...
    /**
     * Reset this OutgoingsHandler and set the frontEndStatus to CONNECTING. If there is a thread
     * waiting for a command to deliver to the FrontEnd it will return null. If there is a thread
     * waiting for a response it will exit with an Exception. The frontEndStatus is set to
     * CONNECTING. Called by HTTPBEDispatcher#handleIncomingConnection()
     */
