...
/*
 * #MIDP_INCLUDE_BEGIN //import javax.microedition.io.*; #MIDP_INCLUDE_END
 */
...
public class HTTPFEDispatcher implements FEConnectionManager, Dispatcher, TimerListener {
  ...
  public BackEnd getBackEnd(FrontEnd fe, Properties p) throws IMTPException {
    ...
    // Compose URL. Note that we build a JICPAddress just to avoid loading the
    // HTTPAddress class.
  ...
  private synchronized void createBackEnd() throws IMTPException {
    ...
    for (int i = -1; i < backEndAddresses.length; i++) {
      ...
      try {
        ...
        myLogger.log(Logger.INFO, "Creating BackEnd on " + hc.getProtocol() + mediatorTA.getHost()
            + ":" + mediatorTA.getPort());
        pkt = deliver(pkt);
  ...
  /**
   * Inner class InputManager This class deals with incoming commands (possibly keep-alive packets)
   */
  private class InputManager extends Thread {
    ...
    public void run() {
      if (cnt == 0) {
        ...
        try {
          myConnectionListener = (ConnectionListener) Class
              .forName(props.getProperty("connection-listener")).newInstance();
        } catch (Exception e) {
          // Just ignore it
        }
      }
      ...
      while (active) {
        try {
          ...
          // Deliver the response to the previous incoming command and wait for the next
          // one
          // If we are delivering the response to an exit command, set active to false to
          // avoid an annoying stack trace
          // (due to the fact that the back-end will close the connection instead of
          // sending further commands) and exit
          if (this == terminator) {
            active = false;
          }
  ...
  private JICPPacket deliver(JICPPacket pkt, Connection c) throws IOException {
    ...
    try {
      ...
      status = 1;
      /*
       * #MIDP_INCLUDE_BEGIN lock(); if (type == JICPProtocol.RESPONSE_TYPE) {
      ...
      if (lastPacket && (pkt.getInfo() & JICPProtocol.TERMINATED_INFO) != 0) {
        // When we send a packet marked with the terminated-info, the back-end may
        // either close
        // the connection (in this case we would have got an Exception) or reply with
        // another
        // packet marked with the terminated-info --> throws an Exception to expose a
        // uniform behaviour
      ...
      /*
       * #MIDP_INCLUDE_BEGIN if (type != JICPProtocol.RESPONSE_TYPE) { // If we delivered a RESPONSE
       * unlock() is already called by the TimerDispatcher unlock(); } #MIDP_INCLUDE_END
       */
    }
  }
  ...
  /**
   * Inner class DisconnectionManager. Manages issues related to disconnection of the device.
   */
  class DisconnectionManager implements Runnable {
    ...
    /**
     * Set the reachability state as "reachable" and notify the InputManager thread in case it is
     * waiting in waitUntilReachable().
     */
    ...
    /**
     * Wait until the device is reachable again. This is executed by the InputManager thread before
     * sending a response
     */
    ...
    public void run() {
      ...
      try {
        ...
        myLogger.log(Logger.SEVERE,
            "Impossible to reconnect to the BackEnd (" + System.currentTimeMillis() + ")", icpe);
        if (myConnectionListener != null) {
  ...
  /**
   * Inner class KeepAliveManager This class is responsible for taking track of keep-alive packets
      ...
      try {
        ...
        if (pkt.getType() == JICPProtocol.ERROR_TYPE) {
          ...
          if (errorMsg.equals(JICPProtocol.NOT_FOUND_ERROR)) {
            // Back-end not found: either the max disconnection time expired server side or
            // there was a fault and restart
  ...
  /*
   * private String[] parseBackEndAddresses(String addressesText) { Vector addrs = new Vector();
   * 
   * if(addressesText != null && !addressesText.equals("")) { // Copy the string with the specifiers
   * into an array of char char[] addressesChars = new char[addressesText.length()];
   * 
   * addressesText.getChars(0, addressesText.length(), addressesChars, 0);
   * 
   * // Create the StringBuffer to hold the first address StringBuffer sbAddr = new StringBuffer();
   * int i = 0;
   * 
   * while(i < addressesChars.length) { char c = addressesChars[i];
   * 
   * if((c != ',') && (c != ';') && (c != ' ') && (c != '\n') && (c != '\t')) { sbAddr.append(c); }
   * else {
   * 
   * // The address is terminated --> Add it to the result list String tmp =
   * sbAddr.toString().trim();
   * 
   * if (tmp.length() > 0) { // Add the Address to the list addrs.addElement(tmp); }
   * 
   * // Create the StringBuffer to hold the next specifier sbAddr = new StringBuffer(); }
   * 
   * ++i; }
   * 
   * // Handle the last specifier String tmp = sbAddr.toString().trim();
   * 
   * if(tmp.length() > 0) { // Add the Address to the list addrs.addElement(tmp); } }
   * 
   * // Convert the list into an array of strings String[] result = new String[addrs.size()];
   * for(int i = 0; i < result.length; i++) { result[i] = (String)addrs.elementAt(i); }
   * 
   * return result;
   * 
   * }
