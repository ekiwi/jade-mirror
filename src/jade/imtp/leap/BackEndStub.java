...
public class BackEndStub extends MicroStub implements BackEnd {
  ...
  protected void handlePostponedCommandExpired(Command c, ICPException exception) {
    ...
    if (c.getCode() == MESSAGE_OUT) {
      ...
      // If a code in the form ...[n]... is specified in the message, cut any message
      // part after
  ...
  /**
   * The method encodes the create mediator request, setting all the common properties retrived in
   * the passed property parameter.
   * 
   * @param pp
  ...
  public static final StringBuffer encodeCreateMediatorRequest(Properties pp) {
    ...
    appendProp(sb, JICPProtocol.MEDIATOR_CLASS_KEY,
        pp.getProperty(JICPProtocol.MEDIATOR_CLASS_KEY));
    appendProp(sb, JICPProtocol.MAX_DISCONNECTION_TIME_KEY,
        pp.getProperty(JICPProtocol.MAX_DISCONNECTION_TIME_KEY));
    appendProp(sb, FrontEnd.REMOTE_BACK_END_ADDRESSES,
        pp.getProperty(FrontEnd.REMOTE_BACK_END_ADDRESSES));
    ...
    appendProp(sb, MicroRuntime.BE_REQUIRED_SERVICES_KEY,
        pp.getProperty(MicroRuntime.BE_REQUIRED_SERVICES_KEY));
    appendProp(sb, JICPProtocol.KEEP_ALIVE_TIME_KEY,
        pp.getProperty(JICPProtocol.KEEP_ALIVE_TIME_KEY));
    ...
    appendProp(sb, JICPProtocol.GET_SERVER_TIME_KEY,
        pp.getProperty(JICPProtocol.GET_SERVER_TIME_KEY));
    return sb;
  }
