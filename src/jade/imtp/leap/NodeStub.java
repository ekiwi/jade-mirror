...
class NodeStub extends Stub implements Node {
   ...
   * @throws IMTPException If a communication error occurs while contacting the remote node.
   */
  public Object accept(HorizontalCommand cmd) throws IMTPException {
    try {
      if (myLogger.isLoggable(Logger.FINER)) {
        myLogger.log(Logger.FINER, "Sending HC " + cmd.getName() + " to remote node " + name
            + " at addresses " + remoteTAs.toString());
      }
      ...
      if (nParam > 1) {
        // An additional parameter is appended to report back the serving time in the
        // remote node
        try {
   ...
   * 
   * @param hang If <code>true</code>, the call hangs until the node exits or is interrupted.
   * @return If the node is currently terminating, <code>true</code> is returned, else
   *         <code>false</code>
   */
    ...
    try {
      if (myLogger.isLoggable(Logger.FINE)) {
        myLogger.log(Logger.FINE,
            "Pinging remote node " + name + " at addresses " + remoteTAs.toString());
      }
  ...
  public void platformManagerDead(String deadPMAddress, String notifyingPMAddress)
  ...
  public String toString() {
    String address =
        (remoteTAs != null && remoteTAs.size() > 0 ? remoteTAs.get(0) : "null").toString();
