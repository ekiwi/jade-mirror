...
public class LEAPIMTPManager implements IMTPManager {
  ...
  public static final String CHANGE_PORT_IF_BUSY =
      "jade_imtp_leap_LEAPIMTPManager_changeportifbusy";
  ...
  public void initialize(Profile p) throws IMTPException {
    ...
    try {
      ...
      localNode =
          new NodeLEAP("No-Name", theProfile.getBooleanProperty(Profile.MAIN, true), theDispatcher);
    ...
    if (masterPMAddr != null) {
      ...
      try {
        ...
        if ((t != null) && (t instanceof UnreachableException)) {
          ...
          logger.log(Logger.INFO, "No master Main Container found at address " + masterPMAddr
              + ". Take the leadership");
          masterPMAddr = null;
  ...
  /**
   * In the Profile ICPS can be specified as follows. 1) If there is only one ICP --> icps =
   * <icp-class-name> <param-key> = <param-value> ....... 2) In general (typically used when there
   * are 2 or more ICPs) icps = <icp1-class-name>(<icp1-id>);<icp2-class-name>(<icp2-id>)...
   * <icp1-id>-<param-key> = <param-value> ....... <icp2-id>-<param-key> = <param-value> .......
   * 
   * If there is no ICP indication set it as follows. a) Peripheral container / J2SE or PJAVA -
   * There is at least 1 ICP already active --> Nothing - There are no ICPs already active -->
   * JICPPeer b) Peripheral container / MIDP --> Throws a ProfileException c) Main container / J2SE
   * or PJAVA --> JICPPeer
   */
