...
public class MicroStub {
  ...
  private Command executeRemotely(Command c, long timeout, int sessionId) throws IMTPException {
    ...
    try {
      ...
      logger.log(Logger.FINE, "Dispatching command " + c.getCode() + ". SF-timeout=" + timeout
          + ", old-SID=" + sessionId);
      byte[] rsp = myDispatcher.dispatch(cmd, flushing, sessionId);
      if (pendingCommands.size() > 0) {
        logger.log(Logger.FINE, "############# Dispatch succeeded with " + pendingCommands.size()
            + " pending commands.");
      }
      ...
      if (r.getCode() == Command.ERROR) {
        if (!((Boolean) r.getParamAt(0)).booleanValue()) {
          ...
          String msg = new String("Exception " + (String) r.getParamAt(1)
              + " occurred in remote site processing command " + c.getCode() + ". "
              + (String) r.getParamAt(2));
      ...
      // Postpone the command if store-and-forward is enabled (timeout > 0 or -1 i.e.
      // INFNITE)
      if (timeout == 0) {
        ...
        logger.log(Logger.WARNING, "Dispatch failed. Command postponed [SF-timeout=" + timeout
            + ", SID=" + dispatchSessionId + "]. " + icpe.getMessage());
        return null;
      }
  ...
  private void postpone(Command c, long timeout, int sessionId, ICPException icpe) {
    if (logger.isLoggable(Logger.FINE)) {
      logger.log(Logger.FINE,
          Thread.currentThread().toString() + ": Command " + c.getCode() + " postponed");
    }
    final PostponedCommand pc = new PostponedCommand(c, sessionId, icpe);
    pendingCommands.addElement(pc);
    if (timeout > 0) {
      logger.log(Logger.INFO,
          Thread.currentThread().toString() + ": Activating Timer for Command " + c.getCode());
      pc.timer = TimerDispatcher.getTimerDispatcher()
          .add(new Timer(System.currentTimeMillis() + timeout, new TimerListener() {
            public void doTimeOut(Timer t) {
              // Timeout expired --> Remove the related postponed command and
              // let subclasses react properly
              if (t == pc.timer) {
                logger.log(Logger.INFO, Thread.currentThread().toString() + ": Timer for Command "
                    + pc.command.getCode() + " expired!!!");
  ...
  public Thread checkFlush() {
    // 1) Lock the buffer of pending commands to ensure mutual exclusion with
    // executeRemotely() (see comment in beginDispatch())
    if (beginFlush()) {
      ...
      flushingThread = new Thread() {
        public void run() {
            ...
            try {
              ...
              if (r.getCode() == Command.ERROR) {
                logger.log(Logger.SEVERE,
                    "Remote exception in command asynchronous delivery. " + r.getParamAt(2));
              }
            } catch (Exception ex) {
              logger.log(Logger.WARNING, "Exception in command asynchronous delivery. " + ex);
              // We are disconnected again --> put the command back in the queue of postponed
              // commands
              // and stop flushing. If there was a Timer it is still there --> No need to do
              // anything
          ...
          logger.log(Logger.FINE,
              "########## " + pendingCommands.size() + " pending commands after flush");
          endFlush();
  ...
  /**
   * Note that normal command-dispatching and postponed command flushing can't occur at the same
   * time, but different commands can be dispatched in parallel. This is the reason for this
   * lock/unlock mechanism instead of a simple synchronization.
   */
  private void beginDispatch() {
    // If this is the flushingThread (of course flush is in progress) do not block
    // it
