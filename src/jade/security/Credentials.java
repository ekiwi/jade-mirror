 ...
 * compliance with the FIPA specifications. Copyright (C) 2002 TILAB S.p.A. GNU Lesser General
...
/**
 * This is a collection of Objects that can be used in order to get privileges to perform an action
 * or access a service.
 * 
...
public interface Credentials extends jade.util.leap.Serializable {
  ...
  /**
   * Look into the various credentials, if there is one that certify the ownership for this
   * credentials, return the JADEPrincipal of the owner. Otherwise return null.
   */
  JADEPrincipal getOwner();

  /**
   * Encode the Credentials into a byte array, so that it can be transported in an ACLMessage. The
   * encoding format is: Credential class | Encoding size | Encoding
   */
