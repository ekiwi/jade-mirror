...
/*
 * #MIDP_INCLUDE_BEGIN import jade.util.leap.EnumIterator; import java.util.Vector;
 * #MIDP_INCLUDE_END
 */
...
/**
 * The class ACLMessage implements an ACL message compliant to the <b>FIPA 2000</b> "FIPA ACL
 * Message Structure Specification" (fipa000061) specifications. All parameters are couples
 * <em>keyword: value</em>. All keywords are <code>private final String</code>. All values can be
 * set by using the methods <em>set</em> and can be read by using the methods <em>get</em>.
 * <p>
 * <b>Warning: </b> since JADE 3.1 an exception might be thrown during the serialization of the
 * ACLMessage parameters (with exception of the content of the ACLMessage) because of a limitation
 * to 65535 in the total number of bytes needed to represent all the characters of a String (see
 * also java.io.DataOutput#writeUTF(String)).
 * <p>
 * The methods <code> setByteSequenceContent() </code> and <code> getByteSequenceContent() </code>
 * allow to send arbitrary sequence of bytes over the content of an ACLMessage.
 * <p>
 * The couple of methods <code> setContentObject() </code> and <code> getContentObject() </code>
 * allow to send serialized Java objects over the content of an ACLMessage. These method are not
 * strictly FIPA compliant so their usage is not encouraged.
 * 
...
public class ACLMessage implements Cloneable, Serializable {
  // #MIDP_EXCLUDE_END
  /*
   * #MIDP_INCLUDE_BEGIN public class ACLMessage implements Serializable { #MIDP_INCLUDE_END
   */
  ...
  /**
   * User defined parameter key specifying, when set to "true", that if the delivery of a message
   * fails, no failure handling action must be performed.
   */
  public static final String IGNORE_FAILURE = "JADE-ignore-failure";

  /**
   * User defined parameter key specifying, when set to "true", that if the delivery of a message
   * fails, no FAILURE notification has to be sent back to the sender. This differs from
   * IGNORE_FAILURE since it does not inhibit the delivery failure handling mechanism (based on the
   * NOTIFY_FAILURE VCommand) at all, but just the delivery of the automatic AMS FAILURE reply.
   */
  public static final String DONT_NOTIFY_FAILURE = "JADE-dont-notify-failure";

  /**
   * User defined parameter key specifying that the JADE tracing mechanism should be activated for
   * this message.
   */
  public static final String TRACE = "JADE-trace";

  /**
   * User defined parameter key specifying that this message does not need to be cloned by the
   * message delivery service. This should be used ONLY when the message object will not be modified
   * after being sent
   */
  public static final String NO_CLONE = "JADE-no-clone";

  /**
   * User defined parameter key specifying that this message must be delivered synchronously. It
   * should be noticed that when using synchronous delivery message order is not guaranteed.
   */
  public static final String SYNCH_DELIVERY = "JADE-synch-delivery";

  /**
   * User defined parameter key specifying the AID of the real sender of a message. This is
   * automatically set by the MessagingService when posting a message where the sender field is
   * different than the real sender.
   */
  public static final String REAL_SENDER = "JADE-real-sender";

  /**
   * User defined parameter key specifying that this message must be stored for a given timeout (in
   * ms) in case it is sent to/from a temporarily disconnected split container. After that timeout a
   * FAILURE message will be sent back to the sender.<br>
  ...
  /**
   * These constants represent the expected size of the 2 array lists used by this class
   **/
  ...
  // #MIDP_EXCLUDE_END
  /*
   * #MIDP_INCLUDE_BEGIN private Vector dests = new Vector(RECEIVERS_EXPECTED_SIZE); private Vector
   * reply_to = null; #MIDP_INCLUDE_END
   */
  ...
  /**
   * @deprecated Since every ACL Message must have a message type, you should use the new
   *             constructor which gets a message type as a parameter. To avoid problems, now this
   *             constructor silently sets the message type to <code>not-understood</code>.
  ...
  /**
   * This constructor creates an ACL message object with the specified performative. If the passed
   * integer does not correspond to any of the known performatives, it silently initializes the
   * message to <code>not-understood</code>.
   **/
  ...
  /**
   * Writes the <code>:sender</code> slot. <em><b>Warning:</b> no checks are made to validate the
   * slot value.</em>
   * 
  ...
  /**
   * Adds a value to <code>:receiver</code> slot. <em><b>Warning:</b> no checks are made to validate
   * the slot value.</em>
   * 
  ...
  /**
   * Removes a value from <code>:receiver</code> slot. <em><b>Warning:</b> no checks are made to
   * validate the slot value.</em>
   * 
  ...
  /**
   * Removes all values from <code>:receiver</code> slot. <em><b>Warning:</b> no checks are made to
   * validate the slot value.</em>
   */
  ...
  /**
   * Adds a value to <code>:reply-to</code> slot. <em><b>Warning:</b> no checks are made to validate
   * the slot value.</em>
   * 
  ...
  public void addReplyTo(AID dest) {
    if (dest != null) {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN reply_to = (reply_to == null ? new Vector(REPLYTO_EXPECTED_SIZE) :
       * reply_to); reply_to.addElement(dest); #MIDP_INCLUDE_END
       */
    }
  }

  /**
   * Removes a value from <code>:reply_to</code> slot. <em><b>Warning:</b> no checks are made to
   * validate the slot value.</em>
   * 
  ...
  /**
   * Removes all values from <code>:reply_to</code> slot. <em><b>Warning:</b> no checks are made to
   * validate the slot value.</em>
   */
  ...
  /**
   * set the performative of this ACL message object to the passed constant. Remind to use the set
   * of constants (i.e. <code> INFORM, REQUEST, ... </code>) defined in this class
   */
  ...
  /**
   * Writes the <code>:content</code> slot. <em><b>Warning:</b> no checks are made to validate the
   * slot value.</em>
   * <p>
   * <p>
   * Notice that, in general, setting a String content and getting back a byte sequence content - or
   * viceversa - does not return the same value, i.e. the following relation does not hold <code>
  ...
  // #MIDP_EXCLUDE_BEGIN
  /**
   * This method sets the content of this ACLMessage to a Java object. It is not FIPA compliant so
   * its usage is not encouraged. For example:<br>
   * 
   * <PRE>
   ...
   * <code>setContentObject</code>. It is not FIPA compliant so its usage is not encouraged. For
   * example to read Java objects from the content
   * 
   * <PRE>
  ...
  /**
   * Writes the <code>:reply-with</code> slot. <em><b>Warning:</b> no checks are made to validate
   * the slot value.</em>
   * 
  ...
  /**
   * Writes the <code>:in-reply-to</code> slot. <em><b>Warning:</b> no checks are made to validate
   * the slot value.</em>
   * 
   * @param reply The new value for the slot.
  ...
  /**
   * Writes the <code>:encoding</code> slot. <em><b>Warning:</b> no checks are made to validate the
   * slot value.</em>
   * 
  ...
  /**
   * Writes the <code>:language</code> slot. <em><b>Warning:</b> no checks are made to validate the
   * slot value.</em>
   * 
   * @param str The new value for the slot.
  ...
  /**
   * Writes the <code>:ontology</code> slot. <em><b>Warning:</b> no checks are made to validate the
   * slot value.</em>
   * 
   * @param str The new value for the slot.
  ...
  /**
   * Writes the <code>:reply-by</code> slot. <em><b>Warning:</b> no checks are made to validate the
   * slot value.</em>
   * 
  ...
  /**
   * Writes the <code>:protocol</code> slot. <em><b>Warning:</b> no checks are made to validate the
   * slot value.</em>
   * 
   * @param str The new value for the slot.
  ...
  /**
   * Writes the <code>:conversation-id</code> slot. <em><b>Warning:</b> no checks are made to
   * validate the slot value.</em>
   * 
   * @param str The new value for the slot.
   ...
   * 
   * @return An <code>Iterator</code> containing the Agent IDs of the receiver agents for this
   *         message.
   */
  public Iterator getAllReceiver() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN return new EnumIterator(dests.elements()); #MIDP_INCLUDE_END
     */
  }
   ...
   * 
   * @return An <code>Iterator</code> containing the Agent IDs of the reply_to agents for this
   *         message.
   */
  public Iterator getAllReplyTo() {
    if (reply_to == null) {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN return new EnumIterator(reply_to.elements()); #MIDP_INCLUDE_END
       */
    }
  }
   ...
   * 
   * @return the string corresponding to the integer for the performative; "NOT-UNDERSTOOD" if the
   ...
   * Reads <code>:reply-by</code> slot.
   * 
   * @return The value of <code>:reply-by</code>slot, as a <code>Date</code> object.
  ...
  /**
   * Searches for the user defined parameter with the specified key. The method returns
  ...
  // #CUSTOM_EXCLUDE_BEGIN
  /**
   * Attaches an envelope to this message. The envelope is used by the <b><it>ACC</it></b> for
  ...
  /**
   * Writes the message envelope for this message, using the <code>:sender</code> and
   * <code>:receiver</code> message slots to fill in the envelope.
   * 
  ...
  public void setDefaultEnvelope() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN Iterator it = new EnumIterator(dests.elements()); #MIDP_INCLUDE_END
     */
    while (it.hasNext())
  ...
  /**
   * Convert an ACL message to its string representation. This method writes a representation of
   * this <code>ACLMessage</code> into a character string. If the content is a bytesequence, then it
   * is automatically converted into Base64 encoding.
   * 
   ...
   * 
   * @return A copy of this <code>ACLMessage</code> object. The copy must be casted back to
   *         <code>ACLMessage</code> type before being used.
   */
  // #MIDP_EXCLUDE_BEGIN
  ...
  // #MIDP_EXCLUDE_END
  /*
   * #MIDP_INCLUDE_BEGIN public synchronized Object clone() { ACLMessage result = new
   * ACLMessage(NOT_UNDERSTOOD); result.performative = performative; result.source = source;
   * result.content = content; result.byteSequenceContent = byteSequenceContent; result.reply_with =
   * reply_with; result.in_reply_to = in_reply_to; result.encoding = encoding; result.language =
   * language; result.ontology = ontology; result.reply_byInMillisec = reply_byInMillisec;
   * result.protocol = protocol; result.conversation_id = conversation_id; result.userDefProps =
   * userDefProps; //#CUSTOM_EXCLUDE_BEGIN if(messageEnvelope != null) { result.messageEnvelope =
   * (Envelope)messageEnvelope.clone(); } //#CUSTOM_EXCLUDE_END result.dests = new
   * Vector(dests.size()); for (int i=0; i<dests.size(); i++)
   * result.dests.addElement(dests.elementAt(i)); if (reply_to != null) { result.reply_to = new
   * Vector(reply_to.size()); for (int i=0; i<reply_to.size(); i++)
   * result.reply_to.addElement(reply_to.elementAt(i)); } return result; } #MIDP_INCLUDE_END
   */
   ...
   *         Envelope ":intended-receiver" first, the Envelope ":to" second and the message
   *         ":receiver" last.
   */
