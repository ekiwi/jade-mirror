...
/**
 * This class contains a set of static methods that convert to/from the Date Time format adopted by
 * FIPA. The FIPA format is based on ISO8601, with the addition of milliseconds. Using the
 * <code>java.text.SimpleDateFormat</code> notation, it is: <code>yyyyMMdd'T'HHmmssSSS'Z'</code> ,
 * where the <code>'T'</code> serves to separate the Day from the Time, and the <code>'Z'</code>
 * indicates that the time is in UTC.
 *
 * The FIPA specs permit either local or UTC time, however, they do express a preference for UTC
 * time (this is particularly helpful when passing messages between agents running on machines in
 * different timezones).
 * <UL>
 ...
 * @version $Date$ $Revision$ Modified by:
...
public class ISO8601 {
  ...
  /**
   * This method converts a FIPA DateTime token to a <code>java.util.Date</code>. It will accept
   ...
   * 
   * @param useUTCtime controls the style used by <code>toString</code>, 'true' generates tokens
   *        using UTC time, 'false' using local time. If you need to send messages to agents
   *        compiled with older versions of Jade, then set this to <code>false</code>.
   * @return a String, e.g. "19640625T073000000Z" to represent 7:30AM on the 25th of June of 1964,
   *         UTC time.
   */
  ...
  /**
   * This method converts a <code>java.util.Date</code> into a FIPA DateTime token by using the UTC
   * time.
   * 
   * @return a String, e.g. "19640625T073000000Z" to represent 7:30AM on the 25th of June of 1964,
   *         UTC time.
  ...
  public static void main(String argv[]) {
    ...
    try {
      System.out.println("ISO8601.toRelativeTimeString(" + argv[0] + ") returns:"
          + ISO8601.toRelativeTimeString(Long.parseLong(argv[0])));

      Date d = new Date(Integer.parseInt(argv[0]));
      System.out
          .println("ISO8601.toString(" + d + ", false) returns:" + ISO8601.toString(d, false));
    } catch (Exception e1) {
    }
