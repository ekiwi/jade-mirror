...
/**
 * Abstract interface for converting ACL messages back and forth between Java objects and raw byte
 * sequences, according to a FIPA ACL message representation.
 * 
...
public interface ACLCodec {
  ...
  // #DOTNET_EXCLUDE_END
  /*
   * #DOTNET_INCLUDE_BEGIN public static final String DEFAULT_CHARSET = "ISO-8859-1";
   * #DOTNET_INCLUDE_END
   */

  /**
   * This exception is thrown when some problem occurs in the concrete parsing subsystem accessed
   * through this interface. If an exception is thrown by the underlying parser, it is wrapped with
   * a <code>Codec.CodecException</code>, which is then rethrown.
   */
  public static class CodecException extends Exception {
     ...
     * 
     * @return the <code>Throwable</code> object that is the exception thrown by the concrete
     *         parsing subsystem.
     */
  ...
  /**
   * Encodes an <code>ACLMessage</code> object into a byte sequence, according to the specific
  ...
  /**
   * Recovers an <code>ACLMessage</code> object back from raw data, using the specific message
   * representation to interpret the byte sequence.
   * 
  ...
  /**
   * Query the name of the message representation handled by this <code>Codec</code> object. The
   * FIPA standard representations have a name starting with <code><b>"fipa.acl.rep."</b></code>.
   * 
