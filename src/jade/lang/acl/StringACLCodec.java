...
/**
 * This class implements the FIPA String codec for ACLMessages. Notice that it is not possible to
 * convey a sequence of bytes over a StringACLCodec because the bytes with the 8th bit ON cannot
 * properly converted into a char.
 * 
...
public class StringACLCodec implements ACLCodec {
  ...
  /**
   * Key of the user-defined parameter used to signal the automatic JADE conversion of the content
   * into Base64 encoding
   **/
  private static final String BASE64ENCODING_KEY = "JADE-Encoding";
  /**
   * Value of the user-defined parameter used to signal the automatic JADE conversion of the content
   * into Base64 encoding
   **/
  ...
  /**
   * constructor for the codec. The standard input is used as an input stream of ACL messages. The
   * standard output is used to write encoded ACL messages.
   */
   ...
   *            <code>new InputStreamReader(System.in)</code> if you want to use the standard input)
   * @parameter w is the writer to write encoded ACL messages (pass
   *            <code>new OutputStreamWriter(System.out)</code> if you want to use the standard
   *            output)
   */
   ...
   * @throws ACLCodec.CodecException if any Exception occurs during the parsing/reading operation
   */
  ...
  /**
   * encodes the message and writes it into the Writer passed in the constructor. Notice that this
   * method does not call <code>flush</code> on the writer. @ param msg is the ACLMessage to encode
   * and write into
   */
  ...
  /**
   * Take a java String and quote it to form a legal FIPA ACL string. Add quotation marks to the
   * beginning/end and escape any quotation marks inside the string.
   */
  ...
  /**
   * If a user-defined parameter contain a blank char inside, then it is skipped for
   * FIPA-compatibility
   * 
  ...
  /**
   * If the content of the message is a byteSequence, then this method encodes the content in Base64
   * and automatically sets the value of the encoding slot.
   * 
  ...
  public ACLMessage decode(byte[] data, String charset) throws ACLCodec.CodecException {
    try {
      ACLMessage msg =
          ACLParser.create().parse(new InputStreamReader(new ByteArrayInputStream(data), charset));
      checkBase64Encoding(msg);
      return msg;
    } catch (jade.lang.acl.TokenMgrError e1) {
      throw new ACLCodec.CodecException(getName() + " ACLMessage decoding token exception", e1);
  ...
  /**
   * append to the passed StringBuffer the slot name and value separated by a blank char and
   * followed by a newline. If the value contains a blank, then it is quoted. if the value is null
   * or its length is zero, the method does nothing.
   **/
  ...
  /**
   * Test if the given string is a legal word using the FIPA ACL spec. In addition to FIPA's
   * restrictions, place the additional restriction that a Word can not contain a '\"', that would
   * confuse the parser at the other end.
   */
