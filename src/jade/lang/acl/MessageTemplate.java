...
/**
 * A pattern for matching incoming ACL messages. This class allows to build complex slot patterns to
 * select ACL messages. These patterns can then be used in <code>receive()</code> operations.
 * 
 * This class provide one method for each attribute of an ACLMessage, that can be combined using the
 * logic operators to create more complex patterns. A user can also create an application-specific
 * pattern. In this case he has to implement the MatchExpression interface, writing the application
 * specific <code>match()</code> method. Then an instance of that class can be used as parameter of
 * the MessageTemplate constructor to define the application specific MessageTemaplate.
 * 
 * 
...
public class MessageTemplate implements Serializable {
  ...
  /**
   * This interface must be overriden in order to define an application specific MessageTemplate. In
   * particular in the method <code> match()</code> the programmer should realize the necessary
   * checks on the ACLMessage in order to return <b>true</b> if the message match with the
   * application specific requirements <b>false</b> otherwise.
   */
  public static interface MatchExpression extends Serializable {
    /**
     * Check whether a given ACL message matches this template. Concrete implementations of this
     * interface will have this method called to accept or refuse an ACL message.
     * 
     * @param msg The ACL message to match against this message template.
     * @return A compliant implementation will return <code>true</code> if the parameter ACL message
     *         matches the template, and <code>false</code> otherwise.
     */
    ...
    public boolean match(ACLMessage msg) {
      switch (slotName) {
        ...
        case CONVERSATION_ID:
          return CaseInsensitiveString.equalsIgnoreCase((String) matchValue,
  ...
  private static class CustomMsgLiteral implements MatchExpression {
    ...
    public boolean match(ACLMessage msg) {
      ...
      if (messageToMatch.hasByteSequenceContent()) {
        ...
        if (!compareByteArrays(messageToMatch.getByteSequenceContent(),
            msg.getByteSequenceContent()))
          return false;
      ...
      while (it1.hasNext()) {
        ...
        if (found == false)
          return false; // when a receiver of the template is not into the receivers of the
                        // ACLMessage.
      } // end while
      ...
      // sender
      if ((messageToMatch.getSender() != null)
          && !(messageToMatch.getSender().equals(msg.getSender())))
        return false;
  ...
  private static class MatchTopic implements MatchExpression {
    ...
    public String toString() {
      String name = (matchAll ? TopicManagementHelper.TOPIC_TEMPLATE_WILDCARD
          : (isTemplate ? topicName + '.' + TopicManagementHelper.TOPIC_TEMPLATE_WILDCARD
              : topicName));
      return "( Topic: " + name + " )";
    }
  }
  ...
  /**
   * This <em>Factory Method</em> returns a message template that matches any message.
   * 
  ...
  /**
   * This <em>Factory Method</em> returns a message template that matches any message with a given
   * <code>:sender</code> slot.
   * 
  ...
  /**
   * This <em>Factory Method</em> returns a message template that matches any message with a given
   * <code>:receiver</code> slot.
   * 
   * @param values An array of Agent IDs against which the value of the message slot will be
   *        matched.
   * @return A new <code>MessageTemplate</code> matching the given value.
   */
  ...
  /**
   * This <em>Factory Method</em> returns a message template that matches any message about a given
   * topic.
   * 
  ...
  /**
   * This <em>Factory Method</em> returns a message template that matches any message with a given
   * <code>:conversation-id</code> slot.
   * 
   * @param value The value the message slot will be matched against.
   * @return A new <code>MessageTemplate</code> matching the given value.
   */
  ...
  /**
   * This <em>Factory Method</em> returns a message template that matches ACL messages against a
   * given one, passed as parameter. The following algorithm is used: When the given
   * <code>ACLMessage</code> has a non <code>null</code> slot, subsequent messages must have the
   * same slot value in that slot to have a match. When the given <code>ACLMessage</code> has a
   * <code>null</code> slot, subsequent messages can have any value for that slot and still match
   * the template. In short, a <code>null</code> value for a slot means <em>don't care</em>.
   * 
   * @param msg The <code>ACLMessage</code> used to build a custom message template.
   * @param matchPerformative a <code>bool</code> value. When <code>true</code>, the performative of
   *        the <code>msg</code> will be considered as a part of the template (i.e. the message
   *        template will match only ACL messages with the same performativa as <code>msg</code>).
   *        When <false>, the performative of <code>msg</code> is ignored and the resulting message
   *        template will not consider it when matching messages.
   * @return A new <code>MessageTemplate</code>, matching the given message according to the above
   *         algorithm.
   */
  ...
  /**
   * Logical <b>and</b> between two <code>MessageTemplate</code> objects. This method creates a new
   * message template that is matched by those ACL messages matching <b><em>both</b></em> message
   * templates given as operands.
   * 
  ...
  /**
   * Logical <b>or</b> between two <code>MessageTemplate</code> objects. This method creates a new
   * message template that is matched by those ACL messages matching <b><em>any</b></em> of the two
  ...
  /**
   * Logical <b>not</b> of a <code>MessageTemplate</code> object. This method creates a new message
   * template that is matched by those ACL messages <b><em>not</em></b> matching the message
   * template given as operand.
   * 
   ...
   * @return <code>true</code> if the ACL message matches this template, <code>false</code>
   *         otherwise.
   */
  public boolean match(ACLMessage msg) {
