...
/**
 * This behaviour subscribes to the AMS to receive notifications about platform-wide events. The
 * <code>installHandlers</code> method must be redefined to define the handlers for events the agent
 * executing this behaviour is interested in.
 * 
...
public abstract class AMSSubscriber extends SimpleBehaviour {
  ...
  // #DOTNET_EXCLUDE_END
  /*
   * #DOTNET_INCLUDE_BEGIN private Map handlers = new TreeMap(new CaseInsensitiveComparator() );
   * #DOTNET_INCLUDE_END
   */
  ...
  /**
   * Construct an AMSSubscriber behaviour to receive notifications about platform events from the
   * local AMS
   */
  ...
  /**
   * This method has to be implemented by concrete subclasses, filling the <code>Map</code> passed
   * as parameter with implementations of the <code>EventHandler</code> interface, using the name of
   * the event as key (see the <code>Event</code> interface.
   * 
   * @param handlersTable The table that associates each event name with a proper handler.
   */
  ...
  /**
   * Retrieve the <code>subscribe</code> ACL message used to subscribe to the AMS. This message is
   * automatically sent when this behaviour is added to an Agent.
   * 
  ...
  /**
   * Retrieve the <code>cancel</code> ACL message used to cancel the subscription to the AMS. Since
   * this behaviour is cyclic (never ends) it is the responsibility of the agent executing this
   * behaviour to send the <code>cancel</code> message to the AMS when notifications are no longer
   * required.
   * 
  ...
  /*
   * #DOTNET_INCLUDE_BEGIN //This class is used to obtain a Comparator for compare two strings
   * //with case ignoring. private class CaseInsensitiveComparator implements java.util.Comparator,
   * java.io.Serializable { // use serialVersionUID from JDK 1.2.2 for interoperability private
   ...
   * int n1=s1.length(), n2=s2.length(); for (int i1=0, i2=0; i1<n1 && i2<n2; i1++, i2++) { char c1
   * = s1.charAt(i1); char c2 = s2.charAt(i2); if (c1 != c2) { c1 = Character.toUpperCase(c1); c2 =
   * Character.toUpperCase(c2); if (c1 != c2) { c1 = Character.toLowerCase(c1); c2 =
   * Character.toLowerCase(c2); if (c1 != c2) { return c1 - c2; } } } } return n1 - n2; } }
   * #DOTNET_INCLUDE_END
   */
}
