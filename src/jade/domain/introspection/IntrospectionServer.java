...
public class IntrospectionServer extends CyclicBehaviour {
  ...
  protected void serveGetKeys(jade.lang.acl.ACLMessage request, Action aExpr, GetKeys action)
      throws Exception {
    ...
    for (int i = 0; i < mm.length; ++i) {
      ...
      if (method.getName().startsWith("get") && method.getParameterTypes().length == 0) {
        ...
        if (retType.isPrimitive()
            || (serializableClass != null && serializableClass.isAssignableFrom(retType))) {
  ...
  protected void serveGetValue(jade.lang.acl.ACLMessage request, Action aExpr, GetValue action)
      throws Exception {
