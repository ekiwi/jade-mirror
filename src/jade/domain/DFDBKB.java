 ...
 * the License. This library is distributed in the hope that it will be useful, but WITHOUT ANY
...
/**
 * This class implements a knowledge base used by the DF which stores its content in an external
 * database.
 * 
...
public class DFDBKB extends DBKB {
  ...
  private class PreparedStatements {
    ...
    private PreparedStatements(Connection conn) throws SQLException {
      // select statements
      stm_selNrOfPropForKey =
          conn.prepareStatement("SELECT COUNT(*) FROM SERVICEPROPERTY  where PROPKEY = ?");
      stm_selNrOfDescrForAID =
          conn.prepareStatement("SELECT COUNT(*) FROM dfagentdescr WHERE aid = ?");
      stm_selAgentAddresses =
          conn.prepareStatement("SELECT address FROM agentaddress WHERE aid = ?");
      stm_selAgentResolverAIDs =
          conn.prepareStatement("SELECT resolveraid FROM agentresolver WHERE aid = ?");
      stm_selAgentUserDefSlot =
          conn.prepareStatement("SELECT slotkey, slotval FROM agentuserdefslot WHERE aid = ?");
      ...
      stm_selServices = conn
          .prepareStatement("SELECT id, sname, stype, sownership FROM service WHERE descrid = ?");
      stm_selServiceProtocols =
          conn.prepareStatement("SELECT protocol FROM serviceprotocol WHERE serviceid = ?");
      stm_selServiceLanguages =
          conn.prepareStatement("SELECT ontology FROM serviceontology WHERE serviceid = ?");
      stm_selServiceOntologies =
          conn.prepareStatement("SELECT language FROM servicelanguage WHERE serviceid = ?");
      stm_selServiceProperties = conn.prepareStatement(
          "SELECT propkey, propval_str, propval_obj FROM serviceproperty WHERE serviceid = ?");
      ...
      stm_selExpiredDescr =
          conn.prepareStatement("SELECT aid FROM dfagentdescr WHERE lease < ? AND lease <> '-1'");
      stm_selSubscriptions = conn.prepareStatement("SELECT * FROM subscription");

      stm_selAllProtocols =
          conn.prepareStatement("SELECT descrid, protocol FROM protocol ORDER BY descrid");
      stm_selCountAllProtocols = conn.prepareStatement("SELECT COUNT(*) FROM protocol");
      stm_selAllLanguages =
          conn.prepareStatement("SELECT descrid, language FROM language ORDER BY descrid");
      stm_selCountAllLanguages = conn.prepareStatement("SELECT COUNT(*) FROM language");
      stm_selAllOntologies =
          conn.prepareStatement("SELECT descrid, ontology FROM ontology ORDER BY descrid");
      ...
      stm_insAgentUserDefSlot =
          conn.prepareStatement("INSERT INTO agentuserdefslot VALUES (?, ?, ?, ?)");
      ...
      stm_insServiceProperty =
          conn.prepareStatement("INSERT INTO serviceproperty VALUES (?, ?, ?, ?, ?)");
      ...
      stm_delServiceLanguage =
          conn.prepareStatement("DELETE FROM servicelanguage WHERE serviceid = ?");
      stm_delServiceOntology =
          conn.prepareStatement("DELETE FROM serviceontology WHERE serviceid = ?");
      stm_delServiceProtocol =
          conn.prepareStatement("DELETE FROM serviceprotocol WHERE serviceid = ?");
      stm_delServiceProperty =
          conn.prepareStatement("DELETE FROM serviceproperty WHERE serviceid = ?");
   ...
   * @param cleanTables specifies whether the KB should delete all existing tables for the DF at
   *        startup
   * @throws SQLException an error occured while opening a connection to the database
   */
  public DFDBKB(int maxResultLimit, String drv, String url, String user, String passwd,
      boolean cleanTables) throws SQLException {
  ...
  protected void createDFTables() {
    ...
    createTable(DFAGENTDESCR, new String[] {"id VARCHAR(" + MAX_PROP_LENGTH + ")",
        "aid VARCHAR(" + MAX_PROP_LENGTH + ")", "lease VARCHAR(20)", "PRIMARY KEY( id )"});

    createTable(AGENTADDRESS,
        new String[] {"id VARCHAR(" + MAX_PROP_LENGTH + ")", "aid VARCHAR(" + MAX_PROP_LENGTH + ")",
            "address VARCHAR(" + MAX_PROP_LENGTH + ")", "PRIMARY KEY( id )"});

    createTable(AGENTRESOLVER,
        new String[] {"id VARCHAR(" + MAX_PROP_LENGTH + ")", "aid VARCHAR(" + MAX_PROP_LENGTH + ")",
            "resolveraid VARCHAR(" + MAX_PROP_LENGTH + ")", "PRIMARY KEY( id )"});

    createTable(AGENTUSERDEFSLOT,
        new String[] {"id VARCHAR(" + MAX_PROP_LENGTH + ")",
            "aid	VARCHAR(" + MAX_PROP_LENGTH + ")", "slotkey	VARCHAR(" + MAX_PROP_LENGTH + ")",
            "slotval	" + LONGVARCHAR_TYPE, "PRIMARY KEY( id )"});

    createTable(ONTOLOGY,
        new String[] {"descrid VARCHAR(" + MAX_PROP_LENGTH + ")", "ontology VARCHAR(64)",
            "PRIMARY KEY( descrid, ontology )",
            "FOREIGN KEY( descrid ) REFERENCES dfagentdescr( id )"});

    createTable(PROTOCOL,
        new String[] {"descrid VARCHAR(" + MAX_PROP_LENGTH + ")", "protocol VARCHAR(64)",
            "PRIMARY KEY( descrid, protocol )",
            "FOREIGN KEY( descrid ) REFERENCES dfagentdescr( id )"});

    createTable(LANGUAGE,
        new String[] {"descrid VARCHAR(" + MAX_PROP_LENGTH + ")", "language VARCHAR(32)",
            "PRIMARY KEY( descrid, language )",
            "FOREIGN KEY( descrid ) REFERENCES dfagentdescr( id )"});

    createTable(SERVICE,
        new String[] {"id VARCHAR(" + MAX_PROP_LENGTH + ")",
            "descrid VARCHAR(" + MAX_PROP_LENGTH + ")", "sname VARCHAR(" + MAX_PROP_LENGTH + ")",
            "stype VARCHAR(64)", "sownership VARCHAR(64)", "PRIMARY KEY( id )",
            "FOREIGN KEY( descrid ) REFERENCES dfagentdescr( id )"});

    createTable(SERVICEPROTOCOL,
        new String[] {"serviceid VARCHAR(" + MAX_PROP_LENGTH + ")", "protocol VARCHAR(64)",
            "PRIMARY KEY( serviceid, protocol )",
            "FOREIGN KEY( serviceid ) REFERENCES service( id )"});

    createTable(SERVICEONTOLOGY,
        new String[] {"serviceid VARCHAR(" + MAX_PROP_LENGTH + ")", "ontology VARCHAR(64)",
            "PRIMARY KEY( serviceid, ontology )",
            "FOREIGN KEY( serviceid ) REFERENCES service( id )"});

    createTable(SERVICELANGUAGE,
        new String[] {"serviceid VARCHAR(" + MAX_PROP_LENGTH + ")", "language VARCHAR(32)",
            "PRIMARY KEY( serviceid, language )",
            "FOREIGN KEY( serviceid ) REFERENCES service( id )"});

    createTable(SERVICEPROPERTY, new String[] {"serviceid VARCHAR(" + MAX_PROP_LENGTH + ")",
        "propkey VARCHAR(" + MAX_PROP_LENGTH + ")", "propval_obj " + LONGVARCHAR_TYPE,
        "propval_str VARCHAR(" + MAX_PROP_LENGTH + ")", "propvalhash VARCHAR(100)",
        "PRIMARY KEY( serviceid, propkey )", "FOREIGN KEY( serviceid ) REFERENCES service( id )"});

    // Tables for subscriptions
    createTable(SUBSCRIPTION, new String[] {"id	 VARCHAR(" + MAX_PROP_LENGTH + ")",
        "aclm " + LONGVARCHAR_TYPE, "PRIMARY KEY( id )"});
          ...
          if (propObj instanceof MultiValueProperty) {
            ...
            while (iterP.hasNext()) {
              ...
              try {
                ...
                if (logger.isLoggable(Logger.SEVERE))
                  logger.log(Logger.SEVERE, "Cannot serialize multi value property '"
                      + prop.getName() + "' for service '" + service.getName() + "'", e);
              }
              index++;
            }
          } else {
  ...
  private void saveProperty(PreparedStatements pss, String serviceId, Property prop)
      throws Exception {
    ...
    // 'propval_obj' field and fill hash field (this will be used in search phase to
    // allow matching Serializable objects)
    if (needSerialization(value)) {
      // System.out.println("DF Handling Object property "+prop.getName()+": value =
      // "+value);
      ...
      // System.out.println("DF Handling String property "+prop.getName()+": value =
      // "+value);
  ...
  protected List searchSingle(Object template, int maxResult) throws SQLException {
    ...
    if (matchingAIDs.size() < 10) {
      ...
      // If we found several matching agents we preload protocols languages and
      // ontologies once for all
      ...
      Map allOntologies =
          preloadIdValueTable(pss.stm_selCountAllOntologies, pss.stm_selAllOntologies);
  ...
  private Map preloadIdValueTable(PreparedStatement cntStm, PreparedStatement stm)
      throws SQLException {
    Map m = null;
    ...
    if (recordCount < MAX_PRELOAD_CNT) {
      // If there are more than MAX_PRELOAD_CNT elements return null. In fact it is
      // more time consuming constructing the preload Map
      ...
      if (true) {
        ...
        while (rs.next()) {
          ...
          l.add(rs.getString(2)); // value (protocol, language, ontology ... depending on the passed
                                  // statement)
        }
        closeResultSet(rs);
      }
   ...
   */
  private DFAgentDescription getDFD(String aidN, Map allLanguages, Map allOntologies,
      Map allProtocols) throws SQLException {
    ...
    try {
      ...
      throw new SQLException(
          "Unexpected error retrieving DFD for agent " + aidN + ". " + e.getMessage());
    } finally {
      closeResultSet(rs);
      closeResultSet(rsS);
    }
  ...
  private final void loadOntologies(String descrId, DFAgentDescription dfd, Map allOntologies)
      throws SQLException {
  ...
  private final void loadLanguages(String descrId, DFAgentDescription dfd, Map allLanguages)
      throws SQLException {
  ...
  private final void loadProtocols(String descrId, DFAgentDescription dfd, Map allProtocols)
      throws SQLException {
  ...
  /**
   * Deletes the AID corresponding to the indicated agent name including all its resolver AIDs (if
   * there are no df descriptions left for them)
   */
  ...
  private void remove(String aid) throws SQLException {
    ...
    try {
      ...
      try {
        ...
        logger.log(Logger.SEVERE,
            "Rollback for incomplete remotion of DFD for agent " + aid + " failed.", se);
      }
      throw sqle;
    } finally {
      closeResultSet(rs);
  ...
  /**
   * Convert a template DFAgentDescription into the SQL SELECT operation that returns the AID names
   * whose DFAgentDescriptions match the given template.
   */
  ...
  /**
   * Removes DF registrations and subscriptions whose lease time has expired. This method is called
   * at startup and each MAX_REGISTER_WITHOUT_CLEAN registrations.
   */
  private void clean() {
  ...
  protected void subscribeSingle(Object dfd, SubscriptionResponder.Subscription s)
      throws SQLException, NotUnderstoodException {
  ...
  public Enumeration getSubscriptions() {
    ...
    try {
      ...
      while (rs.next()) {
        ...
        String aclmStr =
            new String(Base64.decodeBase64(base64Str.getBytes("US-ASCII")), "US-ASCII");
   ...
   * @return <code>true</code> if an entry has been found and removed - otherwise <code>false</code>
   */
  ...
  /**
   * Closes an open result set and logs an appropriate error message when it fails
   */
  ...
  /**
   * Closes an open SQL statement and logs an appropriate error message when it fails
   */
  ...
  /**
   * This method must be used when inserting text values into a DB. It doubles all ' and "
   * characters If the passed parameter is null, then an empty string is returned.
   **/
  ...
  /**
   * Replaces all occurences of a <code>pattern</code> in <code>str</code> with
