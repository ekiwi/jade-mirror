...
/**
 * This behaviour serves search requests according to the iterated-fipa-request protocol.
 * 
    ...
    if (msg != null) {
      if (logger.isLoggable(Logger.FINE)) {
        logger.log(Logger.FINE,
            "DF " + myAgent.getName() + ": Iterated-search request received from "
                + msg.getSender().getName() + ". Conv-ID = " + msg.getConversationId());
      }
      theDF.addBehaviour(new SSIteratedAchieveREResponder(theDF, msg) {
        ...
        protected ACLMessage handleRequest(ACLMessage request)
          ...
          if (logger.isLoggable(Logger.FINE)) {
            logger.log(Logger.FINE, "DF " + myAgent.getName() + ": Iterated-search "
                + request.getConversationId() + ". Serving request # " + cnt);
          }
          cnt++;
          try {
            ...
            if (iterator == null) {
              if (logger.isLoggable(Logger.FINER)) {
                logger.log(Logger.FINER, "DF " + myAgent.getName() + ": Iterated-search "
                    + request.getConversationId() + ". Initializing KBIterator");
              }
              iterator = theDF.iteratedSearchAction(search, request.getSender());
              if (logger.isLoggable(Logger.FINEST)) {
                logger.log(Logger.FINEST, "DF " + myAgent.getName() + ": Iterated-search "
                    + request.getConversationId() + ". KBIterator correctly initialized");
              }
            }
            ...
            for (int i = 0; i < maxResult; ++i) {
              if (iterator.hasNext()) {
                ...
                if (logger.isLoggable(Logger.FINER)) {
                  logger.log(Logger.FINER, "DF " + myAgent.getName() + ": Iterated-search "
                      + request.getConversationId() + ". Closing KBIterator");
                }
                iterator.close();
            ...
            if (logger.isLoggable(Logger.FINE)) {
              logger.log(Logger.FINE, "DF " + myAgent.getName() + ": Iterated-search "
                  + request.getConversationId() + ". Sending back " + ll.size() + " results");
            }
            ...
            try {
              ...
              throw new FailureException(
                  ExceptionVocabulary.INTERNALERROR + " \"" + e.getMessage() + "\"");
            }
            ...
            t.printStackTrace();
            throw new FailureException(
                ExceptionVocabulary.INTERNALERROR + " \"" + t.getMessage() + "\"");
          }
        ...
        protected void handleCancel(ACLMessage cancel) {
          if (logger.isLoggable(Logger.FINE)) {
            logger.log(Logger.FINE, "DF " + myAgent.getName() + ": Iterated-search "
                + cancel.getConversationId() + ". Serving cancel");
          }
          if (iterator != null) {
            if (logger.isLoggable(Logger.FINER)) {
              logger.log(Logger.FINER, "DF " + myAgent.getName() + ": Iterated-search "
                  + cancel.getConversationId() + ". Closing KBIterator");
            }
            iterator.close();
          }
        }
        ...
        /**
         * Avoid conflicts between the main MessageTemplate and that used internally by this
         * IteratedFipaRequestResponder.
         */
