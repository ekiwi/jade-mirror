...
/**
 * This behaviour serves the actions of the JADE management ontology supported by the AMS. Extends
 * RequestManagementBehaviour and implements performAction() to i) call the method of the AMS
 * corresponding to the requested action and ii) prepare the result notification depending on -
 * whether a result should be returned (RESULT or DONE) - whether the notification can be sent
 * immediately or must be delayed at a later time.
 * 
...
class AMSJadeAgentManagementBehaviour extends RequestManagementBehaviour {
  ...
  protected ACLMessage performAction(Action slAction, ACLMessage request)
    ...
    if (action instanceof CreateAgent) {
      theAMS.createAgentAction((CreateAgent) action, request.getSender(), requesterPrincipal,
          requesterCredentials);
      String agentName =
          JADEManagementOntology.adjustAgentName(((CreateAgent) action).getAgentName(),
              new String[] {((CreateAgent) action).getContainer().getName()});
    ...
    // KILL AGENT (asynchronous notification to requester)
      ...
      theAMS.killAgentAction((KillAgent) action, request.getSender(), requesterPrincipal,
          requesterCredentials);
    ...
    // KILL CONTAINER (asynchronous notification to requester)
      ...
      theAMS.killContainerAction((KillContainer) action, request.getSender(), requesterPrincipal,
          requesterCredentials);
    ...
    // QUERY PLATFORM LOCATIONS
      ...
      result = theAMS.queryPlatformLocationsAction((QueryPlatformLocationsAction) action,
          request.getSender());
      resultNeeded = true;
    }
    // QUERY AGENTS ON LOCATION
      ...
      result =
          theAMS.queryAgentsOnLocationAction((QueryAgentsOnLocation) action, request.getSender());
      resultNeeded = true;
    } else {
