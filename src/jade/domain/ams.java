 ...
 * 
 * The updating of this file to JADE 2.0 has been partially supported by the IST-1999-10211 LEAP
 * Project
 * 
...
/**
 * Standard <em>Agent Management System</em> agent. This class implements <em><b>FIPA</b></em>
 * <em>AMS</em> agent. <b>JADE</b> applications cannot use this class directly, but interact with it
...
public class ams extends Agent /* implements AgentManager.Listener */ {
  ...
  /**
   * This constructor creates a new <em>AMS</em> agent. Since a direct reference to an Agent
   * Platform implementation must be passed to it, this constructor cannot be called from
   * application code. Therefore, no other <em>AMS</em> agent can be created beyond the default one.
   */
  ...
  protected void setup() {
    ...
    try {
      ...
      logger.log(Logger.WARNING,
          "Wrong periodic log delay value " + sLogDelay + ". It must be an integer value.");
    }
    ...
    try {
      ...
      logger.log(Logger.WARNING,
          "Wrong max result limit " + sMaxResults + ". It must be an integer value.");
    }
    ...
    // Use fully qualified name to avoid conflict with old
    ...
    MessageTemplate mtF = MessageTemplate.and(MessageTemplate.MatchPerformative(ACLMessage.REQUEST),
    ...
    MessageTemplate mtJ = MessageTemplate.and(MessageTemplate.MatchPerformative(ACLMessage.REQUEST),
        MessageTemplate.or(MessageTemplate.MatchOntology(JADEManagementVocabulary.NAME),
    ...
    Thread t = new Thread() {
      public void run() {
        logger.log(Logger.FINE,
            ">>>>>>>>> Shutdown Hook activated. AMS state = " + ams.this.getState());
        if (!shuttingDown && ams.this.getState() != Agent.AP_DELETED) {
          ...
          try {
            logger.log(Logger.WARNING,
                ">>>>>>>>> Main Container JVM is terminating. Activate platform shutdown");
  ...
  // CREATE AGENT
  void createAgentAction(final CreateAgent ca, final AID requester,
      final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials)
    ...
    if (logger.isLoggable(Logger.FINE))
      logger.log(Logger.FINE, "Agent " + requester + " requesting Create-agent " + agentID
    ...
    Thread auxThread = new Thread() {
      public void run() {
        try {
          myPlatform.create(agentName, className, args, container, owner, initialCredentials,
              requesterPrincipal, requesterCredentials);
        } catch (UnreachableException ue) {
          // Send failure notification to the requester if any. Note that
          // UnreachableException also wraps IMTPException that is not necessarily related
          // to a real un-reachability problem
          sendFailureNotification(ca, agentID, new InternalError(ue.getMessage()));
        } catch (JADESecurityException ae) {
          if (logger.isLoggable(Logger.SEVERE))
            logger.log(Logger.SEVERE, "Agent " + requester.getName()
                + " does not have permission to perform action Create-agent: " + ae);
          // Send failure notification to the requester if any
          sendFailureNotification(ca, agentID, new Unauthorised());
        } catch (NotFoundException nfe) {
          // Send failure notification to the requester if any
          sendFailureNotification(ca, agentID,
              new InternalError("Destination container not found. " + nfe.getMessage()));
  ...
  // KILL AGENT
  void killAgentAction(KillAgent ka, AID requester, JADEPrincipal requesterPrincipal,
      Credentials requesterCredentials) throws FIPAException {
    ...
    try {
      ...
      if (logger.isLoggable(Logger.SEVERE))
        logger.log(Logger.SEVERE, "Agent " + requester.getName()
            + " does not have permission to perform action KillAgent");
  ...
  void cloneAgentAction(CloneAction ca, AID requester) throws FIPAException {
    ...
    if (logger.isLoggable(Logger.CONFIG))
      logger.log(Logger.CONFIG,
          "Agent " + requester + " requesting Clone-agent " + agentID + " on container " + where);
    ...
    try {
      ...
      if (logger.isLoggable(Logger.SEVERE))
        logger.log(Logger.SEVERE, "Agent " + requester.getName()
            + " does not have permission to perform action CloneAgent");
      throw new Unauthorised();
    } catch (UnreachableException ue) {
      throw new InternalError("Container not reachable. " + ue.getMessage());
    } catch (NotFoundException nfe) {
  ...
  void moveAgentAction(MoveAction ma, AID requester) throws FIPAException {
    ...
    if (logger.isLoggable(Logger.FINE))
      logger.log(Logger.FINE,
          "Agent " + requester + " requesting Move-agent " + agentID + " on container " + where);
    ...
    try {
      ...
      if (logger.isLoggable(Logger.SEVERE))
        logger.log(Logger.SEVERE, "Agent " + requester.getName()
            + " does not have permission to perform action MoveAgent");
      throw new Unauthorised();
    } catch (UnreachableException ue) {
      throw new InternalError("Container not reachable. " + ue.getMessage());
    } catch (NotFoundException nfe) {
  ...
  // KILL CONTAINER
  void killContainerAction(final KillContainer kc, final AID requester,
      final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials)
      throws FIPAException {
    ...
    Thread auxThread = new Thread() {
      public void run() {
        try {
          ...
          logger.log(Logger.SEVERE, "Agent " + requester.getName()
              + " does not have permission to perform action Kill-container: " + ae);
          // Send failure notification to the requester if any
          sendFailureNotification(kc, cid, new Unauthorised());
        } catch (NotFoundException nfe) {
          // Send failure notification to the requester if any
          sendFailureNotification(kc, cid,
              new InternalError("Container not found. " + nfe.getMessage()));
        } catch (UnreachableException ue) {
          // Send failure notification to the requester if any
          sendFailureNotification(kc, cid,
              new InternalError("Container unreachable. " + ue.getMessage()));
        } catch (Throwable t) {
          // Send failure notification to the requester if any
  ...
  // SHUTDOWN PLATFORM
  void shutdownPlatformAction(ShutdownPlatform sp, final AID requester,
      final JADEPrincipal requesterPrincipal, final Credentials requesterCredentials)
      throws FIPAException {
    logger.log(Logger.INFO,
        "AMS - Activating platform shutdown. Requester = " + requester.getName());
    ...
    Thread auxThread = new Thread() {
      public void run() {
        try {
          ...
          logger.log(Logger.SEVERE, "Agent " + requester.getName()
              + " does not have permission to perform action Shutdown-Platform: " + ae);
          shuttingDown = false;
        }
      }
    };
  ...
  List queryPlatformLocationsAction(QueryPlatformLocationsAction qpl, AID requester)
      throws FIPAException {
    if (logger.isLoggable(Logger.FINE))
  ...
  void registerAction(Register r, AID requester) throws FIPAException {
    ...
    if (id == null || id.getName() == null || id.getName().length() == 0) {
      throw new MissingParameter(FIPAManagementVocabulary.AMSAGENTDESCRIPTION,
    ...
    try {
      ...
      if (logger.isLoggable(Logger.SEVERE))
        logger.log(Logger.SEVERE, "Agent " + requester.getName()
            + " does not have permission to perform action Register");
      throw new Unauthorised();
    } catch (Exception e) {
      e.printStackTrace();
      throw new InternalError("Unexpected exception. " + e.getMessage());
  ...
  void deregisterAction(Deregister d, AID requester) throws FIPAException {
    ...
    try {
      ...
      if (logger.isLoggable(Logger.SEVERE))
        logger.log(Logger.SEVERE, "Agent " + requester.getName()
            + " does not have permission to perform action Deregister");
      throw new Unauthorised();
    } catch (Exception e) {
      e.printStackTrace();
      throw new InternalError("Unexpected exception. " + e.getMessage());
  ...
  void modifyAction(Modify m, AID requester) throws FIPAException {
    ...
    if (logger.isLoggable(Logger.FINE)) {
      ...
      logger.log(Logger.FINE,
          "New state is " + amsd.getState() + ". New ownership is " + amsd.getOwnership());
    }
    if (id == null || id.getName() == null || id.getName().length() == 0) {
      throw new MissingParameter(FIPAManagementVocabulary.AMSAGENTDESCRIPTION,
          FIPAManagementVocabulary.DFAGENTDESCRIPTION_NAME);
    ...
    try {
      ...
      if (logger.isLoggable(Logger.SEVERE))
        logger.log(Logger.SEVERE,
            "Agent " + requester.getName() + " does not have permission to perform action Modify");
      throw new Unauthorised();
    } catch (Exception e) {
      e.printStackTrace();
      throw new InternalError("Unexpected exception. " + e.getMessage());
  ...
  List searchAction(Search s, AID requester) {
    ...
    return myPlatform.amsSearch((AMSAgentDescription) s.getDescription(),
        getActualMaxResults(s.getConstraints()));
  }
   ...
   *         <li>The AMS internal max result limit if constraints.maxResults < 0 (the FIPA specs
   *         requires it to be infinite, but for practical reason we prefer to limit it)
   *         <li>constraints.maxResults otherwise
   *         </ul>
   **/
  private long getActualMaxResults(SearchConstraints constraints) {
    long maxResult =
        (constraints.getMaxResults() == null ? 1 : constraints.getMaxResults().longValue());
  ...
  //////////////////////////////////////////////////////////////////
  /**
   * Inner calss RegisterToolBehaviour. This behaviour handles subscriptions of tools i.e. agents to
   * be notified about platform events.
   */
  ...
  /**
   * Inner calss DeregisterToolBehaviour. This behaviour handles tools un-subscriptions.
   */
  ...
  /**
   * Inner interface Handler. Perform additional operations related to a given platform event
   */
  ...
  /**
   * Inner class EventManager. This behaviour notifies - all registered tools about all platform
   * events - the agent that had requested the AMS to perform an action about the platform event
   * forced by that action. Note that this is done only for actions that produce an "asynchronous
   * event".
   */
  ...
  private void notifyTools(EventRecord er) throws Exception {
    // Unless explicitly instructed to do so, avoid notifying tools when the
      ...
      try {
        ...
        for (int j = 0; j < agents.length; j++) {
          ...
          if (!amsd.getState().equals(AMSAgentDescription.LATENT)) {
            ...
            // ContainerID is null in case of foreign agents registered with this AMS or
            // virtual agents
   ...
   *
   * public void bornAgent(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); AID agentID =
   * ev.getAgent(); String ownership = ev.getNewOwnership();
   * 
   * BornAgent ba = new BornAgent(); ba.setAgent(agentID); ba.setWhere(cid);
   * ba.setState(AMSAgentDescription.ACTIVE); ba.setOwnership(ownership); ba.setClassName((String)
   * agentID.getAllUserDefinedSlot().get(AID.AGENT_CLASSNAME));
   * 
   * EventRecord er = new EventRecord(ba, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a DeadAgent event in the AMS event queue
   *
   * public void deadAgent(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); AID agentID =
   * ev.getAgent();
   * 
   * DeadAgent da = new DeadAgent(); da.setAgent(agentID); da.setWhere(cid); if
   * (ev.getContainerRemoved()) { da.setContainerRemoved(new Boolean(true)); }
   * 
   * EventRecord er = new EventRecord(da, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a SuspendedAgent event in the AMS event queue
   *
   * public void suspendedAgent(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); AID name =
   * ev.getAgent();
   * 
   * SuspendedAgent sa = new SuspendedAgent(); sa.setAgent(name); sa.setWhere(cid);
   * 
   * EventRecord er = new EventRecord(sa, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a ResumedAgent event in the AMS event queue
   *
   * public void resumedAgent(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); AID name =
   * ev.getAgent();
   * 
   * ResumedAgent ra = new ResumedAgent(); ra.setAgent(name); ra.setWhere(cid);
   * 
   * EventRecord er = new EventRecord(ra, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a FrozenAgent event in the AMS event queue
   *
   * public void frozenAgent(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); AID name =
   ...
   * 
   * EventRecord er = new EventRecord(fa, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a ThawedAgent event in the AMS event queue
   *
   * public void thawedAgent(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); AID name =
   * ev.getAgent(); ContainerID bufferContainer = ev.getNewContainer();
   * 
   ...
   * 
   * EventRecord er = new EventRecord(ta, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a MovedAgent event in the AMS event queue
   *
   * public void movedAgent(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID from = ev.getContainer(); ContainerID to
   * = ev.getNewContainer(); AID agentID = ev.getAgent();
   * 
   * MovedAgent ma = new MovedAgent(); ma.setAgent(agentID); ma.setFrom(from); ma.setTo(to);
   * 
   * EventRecord er = new EventRecord(ma, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a ChangedAgentOwnership event in the AMS event queue
   *
   * public void changedAgentPrincipal(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); AID name =
   * ev.getAgent();
   * 
   * ChangedAgentOwnership cao = new ChangedAgentOwnership(); cao.setAgent(name); cao.setWhere(cid);
   * cao.setFrom(ev.getOldOwnership()); cao.setTo(ev.getNewOwnership());
   * 
   * EventRecord er = new EventRecord(cao, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put an AddedContainer event in the AMS event queue
   *
   * public void addedContainer(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); String name =
   * cid.getName();
   * 
   * AddedContainer ac = new AddedContainer(); ac.setContainer(cid);
   * 
   * EventRecord er = new EventRecord(ac, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */

  /**
   * Put a RemovedContainer event in the AMS event queue
   *
   * public void removedContainer(PlatformEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, ev.toString()); ContainerID cid = ev.getContainer(); String name =
   * cid.getName();
   * 
   * RemovedContainer rc = new RemovedContainer(); rc.setContainer(cid);
   * 
   * EventRecord er = new EventRecord(rc, here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
   */
   ...
   * (logger.isLoggable(Logger.CONFIG)) logger.log(Logger.CONFIG, ev.toString()); // FIXME: There is
   * no element in the IntrospectionOntology // corresponding to this event }
   */

  /**
   * Put a AddedMTP event in the AMS event queue
   *
   * public synchronized void addedMTP(MTPEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, "MTPEvent [added MTP]"); Channel ch = ev.getChannel(); ContainerID
   * cid = ev.getPlace(); String proto = ch.getProtocol(); String address = ch.getAddress();
   * 
   * // Generate a suitable AMS event AddedMTP amtp = new AddedMTP(); amtp.setAddress(address);
   * amtp.setProto(proto); amtp.setWhere(cid);
   * 
   * EventRecord er = new EventRecord(amtp, here()); er.setWhen(ev.getTime()); eventQueue.put(er);
   * 
   * // The PlatformDescription has changed --> Generate a suitable event PlatformDescription ap =
   * new PlatformDescription(); ap.setPlatform(getDescriptionAction(null)); er = new EventRecord(ap,
   ...
   *
   * public synchronized void removedMTP(MTPEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, "MTPEvent [removed MTP]"); Channel ch = ev.getChannel(); ContainerID
   * cid = ev.getPlace(); String proto = ch.getProtocol(); String address = ch.getAddress();
   * 
   * RemovedMTP rmtp = new RemovedMTP(); rmtp.setAddress(address); rmtp.setProto(proto);
   * rmtp.setWhere(cid);
   * 
   * EventRecord er = new EventRecord(rmtp, here()); er.setWhen(ev.getTime()); eventQueue.put(er);
   * 
   * // The PlatformDescription has changed --> Generate a suitable event PlatformDescription ap =
   * new PlatformDescription(); ap.setPlatform(getDescriptionAction(null)); er = new EventRecord(ap,
   * here()); er.setWhen(ev.getTime()); eventQueue.put(er); }
  ...
  /*
   * public void messageIn(MTPEvent ev) { if (logger.isLoggable(Logger.CONFIG))
   * logger.log(Logger.CONFIG, "MTPEvent [message in]"); }
   * 
   * public void messageOut(MTPEvent ev) { if (logger.isLoggable(Logger.CONFIG))
  ...
  /**
   * Store a notification message to be sent at a later time. Package-scoped as it is called by the
   * AMSJadeAgentManagementBehaviour
   */
  ...
  /**
   * This method is executed by an auxiliary thread started by the AMS to perform certain actions
   * (createAgent, killContainer) asynchronously
   */
  private void sendFailureNotification(final Concept action, final Object key,
      final FIPAException fe) {
