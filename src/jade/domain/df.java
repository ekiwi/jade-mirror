...
import jade.domain.DFGUIManagement.GetDescription; // Explicitly imported to avoid conflict with
...
/**
 * <p>
 * Standard <em>Directory Facilitator</em> agent. This class implements <em><b>FIPA</b></em>
 * <em>DF</em> agent. <b>JADE</b> applications typically don't use this class directly, but interact
 * with the DF agent through <em>ACL</em> message passing. The <code>DFService</code> class provides
 * a number of static methods that facilitate this task. More <em>DF</em> agents can be created by
 * application programmers to divide a platform into many <em><b>Agent Domains</b></em>.
 * <p>
 * A DF agent accepts a number of optional configuration parameters that can be set either as
 * command line options or within a properties file (to be passed to the DF as an argument).
 * </p>
 ...
 * <td>If set to <code>true</code>, indicates that the DF will automatically clean up registrations
 * as soon as an agent terminates. The default is <code>false</code></td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_maxleasetime</code></td>
 * <td>Indicates the maximum lease time (in millisecond) that the DF will grant for agent
 * description registrations (defaults to infinite).</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_maxresult</code></td>
 * <td>Indicates the maximum number of items found in a search operation that the DF will return to
 * the requester (defaults to 100).</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_disablevalidation</code></td>
 * <td>Turns off content validation on incoming/outgoing messages (defaults to false)</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-default</code></td>
 * <td>If set to <code>true</code>, indicates that the DF will store its catalogue into an internal
 * HSQL database, running within the same VM. (The HSQL jar files have to added to the Java
 * CLASSPATH)</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-url</code></td>
 * <td>Indicates the JDBC URL of the database the DF will store its catalogue into. This parameter
 * is ignored if <code>jade_domain_df_db-default</code> is set. If neither this parameter nor
 * <code>jade_domain_df_db-default</code> is specified the DF will keep its catalogue in
 * memory.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-driver</code></td>
 * <td>Indicates the JDBC driver to be used to access the DF database (defaults to the ODBC-JDBC
 * bridge). This parameter is ignored if <code>jade_domain_df_db-url</code> is not set or
 * <code>jade_domain_df_db-default</code> is set.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-username</code>, <code>jade_domain_df_db-password</code></td>
 * <td>Indicate the username and password to be used to access the DF database (default to null).
 ...
 * <td>If set to <code>true</code>, indicates that the DF will clean the content of all pre-existing
 * database tables, used by the DF. This parameter is ignored if the catalogue is not stored in a
 * database.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_db-abortonerror</code></td>
 * <td>If set to <code>true</code>, indicates that the DF will immediately terminate in case it
 * cannot connect to the database. This parameter is ignored if the catalogue is not stored in a
 * database.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_kb-factory</code></td>
 * <td>Indicates the name of the factory class that should be used to create the knowledge base
 * objects for the DF. The class has to be a sub class of jade.domain.DFKBFactory.</td>
 * </tr>
 * <tr>
 * <td><code>jade_domain_df_poolsize</code></td>
 * <td>The dimension of the pool of thread dedicated to serving registration, deregistration and
 * search requests. If <code>0</code> (default) is specified then registration, deregistration and
 * search requests are served directly by the df agent Thread. This parameter is ignored when using
 * a volatile (in-memory) knowledge base.</td>
 * </tr>
 * </table>
 * 
 * <p>
 * For instance the following command line will launch a JADE main container with a DF that will
 * store its catalogue into a database accessible at URL jdbc:odbc:dfdb and that will keep agent
 * registrations for 1 hour at most.
 * 
 * <code>
 java jade.Boot -gui -jade_domain_df_db-url jdbc:odbc:dfdb -jade_domain_df_maxleasetime 3600000
 </code>
 * <p>
 * Each DF has a GUI but, by default, it is not visible. The GUI of the agent platform includes a
 * menu item that allows to show the GUI of the default DF. In order to show the GUI, you should
 * simply send the following message to each DF agent:
 * <code>(request :content (action DFName (SHOWGUI))
...
public class df extends GuiAgent implements DFGUIAdapter {
  ...
  /*
   * This is the actual value for the limit on the maximum number of results to be returned in case
   * of an ulimited search. This value is read from the Profile, if no value is set in the Profile,
   * then DEFAULT_MAX_RESULTS is used instead.
   */
  ...
  /**
   * Default constructor. This constructor does nothing; however, applications can create their own
   * DF agents using regular platform management commands. Moreover, customized versions of a
   * Directory Facilitator agent can be built subclassing this class and exploiting its protected
   * interface.
   */
  public df() {}

  /**
   * This method starts all behaviours needed by <em>DF</em> agent to perform its role within
   * <em><b>JADE</b></em> agent platform.
   */
  protected void setup() {
    ...
    logger.log(Logger.CONFIG,
        "Agent " + getLocalName() + " - Max lease time = "
            + (maxLeaseTime != null ? ISO8601.toRelativeTimeString(maxLeaseTime.getTime())
                : "infinite"));
    ...
    logger.log(Logger.CONFIG,
        "Agent " + getLocalName() + " - Max search result = " + maxResultLimit);
    ...
    if (dbDefault) {
      ...
      try {
        agentDescriptions =
            kbFactory.getDFDBKB(maxResultLimit, null, null, null, null, cleanTables);
      } catch (Exception e) {
        logger.log(Logger.SEVERE, "Agent " + getLocalName()
            + " - Error creating persistent KB based on internal HSQLDB database", e);
        if (dbAbortOnError) {
          doDelete();
          return;
        } else {
          logger.log(Logger.WARNING,
              "Agent " + getLocalName() + " - Using volatile (in-memory) KB");
    ...
    if (agentDescriptions == null && dbUrl != null) {
      ...
      try {
        agentDescriptions = kbFactory.getDFDBKB(maxResultLimit, dbDriver, dbUrl, dbUsername,
            dbPassword, cleanTables);
      } catch (Exception e) {
        logger.log(Logger.SEVERE,
    ...
    if (agentDescriptions == null) {
      ...
      if (sPoolsize != null) {
        logger.log(Logger.WARNING, "Agent " + getLocalName() + " - Ignoring pool-size indication ("
            + sPoolsize + "). Parameter not supported when using volatile KB");
        sPoolsize = null;
      }
    }
     ...
     * DFMemKB(Integer.parseInt(getProperty(MAX_RESULTS, DEFAULT_MAX_RESULTS))); #PJAVA_INCLUDE_END
     */
    ...
    if (poolSize == 0) {
      ...
      logger.log(Logger.INFO,
          "Agent " + getLocalName() + " - FIPA request pool-size = " + poolSize);
    ...
    // #PJAVA_EXCLUDE_END
    /*
     * #PJAVA_INCLUDE_BEGIN DFFipaAgentManagementBehaviour fipaRequestResponder = new
     * DFFipaAgentManagementBehaviour(this, mt1); addBehaviour(fipaRequestResponder);
     * #PJAVA_INCLUDE_END
     */
    ...
    DFIteratedSearchManagementBehaviour iteratedSearchResponder =
        new DFIteratedSearchManagementBehaviour(this, mt1);
    ...
    DFJadeAgentManagementBehaviour jadeRequestResponder =
        new DFJadeAgentManagementBehaviour(this, mt1);
    addBehaviour(jadeRequestResponder);

    // Behaviour dealing with DFApplet management actions
    mt1 = MessageTemplate.and(mt,
        MessageTemplate.MatchOntology(DFAppletOntology.getInstance().getName()));
    ...
    agentDescriptions.setLeaseManager(new LeaseManager() {
      ...
      /**
       * Grant a lease to this request according to the policy of the DF: if the requested lease is
       * greater than the max lease policy of this DF, then the granted lease is set to the max of
       * the DF.
       **/
  ...
  private void unsubscribeDeadAgent(AID id) {
    ...
    for (int i = 0; i < ss.size(); i++) {
      try {
        ...
        logger.log(Logger.WARNING, "Agent " + getLocalName()
            + " - Error deregistering subscription of dead agent " + id.getName(), e);
      }
    }
  }
  ...
  /**
   * Cleanup <em>DF</em> on exit. This method performs all necessary cleanup operations during agent
   * shutdown.
   */
  ...
  /**
   * Add the behaviour handling a recursive search. If constraints contains a null search_id, then a
   * new one is generated and the new search_id is stored into searchIdCache for later check (i.e.
   * to avoid search loops).
   */
  private void performRecursiveSearch(List localResults, DFAgentDescription dfd,
      SearchConstraints constraints, Search action) {
    ...
    if (logger.isLoggable(Logger.CONFIG))
      logger.log(Logger.CONFIG,
          "Agent " + getLocalName() + " - Activating recursive search: " + localResults.size()
              + " item(s) found locally. " + maxRes + " expected. Search depth is " + maxDep
              + ". Search ID is " + searchId + ". Propagating search to " + children.size()
              + " federated DF(s)");
  ...
  /**
   * Inner class RecursiveSearchHandler. This is a behaviour handling recursive searches i.e.
   * searches that must be propagated to children (federated) DFs.
   */
  private class RecursiveSearchHandler extends AchieveREInitiator {
     ...
     * 
     * @param results The search results. Initially this includes the items found locally.
     ...
     * @param action The original Search action. This is used as a key to retrieve the incoming
     *        REQUEST message.
     */
    private RecursiveSearchHandler(List results, DFAgentDescription template,
        SearchConstraints constraints, Search action) {
      super(df.this, null);
    ...
    protected Vector prepareRequests(ACLMessage request) {
      ...
      if (incomingRequest != null) {
        ...
        while (it.hasNext()) {
          ...
          ACLMessage msg = DFService.createRequestMessage(myAgent, childDF,
              FIPAManagementVocabulary.SEARCH, template, constraints);
    ...
    /**
     * As long as we receive the replies we update the results. If we reach the max-results we send
     * back the notification to the requester and discard successive replies.
     */
    protected void handleInform(ACLMessage inform) {
      if (logger.isLoggable(Logger.CONFIG))
        logger.log(Logger.CONFIG, "Agent " + getLocalName()
            + " - Recursive search result received from " + inform.getSender().getName() + ".");
      int cnt = 0;
    ...
    protected void handleRefuse(ACLMessage refuse) {
      if (logger.isLoggable(Logger.WARNING))
        logger.log(Logger.WARNING,
            "Agent " + getLocalName() + " - REFUSE received from federated DF "
    ...
    protected void handleFailure(ACLMessage failure) {
      ...
      if (logger.isLoggable(Logger.WARNING))
        logger.log(Logger.WARNING,
            "Agent " + getLocalName() + " - FAILURE received from federated DF "
    ...
    protected void handleNotUnderstood(ACLMessage notUnderstood) {
      if (logger.isLoggable(Logger.WARNING))
        logger.log(Logger.WARNING,
            "Agent " + getLocalName() + " - NOT_UNDERSTOOD received from federated DF "
  ...
  void DFRegister(DFAgentDescription dfd) throws AlreadyRegistered {
    ...
    if (isADF(dfd)) {
      if (logger.isLoggable(Logger.INFO))
        logger.log(Logger.INFO, "Agent " + getLocalName() + " - Added federation "
            + dfd.getName().getName() + " --> " + getName());
  ...
  /**
   * Serve the Register action of the FIPA management ontology. Package scoped since it is called by
   * DFFipaAgentManagementBehaviour.
   */
  ...
  /**
   * Serve the Deregister action of the FIPA management ontology. Package scoped since it is called
   * by DFFipaAgentManagementBehaviour.
   */
  ...
  /**
   * Serve the Modify action of the FIPA management ontology. Package scoped since it is called by
   * DFFipaAgentManagementBehaviour.
   */
  ...
  /**
   * Serve the Search action of the FIPA management ontology. Package scoped since it is called by
   * DFFipaAgentManagementBehaviour.
   * 
   * @return the List of descriptions matching the template specified in the Search action. If no
   *         description is found an empty List is returned. In case a recursive search is required
   *         it returns null to indicate that the result is not yet available.
   */
  ...
  /**
   * Serve a Search action of the FIPA management ontology requested using an iterated-fipa-request
   * protocol. Package scoped since it is called by DFIteratedSearchManagementBehaviour.
   * 
   * @return an iterator over the DFAgentDescription matching the specified search template.
   */
  ...
  /**
   * Serve the ShowGui action of the JADE management ontology. Package scoped since it is called by
   * DFJadeAgentManagementBehaviour.
   * 
   * @exception FailureException If the GUI is already visible or some error occurs creating the
   *            GUI.
   */
  void showGuiAction(ShowGui sg, AID requester) throws FailureException {
    if (logger.isLoggable(Logger.CONFIG))
      logger.log(Logger.CONFIG, "Agent " + getLocalName() + " - Agent " + requester.getName()
          + " requesting action ShowGui");
    if (!showGui()) {
  ...
  /**
   * Serve the GetParents action of the DF-Applet ontology Package scoped since it is called by
   * DFAppletManagementBehaviour.
   */
  ...
  /**
   * Serve the GetDescription action of the DF-Applet ontology Package scoped since it is called by
   * DFAppletManagementBehaviour.
   */
  List getDescriptionAction(GetDescription action, AID requester) {
    if (logger.isLoggable(Logger.CONFIG))
      logger.log(Logger.CONFIG, "Agent " + getLocalName() + " - Agent " + requester
          + " requesting action GetDescription.");
  ...
  /**
   * Serve the GetDescriptionUsed action of the DF-Applet ontology Package scoped since it is called
   * by DFAppletManagementBehaviour.
   */
  ...
  /**
   * Serve the Federate action of the DF-Applet ontology Package scoped since it is called by
   * DFAppletManagementBehaviour.
   */
  ...
  /**
   * Serve the RegisterWith action of the DF-Applet ontology Package scoped since it is called by
   * DFAppletManagementBehaviour.
   */
  ...
  /**
   * Serve the DeregisterFrom action of the DF-Applet ontology Package scoped since it is called by
   * DFAppletManagementBehaviour.
   */
  ...
  /**
   * Serve the ModifyOn action of the DF-Applet ontology Package scoped since it is called by
   * DFAppletManagementBehaviour.
   */
  ...
  /**
   * Serve the SearchOn action of the DF-Applet ontology Package scoped since it is called by
   * DFAppletManagementBehaviour.
   */
            ...
            Behaviour b = new RemoteDFRequester(df, r) {
              public int onEnd() {
                ...
                if (!(result instanceof InternalError)) {
                  ...
                  gui.showStatusMsg(
                      "Error processing request. " + ((InternalError) result).getMessage());
                }
                return 0;
              }
            };
  ...
  protected boolean showGui() {
    if (gui == null) {
      try {
        ...
        gui.setAdapter(df.this); // this method must be called to avoid reflection (the constructor
                                 // of the df
  ...
  /**
   * This method set the description of the df according to the DFAgentDescription passed. The
   * programmers can call this method to provide a different initialization of the description of
   * the df they are implementing. The method is called inside the setup of the agent and set the df
  ...
  protected void addParent(AID dfName, DFAgentDescription dfd) {
    ...
    dscDFParentMap.put(dfName, dfd); // update the table of corrispondence between parents and
                                     // description of this
                                     // df used to federate.
  ...
  /**
   * Store the request message related to an action that is being processed by a Behaviour. This
   * information will be used by the Behaviour to send back the notification to the requester.
   */
  ...
  /**
   * Send the notification related to an action that has been processed by a Behaviour.
   */
  private void sendPendingNotification(Concept action, Object result) {
    ...
    if (request != null) {
      ...
      if (logger.isLoggable(Logger.WARNING))
        logger.log(Logger.WARNING,
            "Agent " + getLocalName() + " - Processed action request not found.");
    }
  }
   ...
   *         <li>maxResultLimit if constraints.maxResults is infinite (i.e. < 0) or greater than
   *         maxResultLimit
  ...
  int getActualMaxResults(SearchConstraints constraints) {
    int maxResult =
        (constraints.getMaxResults() == null ? 1 : constraints.getMaxResults().intValue());
    maxResult = ((maxResult < 0 || maxResult > maxResultLimit) ? maxResultLimit : maxResult); // limit
                                                                                              // the
                                                                                              // max
                                                                                              // num
                                                                                              // of
  ...
  /**
   * Check if this search must be served, i.e. if it has not yet been received. In particular the
   * value of search_id must be different from any prior value that was received. If search_id is
   * not null and it has not yet been received, search_id is added into the cache.
   * 
  ...
  private boolean isADF(DFAgentDescription dfd) {
    try {
      return ((ServiceDescription) dfd.getAllServices().next()).getType()
          .equalsIgnoreCase("fipa-df");
    } catch (Exception e) {
      return false;
    }
  }
