...
/**
 * This class provides a set of static methods to communicate with a DF Service that complies with
 * FIPA specifications. It includes methods to register, deregister, modify and search with a DF.
 * Each of this method has version with all the needed parameters, or with a subset of them where,
 * those parameters that can be omitted have been defaulted to the default DF of the platform, the
 * AID of the sending agent, the default Search Constraints.
 * <p>
 * Notice that all these methods block every activity of the agent until the action (i.e.
 * register/deregister/modify/search) has been successfully executed or a jade.domain.FIPAException
 * exception has been thrown (e.g. because a FAILURE message has been received from the DF).
 * <p>
 * In some cases, instead, it is more convenient to execute these tasks in a non-blocking way. In
 * these cases a <code>jade.proto.AchieveREInitiator</code> or
 * <code>jade.proto.SubscriptionInitiator</code> should be used in conjunction with the
 * <code>createRequestMessage(), createSubscriptionMessage(), 
 * decodeDone(), decodeResult() and decodeNotification()</code> methods that facilitate the
 * preparation and decoding of messages to be sent/received to/from the DF. The following piece of
 * code exemplifies that in the case of an agent subscribing to the default DF.
 * 
 * <pr>
 * <hr>
 * <blockquote>
...
public class DFService extends FIPAService {
  ...
  /**
   * check that the <code>DFAgentDescription</code> contains the mandatory slots, i.e. the agent
   * name and, for each servicedescription, the service name and the service type
   * 
  ...
  static void checkIsValid(DFAgentDescription dfd, boolean checkServices) throws MissingParameter {
    try {
      if (dfd.getName().getName().length() == 0) {
        throw new MissingParameter(FIPAManagementVocabulary.DFAGENTDESCRIPTION,
            FIPAManagementVocabulary.DFAGENTDESCRIPTION_NAME);
      }
    ...
    if (checkServices) {
      ...
      while (i.hasNext()) {
        ...
        if (sd.getName() == null)
          throw new MissingParameter(FIPAManagementVocabulary.SERVICEDESCRIPTION,
              FIPAManagementVocabulary.SERVICEDESCRIPTION_NAME);
        if (sd.getType() == null)
          throw new MissingParameter(FIPAManagementVocabulary.SERVICEDESCRIPTION,
              FIPAManagementVocabulary.SERVICEDESCRIPTION_TYPE);
      }
    }
  }
   ...
   * <p>
   * It should be noted that, depending on the policy adopted by the DF, the granted lease time for
   * the registration can be shorter than the requested one. This can be checked by looking at the
   * DF-Description actually registered by the DF that is made available as the return value of this
   * method. The <code>keepRegistered()</code> method can be used to keep the registration valid
   * until a given time.
   * 
   * @param a is the Agent performing the registration (it is needed in order to send/receive
   *        messages)
   * @param dfName The AID of the <b>DF</b> agent to register with.
   * @param dfd A <code>DFAgentDescription</code> object containing all data necessary to the
   *        registration. If the Agent name is empty, than it is set according to the <code>a</code>
   *        parameter.
   * @return the <code>DFAgentDescription</code> actually registered by the DF
   * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code> or
   *            <code>NOT_UNDERSTOOD</code> message is received from the DF (to indicate some error
   *            condition) or if the supplied DF-Description is not valid.
   * @see #keepRegistered(Agent, AID, DFAgentDescription, Date)
   */
  public static DFAgentDescription register(Agent a, AID dfName, DFAgentDescription dfd)
    ...
    ACLMessage request =
        createRequestMessage(a, dfName, FIPAManagementVocabulary.REGISTER, dfd, null);
  ...
  public static void deregister(Agent a, AID dfName, DFAgentDescription dfd) throws FIPAException {
    ...
    ACLMessage request =
        createRequestMessage(a, dfName, FIPAManagementVocabulary.DEREGISTER, dfd, null);
    ACLMessage reply = doFipaRequestClient(a, request);
    if (reply == null) {
      throw new FIPAException("Missing reply");
    }
  ...
  /**
   * Deregisters a <code>DFAgentDescription</code> from the default DF. A default DF-Description is
   * used which contains only the AID of this agent.
   * 
   * @exception FIPAException
   * @see #deregister(Agent a, AID dfName, DFAgentDescription dfd)
   **/
  ...
  /**
   * Modifies a previously registered DF-Description within a <b>DF</b> agent.
   * <p>
   * It should be noted that, depending on the policy adopted by the DF, the granted lease time for
   * the modified registration can be shorter than the requested one. This can be checked by looking
   * at the DF-Description actually registered by the DF that is made available as the return value
   * of this method. The <code>keepRegistered()</code> method can be used to keep the registration
   * valid until a given time.
   * 
   * @param a is the Agent performing the request of modification
   * @param dfName The AID of the <b>DF</b> agent holding the data to be changed.
   * @param dfd A <code>DFAgentDescription</code> object containing all new data values;
   * @return the <code>DFAgentDescription</code> actually registered (after the modification) by the
   *         DF
   * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code> or
   *            <code>NOT_UNDERSTOOD</code> message is received from the DF (to indicate some error
   *            condition) or if the supplied DF-Description is not valid.
   * @see #keepRegistered(Agent, AID, DFAgentDescription, Date)
   */
  public static DFAgentDescription modify(Agent a, AID dfName, DFAgentDescription dfd)
      throws FIPAException {
    if (dfd == null) {
      dfd = new DFAgentDescription();
    }
    ...
    ACLMessage request =
        createRequestMessage(a, dfName, FIPAManagementVocabulary.MODIFY, dfd, null);
    ACLMessage reply = doFipaRequestClient(a, request);
    if (reply == null) {
      throw new FIPAException("Missing reply");
    }
  ...
  /**
   * Add a suitable behaviour that ensures that a DF-Description currently registered with a DF is
   * kept registered until a given deadline. This method is particularly useful when dealing with a
   * DF agent that grants limited lease time for agent registrations. The following piece of code
   ...
   * @param deadline The time until which the currenlty registered DF-Description must be kept
   *        valid. Use null to indicate an infinite time
   */
   ...
   * @param dfd A <code>DFAgentDescription</code> object containing data to search for; this
   *        parameter is used as a template to match data against.
   * @param constraints of the search
   * @return An array of <code>DFAgentDescription</code> containing all found items matching the
   *         given descriptor, subject to given search constraints for search depth and result size.
   * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code> or
   *            <code>NOT_UNDERSTOOD</code> message is received from the DF (to indicate some error
   *            condition)
   */
  public static DFAgentDescription[] search(Agent a, AID dfName, DFAgentDescription dfd,
    ...
    ACLMessage request =
        createRequestMessage(a, dfName, FIPAManagementVocabulary.SEARCH, dfd, constraints);
    ...
    try {
      ...
      throw new FIPAException(
          "Property " + DF_SEARCH_TIMEOUT_KEY + " is not a valid value for integer parameter");
    }
  ...
  /**
   * The default DF is used. The default SearchConstraints are used. According to FIPA they are
  ...
  /**
   * The default SearchConstraints are used. According to FIPA they are defaulted to null value for
   * all slots.
   * 
   * @exception FIPAException
   * @see #search(Agent a, AID dfName, DFAgentDescription dfd, SearchConstraints constraints)
  ...
  /**
   * Searches the DF and remains blocked until a result is found or the specified timeout has
   * expired.
   * 
   * @param a The agent that is performing the search
   * @param dfName The AID of the <b>DF</b> agent where to search into.
   * @param template A <code>DFAgentDescription</code> object that is used as a template to identify
   *        the DF descriptions to search for.
   * @param constraints The constraints to limit the number of results to be sent back.
   * @param timeout The maximum amount of time that we want to remain blocked waiting for results.
   * @return The DF agent descriptions matching the specified template or <code>null</code> if the
   *         timeout expires.
   * @exception FIPAException If a <code>REFUSE</code>, <code>FAILURE</code> or
   *            <code>NOT_UNDERSTOOD</code> message is received from the DF (to indicate some error
   *            condition) or if the supplied DF-Description template is not valid.
   */
  public static DFAgentDescription[] searchUntilFound(Agent a, AID dfName, DFAgentDescription dfd,
      SearchConstraints constraints, long timeout) throws FIPAException {
  ...
  /**
   * Utility method that creates a suitable message to be used to REQUEST a DF agent to perform a
   * given action of the FIPA-Management-ontology.
   * <p>
   * This method can be fruitfully used in combination with the
   * <code>jade.proto.AchieveREInitiator</code> protocol and with the <code>decodeDone()</code> and
   * <code>decodeResult()</code> methods to interact with a DF in a non-blocking way.
   * 
   ...
   *        </ul>
   * @param dfd A <code>DFAgentDescription</code> object. Depending on the requested action, this is
   *        the description to register/deregister/modify or a template to match data against during
   *        a search.
   * @param constraints The constraints to limit the number of results to be notified. This is
   *        meaningful only if the requested action is SEARCH.
   * @return the request message.
   * @see jade.proto.AchieveREInitiator
   * @see #decodeDone(String)
   * @see #decodeResult(String)
   */
  public static ACLMessage createRequestMessage(Agent a, AID dfName, String action,
  ...
  /**
   * Utility method that creates a suitable message to be used to SUBSCRIBE to a DF agent in order
   * to receive notifications when a new DF-Description matching the indicated template is
   * registererd with that DF.
   * <p>
   * This method can be fruitfully used in combination with the
   * <code>jade.proto.SubscriptionInitiator</code> protocol and with the
   * <code>createCancelMessage()</code> and <code>decodeNotification()</code> methods to interact
   * with a DF in a non-blocking way.
   * 
   * @param a The agent that is subscribing to the DF
   * @param dfName The AID of the <b>DF</b> agent to subscribe to.
   * @param template A <code>DFAgentDescription</code> object that is used as a template to identify
   *        DF description that will be notified
   * @param constraints The constraints to limit the number of results to be notified.
   ...
   */
  public static ACLMessage createSubscriptionMessage(Agent a, AID dfName,
      DFAgentDescription template, SearchConstraints constraints) {
   ...
   */
  public static ACLMessage getSubscriptionMessage(Agent a, AID dfName, DFAgentDescription template,
      SearchConstraints constraints) throws FIPAException {
    return createSubscriptionMessage(a, dfName, template, constraints);
  }

  /**
   * Utility method that creates a suitable message to be used to CANCEL a subscription to a DF
   * agent.
   * 
  ...
  /**
   * Process the content of the final <code>inform (Done)</code> message resulting from a
   * <code>register</code> or <code>deregister</code> action requested to a DF agent, extracting the
   * <code>df-agent-description</code> contained within.
   * 
   * @return The <code>DFAgentDescription</code> object included in the "done" expression used as
   *         the content of the INFORM message send back by the DF in response to a REQUEST to
   *         perform a register, deregister or modify action.
  ...
  /**
   * Process the content of the final <code>inform (result)</code> message resulting from a
   * <code>search</code> action requested to a DF agent, extracting the array of
   * <code>df-agent-description</code> contained within.
   * 
   * @return The <code>DFAgentDescription</code> objects (as an array) included in the "result"
   *         expression used as the content of the INFORM message send back by the DF in response to
   *         a REQUEST to perform a search action.
   * @exception FIPAException If some error occurs while decoding
   */
  public static DFAgentDescription[] decodeResult(String s) throws FIPAException {
    // S has the form:
    // ((result (action...) (sequence (DFD...) (DFD...)) ) )
    // We skip until we find "action", skip until the end of (action...) and start
    // decoding from there.
    try {
  ...
  /**
   * Process the content of the <code>inform</code> message resulting from a subscription with a DF
   * agent, extracting the array of <code>df-agent-description</code> objects contained within.
   * 
   * @return The <code>DFAgentDescription</code> objects (as an array) included in the "(= (iota...)
   *         ...)" expression used as the content of an INFORM message sent back by the DF as a
  ...
  public static DFAgentDescription[] decodeNotification(String s) throws FIPAException {
    ...
    // We skip until we find "iota", skip until the end of (iota...) and start
    // decoding from there.
    try {
  ...
  /**
   * The parser content has the form: df-agent-description ......) <possibly something else>
   */
  ...
  /**
   * The parser content has the form: service-description ......) <possibly something else>
   */
  private static ServiceDescription parseServiceDescription(SimpleSLTokenizer parser)
      throws Exception {
  ...
  /**
   * The parser content has the form: property ......) <possibly something else>
   */
  ...
  /**
   * The parser content has the form: multi-value-property ......) <possibly something else>
   */
  ...
  /**
   * The parser content has the form: agent-identifier ......) <possibly something else>
   */
  ...
  /**
   * The parser content has the form: (sequence <val> <val> ......) <possibly something else> or
  ...
  /**
   * This is package scoped as it is used by DFUpdateBehaviour and DFSearchBehaviour.
   */
  static String encodeAction(AID df, String actionName, DFAgentDescription dfd,
  ...
  static String encodeCancel(AID df, ACLMessage msg) {
    ...
    encodeAggregate(sb, msg.getAllReceiver(), SL0Vocabulary.SEQUENCE,
        SL0Vocabulary.ACLMSG_RECEIVERS);
  ...
  private static void encodeDfd(StringBuffer sb, DFAgentDescription dfd) {
    ...
    encodeAggregate(sb, dfd.getAllProtocols(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.DFAGENTDESCRIPTION_PROTOCOLS);
    encodeAggregate(sb, dfd.getAllLanguages(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.DFAGENTDESCRIPTION_LANGUAGES);
    encodeAggregate(sb, dfd.getAllOntologies(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.DFAGENTDESCRIPTION_ONTOLOGIES);
    encodeAggregate(sb, dfd.getAllServices(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.DFAGENTDESCRIPTION_SERVICES);
    Date lease = dfd.getLeaseTime();
    if (lease != null) {
  ...
  private static void encodeServiceDescription(StringBuffer sb, ServiceDescription sd) {
    ...
    encodeAggregate(sb, sd.getAllProtocols(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.SERVICEDESCRIPTION_PROTOCOLS);
    encodeAggregate(sb, sd.getAllLanguages(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.SERVICEDESCRIPTION_LANGUAGES);
    encodeAggregate(sb, sd.getAllOntologies(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.SERVICEDESCRIPTION_ONTOLOGIES);
    encodeAggregate(sb, sd.getAllProperties(), SL0Vocabulary.SET,
        FIPAManagementVocabulary.SERVICEDESCRIPTION_PROPERTIES);
    sb.append(')');
  }
  ...
  private static void encodeMultiValueProperty(StringBuffer sb, MultiValueProperty mvp) {
    ...
    encodeAggregate(sb, mvp.getValues().iterator(), SL0Vocabulary.SEQUENCE,
        FIPAManagementVocabulary.PROPERTY_VALUE);
    sb.append(')');
  }
  ...
  // #MIDP_EXCLUDE_BEGIN
  /**
   * In some cases it is more convenient to execute this tasks in a non-blocking way. This method
   * returns a non-blocking behaviour that can be added to the queue of the agent behaviours, as
   * usual, by using <code>Agent.addBehaviour()</code>.
   * <p>
   * Several ways are available to get the result of this behaviour and the programmer can select
   * one according to his preferred programming style:
   * <ul>
   * <li>call getLastMsg() and getSearchResults() where both throw a NotYetReadyException if the
   * task has not yet finished;
   * <li>create a SequentialBehaviour composed of two sub-behaviours: the first subbehaviour is the
   * returned RequestFIPAServiceBehaviour, while the second one is application-dependent and is
   * executed only when the first is terminated;
   * <li>use directly the class RequestFIPAServiceBehaviour by extending it and overriding all the
   * handleXXX methods that handle the states of the fipa-request interaction protocol.
   * </ul>
   * 
   * @param a is the agent performing the task
   * @param dfName is the AID of the DF that should perform the requested action
   * @param actionName is the name of the action (one of the constants defined in
   *        FIPAManagementOntology: REGISTER / DEREGISTER / MODIFY / SEARCH).
   * @param dfd is the agent description
   * @param constraints are the search constraints (can be null if this is not a search operation)
   * @return the behaviour to be added to the agent
   * @exception FIPAException A suitable exception can be thrown to indicate some error condition
  ...
  public static RequestFIPAServiceBehaviour getNonBlockingBehaviour(Agent a, AID dfName,
      String actionName, DFAgentDescription dfd, SearchConstraints constraints)
      throws FIPAException {
   ...
   * The default DF is used.
   * 
   * @see #getNonBlockingBehaviour(Agent a, AID dfName, String actionName, DFAgentDescription dfd,
   *      SearchConstraints constraints)
   * @deprecated Use <code>AchieveREInitiator</code> instead
   **/
  public static RequestFIPAServiceBehaviour getNonBlockingBehaviour(Agent a, String actionName,
      DFAgentDescription dfd, SearchConstraints constraints) throws FIPAException {
    return getNonBlockingBehaviour(a, a.getDefaultDF(), actionName, dfd, constraints);
  }

  /**
   * The default DF is used. the default SearchContraints are used. a default AgentDescription is
  ...
  /**
   * the default SearchContraints are used. a default AgentDescription is used, where only the agent
   * AID is set.
   * 
   * @see #getNonBlockingBehaviour(Agent a, AID dfName, String actionName, DFAgentDescription dfd,
   *      SearchConstraints constraints)
   ...
   **/
  public static RequestFIPAServiceBehaviour getNonBlockingBehaviour(Agent a, AID dfName,
      String actionName) throws FIPAException {
    DFAgentDescription dfd = new DFAgentDescription();
    dfd.setName(a.getAID());
    SearchConstraints constraints = new SearchConstraints();
    constraints.setMaxResults(MINUSONE);
   ...
   **/
  public static RequestFIPAServiceBehaviour getNonBlockingBehaviour(Agent a, String actionName,
      DFAgentDescription dfd) throws FIPAException {
    SearchConstraints constraints = new SearchConstraints();
    constraints.setMaxResults(MINUSONE);
    return getNonBlockingBehaviour(a, a.getDefaultDF(), actionName, dfd, constraints);
  }
   ...
   **/
  public static RequestFIPAServiceBehaviour getNonBlockingBehaviour(Agent a, AID dfName,
      String actionName, DFAgentDescription dfd) throws FIPAException {
    SearchConstraints constraints = new SearchConstraints();
    constraints.setMaxResults(MINUSONE);
    return getNonBlockingBehaviour(a, dfName, actionName, dfd, constraints);
  }
