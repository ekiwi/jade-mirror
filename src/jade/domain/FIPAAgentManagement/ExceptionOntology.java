...
/**
 * This class groups into a separated ontology the elements of the FIPA-Agent-Management-ontology
 * (see FIPA specification document no. 23) representing generic exceptions. This allows reusing
 * these elements into other ontologies simply including tis Exception-ontology
 * <p>
 * The actual <code>Ontology</code> object representing the Exception-ontology is a singleton and is
...
public class ExceptionOntology extends Ontology implements ExceptionVocabulary {
  ...
  private ExceptionOntology() {
    ...
    try {
      ...
      /*
       * #MIDP_INCLUDE_BEGIN add(new PredicateSchema(UNAUTHORISED)); add(new
       * PredicateSchema(UNSUPPORTEDACT)); add(new PredicateSchema(UNEXPECTEDACT)); add(new
       * PredicateSchema(UNSUPPORTEDVALUE)); add(new PredicateSchema(UNRECOGNISEDVALUE)); add(new
       * PredicateSchema(UNSUPPORTEDFUNCTION)); add(new PredicateSchema(MISSINGARGUMENT)); add(new
       * PredicateSchema(UNEXPECTEDARGUMENT)); add(new PredicateSchema(UNEXPECTEDARGUMENTCOUNT));
       * add(new PredicateSchema(MISSINGPARAMETER)); add(new PredicateSchema(UNEXPECTEDPARAMETER));
       * add(new PredicateSchema(UNRECOGNISEDPARAMETERVALUE)); add(new
       * PredicateSchema(INTERNALERROR)); #MIDP_INCLUDE_END
       */

      PredicateSchema ps = (PredicateSchema) getSchema(UNSUPPORTEDACT);
      ps.add(UNSUPPORTEDACT_ACT, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.MANDATORY);

      ps = (PredicateSchema) getSchema(UNEXPECTEDACT);
      ps.add(UNEXPECTEDACT_ACT, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.MANDATORY);
      ...
      ps.add(MISSINGARGUMENT_ARGUMENT, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.MANDATORY);

      ps = (PredicateSchema) getSchema(UNEXPECTEDARGUMENT);
      ps.add(UNEXPECTEDARGUMENT_ARGUMENT, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.MANDATORY);
      ...
      ps.add(UNRECOGNISEDPARAMETERVALUE_PARAMETER_NAME,
          (PrimitiveSchema) getSchema(BasicOntology.STRING), ObjectSchema.MANDATORY);
      ps.add(UNRECOGNISEDPARAMETERVALUE_PARAMETER_VALUE,
          (PrimitiveSchema) getSchema(BasicOntology.STRING), ObjectSchema.MANDATORY);

      ps = (PredicateSchema) getSchema(INTERNALERROR);
      ps.add(INTERNALERROR_MESSAGE, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.MANDATORY);
