...
/**
 * This class represents the JADE-Agent-Management-ontology i.e. the set of concepts, actions and
 * predicates that relates to the JADE specific actions that can be requested to the JADE AMS and
 * DF.
 * <p>
 * The actual <code>Ontology</code> object representing the JADE-Agent-Management-ontology is a
...
public class JADEManagementOntology extends Ontology implements JADEManagementVocabulary {
  ...
  private JADEManagementOntology() {
    ...
    try {
      ...
      /*
       * #MIDP_INCLUDE_BEGIN // Concepts definitions add(new ConceptSchema(LOCATION)); add(new
       * ConceptSchema(CONTAINERID)); add(new ConceptSchema(PLATFORMID));
       * 
       * // AgentActions definitions add(new AgentActionSchema(SHUTDOWNPLATFORM)); add(new
       * AgentActionSchema(KILLCONTAINER)); add(new AgentActionSchema(CREATEAGENT)); add(new
       * AgentActionSchema(KILLAGENT)); add(new AgentActionSchema(INSTALLMTP)); add(new
       * AgentActionSchema(UNINSTALLMTP)); add(new AgentActionSchema(SNIFFON)); add(new
       * AgentActionSchema(SNIFFOFF)); add(new AgentActionSchema(DEBUGON)); add(new
       * AgentActionSchema(DEBUGOFF)); add(new AgentActionSchema(SHOWGUI)); add(new
       * AgentActionSchema(WHEREISAGENT)); add(new AgentActionSchema(QUERYAGENTSONLOCATION));
      ...
      cs.add(LOCATION_PROTOCOL, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
      cs.add(LOCATION_ADDRESS, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);

      cs = (ConceptSchema) getSchema(CONTAINERID);
      cs.addSuperSchema((ConceptSchema) getSchema(LOCATION));
      cs.add(CONTAINERID_MAIN, (PrimitiveSchema) getSchema(BasicOntology.BOOLEAN),
          ObjectSchema.OPTIONAL);
      cs.add(CONTAINERID_PORT, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
      cs.add(CONTAINERID_PROTOCOL, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
      ...
      as.add(KILLCONTAINER_PASSWORD, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
      ...
      as.add(CREATEAGENT_ARGUMENTS, (TermSchema) TermSchema.getBaseSchema(), 0,
      ...
      as.add(CREATEAGENT_INITIAL_CREDENTIALS, (TermSchema) TermSchema.getBaseSchema(),
          ObjectSchema.OPTIONAL);
      // #MIDP_EXCLUDE_END

      as = (AgentActionSchema) getSchema(KILLAGENT);
      as.add(KILLAGENT_AGENT, (ConceptSchema) getSchema(BasicOntology.AID));
      as.add(KILLAGENT_PASSWORD, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);

      as = (AgentActionSchema) getSchema(INSTALLMTP);
      as.add(INSTALLMTP_ADDRESS, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
      ...
      as.add(SNIFFON_SNIFFED_AGENTS, (ConceptSchema) getSchema(BasicOntology.AID), 1,
          ObjectSchema.UNLIMITED);
      as.add(SNIFFON_PASSWORD, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);

      as = (AgentActionSchema) getSchema(SNIFFOFF);
      as.add(SNIFFOFF_SNIFFER, (ConceptSchema) getSchema(BasicOntology.AID));
      as.add(SNIFFOFF_SNIFFED_AGENTS, (ConceptSchema) getSchema(BasicOntology.AID), 1,
          ObjectSchema.UNLIMITED);
      as.add(SNIFFOFF_PASSWORD, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);

      as = (AgentActionSchema) getSchema(DEBUGON);
      as.add(DEBUGON_DEBUGGER, (ConceptSchema) getSchema(BasicOntology.AID));
      as.add(DEBUGON_DEBUGGED_AGENTS, (ConceptSchema) getSchema(BasicOntology.AID), 1,
          ObjectSchema.UNLIMITED);
      as.add(DEBUGON_PASSWORD, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);

      as = (AgentActionSchema) getSchema(DEBUGOFF);
      as.add(DEBUGOFF_DEBUGGER, (ConceptSchema) getSchema(BasicOntology.AID));
      as.add(DEBUGOFF_DEBUGGED_AGENTS, (ConceptSchema) getSchema(BasicOntology.AID), 1,
          ObjectSchema.UNLIMITED);
      as.add(DEBUGOFF_PASSWORD, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
  ...
  public static String adjustAgentName(String nameWithWildcards, String[] values) {
    ...
    // #DOTNET_EXCLUDE_END
    /*
     * #DOTNET_INCLUDE_BEGIN System.Text.StringBuilder out = new
     * System.Text.StringBuilder(nameWithWildcards); #DOTNET_INCLUDE_END
     */
    ...
    for (int i = 0; i < minLength; i++) {
      ...
      // #DOTNET_EXCLUDE_END
      /*
       * #DOTNET_INCLUDE_BEGIN int index = out.ToString().indexOf(WILDCARDS[i]); #DOTNET_INCLUDE_END
       */
      if (index >= 0) {
        ...
        while (index >= 0) {
          ...
          // #DOTNET_EXCLUDE_END
          /*
           * #DOTNET_INCLUDE_BEGIN out = out.Replace(WILDCARDS[i], values[i], index,
           * index+WILDCARDS[i].length()); index = out.ToString().indexOf(WILDCARDS[i]);
           * #DOTNET_INCLUDE_END
           */
        }
      }
    ...
    /*
     * #J2ME_INCLUDE_BEGIN return nameWithWildcards; #J2ME_INCLUDE_END
     */
  }
