...
/**
 * Base class for AMS and DF behaviours managing requests from agents. This class handles the
 * FIPA-request protocol and in particular prepares the response taking into account all possible
 * exceptions. The preparation of the result notification is delegated to subclasses as its form
 * (RESULT or DONE) and sending time (i.e. whether it can be sent immediately or must be delayed at
 * a later time) depends on the specific action.
 * 
...
public abstract class RequestManagementBehaviour extends SimpleAchieveREResponder {
  ...
  /**
   * @return null when the AGREE message can be skipper, the AGREE message otherwise.
   */
  protected ACLMessage prepareResponse(ACLMessage request)
    ...
    try {
      ...
      notification.setContent(prepareErrorContent(request.getContent(),
          ExceptionVocabulary.INTERNALERROR + " \"" + tr + "\""));
    }
    if (t != null) {
      myLogger.log(Logger.WARNING, "Agent " + myAgent.getLocalName() + " - Error handling request",
          t);
    }
    return response;
  }
