...
/**
 * This class represents the JADE-Persistence ontology i.e. the set of concepts, actions and
 * predicates that relates to the JADE Persistence Service features that can be accessed through the
 * JADE AMS.
 * <p>
 * The actual <code>Ontology</code> object representing the JADE-Persistence ontology is a singleton
 * and is accessible through the static method <code>getInstance()</code>
 * 
...
public class PersistenceOntology extends Ontology implements PersistenceVocabulary {
  ...
  private PersistenceOntology() {
    ...
    /*
     * #MIDP_INCLUDE_BEGIN super(NAME, BasicOntology.getInstance(), null); #MIDP_INCLUDE_END
     */
    ...
    try {
      ...
      /*
       * #MIDP_INCLUDE_BEGIN // Concepts definitions add(new ConceptSchema(AGENTGROUP)); add(new
       * ConceptSchema(CONTAINERID)); add(new ConceptSchema(LOCATION));
       * 
       * // AgentActions definitions add(new AgentActionSchema(LOADAGENT)); add(new
       * AgentActionSchema(RELOADAGENT)); add(new AgentActionSchema(SAVEAGENT)); add(new
       * AgentActionSchema(DELETEAGENT)); add(new AgentActionSchema(FREEZEAGENT)); add(new
       * AgentActionSchema(THAWAGENT)); add(new AgentActionSchema(SAVECONTAINER)); add(new
       * AgentActionSchema(LOADCONTAINER)); add(new AgentActionSchema(DELETECONTAINER)); add(new
       * AgentActionSchema(LOADAGENTGROUP)); add(new AgentActionSchema(SAVEAGENTGROUP)); add(new
       * AgentActionSchema(DELETEAGENTGROUP));
       * 
       * // Predicates definitions add(new PredicateSchema(UNSUPPORTEDVALUE)); add(new
       * PredicateSchema(UNRECOGNISEDVALUE)); add(new PredicateSchema(UNSUPPORTEDFUNCTION)); add(new
       * PredicateSchema(MISSINGPARAMETER)); add(new PredicateSchema(UNEXPECTEDPARAMETER)); add(new
       * PredicateSchema(UNRECOGNISEDPARAMETERVALUE)); add(new PredicateSchema(NOTREGISTERED));
       * add(new PredicateSchema(INTERNALERROR)); #MIDP_INCLUDE_END
       */
      ...
      as.add(FREEZEAGENT_BUFFERCONTAINER, (ConceptSchema) getSchema(CONTAINERID),
          ObjectSchema.OPTIONAL);
