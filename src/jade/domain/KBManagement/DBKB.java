...
/**
 * This class provides an abstract implementation of the <code>KB</code> interface where information
...
public abstract class DBKB extends KB {
  ...
  /**
   * This ThreadLocal is used to hold connections and associated additional information (such as
   * prepared statements) currently used by each Thread
   */
  ...
  /**
   * Constructs a new <code>DFKB</code> and establishes a connection to the database at the given
   * URL using the <code>sun.jdbc.odbc.JdbcOdbcDriver</code> driver.
   * 
   ...
   * @throws SQLException an error occured while opening a connection to the database
   */
  public DBKB(String drv, String url, String username, String password, int maxResultLimit,
  ...
  /**
   * This method is called by the KB Factory and is a placeholder for implementation specific KB
   * initializations.
   */
   ...
   * 
   * @param drv driver name or <code>null</code> </ br> if the default JDBC-ODBC driver should be
   *        used
  ...
  protected Connection createDBConnection(String url, String username, String password)
      throws SQLException {
    if (username != null) {
  ...
  /**
   * Subclasses can redefine this method to provide implementation specific ConnectionWrapper
   * initializations
   */
  ...
  private void invalidateConnectionWrapper() throws SQLException {
    ...
    if (wrapper != null) {
      ...
      // FIXME: For JDK1.4 compatibility we can't use remove(). For our purposes there
      // is no difference
    ...
    try {
      ...
      try {
        ...
        logger.log(Logger.SEVERE,
            "DB error inserting DFD for agent " + ((DFAgentDescription) fact).getName().getName(),
            sqle);
        try {
          invalidateConnectionWrapper();
        } catch (Exception e1) {
  ...
  protected Object remove(Object name) {
    try {
      ...
      try {
        ...
        // Log the original error
        logger.log(Logger.SEVERE, "DB error removing DFD for agent " + ((AID) name).getName(),
            sqle);
        try {
          invalidateConnectionWrapper();
        } catch (Exception e1) {
  ...
  public void subscribe(Object template, SubscriptionResponder.Subscription s)
  ...
  protected abstract void subscribeSingle(Object template, SubscriptionResponder.Subscription s)
      throws SQLException, NotUnderstoodException;

  // Note that getSubscriptions() is only called just after a
  // registration/deregistration/modification -->
  ...
  protected abstract void unsubscribeSingle(SubscriptionResponder.Subscription s)
      throws SQLException;
