...
class KBSubscriptionManager implements SubscriptionResponder.SubscriptionManager {
  ...
  public boolean register(SubscriptionResponder.Subscription sub)
    ...
    try {
      ...
      if (myLogger.isLoggable(Logger.CONFIG)) {
        myLogger.log(Logger.CONFIG, "Registering subscription " + subMessage.getConversationId()
            + ". Agent is " + subMessage.getSender().getName());
      }

      absIota = (AbsIRE) cm.extractAbsContent(subMessage);
      AbsPredicate absResult = absIota.getProposition();
      AbsAgentAction absAction =
          (AbsAgentAction) absResult.getAbsObject(BasicOntology.RESULT_ACTION);
      AbsAgentAction absSearch =
          (AbsAgentAction) absAction.getAbsObject(BasicOntology.ACTION_ACTION);
      ...
      synchronized (subscriptionsCache) {
        if (subscriptions != null) {
          subscriptionsCache.put(subMessage.getConversationId(),
              new SubscriptionInfo(sub, dfdTemplate, absIota));
  ...
  public boolean deregister(SubscriptionResponder.Subscription sub) throws FailureException {
    if (myLogger.isLoggable(Logger.CONFIG)) {
      ACLMessage subMessage = sub.getMessage();
      myLogger.log(Logger.CONFIG, "Deregistering subscription " + subMessage.getConversationId()
          + ". Agent is " + subMessage.getSender().getName());
    }
  ...
  /**
   * Handle registrations/deregistrations/modifications by notifying subscribed agents if necessary.
  ...
  void handleChange(DFAgentDescription dfd, DFAgentDescription oldDfd) {
    ...
    for (int i = 0; i < subscriptions.length; ++i) {
      ...
      if (DFMemKB.compare(template, dfd)
          || ((oldDfd != null) && DFMemKB.compare(template, oldDfd))) {
        // This subscriber must be notified
        List results = new ArrayList();
        results.add(dfd);
        if (myLogger.isLoggable(Logger.FINE)) {
          ACLMessage subMessage = info.getSubscription().getMessage();
          myLogger.log(Logger.FINE, "Notifying subscribed agent " + subMessage.getSender().getName()
              + " [" + subMessage.getConversationId() + "] ");
        }
  ...
  private void notify(SubscriptionResponder.Subscription sub, List results, AbsIRE absIota) {
    try {
      ...
      absEquals.set(SLVocabulary.EQUALS_RIGHT,
          FIPAManagementOntology.getInstance().fromObject(results));
  ...
  /**
   * Inner class SubscriptionInfo This class associates a Subscription object with the
   * DFAgentDescription that acts as template for that Subscription object
   */
  private class SubscriptionInfo {
    ...
    private SubscriptionInfo(SubscriptionResponder.Subscription subscription,
        DFAgentDescription template, AbsIRE absIota) {
