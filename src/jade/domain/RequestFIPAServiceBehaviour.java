...
/**
 * This class extends the <code>FipaRequestIntiatorBehaviour</code> in order to request an agent,
 * e.g. <em>DF or AMS</em> to perform a specific action. <br>
 * This class implements all the abstract method of the super classes, therefore the behaviour can
 * be immediately added to an agent. <br>
 * In some cases, it might be usefull to extend this class to override some of its methods and react
 * to the received messages in an application-specific manner. <br>
 * The class has two constructor. The first generic constructor can be used for all the action an
 * agent can perform. In the case of a a search action the default search constraints are used. <br>
 * The second constructor is specific for a search action and it allows to specify additional search
 * constraints.
 * 
...
public class RequestFIPAServiceBehaviour extends SimpleAchieveREInitiator {
   ...
   * <code>ACLMessage</code> from an <code>Handle</code> and no message has been yet received and
   * the timeout limit is not yet elapsed.
   * 
  ...
  /**
   * Create a behaviour to request an agent to perform a specific action. Using this constructor, is
   * possible to pass all information necessary to request a search operation.
   * 
   * @param a The agent this behaviour belongs to, i.e. the agent who is interested in the search
   *        result.
   ...
   * @exception FIPAException A suitable exception can be thrown when the method locally discovers
   *            that the passed parameters are not valid.
   * @see jade.domain.FIPAAgentManagement.SearchConstraints
   */
  public RequestFIPAServiceBehaviour(Agent a, AID receiver, String actionName,
      Object agentDescription, SearchConstraints constraints) throws FIPAException {
  ...
  /**
   * Create a behaviour to request an agent to perform a specific action. The default search
   ...
   * @see #RequestFIPAServiceBehaviour(Agent a, AID receiver, String actionName, Object
   *      agentDescription, SearchConstraints constraints)
   */
  public RequestFIPAServiceBehaviour(Agent a, AID receiver, String actionName,
      Object agentDescription) throws FIPAException {
   ...
   * 
   * @param reply The actual ACL message received. It is of <code>not-understood</code> type and
   *        matches the conversation template.
   */
   ...
   * 
   * @param reply The actual ACL message received. It is of <code>refuse</code> type and matches the
   ...
   * 
   * @param reply The actual ACL message received. It is of <code>agree</code> type and matches the
   *        conversation template.
   */
   ...
   * 
   * @param reply The actual ACL message received. It is of <code>failure</code> type and matches
   ...
   * 
   * @param reply The actual ACL message received. It is of <code>inform</code> type and matches the
   *        conversation template.
   */
  ...
  /**
   * This method handle is called after receiving all the responses and when the timeout is expired(
   * in this case no reply has been received.
   */
  ...
  /**
   * This public method allows to get the INFORM message received in the final state of this
   * FIPA-Request protocol.
   * 
   * @return the ACLMessage received
   * @exception FIPAException A suitable exception can be thrown when the protocol was finished with
   *            a FAILURE/REFUSE or NOT-UNDERSTOOD performative.
   ...
   * This public method allows to get the results of a search operation.
   * 
   * @return An array of Objects containing the items found as the result of the search.
   * @exception FIPAException A suitable exception can be thrown when the protocol was finished with
   *            a FAILURE/REFUSE or NOT-UNDERSTOOD performative.
   * @exception NotYetReady is thrown if the protocol is not yet finished.
   **/
