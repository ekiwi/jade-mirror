...
/**
 * This class provides a set of basic and static methods to perform the FIPA Agent Management
 * actions. However, developers should use <code>DFService</code> and <code>AMSService</code> which
 * provide specialized methods to communicate with the DF and the AMS.
 * 
...
public class FIPAService {
   ...
   * :reply-with xxx :conversation-id xxx) </code> where <code>xxx</code> are unique words generated
   * on the basis of the sender's name and the current time.
   * 
  ...
  /**
   * This method plays the initiator role in the Fipa-Request interaction protocol and performs all
   * the steps of the protocol. The method uses the
   * <code>:reply-with</code>/<code>:in-reply-to</code> ACL message slots as a mechanism to match
   * the protocol replies. Take care because the method blocks until all the response messages are
   * received. Under error conditions, or if the responder does not wish to respond, that might
   * block for ever the execution of the agent. For this reason, the
   * <code>FipaRequestInitiatorBehaviour</code> is the preferred way to play the protocol.
   * 
   * @param a is the Agent playing the initiator role
   * @param request is the ACLMessage to be sent. Notice that all the slots of the message must have
   *        already been filled by the caller. If the <code>:reply-with</code> message slot is not
   *        set, a default one will be generated automatically.
   * @return the INFORM message received in the final state of the protocol, if the protocol
   *         succeeded, otherwise it throws an Exception
   */
  ...
  /**
   * This method plays the initiator role in the Fipa-Request interaction protocol and performs all
   * the steps of the protocol, and additionally sets a conversation timeout. The method uses the
   * <code>:reply-with</code>/<code>:in-reply-to</code> ACL message slots as a mechanism to match
   * the protocol replies. Under error conditions, or if the responder does not wish to respond,
   * that might block the execution of the agent until the conversation timeout expires. For this
   * reason, the <code>FipaRequestInitiatorBehaviour</code> is the preferred way to play the
   * protocol.
   * 
   * @param a is the Agent playing the initiator role
   * @param request is the ACLMessage to be sent. Notice that all the slots of the message must have
   ...
   *         succeeded, otherwise it throws an Exception
   */
  public static ACLMessage doFipaRequestClient(Agent a, ACLMessage request, long timeout)
      throws FIPAException {
    String key = null;
    // If the request message does not have a ':reply-with' and :conversation-id
    // slots set them
