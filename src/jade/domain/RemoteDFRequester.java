...
/**
 * This behaviour is used by the df agent only (it is not defined as an Inner class due to
 * compilation problems in PJAVA) and handles a request to another DF. These requests can be
 * triggered by an action of the DF-Applet ontology (e.g. Federate) or by a DF GUI event. The
 * onEnd() method must be re-defined to manage the result properly.
 * 
...
class RemoteDFRequester extends SimpleAchieveREInitiator {
  ...
  protected void handleRefuse(ACLMessage refuse) {
    result = new InternalError("Agent " + remoteDF.getName() + " replyed with "
        + ACLMessage.getPerformative(refuse.getPerformative()));
  }

  protected void handleNotUnderstood(ACLMessage notUnderstood) {
    result = new InternalError("Agent " + remoteDF.getName() + " replyed with "
        + ACLMessage.getPerformative(notUnderstood.getPerformative()));
  }

  protected void handleFailure(ACLMessage failure) {
    result = new InternalError("Agent " + remoteDF.getName() + " replyed with "
        + ACLMessage.getPerformative(failure.getPerformative()));
  }
