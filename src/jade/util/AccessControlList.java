...
/**
 *
 * An ACL object represents an Access Control List and wraps a White list and a Black list, Both
 * lists make use of regular expressions for allowing/denying access to a certain resource.
 * 
 ...
 * </code>
 * 
 * and the two files look as follows: <strong>white.txt</strong> <code>
 *  # this is a commant
 ...
 *     39335.+?
 * </code> <strong>black.txt</strong> <code>
 *  user:
 ...
 * <ul>
 * <li>it is not present into the black list, AND</li>
...
public class AccessControlList {
  ...
  // constructor
  public AccessControlList() {} // end constructor
   ...
   * </ul>
   *
   * <code>isAllow("section", value)</code> returns is the client having that specific value for
  ...
  private boolean isInList(InternalACL iacl, String section, String value) {
    ...
    try {
      ...
      while (pos < iacl.size) {
        ...
        // System.out.println( "+++ pos="+pos+" pattern="+p+"
        // iacl.sectionName[pos]="+iacl.sectionName[pos] );
        ...
        if (logger.isLoggable(Logger.FINER)) {
          logger.log(Logger.FINER, "(" + iacl.fileName + ")  " + "  pattern="
              + iacl.pat[pos].pattern() + "  matcher=" + value + "\n");
        }
        // check the matching
        boolean b = m.matches();
        if (logger.isLoggable(Logger.FINER))
    ...
    while ((str = in.readLine()) != null) {
      ...
      try {
        ...
        logger.log(Logger.WARNING,
            "Error in expression acc.control list file:" + fileName + " (line:" + pos + ")");
      }
    }
    in.close();
   ...
   *
   * 
   * public static void main(String args[]){ createTestACL(); AccessControlList acl = new
   * AccessControlList(); acl.setBlack( blackFilename ); acl.setWhite( whiteFilename );
   * acl.setLogLevel( Level.FINE );
   * 
   * testAndPrint( acl, "user", "goodboy" , true); testAndPrint( acl, "user", "sfogliatella9814",
   * false); testAndPrint( acl, "section2", "forbiddenvalue" , false); testAndPrint( acl,
   * "section2", "goodvalue" , true); testAndPrint( acl, "sectionX", "anyvalue" , false);
   * 
   * }
   * 
   * private static void testAndPrint(AccessControlList acl, String section, String value, boolean
   * expected){ boolean ok = acl.isAllowed( section, value ); System.out.println(
   * "section="+section+"  value="+value +"  ok="+(ok+" ").toUpperCase() +""+( (ok==expected) ?
   * ":-)": "!!!  [:-( expected="+expected+" !!!") +"\n\n"); }
   * 
   * private static final String blackFilename="black.txt"; private static final String
   * whiteFilename="white.txt"; private static void createTestACL(){ try { PrintWriter out;
   * 
   * out = new PrintWriter(new BufferedWriter(new FileWriter(blackFilename))); out.println( "\n"
   * +"user:\n" +"	sfogliatella.+?  \n" +"section2:\n" +"	forbiddenvalue.*  \n" +"section3:\n"
   * +"	.+?  \n" ); out.flush(); out.close(); out = new PrintWriter(new BufferedWriter(new
   * FileWriter(whiteFilename))); out.println( "\n" +"user:\n" +"	.+?  \n" +"section2:\n"
   * +"	good.+?  \n" +"section3:\n" +"	.+?  \n" ); out.flush(); out.close();
   * 
   * } catch (Exception e) { e.printStackTrace();} }
   * 
   * private void setLogLevel(Level lev){ logger.setLevel( lev ); logger.getParent().setLevel( lev
   * );
   * 
   * //Set level for handlers associated to logger Handler[] pHandlers =
   * logger.getParent().getHandlers(); Handler[] handlers = logger.getHandlers(); for (int i=0;
   * i<pHandlers.length; i++){ pHandlers[i].setLevel(lev); } for (int j=0; j<handlers.length; j++){
