/*
 * (c) Copyright Hewlett-Packard Company 2001 This program is free software; you can redistribute it
 * and/or modify it under the terms of the GNU Lesser General Public License as published by the
 * Free Software Foundation; either version 2 of the License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY; without
 * even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE and no warranty
 * that the program does not infringe the Intellectual Property rights of a third party. See the GNU
 ...
 * 1) Allows specifying property values containing strings of the form <b><tt>${key}</tt></b>. Such
 * strings are properly replaced with the value of the <tt>key</tt> variable that may represent
 * another property, a system property or an environment variable.
 * <p>
 * 2) Allows specifying boolean properties in the form <tt>-key</tt>. Such format is equivalent to
 * <tt>key=true</tt>.
 * <p>
 ...
 * <p>
 * 4) Allows declaring a property as read-only. In order to do that it is sufficient to end its key
 * with a '!'. For example:
 * 
 * <pre>
...
public class ExtendedProperties extends Properties {
  ...
  /**
   * For testing. Simply pass command line arguments to constructor then display all key=value pairs
   * using sorted enumeration.
   */
  ...
  /**
   * Add properties from a specified InputStream. Properties will be added to any existing
   * collection.
   * 
  ...
  /**
   * Writes this property collection to the output stream in a format suitable for loading into a
  ...
  /**
   * Add to this Properties object a stringified property of the form <tt>key = value</tt> or
   * <tt>-key</tt>
   * 
  ...
  /**
   * Retrieve the position of the first valid key-value separator character ('=' or ':') in a
  ...
  /**
   * Create a new Properties object by coping those properties whose key begins with a particular
   * prefix string. The prefix is removed from the keys inserted into the extracted Properties
   * object
   * 
   ...
   * perform substitutions of variables of the form ${x} in property values since such values may be
   * non-string objects.
   * 
  ...
  public Object get(Object aKey) {
    ...
    if (aKey instanceof String) {
      ...
      if (value != null && value instanceof String) {
        ...
        // This synchronized block prevents a "Circular argument substitution key" error
        // in case two threads
   ...
   * 
   * @param aKey The key used to store the data. The key may contain strings of the form
   *        <b><tt>${key}</tt></b> which will be evaluated first.
  ...
  public Object put(Object aKey, Object aValue) {
    if (aKey instanceof String) {
      ...
      // aKey may have the form kkk or kkk!. In both cases if a property with key =
      // kkk! exists throws an exception
      String testKey =
          (actualKey.endsWith("!")) ? actualKey.substring(0, actualKey.length()) : actualKey;
      if (super.containsKey(testKey + "!")) {
        throw new PropertiesException("Attempt to alter read only property:" + testKey);
      }

      // If the key to be inserted is the "import" key, do not insert it, but add all
      // properties
  ...
  /**
   * Override getProperty in base class so all occurances of the form <b><tt>${key}</tt></b> are
   * replaced by their associated value.
   * 
   * @param aKey Key for desired property.
  ...
  /**
   * Perform substitution when a value is fetched. Traps circular definitions, and calls valueFilter
   * with value prior to returning it.
   * 
   * @param aKey The property key.
   * @param defaultValue Value to return if property not defined. May be null. If non null it will
   *        be passes to valueFilter first.
   ...
   */
  /*
   * public Object setProperty(String aKey, String aValue) { String actualKey =
   * doSubstitutions(aKey);
   * 
   * String testKey = (actualKey.endsWith("!")) ? actualKey.substring(0, actualKey.length()) :
   * actualKey; if (super.containsKey(testKey + "!")) { throw new
   * PropertiesException("Attempt to alter read only property:" + testKey); }
   * 
   * if (aValue == null) { return super.remove(actualKey); } else { return super.put(actualKey,
   * aValue); } }
   */
   ...
   * @return Null if store was done, non-null indicates store not done and the returned value in the
  ...
  /**
   * Fetch property value for key which may contain strings of the form <b><tt>${key}</tt></b>.
   * 
   * @param aKey Key for desired property.
  ...
  /**
   * Called by getProperty(key, default) to perform any post processing of the value string. By
   * default, this method provides special processing on the value associated with any property
   * whose key name has the string "path" as part of it (ex: "classpath", "sourcepath", "mypath").
   * When the value for such keys is fetched any occurance of '|' will be converted to a ':' on Unix
   * systems and a ';' on Windows systems. Therefore to increase the direct reuse of your property
   * files, always use a '|' as a separator and always assign a key name which has "path" as part of
   * it.
   * 
  ...
  protected String valueFilter(String key, String value) {
    if (key.toLowerCase().indexOf("path") >= 0) { // convert separators to be correct for this
                                                  // system
  ...
  /**
   * Extract a string value and convert it to an integer. If there isn't one or there is a problem
   * with the conversion, return the default value.
   * 
  ...
  /**
   * Extract a string value ("true" or "false") and convert it to a boolean. If there isn't one or
   * there is a problem with the conversion, return the default value.
   * 
   * @param aKey The key which will be used to fetch the attribute.
   ...
   * @return Non null is former value of object associated with new key. Null indicates that either
   *         the existing key didn't exist or there was no former value associated with the new key.
   *         i.e. null => success.
   */
  ...
  /**
   * Replace all substrings of the form ${xxx} with the property value using the key xxx. Calls
   * doSubstitutions(anInputString, false).
   * 
  ...
  /**
   * Replace all substrings of the form ${xxx} with the property value using the key xxx. If the key
   * is all caps then the property is considered to be a system property.
   * 
   * @param anInputString The input string - may be null.
   * @param allowUndefined If true, undefined strings will remain as is, if false, an exception will
   *        be thrown.
   * @return The resultant line with all substitutions done or null if input string was.
   */
  public String doSubstitutions(String anInputString, boolean allowUndefined) {
    ...
    do {
      ...
      if ((oi == si) && (ci > oi + 2)) { // ${key}xxxxx
        ...
        // Try as another property first (this allows the user to override system or
        // environment setting)
  ...
  /**
   * Add properties from Reader. Explicitly handled so as to enable handling of import=<file>
   * directive. Blank lines as well as those beginning with a '#' character (comments) are ignored.
   * 
   * @param reader The buffered reader to read from. to catch circular imports.
   * @throws IOException if anything goes wrong.
   */
   ...
   * @return The resultant logical line which may have been constructed from one or more physical
   *         lines.
   * @throws IOException if anything goes wrong.
   */
  ...
  /**
   * Read one line from the Reader. A line may be terminated by a single CR or LF, or the pair CR
   * LF.
   * 
  ...
  /**
   * List properties to provided PrintStream. Output will be in sorted key sequence. If a value is
   * null, it will appear as "key=".
   * 
  ...
  /**
   * Create a String[] for the properties with one key=value pair per array entry. If a value is
   * null, it will appear as "key=".
   * 
