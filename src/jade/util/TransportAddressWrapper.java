...
/**
 * This class wraps a Transport Address and implements the equals() and hashCode() methods so that
 * two Transport Addresses representing the same address, even if possibly expressed in different
 * forms, are actually considered equals.
 * 
 * @author 00917536
 *
 */
public class TransportAddressWrapper {
  ...
  public TransportAddressWrapper(TransportAddress t) {
    ...
    if (host != null) {
      if (Profile.isLocalHost(host)) {
        ...
        // that if two stringified hosts represent the same host the hashCode-s are
        // equals,
        // while if they represent different hosts very likely the hashCode-s are
        // different,
        // we use a conservative approach: if the host represents a remote host, it
        // does not contribute to the hashCode generation. This makes hash-based
        // searches
  ...
  public static TransportAddressWrapper getWrapper(String stringifiedTransportAddress,
      IMTPManager imtpManager) throws IMTPException {
