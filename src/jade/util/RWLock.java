...
/**
 * This class provides support for synchronizing threads acting on a generic resource in such a way
 * that - If a thread is writing the resource no other thread can act on it in any way - Several
 * threads can read the resource at the same time - If one or more threads are reading the resource
 * no thread can write it
 * 
...
public class RWLock {
  ...
  /**
   * This placeholder method is called every time a thread actually acquires the protected resource
   * with writing privileges (this means that, in case of multiple recursive locking by the same
   * thread, this method is called only the first time). Subclasses can exploit this to
   * transparently trigger a resource acquisition prolog.
   */
  protected void onWriteStart() {}

  /**
   * This placeholder method is called every time a thread actually releases the protected resource
   * with writing privileges (this means that, in case of multiple recursive unlocking by the same
   * thread, this method is called only the last time). Subclasses can exploit this to transparently
   * trigger a resource release epilog.
   */
