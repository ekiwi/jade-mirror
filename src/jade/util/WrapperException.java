...
/**
 * 
 * This class acts as a base class for all the exceptions that wrap another (nested) exception. The
 * typical usage for descendents of this class is to throw them within the <code>catch</code> block
 * for their nested exception.
 * 
...
public abstract class WrapperException extends Exception {
   ...
   * 
   * @return The exception message, including the nested exception text if present.
   */
  public String getMessage() {
    if ((nested != null)) {
      return super.getMessage() + " - Caused by:  "
          + (nested.getMessage() != null ? nested.getMessage() : nested.toString());
    }
  ...
  // #MIDP_EXCLUDE_END
  /*
   * #MIDP_INCLUDE_BEGIN public void printStackTrace() { super.printStackTrace(); if(nested != null)
   * { System.err.println("Nested Exception:"); nested.printStackTrace(); } } #MIDP_INCLUDE_END
   */

  /**
   * Reads the exception wrapped by this object.
   * 
   * @return the <code>Throwable</code> object that is the exception that was originally thrown.
   */
