...
public class RoundList implements Serializable {
  ...
  /**
   * Inserts the <code>element</code> before the current element. If the list was empty, the
   * inserted element becomes also the current element. <b> Note that this implementation uses a
   * <code>LinkedList</code> and therefore it is not synchronized.
   * 
  ...
  /**
   * Returns the current <code>element</code> in the list and updates the pointer such that the
   * current becomes the next element in the list. <br>
   * Notice that if the list contains just 1 element each call to this method will return the same
   * element. <br>
   * Take care in avoiding infinite loops in calling this method. It must be called no more than
   * <code>size()</code> times
   * 
  ...
  /**
   * Removes the first occurrence of the specified element in this list and updates the pointer to
   * the current element. If the list does not contain the element, it is unchanged. More formally,
   * removes the element with the lowest index i such that
   * <code>(element==null ? get(i)==null : element.equals(get(i))) </code> (if such an element
   * exists).
   * 
   ...
   * <code>(element==null ? e==null : element.equals(e)).</code>
   * 
   ...
   * list of the collection's elements in the order they are returned by its get() method, enclosed
   * in square brackets ("[]"). Adjacent elements are separated by the characters ", " (comma and
   * space). Elements are converted to strings as by <code>String.valueOf(Object).</code>
   * 
