...
class Connection extends Thread {
  ...
  /**
   * Parses ACL messages from the input stream. It is written to take multiple messages from the
   * stream. For each message it gets from the input stream it 1) sends it into the Jade world and
   * 2) starts a new behavior to wait for the response. When that behavior gets the response (or
   * times out) it will send a message back out the socket.
   */
  public void run() {
    ...
    try {
      ...
      while (true) {
        ...
        if (allValidReceivers(msg)) {
          ...
          if ((msg.getReplyWith() == null) || (msg.getReplyWith().length() < 1)) {
            msg.setReplyWith(myAgent.getLocalName() + "." + getName() + "."
      ...
      if ((gotOneMessage) && (any.getMessage() != null)
          && (any.getMessage().indexOf("<EOF>") >= 0)) {
        // ignore it
      } else {
