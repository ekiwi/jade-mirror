...
class Server extends Thread {
  ...
  /**
   * Constructor of the class. It creates a ServerSocket to listen for connections on.
   * 
   * @param port is the port number to listen for. If 0, then it uses the default port number.
   * @param a is the pointer to agent to be used to send messages.
   * @param receivers vector with the names of all the agents that wish to receive messages through
   *        this proxy.
   */
  ...
  /**
   * The body of the server thread. It is executed when the start() method of the server object is
   * called. Loops forever, listening for and accepting connections from clients. For each
   * connection, creates a Connection object to handle communication through the new Socket. Each
   * Connection object is a new thread. The maximum queue length for incoming connection indications
   * (a request to connect) is set to 50 (that is the default for the ServerSocket constructor). If
   * a connection indication arrives when the queue is full, the connection is refused.
   */
  public void run() {
    try {
      done = false;
      while (!done) {
        ...
        if (logger.isLoggable(Logger.FINE)) {
          logger.log(Logger.FINE, "New Connection with " + client_socket.getInetAddress().toString()
