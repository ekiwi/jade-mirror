...
/**
 * Send an ACL message to the JADE socket proxy agent which is running on a host system and
...
public class JadeBridge {
   ...
   */
  public ACLMessage sendACL(String aMsg, long timeout)
    ...
    if (logger.isLoggable(Logger.FINE)) {
      logger.log(Logger.FINE,
          "created socket to host \"" + host + "\", port " + port + ", timeout " + timeout + "ms");
    }
    ...
    try {
      ...
      if (millisecs > timeout * 95 / 100) {
        ...
        response.setContent(
            "( \"JadeBridge error in parsing ACL response from SocketProxy:" + any + "\" )");
      }
    }
