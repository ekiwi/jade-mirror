...
/**
 * Agent to bridge ACL messages between JADE and the outside using a socket. The agent(s) to talk to
 * and the port to be used may be specified:
 * <ol>
 * <li>as the two String arguments to the agent, or</li>
 * <li>if no agent argument, then in a configuration file <i>agentname</i><code>.inf</code>, or if
 * no configuration file, then</li>
 ...
 * </ol>
 * Specifying an asterisk (<code>*</code>) for the agents will permit all agents to be legal
 * addressees.
 * 
...
public class SocketProxyAgent extends Agent {
  ...
  protected void setup() {
    try {
      ...
      if (logger.isLoggable(Logger.FINE)) {
        logger.log(Logger.FINE, "Attempting to open a server socket on port: " + portNumber);
