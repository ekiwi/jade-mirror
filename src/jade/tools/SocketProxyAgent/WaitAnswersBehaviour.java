...
class WaitAnswersBehaviour extends SimpleBehaviour {
  ...
  /** my logger */
  private final static Logger logger = Logger.getMyLogger(WaitAnswersBehaviour.class.getName());
  ...
  WaitAnswersBehaviour(Agent a, ACLMessage m, PrintStream o) {
    ...
    logger.log(Logger.CONFIG,
        myThreadName + ": Constructed " + this.getClass().getName() + " using message template "
            + mt + " and timeout " + timeout + " to wait for answer to:" + m);
  }

  /**
   * behaviour action blocking receive for message, followed by copying that message to the output
   * stream.
   */
  public void action() {
    if (logger.isLoggable(Logger.FINE)) {
      logger.log(Logger.FINE, myThreadName + ": About to block, message template " + mt
          + ", timeout " + timeout + "ms, waiting for reply...");
    }
    msg = myAgent.blockingReceive(mt, timeout);

    if (logger.isLoggable(Logger.FINE)) {
      logger.log(Logger.FINE, myThreadName + ": No longer blocked");
    }
    ...
    if (logger.isLoggable(Logger.FINE)) {
      logger.log(Logger.FINE, myThreadName + ": writing reply...:" + msg);
    }
    ...
    if (logger.isLoggable(Logger.FINE)) {
      logger.log(Logger.FINE, myThreadName + ": set finished=" + finished);
    }
