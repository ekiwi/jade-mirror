...
public class StartDialog extends JDialog implements ActionListener {
  ...
  private static class AgentClassFilter implements ClassFinderFilter {
    private final static String[] excluded = new String[] {"jade.domain.ams",
        "jade.tools.ToolNotifier", "jade.tools.logging.LogHelperAgent"};

    // Exclude all classes that are
    // - Not concrete (abstract or interfaces)
    // - Contained in the jade.core package (a part from the jade.core.Agent class
    // itself)
    // - Explicitly mentioned in the "exclude" array
    public boolean include(Class superClazz, Class clazz) {
      String clazzName = clazz.getName();
      int modifiers = clazz.getModifiers();
      boolean doInclude = ((modifiers
          & (ClassSelectionDialog.ACC_ABSTRACT | ClassSelectionDialog.ACC_INTERFACE)) == 0);
      if (doInclude) {
  ...
  void updateOkButtonEnabled() {
    ...
    boolean okEnabled = selClassname != null && selClassname.length() > 0 && currAgentName != null
        && currAgentName.length() > 0;
