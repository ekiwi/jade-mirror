...
class MainPanel extends JPanel implements DropTargetListener, TreeSelectionListener {
  ...
  public MainPanel(rma anRMA, MainWindow mainWnd) {
    ...
    pan = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(treeAgent.tree),
        table.createTable());
    ...
    if (paths != null) {
      ...
      for (int i = 0; i < paths.length; i++) {
        ...
        for (int j = 0; j < relCur.length; j++) {
          ...
          else if (relCur[j] instanceof AgentTree.SuperContainer) {// show the APDescription in the
                                                                   // TextArea
          }
        }
      }
  ...
  public void adjustDividersLocation() {
    // int rootSize = pane.getDividerLocation(); // This is the height of a single
    // tree folder
    // pane.setDividerLocation(7*rootSize); // The initial agent tree has 6
    // elements; one more empty space
