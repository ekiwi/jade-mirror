...
class ActionProcessor {
  ...
  public static final String REGISTERREMOTEAGENTWITHAMS_ACTION =
      "Register Remote Agent with local AMS";
  ...
  public ActionProcessor(rma anRma, MainWindow mWnd, MainPanel panel) {
    ...
    actions.put(ADDREMOTEPLATFORMFROMURL_ACTION,
        new AddRemotePlatformFromURLAction(anRma, this, mWnd));
  ...
  public void process(RMAAction a) {
    ...
    // means that, if no selection is present, no action is performed.
      ...
      if (paths != null) {
        ...
        // path null
        if (action instanceof AgentAction)
          JOptionPane.showMessageDialog(new JFrame(), "You must select an agent in the Tree",
              "Start Procedure Error", JOptionPane.ERROR_MESSAGE);
        else if (action instanceof DummyAgentAction || action instanceof SnifferAction
            || action instanceof IntrospectorAction || action instanceof LogManagerAgentAction)
          containerAct(null);
        else if (action instanceof PlatformAction)
          JOptionPane.showMessageDialog(new JFrame(), "You must select a platform", "Error",
              JOptionPane.ERROR_MESSAGE);
        else
          JOptionPane.showMessageDialog(new JFrame(),
              "You must select an agent-platform or a agent-container in the Tree",
              "Start Procedure Error", JOptionPane.ERROR_MESSAGE);
  ...
  private void containerAct(AgentTree.Node node) {
    ...
    if ((ac instanceof DummyAgentAction || ac instanceof SnifferAction
        || ac instanceof IntrospectorAction || ac instanceof LogManagerAgentAction)
        && (node == null || node instanceof AgentTree.SuperContainer))
      ac.doAction(null);
    else
      try {
  ...
  private void platformAct(AgentTree.Node node) {
    ...
    if ((node instanceof AgentTree.LocalPlatformFolderNode)
        || (node instanceof AgentTree.RemotePlatformNode))
      ac.doAction(node);
