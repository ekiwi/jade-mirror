...
public class SLFormatter {
  ...
  /**
   * Private class to iterate over a string. This has a next() and hasNext() inspired by the
   * standard Iterator interface. (It might appear that java.text.StringCharacterIterator would
   * work, but its implementation of next() doesn't match the standard iterator - it returns the
   * value after incrementing the index, and so always skips the first character).
   */
  ...
  /**
   * Formats an ACL/SL string. This takes an input string, formats it (adding new-lines and spaces
   * in an attempt to improve readability), and returns the result. The formatted output is similar
   * to that used for the examples in the FIPA SL spec. The formatter attempts to keep short
   * expressions on one line, while indenting longer expressions to show the structure. As an
   * example, the result of:
   * 
   * <pre>
   * System.out.println(SLFormatter.format(
   *     "(vehicle :color red :max-speed 100 :owner (Person :name Luis :nationality Potuguese))"));
   * </pre>
   * 
   * is the output
   * 
   ...
   * </pre>
   * 
   * Usually, the input string will come from a call to ACLMessage.getContent(), for example if you
   * have an ACLMessage called msg, you can print formatted content:
   * 
   * <pre>
   *    System.out.println(SLFormatter.format(msg.getContent());
   * </pre>
   * 
   * Since text ACL messages follow an SL style, you can also use this function to format entire
   * messages by formatting the return value from msg.toString().
   *
  ...
  /**
   * Main routine is helpful for debugging. It asks for an SL expression and then prints the result
   * of calling SLFormatter.format.
   * 
