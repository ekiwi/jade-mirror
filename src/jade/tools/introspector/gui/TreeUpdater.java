...
public class TreeUpdater implements Runnable {
  ...
  public TreeUpdater(ChangedBehaviourState b, BehaviourPanel bp) {
    ...
    /*
     * if (b.getTo().equals(Behaviour.STATE_BLOCKED)) { blocked = true; } else { blocked = false; }
     */
    state = b.getTo();
  }
  ...
  public void createTree(DefaultMutableTreeNode r, Iterator v) {
    while (v.hasNext()) {
      ...
      // DefaultMutableTreeNode rc = new DefaultMutableTreeNode(new
      // BehaviourTreeNode(b, blocked));
    ...
    if (action == CHANGE_NODE) {
      ...
      while (e.hasMoreElements()) {
        ...
        if (b.getCode() == behaviour.getCode() && b.equals(behaviour)) {
          ...
          /*
           * if ((blocked && !bNode.isBlocked()) || (!blocked && bNode.isBlocked())) {
           * bNode.setBlocked(blocked); model.nodeChanged(node); }
           */
      ...
      /*
       * If we didn't find the node in the tree, add it now. if (!bFound) action = ADD_NODE;
       */
    }

    if (action == ADD_NODE) {
      // DefaultMutableTreeNode beh = new DefaultMutableTreeNode(new
      // BehaviourTreeNode(behaviour, blocked));
      DefaultMutableTreeNode beh =
          new DefaultMutableTreeNode(new BehaviourTreeNode(behaviour, state));
