...
/**
 * This is the main window, containing the agent tree and a JDesktopPane to whom internal debugging
 * window are added.
 * 
...
public class IntrospectorGUI extends JFrame implements WindowListener {
  ...
  public IntrospectorGUI(Introspector i) {
    ...
    panel.treeAgent.setNewPopupMenu(AgentTree.AGENT_TYPE,
        new TreeAgentPopupMenu(debugger, panel.treeAgent));
  ...
  public void showError(String errMsg) {
    JOptionPane.showMessageDialog(null, errMsg, "Error in " + debugger.getName(),
  ...
  /*
   * public void messageEvent(MessageEvent e, MainWindow f){ MessagePanel mp=f.getMessagePanel();
   * EventQueue.invokeLater(new TableUpdater(e, mp)); } public void stateEvent(StateEvent
   * e,MainWindow f){ StatePanel sp=f.getStatePanel(); EventQueue.invokeLater(new
   * StateUpdater(e,sp)); }
   * 
   * 
   * //l'initEvent viene scomposto in tanti eventi //minori.Questo lo rende + lento ma meno
   * complesso public void initEvent(InitEvent e,MainWindow f){
   * 
   * Iterator it=e.getAllBehaviours(); BehaviourPanel bp=f.getBehaviourPanel(); while(it.hasNext()){
   * BehaviourRapp b=(BehaviourRapp)it.next(); BehaviourEvent event=new
   * BehaviourEvent(b,false,true); EventQueue.invokeLater(new TreeUpdater(event,bp)); }
   * 
   * it=e.getAllinMessages(); MessagePanel mp=f.getMessagePanel(); while(it.hasNext()){ MessageRapp
   * b=(MessageRapp)it.next(); MessageEvent event=new MessageEvent(b,true,true);
   * EventQueue.invokeLater(new TableUpdater(event,mp)); }
   * 
   * it=e.getAlloutMessages(); while(it.hasNext()){ MessageRapp b=(MessageRapp)it.next();
   * MessageEvent event=new MessageEvent(b,false,true); EventQueue.invokeLater(new
   * TableUpdater(event,mp)); }
   * 
   * StateEvent se=new StateEvent(e.getState()); StatePanel sp=f.getStatePanel();
  ...
  public void addAgent(final String containerName, final AID agentID) {
    ...
    Runnable addIt = new Runnable() {
      public void run() {
        ...
        // panel.treeAgent.addAgentNode((AgentTree.AgentNode)node, containerName,
  ...
  public void addContainer(final String name, final InetAddress addr) {
    Runnable addIt = new Runnable() {
      public void run() {
        ...
        // panel.treeAgent.addContainerNode((AgentTree.ContainerNode)node,
        // "FIPACONTAINER", addr);
