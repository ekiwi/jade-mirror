...
/*
 * This class represents the Introspector Agent. This agent registers with the AMS as a tool, to
 * manage an AgentTree component, then activates its GUI. The agent listens for ACL messages
 * containing introspection events and updates the display through the IntrospectorGUI class.
 * 
...
public class Introspector extends ToolAgent {
  ...
  /*
   * Adds an agent to the debugged agents map, and asks the AMS to start debugging mode on that
   * agent.
   */
  ...
  /*
   * Removes an agent from the debugged agents map, and closes its window. Moreover,it and asks the
   * AMS to stop debugging mode on that agent.
   */
  ...
  /**
   * Cleanup during agent shutdown. This method cleans things up when <em>RMA</em> agent is
   * destroyed, disconnecting from <em>AMS</em> agent and closing down the platform administration
   * <em>GUI</em>.
   */
  ...
  /**
   * Inner class ControlListenerBehaviour. This is a behaviour that listen for messages from
   * ToolNotifiers informing that they have started notifying events about a given agent. These
   * information are used to keep the map between observed agents and ToolNotifiers up to date.
   */
  ...
  /**
   * The doDelete() method is re-defined because if the Introspector is killed while it is debugging
   * the AMS a deadlock occurs. In fact, while exiting, the Introspector can't make the debugged
   * agents proceed. At the same time however the Introspector can't proceed as it is waiting for
   * the answer to its AMS deregistration
   */
  ...
  /**
   * Inner class RequestListenerBehaviour. This behaviour serves requests to start debugging agents.
   * If an agent does not exist it is put into the preload table so that it will be debugged as soon
   * as it starts.
   */
