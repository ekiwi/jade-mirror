...
public class ToolNotifier extends ToolAgent implements MessageListener, AgentListener {
  ...
  /**
   * Inner class SynchEventInformer. When the observation of an event must be synchronous (i.e. the
   * thread that generated the event must block until the observer has finished observing the event)
   * we must - inform the observer - wait for the observer to send back a proper indication - wake
