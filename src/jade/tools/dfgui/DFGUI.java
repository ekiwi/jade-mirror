...
/**
 * This class implements the GUI of the Directory Facilitator. The gui shows a tabbed pane with
 * three different views of the functions provided by a Directory Facilitator. The three views are:
 * <ul>
 * <li><b>Registrations with this DF</b> shows a table with all the agents registered with the DF.
 * <li><b>Search Result</b> shows a table with the list of agent descriptions that were returned as
 * a result of the last search operation on a specified df.
 * <li><b>DF Federation</b> shows the DF federation. The Parents table shows the list of DF's with
 * which this DF is federated, while the Children table shows the list of DF's that are registered
 * with this DF.
 * </ul>
 ...
 * <li><b>Register</b> an agent with the DF. The user is then requested to fill in an agent
 * description, notice that some values are mandatory for registration,
 * <li><b>Deregister</b> an agent selected in the table.
 * <li><b>Search</b> for agent descriptions with this DF. The user must first insert the search
 * constraint: <code> maximum depth</code> the depth of propagation of the search operation to the
 * federate DF's (children);<code> maximum number of results </code>. If no values are inserted then
 * the default one are used :local search on this df and all agents found returned.The the user must
 * provide an agent description. If no value is inserted in the agent description, the search action
 * returns all the active agents currently registered with this DF.
 * <li><b>Federate</b> allow to federate this DF with another DF. First of all, the user must
 * provide the full name of the DF with which to federate and then the description of this DF that
 * must be registered with the specified DF.
 * </ul>
 * <b>Search Result</b>
 * <ul>
 * <li><b>View</b> the description of a selected agent on the table of the results.
 * <li><b>Register</b> a new agent with last DF used for the search operation (indicated in the
 * tab).
 * <li><b>Modify</b> the agent description of the agent selected in the table (with the appropriate
 * df).
 * <li><b>Search</b> for agent descriptions with the DF involved in the last search operation. (see
 * above)
 * <li><b>Federation</b> (see above)
 * </ul>
 * <b>DF Federation</b>
 * <ul>
 * <li><b>View</b> the description of an agent selected in one of the two tables. If the agent
 * selected is a parent, then the description of this DF used to fedearate is shown. Otherwise if
 * the selected agent is a child,then the description of this child DF is shown.
 * <li><b>Register</b> a new agent with the DF selected in one of the two tables.
 * <li><b>Deregister</b> If the selected agent is a parent then this DF is deregistered from the
 * selected one, otherwise, if the agent selected is a child, this child is deregistered from this
 * DF.
...
public class DFGUI extends JFrame implements DFGUIInterface {
  ...
  // CONSTRUCTORS
  /**
   * Constructor without parameter. Used by the df to avoid reflection, Using this constructor, the
   * method setAdapter must be called (after the constructor) to set the agent with which the gui
   * interacts.
   */
  public DFGUI() {
    //////////////////////////
    ...
    foundTable.setToolTipText(
        "Double click on agent name to view the services provided by the selected agent");
    // Column names
  ...
  /*
   * This method must be used after the constructor of the gui to set the object implementing the
   * DFGUIAdapter interface. It's necessary to avoid any dipendency between the JADE tools and the
   * class of the domain package.
   */
  ...
  /*
   * private void setSearch(boolean value) { searchB.setEnabled(value);
  ...
  /**
   * This method returns the <code>DFAgentDescription</code> of an agent found in a search
   * operation.
   * 
  ...
  /*
   * This method must be called after the constructor to enable the RefreshGui action. It's
   * necessary for the DFapplet.
   */
