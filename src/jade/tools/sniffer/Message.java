...
/**
 * A <em>Message</em> extends the meaning of an ACLMessage (thus extending jade.lang.acl.ACLMessage)
 * providing an ACLMessage with the graphic part: the arrow going from the sender of the message to
 * the receiver of the message to be drawn on the Message Canvas
 *
...
public class Message extends jade.lang.acl.ACLMessage implements Serializable {
  ...
  /*
   * TO BE REMOVED public Message(AID s, AID r){
   * 
   * super(ACLMessage.INFORM);
   * 
   * this.setSender(s); this.clearAllReceiver(); this.addReceiver(r); }
   */
  ...
  public Message(ACLMessage msg, AID r) {
    ...
    for (Iterator i = msg.getAllReceiver(); i.hasNext();)
      // #DOTNET_EXCLUDE_END
      /*
       * #DOTNET_INCLUDE_BEGIN for (jade.util.leap.Iterator i=msg.getAllReceiver(); i.hasNext(); )
       * #DOTNET_INCLUDE_END
       */
    ...
    for (Iterator i = msg.getAllReplyTo(); i.hasNext();)
      // #DOTNET_EXCLUDE_END
      /*
       * #DOTNET_INCLUDE_BEGIN for (jade.util.leap.Iterator i=msg.getAllReplyTo(); i.hasNext(); )
       * #DOTNET_INCLUDE_END
       */
