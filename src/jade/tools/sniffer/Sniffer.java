 ...
 * This class implements the low level part of the Sniffer, interacting with Jade environment and
 * with the sniffer GUI.<br>
 * At startup, the sniffer subscribes itself as an rma to be informed every time an agent is born or
 * dead, a container is created or deleted. <br>
 * For more information see <a href="../../../../intro.htm" target="_top">Introduction to the
 * Sniffer</a>.
 * <p>
 * A properties file while may be used to control different sniffer properties. These optional
 * properties are as follows:
 * <ul>
 * <li>preload - A list of preload descriptions seperated by a semi-colon. Each description consists
 * of an agent name match string and optional list of performatives each seperated by a space. For
 * details on the agent name match string, see the method isMatch(). If there is no @ in the agent
 * name, it assumes the current HAP for it. If the performative list is not present, then the
 * sniffer will display all messages; otherwise, only those messages that have a matching
 * performative mentioned will be displayed. <br>
 * Examples:
 * 
 * <pre>
 ...
 * </pre>
 * </ul>
 * The property file is looked for in the current directory, and if not found, it looks in the
 * parent directory and continues this until the file is either found or there isn't a parent
 * directory.
 * <p>
 * The original implementation processed a .inf file. For backward compatability this has been
 * preserved but its usage should be converted to use the new .properties file. The format of the
 * .inf file is each line contains an agent name and optional list of performatives. <br>
 * Example:
 * 
 * <pre>
 * da0
 ...
 * Notes:
 * <ol>
 * <li>If a message is one that is to be ignored, then it is dropped totally. If you look at the
 * sniffer dump of messages, it will not be there. Might want to change this.
 * <li>Should develop a GUI to allow dynamically setting which messages are filtered instead of
 * forcing them to be in the properties file.
 * <li>Probably should allow one to turn on and off the display of the performative name. Although,
 * it seems pretty nice to have this information and although one might consider that it clutters
 * the display, it sure provides a lot of information with it.
 * </ol>
 *
 * @author <a href="mailto:alessandro.beneventi@re.nettuno.it"> Alessandro Beneventi
 *         </a>(Developement)
...
public class Sniffer extends ToolAgent {
    ...
    public void action() {
      ...
      if (current != null) {
        ...
        try {
          ...
          /*
           * If this is a 'posted-message' event and the sender is // currently under sniff, then
           * the message was already // displayed when the 'sent-message' event occurred. In that //
           * case, we simply skip this message. if(ev instanceof PostedMessage) { Agent a = new
           * Agent(msg.getSender()); if(agentsUnderSniff.contains(a)) return; }
           */
  ...
  /**
   * Given two strings determine if they match. We iterate over the match expression string from
   ...
   * <li>If we encounter a '?' in the expression token we ignore the subject string's character and
   * move on to the next iteration.
   * <li>If the character in the expression token isn't equal to the character in the subject string
   * they don't match.
   * </ol>
   * If we complete the iteration they match only if there are the same number of characters in both
   * strings.
   * 
  ...
  /**
   * ACLMessages for subscription and unsubscription as <em>rma</em> are created and corresponding
   * behaviours are set up.
   */
  public void toolSetup() {
    ...
    /*
     * preload agents from argument array if arguments present, otherwise load sniffer.properties
     * file.
     */
  ...
  /*
   * #DOTNET_INCLUDE_BEGIN private void createUI() { // Show Graphical User Interface myGUI = new
   * MainWindow(this, properties); myGUI.ShowCorrect(); System.Windows.Forms.Application.Run(); }
   * 
   ...
   * 
   * // Schedule Behaviours for execution addBehaviour(AMSSubscribe); addBehaviour(new
   * SniffListenerBehaviour()); } #DOTNET_INCLUDE_END
   */
  ...
  /**
   * Private function to read configuration file containing names of agents to be preloaded. Also
   * supports message filtering based on performatives. Each line of the file lists an agent,
   * optionally followed by a set of messages to sniff. If there are no tokens, then we assume that
   * means to sniff all. The tokens are the name of the performative type such as INFORM, QUERY,
   * etc.
   * 
  ...
  /**
   * Cleanup during agent shutdown. This method cleans things up when <em>Sniffer</em> agent is
   * destroyed, disconnecting from <em>AMS</em> agent and closing down the Sniffer administration
   * <em>GUI</em>. Currently sniffed agents are also unsniffed to avoid errors.
   */
  ...
  /**
   * Creates the ACLMessage to be sent to the <em>Ams</em> with the list of the agents to be
   * sniffed/unsniffed. The internal list of sniffed agents is also updated.
   *
  ...
  /**
   * Inner class RequestListenerBehaviour. This behaviour serves requests to start sniffing agents.
   * If an agent does not exist it is put into the preload table so that it will be sniffed as soon
   * as it starts.
   */
      ...
      try {
        ...
        if (aa instanceof SniffOn || aa instanceof SniffOff) {
          if (getAID().equals(requestAction.getActor())) {
            ...
            response.setContent(
                "((unrecognised-parameter-value actor " + requestAction.getActor() + "))");
          }
        } else {
          response.setPerformative(ACLMessage.REFUSE);
