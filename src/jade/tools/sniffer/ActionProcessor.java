...
/*
 * #DOTNET_INCLUDE_BEGIN import System.Windows.Forms.*; #DOTNET_INCLUDE_END
 */
...
class ActionProcessor {
  ...
  public void process(SnifferAction act) {
    ...
    // #DOTNET_EXCLUDE_END
    /*
     * #DOTNET_INCLUDE_BEGIN System.Collections.IList paths = new System.Collections.ArrayList();
     * TreeNode node;
     * 
     * TreeNode aNode = mainPanel.treeAgent.tree.get_SelectedNode(); while (aNode != null) {
     * paths.Add( aNode ); aNode = aNode.get_Parent(); if ( !(aNode instanceof AgentTree.AgentNode)
     * ) aNode = null; } #DOTNET_INCLUDE_END
     */
    ...
    // means that, if no selection is present, no action is performed.
      ...
      if (paths != null) {
        ...
        // #DOTNET_EXCLUDE_END
        /*
         * #DOTNET_INCLUDE_BEGIN lungpath=paths.get_Count(); for (int i=0;i<lungpath;i++) { try {
         * now = (AgentTree.Node) (paths.get_Item(i)); agentAct(now); } catch (ClassCastException
         * cce) {} } #DOTNET_INCLUDE_END
         */
      }
    }
