 ...
 * The socket server is implemented by the agent jade.tools.SocketProxyAgent.
 * 
...
public class DFAppletCommunicator implements DFGUIAdapter {
  ...
  /**
   * Create a socket to communicate with a server on port 6789 of the host that the applet's code is
   * on. Create streams to use with the socket. Finally, gets the value of the parameter
   * <code>JADEAddress</code> from the HTML file.
   */
  public DFAppletCommunicator(Applet applet) {
    try {
      ...
      if (logger.isLoggable(Logger.CONFIG))
        logger.log(Logger.CONFIG, "DFAppletClient connected to local port " + s.getLocalPort()
            + " and remote port " + s.getPort());
  ...
  /**
   * This method allows this class to call the method showStatusMsg implemented by DFGUI
   */
  void setGUI(DFGUI g) {
    gui = g;
  }
  ...
  /**
   * returns "df" that is the name of the default DF. In fact, so far this applet can be used only
   * to interact with the default DF.
   */
  ...
  /**
   * Refresh the gui of the applet. First of all makes a search for all agent registered with the
   * df, then updates the federate view, requesting the parents to the df.
   */
  ...
  public void refreshDFGUI() {
    ...
    try {
      ...
      FIPAAppletRequestProto arp =
          new FIPAAppletRequestProto(this, df, FIPAManagementVocabulary.SEARCH, dfd, sc);
      arp.doProto();
      ...
      JADEAppletRequestProto getParent = new JADEAppletRequestProto(this,
          getDescriptionOfThisDF().getName(), DFAppletVocabulary.GETPARENTS, null, null);
      getParent.doProto();
  ...
  private void RegisterNewAgent(GuiEvent event) {
    ...
    if (df.getName().equalsIgnoreCase(thisDF.getName().getName()))
      try {
        ...
        FIPAAppletRequestProto rf =
            new FIPAAppletRequestProto(this, df, FIPAManagementVocabulary.REGISTER, dfd, null);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
      ...
      try {
        JADEAppletRequestProto requestBehav = new JADEAppletRequestProto(this,
            getDescriptionOfThisDF().getName(), DFAppletVocabulary.REGISTERWITH, dfd, df);
  ...
  private void DeregisterAgent(GuiEvent event) {
    ...
    if (df.getName().equalsIgnoreCase(thisDF.getName().getName()))
      try {
        FIPAAppletRequestProto rf =
            new FIPAAppletRequestProto(this, df, FIPAManagementVocabulary.DEREGISTER, dfd, null);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
      ...
      try {
        JADEAppletRequestProto rf = new JADEAppletRequestProto(this,
            getDescriptionOfThisDF().getName(), DFAppletVocabulary.DEREGISTERFROM, dfd, df);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
  ...
  private void SearchAgents(GuiEvent event) {
    ...
    if (df.getName().equalsIgnoreCase(thisDF.getName().getName()))
      ...
      try {
        JADEAppletRequestProto rf = new JADEAppletRequestProto(this,
            getDescriptionOfThisDF().getName(), DFAppletVocabulary.SEARCHON, dfd, df, sc);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
  ...
  private void Modify(GuiEvent event) {
    ...
    if (df.equals(thisDF.getName()))
      try {
        FIPAAppletRequestProto rf =
            new FIPAAppletRequestProto(this, df, FIPAManagementVocabulary.MODIFY, dfd, null);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
      ...
      try {
        JADEAppletRequestProto rf = new JADEAppletRequestProto(this, thisDF.getName(),
            DFAppletVocabulary.MODIFYON, dfd, df);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
  ...
  private void Federate(GuiEvent event) {
    ...
    if (parentDF.equals(thisDF.getName())) {
      ...
      try {
        JADEAppletRequestProto rf = new JADEAppletRequestProto(this,
            getDescriptionOfThisDF().getName(), DFAppletVocabulary.FEDERATE, dfd, parentDF);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
  ...
  public DFAgentDescription getDescriptionOfThisDF() {
    if (thisDF == null) {
      ...
      try {
        JADEAppletRequestProto rf =
            new JADEAppletRequestProto(this, df, DFAppletVocabulary.GETDESCRIPTION, null, null);
        rf.doProto();
      } catch (FIPAException e) {
        e.printStackTrace();
      }
  ...
  public DFAgentDescription getDescriptionOfThisDF(AID df) {
    if (logger.isLoggable(Logger.FINEST))
      logger.log(Logger.FINEST,
          "CALLED METHOD: getDescriptionOfThisDF(aid) into DFAppletCommunicator");
    DFAgentDescription output = null;
    try {

      JADEAppletRequestProto rf = new JADEAppletRequestProto(this,
          getDescriptionOfThisDF().getName(), DFAppletVocabulary.GETDESCRIPTIONUSED, null, df);
      rf.doProto();
  ...
  boolean isADF(DFAgentDescription dfd) {
    try {
      ServiceDescription sd =
          (ServiceDescription) ((DFAgentDescription) dfd).getAllServices().next();
