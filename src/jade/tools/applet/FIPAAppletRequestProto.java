...
public class FIPAAppletRequestProto extends AppletRequestProto {
   ...
   */
  FIPAAppletRequestProto(DFAppletCommunicator communicator, AID receiver, String actionName,
      Object agentDescription, SearchConstraints sc) throws FIPAException {
    ...
    // FIXME for applet I have not the agent c =
    // sender.lookupLanguage(SL0Codec.NAME);
    // if (c == null)
    c = new SLCodec();
  ...
  public void handleInform(ACLMessage msg) {
    ...
    try {
      if (this.action.equalsIgnoreCase(FIPAManagementVocabulary.REGISTER)) { // refresh the gui
        ...
        gui.showStatusMsg("Registration of agent:"
            + ((DFAgentDescription) this.dfd).getName().getName() + " done.");

      } else if (this.action.equalsIgnoreCase(FIPAManagementVocabulary.SEARCH)) { // extract the
                                                                                  // results and
                                                                                  // update the gui
        try {
        ...
        // update the gui
        try {
          gui.removeChildren(((DFAgentDescription) dfd).getName()); // if the agent deregistered was
                                                                    // a child
        } catch (Exception e) {
        }

        gui.removeAgentDesc(((DFAgentDescription) dfd).getName(),
            dfApplet.getDescriptionOfThisDF().getName());
        gui.showStatusMsg("Deregistration of agent: "
            + ((DFAgentDescription) dfd).getName().getName() + " done.");
        ...
        gui.showStatusMsg(
            "Modify of Agent: " + ((DFAgentDescription) dfd).getName().getName() + " done.");
      }
