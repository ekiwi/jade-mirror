...
/**
 * This class represents the Log-Agent-Management-ontology i.e. the set of concepts, actions and
 * predicates needed by the LogManagerAgent and the LogHelperAgent to manage logs on JADE containers
 * at runtime.
 * 
...
public class LogManagementOntology extends Ontology implements LogManagementVocabulary {
  ...
  private LogManagementOntology() {
    ...
    try {
      ...
      cs.add(LOGGER_INFO_HANDLERS, (PrimitiveSchema) getSchema(BasicOntology.STRING), 0,
          ObjectSchema.UNLIMITED);
      cs.add(LOGGER_INFO_FILE, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);

      AgentActionSchema as = (AgentActionSchema) getSchema(GET_ALL_LOGGERS);
      as.add(GET_ALL_LOGGERS_TYPE, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
      as.add(GET_ALL_LOGGERS_FILTER, (PrimitiveSchema) getSchema(BasicOntology.STRING),
          ObjectSchema.OPTIONAL);
