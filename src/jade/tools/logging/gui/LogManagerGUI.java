...
public class LogManagerGUI extends JFrame {
  private static final String DEFAULT_LOG_MANAGER_CLASS =
  ...
  private AbstractAction startManagingLogAction, stopManagingLogAction,
      setDefaultLoggingSystemAction, exitAction;
  ...
  public LogManagerGUI(Agent a) {
    ...
    jsp = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, new JScrollPane(agentTree.tree),
    ...
    if (node != null && node instanceof AgentTree.ContainerNode) {
      ...
      if (window != null) {
        ...
        try {
          ...
          window =
              new ContainerLogWindow(myAgent, containerName, controller, defaultLogManager, this);
          window.pack();
          ...
          String msg = (state == 0 ? "Cannot create Log Helper agent on container " + containerName
              : "Cannot retrieve logging information from container " + containerName);
  ...
  void stopManagingLog() {
    ...
    if (node != null && node instanceof AgentTree.ContainerNode) {
      String containerName = node.getName();
      final ContainerLogWindow window =
          (ContainerLogWindow) managedContainers.remove(containerName);
      if (window != null) {
  ...
  LogManager initializeLogManager() {
    ...
    try {
      className = JOptionPane.showInputDialog(this,
          "Insert the fully qualified class name of the LogManager implementation for the desired logging system");
      if (className != null) {
        return (LogManager) Class.forName(className).newInstance();
      }
    } catch (Exception e) {
      JOptionPane.showMessageDialog(this,
          "Cannot create a LogManager of class " + className + " [" + e + "]");
    }
    return null;
  }
