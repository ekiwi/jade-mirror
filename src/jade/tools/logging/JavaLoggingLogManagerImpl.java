...
public class JavaLoggingLogManagerImpl implements LogManager {
    ...
    if (this.loggers == null) {
      ...
      for (Enumeration e = logManager.getLoggerNames(); e.hasMoreElements();) {
        ...
        try {
          ...
          // If the result is null, this logger's effective level will be inherited from
          // its parent.
          // The value is the one specified for the property level in the configuration
          // file
          ...
          // if a FileHandler has been specified it's not possibile to retrieve the
          // fileName

          // retrieves all the handlers associated to the logger
          // root handlers are inherited by default
          List loggerHandlers =
              (this.rootHandlers == null ? new ArrayList() : new ArrayList(this.rootHandlers));
          ...
          if (!logName.equals("")) {
            ...
            // add the file handler specified by the user only if fileHandler is not a root
            // handler
            for (int i = 0; i < handlers.length; i++) {
              String temp = handlers[i].toString();
              if (!fhExists) {
                String userHandler =
                    (temp.indexOf('@') < 0 ? temp : temp.substring(0, temp.indexOf('@')));
          ...
          this.loggers.add(logInfoElem); // non sono in ordine alfabetico (dovrebbe essere la gui a
                                         // mostrarle
                                         // in quell'ordine.
        } catch (Exception ex) {
          ex.printStackTrace();
  ...
  /**
   * Returns a list of <code>LevelInfo</code> object each one describing a level valid for the
   * selected logging system.
   * 
   * @return a list of <code>LevelInfo</code> object each one describing a level valid for the
   *         selected logging system.
   */
