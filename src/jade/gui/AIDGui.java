...
public class AIDGui extends JDialog {
   ...
   * @param checkMandatorySlots is true when the returned AID must contain all the mandatory slots,
   *        as defined by FIPA specs
  ...
  public AID ShowAIDGui(AID agentIdentifier, boolean ed, boolean checkMandatorySlots) {
    ...
    // #DOTNET_EXCLUDE_END
    /*
     * #DOTNET_INCLUDE_BEGIN java.util.ArrayList aList = new java.util.ArrayList();
     * jade.util.leap.Iterator it = agentAID.getAllAddresses(); while ( it.hasNext() ) { aList.add(
     * it.next() ); } addressListPanel = new VisualStringList(aList.iterator(), getChildrenOwner());
     * #DOTNET_INCLUDE_END
     */
    ...
    // #DOTNET_EXCLUDE_END
    /*
     * #DOTNET_INCLUDE_BEGIN java.util.ArrayList bList = new java.util.ArrayList();
     * jade.util.leap.Iterator it2 = agentAID.getAllResolvers(); while ( it2.hasNext() ) {
     * bList.add( it2.next() ); } resolverListPanel = new VisualAIDList(bList.iterator(),
     * getChildrenOwner()); #DOTNET_INCLUDE_END
     */
    ...
    propertiesListPanel =
        new VisualPropertiesList(agentAID.getAllUserDefinedSlot(), getChildrenOwner());
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if (param.equals("OK")) {
          ...
          if (editable) {
            ...
            if (checkSlots)
              if (name.length() == 0) {
                JOptionPane.showMessageDialog(AIDGui.this, "AID must have a non-empty name.",
  ...
  private void ShowCorrect() {
    pack();
    if (parentGUI != null) {
      setLocation(parentGUI.getX() + (parentGUI.getWidth() - getWidth()) / 2,
          parentGUI.getY() + (parentGUI.getHeight() - getHeight()) / 2);
    }
    setVisible(true);
    toFront();
  }
