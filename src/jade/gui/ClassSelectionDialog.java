...
public class ClassSelectionDialog extends JDialog
  ...
  private void updateLabelStatus(boolean stillSearching) {
    if (stillSearching) {
      jLabelStatus.setText("Searching in classpath for classes that extend " + classname + " ("
          + jTableModel.getRowCount() + " found so far)");
    } else {
      setCursor(new Cursor(Cursor.DEFAULT_CURSOR));
      jLabelStatus.setText(
          "Classpath contains " + jTableModel.getRowCount() + " classes that extend " + classname);
    }
  }
  ...
  public void windowOpened(WindowEvent e) {
    if (!classesLoaded) {
      ...
      ClassUpdater cu =
          new ClassUpdater(classname, classfilter == null ? new ClassFilter() : classfilter);
      new Thread(cu).start();
    }
  }
