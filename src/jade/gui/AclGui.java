...
/**
 * The AclGui class extends the Swing JPanel class by adding all the controls required to properly
 * edit/show the fields of an an ACL message compliant to the <b>FIPA 97</b> specs.
 * <p>
 * <p>
 * There are basically two ways of using the AclGui class.
 * <ul>
 * <li><b>Non Static Mode</b>. As AclGui extends JPanel, an instance of AclGui can be directly added
 * to whatever Container thus providing an easy way to permanently insert into a GUI a panel for the
 * editing/display of an ACL message.<br>
 * The <em>setMsg()</em> and <em>getMsg()</em> methods can be used to display a given ACL message in
 * the panel and to retrieve the ACL message currently displayed.<br>
 * The <em>setEnabled()</em> and <em>setSenderEnabled()</em> methods can be used to enable/disable
 * modifications to all fields in the ACL message and/or the sender field respectively.<br>
 * E.g.<br>
 * This code creates an agent GUI with a panel (in the left part of the GUI) that displays each new
 * message received by the agent <code>
 * .....<br>
 * AclGui acl;<br>
 * .....<br>
 ...
 * .....<br>
 * </code> Each time a new message is received (assuming the message has been stored in the msg
 * variable of type ACLMessage) <code>
 * acl.setMsg(msg);<br>
 * </code></li>
 * <li><b>Static Mode</b>. The AclGui class also provides the <em>editMsgInDialog()</em> and
 * <em>showMsgInDlg()</em> static methods that pop up a temporary dialog window (including an AclGui
 * panel and the proper OK and Cancel buttons) by means of which it is possible to edit and show a
 * given ACL message.<br>
 * E.g.<br>
 * This code creates a button that allows the user to edit an ACL message by means of a temporary
 * dialog window <code>
 * .....<br>
 * ACLMessage msg;<br>
 * .....<br>
...
public class AclGui extends JPanel {
  ...
  private static String fipaProtocols[] = {FIPANames.InteractionProtocol.FIPA_ENGLISH_AUCTION,
      ...
      FIPANames.InteractionProtocol.FIPA_QUERY, FIPANames.InteractionProtocol.FIPA_REQUEST,
      FIPANames.InteractionProtocol.FIPA_REQUEST_WHEN, FIPANames.InteractionProtocol.FIPA_PROPOSE};
  ...
  // #DOTNET_EXCLUDE_END
  {
    AclMessagePanel() {
      ...
      senderEnabledFlag = true; // The sender field is enabled by default, but can be disabled with
                                // the
  ...
  public ACLMessage getMsg() {
    ...
    /*
     * if ( ((param = sender.getText()).trim()).length() > 0 ) SenderAID.setName(param);
     */
    ...
    /*
     * ReceivedObject recObject = new ReceivedObject(); boolean filled = false; param =
     * by.getText().trim();
     * 
     * if(param.length()>0) { filled = true; recObject.setBy(param); } param =
     * fromRec.getText().trim(); if(param.length()>0) { filled = true; recObject.setFrom(param); }
     * 
     * if (dateRecDate != null) { filled = true; recObject.setDate(dateRecDate); }
     * 
     * param = id.getText().trim(); if(param.length()>0) { filled = true; recObject.setId(param); }
     * 
     * param = via.getText().trim(); if(param.length()>0) { filled = true; recObject.setVia(param);
     * }
     * 
   ...
   */
  /*
   * public void setBorder(Border b) { if (aclPanel != null) aclPanel.setBorder(b); }
   */
   ...
   * @param parent The parent window of the dialog window
   * @return The ACL message displayed in the dialog window or null depending on whether the user
   *         close the window by clicking the OK or Cancel button
   ...
   * 
   * ACLMessage msg = new ACLMessage(ACLMessage.INFORM); msg.setLanguage("language");
   * msg.setOntology("onto"); Envelope env = new Envelope(); env.setComments("Commento");
