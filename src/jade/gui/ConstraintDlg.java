...
public class ConstraintDlg extends JDialog {
  ...
  /*
   * #DOTNET_INCLUDE_BEGIN Component myComponent; private Component getOwner() {return myComponent;}
   * #DOTNET_INCLUDE_END
   */
   ...
   * 
   * @param parent The <code>Frame</code> that is the parent window of this dialog.
   */
   ...
   * Create a dialog with the given parent.
   * 
   * @param parent The <code>Dialog</code> that is the parent window of this dialog.
   */
  ...
  /**
   * This method display a gui to insert the search constraints. Return a
   * <code>SearchConstraints </code> if the OK button is pressed null otherwise.
   */
  public SearchConstraints setConstraint() {
    ...
    mainPanel.add(p);
    /*
     * The maximum nuber of result to return for the search. A negative value indicates that the
     * sender agent is willing to allow the search to propagate across all DFs. The default value is
     * 1
     */
    p = new JPanel();
          ...
          try {
            ...
            if (depth.length() != 0) {
              ...
              if (d.compareTo(new Long(0)) >= 0)
                ...
                JOptionPane.showMessageDialog(null, "The propagation depth must be positive !!!.",
            ...
            if (result.length() != 0) {
              ...
              if (r.compareTo(new Long(0)) >= 0)
                ...
                JOptionPane.showMessageDialog(null, "The max result must be positive !!!.",
                    "Error Message", JOptionPane.ERROR_MESSAGE);
                return;
              }
            } else {
              /*
               * the user didnt set any value so set the value of max-result to the default value
               */
            ...
            JOptionPane.showMessageDialog(null, "The inserted values must be numbers !!!.",
                "Error Message", JOptionPane.ERROR_MESSAGE);
            return;
          }
        }
  ...
  private void ShowCorrect() {
    ...
    try {
      ...
      // #DOTNET_EXCLUDE_END
      /*
       * #DOTNET_INCLUDE_BEGIN int x = getOwner().getX() + (getOwner().WIDTH - getWidth()) / 2; int
       * y = getOwner().getY() + (getOwner().HEIGHT - getHeight()) / 2; #DOTNET_INCLUDE_END
       */
