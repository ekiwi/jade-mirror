...
/**
 * The <code>ServiceManagerImpl</code> class is the actual implementation of JADE platform
 * <i>Service Manager</i> and <i>Service Finder</i> components. It holds a set of services and
 * manages them.
 * 
...
public class PlatformManagerImpl implements PlatformManager {
  ...
  // FIXME: The association between MainContainer and PlatformManagerImpl need be
  // clarified...
  ...
  // By convention, nodes hosting containers with a local copy of the Platform
  // Manager are called
  // Main-Container-<N>, hosting containers whereas nodes without their own
  // Platform Manager are
  // called Container-<M>.
  ...
  /**
   * Constructs a new Service Manager implementation complying with a given JADE profile. This
   * constructor is package-scoped, so that only the JADE kernel is allowed to create a new Service
   * Manager implementation.
   * 
   * @param p The platform profile describing how the JADE platform is to be configured.
   */
   ...
   * @param propagated Flag indicating whether the new-node event was a propagated event within the
  ...
  private String localAddNode(NodeDescriptor dsc, Vector nodeServices, boolean propagated)
      throws IMTPException, ServiceException, JADESecurityException {
    ...
    // Check if the newly starting node is reachable and, in case, issue a NEW_NODE
    // vertical command
    if (!propagated) {
      checkReachability(dsc);
      // In this case we issue the command before adding the node for authorization
      // purposes
    ...
    // Do not broadcast since this information is already conveied when broadcasting
    // the add-node event
  ...
  public void removeNode(NodeDescriptor dsc, boolean propagated)
  ...
  private void localRemoveNode(NodeDescriptor dsc, boolean propagated) throws ServiceException {
    ...
    if (parent != null) {
      // If the dead node had a parent, notify the failure-monitor monitoring the
      // parent
    ...
    if (!propagated) {
      ...
      if (result instanceof Throwable) {
        myLogger.log(Logger.WARNING,
            "Unexpected error processing DEAD_NODE command. Node is " + dsc.getName(),
            (Throwable) result);
      }
    }
  }
  ...
  // This may throw IMTPException since IMTPManager.createSliceProxy throws
  // IMTPException
  private void localAddSlice(ServiceDescriptor serviceDsc, NodeDescriptor dsc, boolean propagated)
      throws IMTPException, ServiceException {
    ...
    if (myLogger.isLoggable(Logger.CONFIG)) {
      myLogger.log(Logger.CONFIG,
          "Adding slice for service <" + serviceKey + "> on node <" + dsc.getName() + ">");
    }
  ...
  private void broadcastAddSlice(ServiceDescriptor service, NodeDescriptor dsc)
  ...
  private void localRemoveSlice(String serviceKey, String sliceKey, boolean propagated)
    ...
    if (e != null) {
      if (e.removeSlice(sliceKey) != null) {
        if (myLogger.isLoggable(Logger.CONFIG)) {
          myLogger.log(Logger.CONFIG,
              "Removing slice for service <" + serviceKey + "> on node <" + sliceKey + ">");
        }
      }
      // Clear the cache
      ...
      if (!propagated) {
        ...
        Object result = myCommandProcessor.processIncoming(gCmd);
        if (result instanceof Throwable) {
          myLogger.log(Logger.WARNING, "Unexpected error processing DEAD_SLICE command. Service is "
              + serviceKey + " node is " + sliceKey, (Throwable) result);
        }
      }
    }
  }
  ...
  public void addReplica(String newAddr, boolean propagated)
      throws IMTPException, ServiceException {
    ...
    replicas.put(TransportAddressWrapper.getWrapper(newReplica.getLocalAddress(), myIMTPManager),
        newReplica);
  }

  // This may throw IMTPException since the new replica must be informed about the
  // platform status
  private void localAddReplica(PlatformManager newReplica, boolean propagated)
      throws IMTPException, ServiceException {
    if (myLogger.isLoggable(Logger.INFO)) {
      myLogger.log(Logger.INFO,
          "Adding replica <" + newReplica.getLocalAddress() + "> to the platform");
    }
    ...
    if (result instanceof Throwable) {
      myLogger.log(Logger.WARNING,
          "Unexpected error processing NEW_REPLICA command. Replica address is "
              + newReplica.getLocalAddress(),
          (Throwable) result);
    }
  }
  ...
  public void removeReplica(String address, boolean propagated)
      throws IMTPException, ServiceException {
  ...
  private void localRemoveReplica(String address, boolean propagated) throws ServiceException {
    ...
    try {
      ...
      if (replicas.remove(key) == null) {
        // The replica to remove is not among the known replicas --> Print a warning
        // since this should never happen
        StringBuffer sb = new StringBuffer("Replica " + key.hashCode() + ", " + key.getAddress()
            + "not found! Known replicas are: {");
      ...
      // This should never happen as our IMTPManager must be able to deal with a
      // replica address
    ...
    if (result instanceof Throwable) {
      myLogger.log(Logger.WARNING,
          "Unexpected error processing DEAD_REPLICA command. Replica address is " + address,
          (Throwable) result);
    }
  }
  ...
  public void adopt(Node n, Node[] children) throws IMTPException {
    ...
    if (dsc != null) {
      ...
      if (result instanceof Throwable) {
        myLogger.log(Logger.WARNING,
            "Unexpected error processing ADOPTED_NODE command. Node is " + dsc.getName(),
            (Throwable) result);
      }
      myLogger.log(Logger.INFO, "Node <" + n.getName() + "> adopted");
    } else {
      myLogger.log(Logger.WARNING,
          "NO descriptor found for node <" + n.getName() + "> requesting adoption. Ignore...");
    }
  }
   ...
   */
  public void addMainContainerNode(NodeDescriptor dsc, Vector services)
  ...
  ////////////////////////////////
  public Service.Slice findSlice(String serviceKey, String sliceKey)
      throws IMTPException, ServiceException {
    ServiceEntry e = (ServiceEntry) services.get(serviceKey);
  ...
  private void issueNewNodeCommand(NodeDescriptor dsc) throws JADESecurityException {
    ...
    if (result instanceof JADESecurityException) {
      ...
      myLogger.log(Logger.WARNING,
          "Unexpected error processing NEW_NODE command. Node is " + dsc.getName(),
          (Throwable) result);
    }
  }
  ...
  private void issueNewSliceCommand(String serviceName, String sliceKey) {
    ...
    Object result = myCommandProcessor.processIncoming(gCmd);
    if (result instanceof Throwable) {
      myLogger.log(Logger.WARNING, "Unexpected error processing NEW_SLICE command. Service is "
          + serviceName + " node is " + sliceKey, (Throwable) result);
    }
  }
    ...
    if (dsc.getParentNode() == null) {
      ...
      if (!isLocalNode(node)) {
        try {
          ...
          myLogger.log(Logger.WARNING,
              "Node <" + dsc.getName()
                  + "> is trying to join the platform but is not reachable. Node details: " + node
                  + ". Exception message: " + imtpe.getMessage());
          // The following exception will be displayed in the joining node output
          throw new IMTPException("Main container cannot reach newly starting node " + node + ": "
              + imtpe.getMessage());
        }
      }
    }
  ...
  private void adjustContainerName(Node n, ContainerID cid) {
    ...
    if (cid.getName() == null || cid.getName().equals(NO_NAME)) {
      if (n.hasPlatformManager()) {
        ...
        do {
          name = AgentContainer.MAIN_CONTAINER_NAME
              + (mainContainerNo == 0 ? "" : "-" + mainContainerNo);
          mainContainerNo++;
          old = (NodeDescriptor) nodes.get(name);
  ...
  private void monitor(NodeDescriptor dsc) {
    ...
    Node parent = dsc.getParentNode();
    if (parent != null) {
      // This is a child node --> Do not monitor it directly if the parent is already
      // monitored or is the local node
      if (isLocalNode(parent)) {
        ...
        if (failureMonitor != null) {
          ...
          if (myLogger.isLoggable(Logger.CONFIG)) {
            myLogger.log(Logger.INFO,
                "Node <" + node.getName() + "> added as child of node " + parent.getName());
          }
          needMonitor = false;
        }
      }
  ...
  private NodeFailureMonitor monitor(Node target) {
    ...
    if (requiredService != null) {
      ...
      if (s == null) {
        ...
        myLogger.log(Logger.WARNING, "Service " + requiredService + " not found in node "
            + target.getName() + " --> Use default node monitoring");
  ...
  /**
   * Inner class NodeInfo. Embeds the node descriptor and the services currently installed on the
   * node
   */
