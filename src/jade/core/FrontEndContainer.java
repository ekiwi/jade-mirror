...
class FrontEndContainer implements FrontEnd, AgentToolkit, Runnable {
  ...
  void start(Properties p) {
    ...
    try {
      ...
      for (Enumeration en = specs.elements(); en.hasMoreElements();) {
        ...
        try {
          localAgents.put(s.getName(),
              initAgentInstance(s.getName(), s.getClassName(), s.getArgs()));
          successfulAgents.addElement(s);
        } catch (Throwable t) {
          logger.log(Logger.SEVERE, "Exception creating agent " + t);
        }
      }
      configProperties.setProperty(MicroRuntime.AGENTS_KEY,
          Specifier.encodeSpecifierList(successfulAgents));
    } catch (Exception e1) {
        ...
        if (a != null) {
          ...
          if ((args != null) && args.length > 0) {
            ...
            logger.log(Logger.SEVERE,
                "Error starting agent " + sp.getName() + ". " + sp.getClassName() + " " + args[0]);
          } else {
   ...
   */
  public final void messageIn(ACLMessage msg, String receiver)
      throws NotFoundException, IMTPException {
    waitUntilStarted();
    if (receiver != null) {
  ...
  /**
   * It may happen that a message for a bootstrap agent using wildcards in its name is received
   * before the actual agent name is assigned --> This method is called in messageIn() and other
   * agent-related methods to avoid that.
   */
  ...
  public void handleSave(AID agentID, String repository)
      throws ServiceException, NotFoundException, IMTPException {}

  public void handleReload(AID agentID, String repository)
      throws ServiceException, NotFoundException, IMTPException {}
  ...
  final void initInfo(Properties pp) {
    ...
    platformAddresses =
        Specifier.parseList(pp.getProperty(MicroRuntime.PLATFORM_ADDRESSES_KEY), ';');
  ...
  private final Agent initAgentInstance(String name, String className, Object[] args)
      throws Exception {
    Agent agent = null;
    // #J2ME_EXCLUDE_BEGIN
