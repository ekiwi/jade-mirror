...
/**
 * Processes JADE kernel-level commands, managing a filter/sink system to support dynamically
...
class CommandProcessor {
   ...
   * @param direction Whether to add this filter to the outgoing or incoming filter chain.
   */
   ...
   * 
   * @param snk A service-specific implementation of the <code>Sink</code> interface, managing a set
   *        of vertical commands.
   ...
   * @param commandNames An array containing all the names of the vertical commands the new sink
   *        wants to handle.
   * @throws ServiceException If some other sink is already registered for a member of the
   *         <code>commandNames</code> set.
   */
  public synchronized void registerSink(Sink snk, boolean side, String serviceName)
   ...
   * 
   * @param side One of the two constants <code>Sink.COMMAND_SOURCE</code> or
   *        <code>Sink.COMMAND_TARGET</code>, to state whether the sink to be removed is handling
   *        locally issued commands or commands incoming from remote nodes.
   * @param commandNames An array containing all the names of the vertical commands currently
   *        handled by the sink to be removed.
   * @throws ServiceException If a member of the <code>commandNames</code> set has no associated
   *         command sink.
   */
  public synchronized void deregisterSink(boolean side, String serviceName)
      throws ServiceException {
  ...
  /**
   * Process an outgoing command object, carrying out the action it represents. This method is not
   * synchronized and it must be reentrant and as fast as possible, because it is going to be a
  ...
  /**
   * Process an incoming command object, carrying out the action it represents. This method is not
   * synchronized and it must be reentrant and as fast as possible, because it is going to be a
   * bottleneck of the kernel call flow.
   * 
   * @param cmd The <code>VerticalCommand</code> object to process.
  ...
  /**
   * Inner class SinksFilter. This class makes the set of sinks in a given direction look like a
   * single filter that always stands at the end of the filter chain.
   */
