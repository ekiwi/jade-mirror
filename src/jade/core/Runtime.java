...
/**
 * The singleton instance (accessible through the <code>instance()</code> static method) of this
 * class allows controlling the JADE runtime system from an external application. Two different
 * modalities of controlling the JADE runtime system exist:
 * <ul>
 * <li>Multiple-container: Several containers (belonging to the same platform) can be executed in
 * the local JVM. This modality is activated by means of the <code>createAgentContainer()</code> and
 * <code>createMainContainer()</code> methods plus the classes included in the
 * <code>jade.wrapper</code> package.</li>
 * <li>Single-container: Only one container can be executed in the local JVM. This modality is
 * activated by means of the <code>startUp()</code> and <code>shutDown()</code> methods</li>
 * </ul>
 * Once a modality has been activated (by calling one of the above methods) calling one of the
 * methods for the other modality cause an <code>IllegalStateException</code> to be thrown.
 * <p>
 * It should be noted that the Single-container modality only provides a limited control of the JADE
 * runtime system (e.g. it does not allow creating and killing agents), but is the only one
 * supported both in J2SE, PJAVA/CDC and MIDP when using the LEAP add-on.
 * 
...
public class Runtime {
  ...
  /**
   * This method returns the singleton instance of this class that should be then used to create
   * agent containers.
   **/
  ...
  // #MIDP_EXCLUDE_BEGIN
  /**
   * Creates a new agent container in the current JVM, providing access through a proxy object. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   * @param p the profile containing boostrap and configuration data for this container
   * @return A proxy object, through which services can be requested from the real JADE container.
   * @exception IllegalStateException if the Single-container modality was previously activated by
   *            calling the <code>startUp()</code> method.
   */
  ...
  /**
   * Creates a new main container in the current JVM, providing access through a proxy object. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   * @param p the profile containing boostrap and configuration data for this container
   * @return A proxy object, through which services can be requested from the real JADE main
   *         container.
   * @exception IllegalStateException if the Single-container modality was previously activated by
   *            calling the <code>startUp()</code> method.
   */
  ...
  /**
   * Starts a JADE container in the Single-container modality. Successive calls to this method will
   * take no effect.
   * 
   * @param p the profile containing boostrap and configuration data for this container
   * @exception IllegalStateException if the Multiple-container modality was previously activated by
   *            calling the <code>createAgentContainer()</code> or
  ...
  /**
   * Allows setting a <code>Runnable</code> that is executed when the last container in this JVM
   * terminates.
   */
