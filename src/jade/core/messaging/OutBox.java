...
/**
 * Object to mantain message to send and to preserve the order for sending.
 * 
...
class OutBox {
  ...
  /**
   * Add a message to the tail of the Box of messages for the indicated receiver. If a Box for the
   * indicated receiver is not yet present, a new one is created. This method is executed by an
   * agent's thread requesting to deliver a new message.
   */
  void addLast(AID receiverID, GenericMessage msg, Channel ch) {
    ...
    if ((size + msg.length()) > maxSize) {
      ...
      synchronized (this) {
        if ((time - lastDiscardedLogTime) > 1000) {
          ...
          String servedWhileDiscardingStr =
              continuousDiscarding ? " (" + servedCnt + " messages served in the meanwhile)" : "";
          myLogger.log(Logger.SEVERE,
              String.valueOf(discardedSinceLastLogCnt + 1)
                  + " message(s) discarded by MessageManager! Current-queue-size = " + size
                  + ", max-size = " + maxSize + ", number of pending messages = " + pendingCnt
                  + ", size of last message = " + msg.length() + servedWhileDiscardingStr);
    ...
    // This must fall outside the synchronized block because the method may call
    // Thread.sleep
    ...
    synchronized (this) {
      ...
      if (logActivated) {
        String msgDebug = (b == null) ? "No box for receiver " + receiverID.getName()
            : "Box for receiver " + receiverID.getName() + " busy ?  " + b.isBusy();
  ...
  /**
   * Add a message to the head of the Box of messages for the indicated receiver. This method is
   * executed by the TimerDispatcher Thread when a retransmission timer expires. Therefore a Box of
   * messages for the indicated receiver must already exist. Moreover the busy flag of this Box must
   * be reset to allow deliverers to handle messages in it
   *
   * synchronized void addFirst(PendingMsg pm){ Box b = (Box)
   * messagesByReceiver.get(pm.getReceiver()); b.addFirst(pm); b.setBusy(false); // Wakes up all
   * deliverers notifyAll(); }
   */

  /**
   * Get the first message for the first idle (i.e. not busy) receiver. This is executed by a
   * Deliverer thread just before delivering a message.
   */
   ...
   * 
   * @return null if all receivers are currently busy This method does not need to be synchronized
  ...
  /**
   * Some messages for the receiver receiverID have been served If the Box of messages for that
   * receiver is now empty --> remove it. Otherwise just mark it as idle (not busy).
   */
  ...
  private void increaseSize(int k) {
    ...
    synchronized (this) {
      ...
      if (size > warningSize) {
        if (!overWarningSize) {
          myLogger.log(Logger.WARNING, "MessageManager queue size (" + size + ") > " + warningSize
              + ". Number of pending messages = " + pendingCnt + ", size of last message = " + k);
  ...
  /**
   * The method decreases the value of size and, eventually, set to false the value of overMaxSize.
   * <p>
   * This method should have been declared synchronized. For possible better peformance, it is not
   * declared synchronized because it is a private method and it is called only by the method get()
  ...
  /**
   * This class represents a Box of messages to be delivered to a single receiver
   */
  private class Box {
    ...
    public String toString() {
      return "(" + receiver.getName() + " :busy " + busy + (owner != null ? " :owner " + owner : "")
          + " :message-cnt " + messages.size() + ")";
    }
