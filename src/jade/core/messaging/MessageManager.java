...
/**
 * This class manages the delivery of ACLMessages to remote destinations in an asynchronous way. If
 * network problems prevent the delivery of a message, this class also embeds a mechanism to buffer
 * the message and periodically retry to deliver it.
 * 
...
class MessageManager {
  ...
  public interface Channel {
    void deliverNow(GenericMessage msg, AID receiverID)
  ...
  private static MessageManager theInstance; // FIXME: Maybe a table, indexed by a profile subset,
                                             // would be better?
  ...
  public void initialize(Profile p) {
    ...
    try {
      String tmp =
          p.getParameter("jade_core_messaging_MessageManager_deliverytimethreshold2", null);
    ...
    // MULTIPLE_DELIVERY
    boolean enableMultipleDelivery =
        p.getBooleanProperty("jade_core_messaging_MessageManager_enablemultipledelivery", true);
      ...
      for (int i = 0; i < poolSize; ++i) {
        ...
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE,
              "Starting deliverer " + name + ". Thread=" + delivererThreads[i]);
        }
  ...
  public void deliver(GenericMessage msg, AID receiverID, Channel ch) {
    if (active) {
      ...
      myLogger.log(Logger.WARNING,
          "MessageManager NOT active. Cannot deliver message " + stringify(msg));
    }
  }
        ...
        if (ch != null) {
          ...
          try {
            if (deliveryTimeThreshold > 0) {
              long threshold =
                  k == 1 ? deliveryTimeThreshold : Math.min(deliveryTimeThreshold * k, 30000);
              if (deliveryTime > threshold) {
                ...
                long threshold2 =
                    k == 1 ? deliveryTimeThreshold2 : Math.min(deliveryTimeThreshold2 * k, 150000);
  ...
  String[] getThreadPoolStatus() {
    ...
    for (int i = 0; i < delivererThreads.length; ++i) {
      status[i] = "(Deliverer-" + i + " :alive " + delivererThreads[i].isAlive()
          + " :Served-messages " + deliverers[i].getServedCnt() + ")";
    }
    return status;
  }
