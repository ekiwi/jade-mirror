...
public class OutgoingEncodingFilter extends Filter {
  ...
  /**
   * Process the SEND_MESSAGE VCommand encoding the ACLMessage with the proper representation and
   * adjusting Envelope fields: 1) If the receiver lives in the local container --> Do not encode
   * (to speed up performances) --> Don't touch the envelope 2) If the receiver lives in a remote
   * container --> Encode using the specified representation or "LEAP" if no representation is
   * specified --> If an envelope is present adjust its fields 3) If the receiver lives in a remote
   * platform --> Encode using the specified representation or "String" if no representation is
   * specified --> Create a default envelope if not present and adjust its fields
   */
  ...
  /**
   * Encodes an ACL message according to the acl-representation described in the envelope. If there
   * is no explicit acl-representation in the envelope, uses the String representation
   * 
  ...
  public byte[] encodeMessage(ACLMessage msg) throws MessagingService.UnknownACLEncodingException {
    ...
    if (enc != null) { // A Codec was selected
      ...
      if (codec != null) {
        ...
        // Codec)
        throw new MessagingService.UnknownACLEncodingException(
            "Unknown ACL encoding: " + enc + ".");
      }
    } else {
      // no codec indicated.
