...
class FileMessageStorage implements MessageStorage {
  ...
  public synchronized void delete(String storeName, AID receiver) throws IOException {
    ...
    // Decrement the counter (if 0, it deletes the file). If the subdirectory is
    ...
    for (int i = 0; i < subdirs.length; i++) {
      ...
      for (int j = 0; j < files.length; j++) {
        ...
        try {
          ...
          // String.getBytes is, in general, an irreversible operation. However, in this
          // case, because
          // the content was previously encoded Base64, we can expect that we will have
          // only valid Base64 chars.
          ByteArrayInputStream istream =
              new ByteArrayInputStream(Base64.decodeBase64(encodedMsg.getBytes("US-ASCII")));
          ...
          System.err
              .println("Error reading file " + toRead.getName() + " [" + ce.getMessage() + "]");
        } catch (ClassNotFoundException cnfe) {
          System.err
              .println("Error reading file " + toRead.getName() + " [" + cnfe.getMessage() + "]");
        } finally {
          in.close();
        }
      }
  ...
  private void createMessageFile(File toStore, GenericMessage msg, AID receiver)
      throws IOException {
