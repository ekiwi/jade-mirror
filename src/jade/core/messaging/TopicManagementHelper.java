...
/**
 * The TopicManagementHelper provides methods that allows creating topic objects and
 * registering/deregistering to topics. Topics are represented by means of <code>AID</code> objects
 * so that they can be used as receivers of ACLMessages. In this way sending a message to an agent
 * or sending a message about a topic is completely uniform.
 * 
...
public interface TopicManagementHelper extends ServiceHelper {
   ...
   * <code>getHelper()</code> method of the <code>Agent</code> class to retrieve the helper of the
   * local TopicManagementService.
   */
   ...
   * @return <code>true</code> if the given <code>AID</code> represents a topic. <code>false</code>
   *         otherwise
   */
  ...
  /**
   * Register a given AID to a given topic. Registering a specific AID instead of the agent AID,
   * allows registering an Alias or a Virtual agent AID
   * 
