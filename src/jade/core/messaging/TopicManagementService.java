...
public class TopicManagementService extends BaseService {
    ...
    try {
      if (myContainer.getPlatformID().equals(TopicManagementHelper.TOPIC_SUFFIX)) {
        throw new ServiceException(
            "The TopicManagementService cannot be used within a platform called with the reserved name "
                + TopicManagementHelper.TOPIC_SUFFIX);
      }
      theMessagingService =
  ...
  /**
   * Inner class CommandOutgoingFilter. Intercepts the SEND_MESSAGE VCommand and broadcast messages
   * directed to a topic to all agents interested in that topic
   */
      ...
      if (name.equals(MessagingSlice.SEND_MESSAGE)) {
        ...
        if (TopicUtility.isTopic(receiver)) {
          ...
          if (interestedAgents.size() > 0) {
            ...
            // Note that if no agents are currently listening to this topic, the message is
            // simply swallowed
        ...
        myLogger.log(Logger.INFO,
            "TopicManagentService: platform shutdown process initiation detected");
    ...
    public boolean accept(VerticalCommand cmd) {
      ...
      if (myMain != null) {
        ...
        if (name.equals(Service.NEW_SLICE)) {
          // If the new slice is a TopicManagementSlice, notify it about the currently
          // registered agents
          handleNewSlice(cmd);
        }
      } else {
  ...
  //////////////////////////////////////////////////
  /**
   * If the dead agent was interested in some topic, notify all slices that its interest is no
   * longer valid
   */
      ...
      if (topics.size() > 0) {
        try {
          ...
          myLogger.log(Logger.WARNING,
              "Error retrieving topic-management-slices when trying to broadcast topic de-registration due to agent death. ",
              t);
        }
      }
    }
  ...
  private void handleNewSlice(VerticalCommand cmd) {
    if (cmd.getService().equals(NAME)) {
      ...
      try {
        ...
        myLogger.log(Logger.WARNING,
            "Error notifying new slice " + newSliceName + " about current topic registrations", t);
      }
    }
  }

  /**
   * The Main lost all information about the local node --> Notify the Main slice about all local
   * registrations
   */
      ...
      while (it.hasNext()) {
        ...
        if (myContainer.acquireLocalAgent(aid) != null) {
          try {
            ...
            myLogger.log(Logger.WARNING,
                "Error notifying main slice about current local topic registrations", e);
          }
    ...
    public VerticalCommand serve(HorizontalCommand cmd) {
      try {
        ...
        if (cmdName.equals(TopicManagementSlice.H_REGISTER)) {
          ...
          // System.out.println("Received registration of agent "+aid.getName()+" to topic
          // "+topic.getLocalName());
          if (myLogger.isLoggable(Logger.FINE)) {
            myLogger.log(Logger.FINE, "Received registration of agent " + aid.getName()
                + " to topic " + topic.getLocalName());
          }
          register(aid, topic);
        } else if (cmdName.equals(TopicManagementSlice.H_DEREGISTER)) {
          AID aid = (AID) params[0];
          AID topic = (AID) params[1];
          // System.out.println("Received deregistration of agent "+aid.getName()+" from
          // topic "+topic.getLocalName());
          if (myLogger.isLoggable(Logger.FINE)) {
            myLogger.log(Logger.FINE, "Received deregistration of agent " + aid.getName()
                + " from topic " + topic.getLocalName());
          }
  ...
  // Utility methods
  ///////////////////////////////////////////////////
  private void broadcastRegistration(AID aid, AID topic, Service.Slice[] slices)
      throws ServiceException {
    if (myLogger.isLoggable(Logger.CONFIG)) {
      myLogger.log(Logger.CONFIG,
          "Registering agent " + aid.getName() + " to topic " + topic.getLocalName());
    }
      ...
      try {
        ...
        if (myLogger.isLoggable(Logger.FINER)) {
          myLogger.log(Logger.FINER,
              "Propagating registration of agent " + aid.getName() + " to slice " + sliceName);
        }
        slice.register(aid, topic);
      } catch (Throwable t) {
        // NOTE that slices are always retrieved from the main and not from the cache
        // --> No need to retry in case of failure
        myLogger.log(Logger.WARNING, "Error propagating topic registration to slice  " + sliceName,
            t);
      }
    }
  }

  private void broadcastDeregistration(AID aid, AID topic, Service.Slice[] slices)
      throws ServiceException {
    if (myLogger.isLoggable(Logger.CONFIG)) {
      myLogger.log(Logger.CONFIG,
          "Deregistering agent " + aid.getName() + " from topic " + topic.getLocalName());
    }
    for (int i = 0; i < slices.length; i++) {
      String sliceName = null;
      try {
        ...
        if (myLogger.isLoggable(Logger.FINER)) {
          myLogger.log(Logger.FINER,
              "Propagating deregistration of agent " + aid.getName() + " to slice " + sliceName);
        }
        ...
        // --> No need to retry in case of failure
        myLogger.log(Logger.WARNING,
            "Error propagating topic de-registration to slice  " + sliceName, t);
      }
    }
  }
}
