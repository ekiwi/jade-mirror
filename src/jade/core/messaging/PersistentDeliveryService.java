...
/**
 * 
 * The JADE service to manage the persistent storage of undelivered ACL messages installed on the
 * platform.
 * 
...
public class PersistentDeliveryService extends BaseService {
  /**
   * This constant is the name of the property whose value contains the name of the
   * application-specific class that will be used by the PersistentDeliveryService on the local
   * container as a filter for undelivered ACL messages
   */
  public static final String PERSISTENT_DELIVERY_FILTER = "persistent-delivery-filter";

  /**
   * This constant is the name of the property whose value contains the semicolon separated names of
   * the nodes where messages can be stored. By default all nodes are considered. This implies that
   * whenever the delivery of a message fails all slices are asked to see if they have a filer that
   * must store the message. Similarly whenever a new agent is created all slices are asked to see
   * if they have messages to flush. When the platform is composed by 100 or more containers (e.g.
   * when there are mobile terminals running split containers) this may be very un-efficient -->
   * using this option to limit the number of slices to be asked is highly recommended
   */
  public static final String PERSISTENT_DELIVERY_STORAGENODES = "persistent-delivery-storagenodes";

  /**
   * This constant is the name of the property whose value contains an integer representing how
   * often (in milliseconds) the PersistentDeliveryService will try to send again previously
   * undelivered ACL messages which have been buffered.
   */
  public static final String PERSISTENT_DELIVERY_SENDFAILUREPERIOD =
      "persistent-delivery-sendfailureperiod";

  /**
   * This constant is the name of the property whose value contains the storage method used to
   * persist undelivered ACL messages by the PersistentDeliveryService on the local container. The
   * supported values for this parameter are:
   * <ul>
   * <li><b>file</b> - A directory tree on the local filesystem is used.</li>
   * </ul>
   * If this property is not specified undelivered ACL messages are kept in memory and not persisted
   * at all.
   */
  public static final String PERSISTENT_DELIVERY_STORAGEMETHOD =
      "persistent-delivery-storagemethod";

  /**
   * This constant is the name of the property whose value contains the root of the directory tree
   * that is used to persist undelivered ACL messages when the <i>file</i> storage method is
   * selected.
   */
  ...
  private class CommandOutgoingFilter extends Filter {
    ...
    private boolean handleNotifyFailure(VerticalCommand cmd)
      ...
      if (myLogger.isLoggable(Logger.FINE))
        myLogger.log(Logger.FINE, "Persistent-Delivery - Processing failed message "
      ...
      for (int i = 0; i < slices.length; i++) {
        ...
        try {
          ...
          if (accepted) {
            myLogger.log((firstTime ? Logger.INFO : Logger.FINE),
                "Persistent-Delivery - Message " + MessageManager.stringify(msg) + " for agent "
                    + receiver.getName() + " stored on node " + sliceName);
            // The message was stored --> Veto the NOTIFY_FAILURE command
            return false;
          }
        } catch (Exception e) {
          myLogger.log(Logger.WARNING,
              "Persistent-Delivery - Error trying to store message " + MessageManager.stringify(msg)
                  + " for agent " + receiver.getName() + " on node " + sliceName);
  ...
  /**
   * Incoming command FILTER. Processes the INFORM_CREATED command. Note that we do this in the
   * postProcess() method so that we are sure the newly born agent is already in the GADT.
   */
  private class CommandIncomingFilter extends Filter {
    ...
    private void handleInformCreated(VerticalCommand cmd) throws IMTPException, ServiceException {
      ...
      myLogger.log(Logger.FINE,
          "Persistent-Delivery - Flushing persisted messages (if any) for newly started agent "
  ...
  private class ServiceComponent implements Service.Slice {
    ...
    /**
     * This is called following a message delivery failure to check whether or not the message must
     * be stored.
     */
    private boolean storeMessage(String storeName, GenericMessage msg, AID receiver)
        throws IMTPException, ServiceException {
      ...
      if (messageFilter != null) {
        ...
        try {
          ...
          if (delay != PersistentDeliveryFilter.NOW) {
            ...
            msg.getACLMessage().addUserDefinedParameter(ACL_USERDEF_DUE_DATE,
            ...
            if (firstTime) {
              if (myLogger.isLoggable(Logger.INFO))
                myLogger.log(Logger.INFO,
                    "Persistent-Delivery - Storing message\n" + MessageManager.stringify(msg)
                        + " for agent " + receiver.getName() + "\nDue date is " + dueDate);
            } else {
              if (myLogger.isLoggable(Logger.FINE))
                myLogger.log(Logger.FINE,
                    "Persistent-Delivery - Re-storing message\n" + MessageManager.stringify(msg)
                        + " for agent " + receiver.getName() + "\nDue date is " + dueDate);
            }
    ...
    /**
     * This is called when a new agent is born to send him the stored messages (if any)
     */
    private void flushMessages(AID receiver) {
      myLogger.log(Logger.FINE,
          "Persistent-Delivery - flushing messages for agent " + receiver.getLocalName());
      int cnt = myManager.flushMessages(receiver);
      if (cnt > 0) {
        myLogger.log(Logger.INFO,
            "Persistent-Delivery - " + cnt + " messages delivered to agent " + receiver);
      }
    }
  ...
  /**
   * Activate the PersistentDeliveryManager and instantiate the PersistentDeliveryFilter. Note that
   * getting the MessagingService (required to instantiate the PersistentDeliveryManager) cannot be
   * done in the init() method since at that time the MessagingService may not be installed yet.
   */
  public void boot(Profile myProfile) throws ServiceException {
    ...
    try {
      ...
      MessageManager.Channel ch =
          (MessageManager.Channel) myServiceFinder.findService(MessagingSlice.NAME);
      if (ch == null)
    ...
    try {
      ...
      if (className != null) {
        ...
        myLogger.log(Logger.INFO, "Persistent-Delivery - Using message filter of type "
            + messageFilter.getClass().getName());
      }
    } catch (Exception e) {
  ...
  /**
   * Requests all slices to flush the stored messages for a newly born target agent. Do it in a
   * separated thread since this may take time. This happens on the main container only.
   */
        ...
        try {
          ...
          for (int i = 0; i < slices.length; i++) {
            ...
            try {
              ...
              myLogger.log(Logger.WARNING,
                  "Persistent-Delivery - Error trying to flush messages for agent "
                      + target.getName() + " on node " + sliceName);
              // Ignore it and try other slices...
            }
          }
        } catch (ServiceException se) {
          myLogger.log(Logger.WARNING,
              "Persistent-Delivery - Error retrieving storage-enabled slices to flush persisted messages for agent "
                  + target.getName());
        }
      }
    };
