...
/**
 * This class supports the ACL persistent delivery service, managing actual ACL messages storage,
 * scheduled message delivery and other utility tasks related to the service.
 *
...
class PersistentDeliveryManager {
  ...
  public static synchronized PersistentDeliveryManager instance(Profile p,
  ...
  private static final String DEFAULT_STORAGE =
      "jade.core.messaging.PersistentDeliveryManager$DummyStorage";
  ...
  public void initialize(Profile p, MessageManager.Channel ch) {
    ...
    try {
      ...
      String storageClass = p.getParameter(
          PersistentDeliveryService.PERSISTENT_DELIVERY_STORAGEMETHOD, DEFAULT_STORAGE);
    ...
    String s =
        p.getParameter(PersistentDeliveryService.PERSISTENT_DELIVERY_SENDFAILUREPERIOD, null);
    if (s != null) {
      try {
  ...
  private static PersistentDeliveryManager theInstance; // FIXME: Maybe a table, indexed by a
                                                        // profile subset, would be
                                                        // better?
