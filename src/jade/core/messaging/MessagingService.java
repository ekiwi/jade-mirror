...
public class MessagingService extends BaseService implements MessageManager.Channel {
  ...
  public static final String ATTACH_PLATFORM_INFO =
      "jade_core_messaging_MessagingService_attachplatforminfo";
  ...
  public static final String DELIVERY_TIME_MEASUREMENT_RATE =
      "jade_core_messaging_MessagingService_deliverytimemeasurementrate";
  public static final int DELIVERY_TIME_MEASUREMENT_RATE_DEFAULT = -1; // Delivery time measurement
                                                                       // disabled by
                                                                       // default. Set it to N to
                                                                       // measure delivery
                                                                       // time 1 out of N delivered
                                                                       // messages
  public static final String ENABLE_POSTED_MESSAGE_COUNT =
      "jade_core_messaging_MessagingService_enablepostedmessagecount";
  public static final String ENABLE_MESSAGE_MANAGER_METRICS =
      "jade_core_messaging_MessagingService_enablemessagemanagermetrics";
  ...
  /**
   * Performs the active initialization step of a kernel-level service: Activates the ACL codecs and
   * MTPs as specified in the given <code>Profile</code> instance.
   *
   * @param myProfile The <code>Profile</code> instance containing the list of ACL codecs and MTPs
   *        to activate on this node.
    ...
    try {
      ...
      while (codecs.hasNext()) {
        ...
        try {
          ...
          throw new jade.lang.acl.ACLCodec.CodecException(
              "The class " + className + " raised InstantiationException (see NestedException)",
              ie);
        } catch (IllegalAccessException iae) {
          throw new jade.lang.acl.ACLCodec.CodecException(
              "The class " + className + " raised IllegalAccessException (see nested exception)",
              iae);
        }
      }
  ...
  private void initializeSAM() {
    ...
    try {
      ...
      if (sam != null) {
        ...
        try {
          deliveryTimeMeasurementRate =
              Integer.parseInt(myProfile.getParameter(DELIVERY_TIME_MEASUREMENT_RATE, null));
        } catch (Exception e) {
          // Keep default
        }
        ...
        boolean enablePostedMessageCount =
            "true".equalsIgnoreCase(myProfile.getParameter(ENABLE_POSTED_MESSAGE_COUNT, "false"));
        ...
        if (enableMessageManagerMetrics) {
          ...
          samHelper.addCounterValueProvider(
              "Message-Manager-served-count#" + myContainer.getID().getName(),
              new AbsoluteCounterValueProvider() {
                @Override
                public long getValue() {
  ...
  public void newAlias(AID alias, AID agent) {
    myLogger.log(Logger.INFO,
        "Adding global alias entry: " + alias.getLocalName() + "-->" + agent.getLocalName());
  ...
  void transferLocalAliases(AID agent, ContainerID dest) {
    myLogger.log(Logger.FINE, "Transferring all local alias entries for agent "
        + agent.getLocalName() + " to " + dest.getName());
    ...
    if (aliases.size() > 0) {
      try {
        ...
        myLogger.log(Logger.SEVERE, "Error transferring local aliases for migrated agent "
            + agent.getName() + " to new location " + dest.getName());
      }
    }
  }
  ...
  public void removeGlobalAliases(AID agent) {
    myLogger.log(Logger.FINE,
        "Removing all global alias entries for agent " + agent.getLocalName());
   ...
   * installed within the local command processing engine.
   * 
    ...
    while (it.hasNext()) {
      ...
      try {
        ...
        myLogger.log(Logger.WARNING,
            "Error notifying local MTP " + mtp.getName() + " to Main Container.", e);
      }
    }
  }
  ...
  /**
   * Inner class CommandSourceSink This inner class handles the messaging commands on the command
   * issuer side, turning them into horizontal commands and forwarding them to remote slices when
   * necessary.
   */
    ...
    private void handleNotifyFailure(VerticalCommand cmd) {
      ...
      try {
        GenericCommand command =
            new GenericCommand(MessagingSlice.SEND_MESSAGE, MessagingSlice.NAME, null);
  ...
  private class CommandTargetSink implements Sink {
    ...
    private void handleSendMessage(VerticalCommand cmd) throws NotFoundException {
      ...
      if (gMsg.getTraceID() != null) {
        myLogger.log(Logger.INFO,
            gMsg.getTraceID() + " - MessagingService target sink posting message to receiver "
                + receiverID.getLocalName());
      }
    ...
    private void manageMessage(AID senderID, GenericMessage msg, AID receiverID)
      ...
      if (msg instanceof MultipleGenericMessage) {
        System.out.println(
            "Multiple delivery: posting " + ((MultipleGenericMessage) msg).getMessages().size()
                + " messages to receiver " + receiverID.getLocalName());
      ...
      if (msg.getTraceID() != null) {
        myLogger.log(Logger.INFO,
            msg.getTraceID() + " - MessagingService target sink posting message to receiver "
                + receiverID.getLocalName());
      }
    ...
    private void postMessage(AID senderID, ACLMessage msg, AID receiverID)
        throws NotFoundException {
      if (!msg.getSender().equals(senderID)) {
        myLogger.log(Logger.FINE,
            "Attaching real-sender user defined parameter: " + senderID.getName());
        // Sender indicated in the message different than the real sender --> store the
        // latter in the REAL_SENDER user defined param
    ...
    private MTPDescriptor installMTP(String address, String className)
      ...
      /*
       * #DOTNET_INCLUDE_BEGIN catch(System.TypeLoadException tle) { ClassNotFoundException cnfe =
       * new ClassNotFoundException(tle.get_Message()); throw new MTPException("The class " +
       * className + " raised IllegalAccessException (see nested exception)", cnfe); }
       * catch(System.TypeInitializationException tie) { InstantiationException ie = new
       * InstantiationException(tie.get_Message()); throw new MTPException("The class " + className
       * + " raised InstantiationException (see nested exception)", ie); } #DOTNET_INCLUDE_END
       */
      catch (ClassNotFoundException cnfe) {
        throw new MTPException(
            "ERROR: The class " + className + " for the " + address + " MTP was not found");
      } catch (InstantiationException ie) {
        throw new MTPException(
    ...
    private void uninstallMTP(String address)
      ...
      if (info != null) {
        ...
        GenericCommand gCmd =
            new GenericCommand(MessagingSlice.DEAD_MTP, MessagingSlice.NAME, null);
        gCmd.addParam(desc);
        gCmd.addParam(myContainer.getID());
        submit(gCmd);
      } else {
    ...
    private void deadMTP(MTPDescriptor mtp, ContainerID cid)
  ...
  /**
   * Inner class for this service: this class receives commands from service <code>Sink</code> and
   * serves them, coordinating with remote parts of this service through the <code>Slice</code>
   * interface (that extends the <code>Service.Slice</code> interface).
   */
  private class ServiceComponent implements Service.Slice {
    ...
    public VerticalCommand serve(HorizontalCommand cmd) {
      ...
      try {
        ...
        if (cmdName.equals(MessagingSlice.H_DISPATCHLOCALLY)) {
          ...
          GenericCommand gCmd =
              new GenericCommand(MessagingSlice.INSTALL_MTP, MessagingSlice.NAME, null);
          String address = (String) params[0];
          String className = (String) params[1];
          ...
          GenericCommand gCmd =
              new GenericCommand(MessagingSlice.UNINSTALL_MTP, MessagingSlice.NAME, null);
          String address = (String) params[0];
          gCmd.addParam(address);
          ...
          GenericCommand gCmd =
              new GenericCommand(MessagingSlice.NEW_MTP, MessagingSlice.NAME, null);
          gCmd.addParam(mtp);
          gCmd.addParam(cid);
    ...
    private void routeOut(Envelope env, byte[] payload, AID receiverID, String address)
      ...
      if (myLogger.isLoggable(Logger.FINE))
        myLogger.log(Logger.FINE,
            "Routing message to " + receiverID.getName() + " towards port " + out);
    ...
    private void addRoute(MTPDescriptor mtp, String sliceName)
        throws IMTPException, ServiceException {
      ...
      if (routes.addRemoteMTP(mtp, sliceName, slice)) {
        ...
        for (int i = 0; i < pp.length; ++i) {
          if (myLogger.isLoggable(Logger.CONFIG))
            myLogger.log(Logger.CONFIG,
                "Added Route-Via-Slice(" + sliceName + ") for protocol " + pp[i]);
        }
    ...
    private void removeRoute(MTPDescriptor mtp, String sliceName)
        throws IMTPException, ServiceException {
      ...
      for (int i = 0; i < pp.length; ++i) {
        if (myLogger.isLoggable(Logger.CONFIG))
          myLogger.log(Logger.CONFIG,
              "Removed Route-Via-Slice(" + sliceName + ") for protocol " + pp[i]);
  ...
  private boolean isPersistentDeliveryRetry(GenericMessage msg) {
    ...
    if (acl != null) {
      ret = acl.getAllUserDefinedParameters()
          .containsKey(PersistentDeliveryService.ACL_USERDEF_DUE_DATE);
    }
    // #J2ME_EXCLUDE_END
    return ret;
  }

  void deliverInLocalPlatfrom(GenericMessage msg, AID receiverID)
  ...
  private MessagingSlice oneShotDeliver(ContainerID cid, GenericMessage msg, AID receiverID)
    ...
    try {
      try {
        if (msg.getTraceID() != null) {
          myLogger.log(Logger.INFO, msg.getTraceID() + " - Delivering message to slice "
              + targetSlice.getNode().getName());
        }
        targetSlice.dispatchLocally(msg.getSender(), msg, receiverID);
      } catch (IMTPException imtpe) {
        // Try to get a newer slice and repeat...
        if (msg.getTraceID() != null) {
          myLogger.log(Logger.FINER, msg.getTraceID() + " - Messaging slice on container "
              + cid.getName() + " unreachable. Try to get a fresh one.");
        }
      ...
      if (msg.getTraceID() != null) {
        myLogger.log(Logger.FINER,
            msg.getTraceID() + " - Receiver " + receiverID.getLocalName()
                + " not found on container " + cid.getName()
                + ". Possibly he moved elsewhere --> Retry");
      }
  ...
  private void forwardMessage(GenericMessage msg, AID receiver, String address)
    ...
    try {
      if (msg.getTraceID() != null) {
        myLogger.log(Logger.INFO,
            msg.getTraceID() + " - Routing message out to address " + address);
  ...
  /**
   * This method is used internally by the platform in order to notify the sender of a message that
   * a failure was reported by the Message Transport Service.
   */
  public void notifyFailureToSender(GenericMessage msg, AID receiver, InternalError ie) {
    ...
    if (acl == null) {
      ...
      try {
        acl = ((IncomingEncodingFilter) encInFilter).decodeMessage(msg.getEnvelope(),
            msg.getPayload());
    ...
    if (acl == null) {
      myLogger.log(Logger.WARNING,
          "Cannot notify failure to sender: GenericMessage contains no ACLMessage");
      return;
    }
    ...
    GenericCommand cmd =
        new GenericCommand(MessagingSlice.NOTIFY_FAILURE, MessagingSlice.NAME, null);
    cmd.addParam(msg);
  ...
  /*
   * This method is called before preparing the Envelope of an outgoing message. It checks for all
   * the AIDs present in the message and adds the addresses, if not present
   **/
  ...
  private void checkTracing(GenericMessage msg) {
    ...
    if (acl != null) {
      if (myLogger.isLoggable(Logger.FINE)
          || "true".equals(acl.getUserDefinedParameter(ACLMessage.TRACE))) {
        msg.setTraceID(ACLMessage.getPerformative(acl.getPerformative()) + "-"
            + msg.getSender().getLocalName() + "-" + traceCnt);
        traceCnt++;
      }
    }
  }
