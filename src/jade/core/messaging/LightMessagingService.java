...
/**
 * A minimal version of the JADE service to manage the message passing subsystem installed on the
 * platform. This class just supports direct ACL message delivery, and relies on another one for any
...
public class LightMessagingService extends BaseService implements MessageManager.Channel {
    ...
    try {
      if (livesHere(receiverID)) {
        ...
        notifyFailureToSender(msg, receiverID,
            new InternalError("No valid address contained within the AID " + receiverID.getName()));
      }
   ...
   * a failure was reported by the Message Transport Service. Package scoped as it can be called by
   * the MessageManager
   */
  ...
  private void handleNotifyFailure(VerticalCommand cmd) throws JADESecurityException {
    ...
    if ((aclmsg.getSender() == null) || ((aclmsg.getSender().equals(myContainer.getAMS()))
        && (aclmsg.getPerformative() == ACLMessage.FAILURE))) { // sanity check to avoid infinite
                                                                // loops
    ...
    content =
        content + " (ACLMessage) ) (MTS-error " + receiver + " \"" + ie.getMessage() + "\") )";
  ...
  private class ServiceComponent extends Filter implements Service.Slice {
    ...
    /**
     * Builds a new messaging service lightweight component, relying on a remote slice for most
     * operations.
     **/
    ...
    public void dispatchLocally(AID senderAID, GenericMessage msg, AID receiverID)
    ...
    public void routeOut(Envelope env, byte[] payload, AID receiverID, String address)
  ...
  /*
   * Activates the ACL codecs and MTPs as specified in the given <code>Profile</code> instance.
   * 
