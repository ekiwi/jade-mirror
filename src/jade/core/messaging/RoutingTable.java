...
class RoutingTable {
  ...
  private static class OutViaSlice implements OutPort {
    ...
    public void route(Envelope env, byte[] payload, AID receiver, String address)
        throws MTPException {
      try {
  ...
  private static class OutViaMTP implements OutPort {
    ...
    public void route(Envelope env, byte[] payload, AID receiver, String address)
      ...
      if (platformInfo != null) {
        ...
        env.addProperties(
            new Property(MessagingService.MTP_IDENTIFIER, myChannel.getClass().getName()));
      }
  ...
  /**
   * Inner class MTPInfo This class just provides the association between a local MTP and its
   * descriptor
   */
  class MTPInfo {
    private MTP mtp;
  ...
  public RoutingTable(boolean attachPlatformInfo) {
    if (attachPlatformInfo) {
      platformInfo = Runtime.instance().getVersionInfo() + " (" + System.getProperty("java.version")
  ...
  public synchronized MTPInfo removeLocalMTP(String url) {
    ...
    /*
     * java.util.Iterator it = outPorts.keySet().iterator(); while(it.hasNext()) { String name =
     * (String)it.next(); OutPortList l = (OutPortList)outPorts.get(name); System.out.println("<" +
     * name + "> ==> " + l.size()); }
     */
  ...
  public synchronized boolean addRemoteMTP(MTPDescriptor mtp, String sliceName,
      MessagingSlice where) {
   ...
   */
  public synchronized void removeRemoteMTP(MTPDescriptor mtp, String sliceName,
      MessagingSlice where) {
  ...
  /**
   * Retrieves an outgoing channel object suitable for reaching the address <code>url</code>.
   */
