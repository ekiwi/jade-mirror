...
/*
 * #MIDP_INCLUDE_BEGIN import jade.content.lang.sl.SimpleSLTokenizer; #MIDP_INCLUDE_END
 */
 ...
 * record agent names and addresses.
 */
public class AID implements Comparable, Serializable {
  ...
  // #MIDP_EXCLUDE_END
  /*
   * #MIDP_INCLUDE_BEGIN private Vector addresses = new Vector(EXPECTED_ADDRESSES_SIZE,1); private
   * Vector resolvers = new Vector(EXPECTED_RESOLVERS_SIZE,1); #MIDP_INCLUDE_END
   */
  ...
  /**
   * Constructor for an Agent-identifier This constructor (which is deprecated), examines the name
   * to see if the "@" chararcter is present. If so, it calls <code> this(name, ISGUID)<code> 
   * otherwise it calls <code>this(name, ISLOCALNAME)</code> This ensures better compatibility with
   * JADE2.2 code.
   * 
   * @param guid is the Globally Unique identifer for the agent. The slot name assumes that value in
   *        the constructed object.
   * @deprecated This constructor might generate a wrong AID, if the passed parameter is not a guid
   *             (globally unique identifier), but the local name of an agent (e.g. "da0").
   ...
   * @param isGUID indicates if the passed <code>name</code> is already a globally unique identifier
   *        or not. Two constants <code>ISGUID</code>, <code>ISLOCALNAME</code> have also been
   *        defined for setting a value for this parameter. If the name is a local name, then the
   *        HAP (Home Agent Platform) is concatenated to the name, separated by "@".
   **/
  ...
  /**
   * Key to retrieve the agent class name as a user defined slot of the AID included in the
   * AMSAgentDescription registered with the AMS.
   */
  public static final String AGENT_CLASSNAME = "JADE-agent-classname";

  /**
   * This method permits to set the symbolic name of an agent. The passed parameter must be a GUID
  ...
  /**
   * This method permits to set the symbolic name of an agent. The passed parameter must be a local
   * name.
   */
  ...
  /**
   * This method permits to add a transport address where the agent can be contacted. The address is
   * added only if not yet present
   */
  public void addAddresses(String url) {
    if (!addresses.contains(url)) {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN addresses.addElement(url); #MIDP_INCLUDE_END
       */
    }
  }
  ...
  public boolean removeAddresses(String url) {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN return addresses.removeElement(url); #MIDP_INCLUDE_END
     */
  }
  ...
  public void clearAllAddresses() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN addresses.removeAllElements(); #MIDP_INCLUDE_END
     */
  }
  ...
  public Iterator getAllAddresses() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN return new EnumIterator(addresses.elements()); #MIDP_INCLUDE_END
     */
  }

  /**
   * This method permits to add the AID of a resolver (an agent where name resolution services for
  ...
  public void addResolvers(AID aid) {
    if (!resolvers.contains(aid)) {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN resolvers.addElement(aid); #MIDP_INCLUDE_END
       */
    }
  }
  ...
  public boolean removeResolvers(AID aid) {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN return resolvers.removeElement(aid); #MIDP_INCLUDE_END
     */
  }
  ...
  public void clearAllResolvers() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN resolvers.removeAllElements(); #MIDP_INCLUDE_END
     */
  }
  ...
  public Iterator getAllResolvers() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN return new EnumIterator(resolvers.elements()); #MIDP_INCLUDE_END
     */
  }
  ...
  public String[] getAddressesArray() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN String[] result = new String[addresses.size()];
     * addresses.copyInto(result); #MIDP_INCLUDE_END
     */
    return result;
  }
  ...
  public AID[] getResolversArray() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN AID[] result = new AID[resolvers.size()]; resolvers.copyInto(result);
     * #MIDP_INCLUDE_END
     */
    return result;
  }
  ...
  public String toString() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN s.append(":name "); s.append(SimpleSLTokenizer.isAWord(name) ? name :
     * SimpleSLTokenizer.quoteString(name)); #MIDP_INCLUDE_END
     */
    ...
    for (int i = 0; i < addresses.size(); i++)
      try {
        ...
        // #MIDP_EXCLUDE_END
        /*
         * #MIDP_INCLUDE_BEGIN s.append((String)addresses.elementAt(i)); #MIDP_INCLUDE_END
         */
        s.append(" ");
    ...
    for (int i = 0; i < resolvers.size(); i++) {
      try {
        ...
        // #MIDP_EXCLUDE_END
        /*
         * #MIDP_INCLUDE_BEGIN s.append(resolvers.elementAt(i).toString()); #MIDP_INCLUDE_END
         */
      } catch (IndexOutOfBoundsException e) {
        e.printStackTrace();
      }
    ...
    while (e.hasMoreElements()) {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN s.append(key); s.append(" "); s.append(SimpleSLTokenizer.isAWord(value)
       * ? value : SimpleSLTokenizer.quoteString(value)); #MIDP_INCLUDE_END
       */
    }
    s.append(")");
    return s.toString();
  ...
  public synchronized Object clone() {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN result.addresses = new Vector(addresses.size()); for (int i=0;
     * i<addresses.size(); i++) result.addresses.addElement(addresses.elementAt(i));
     * result.resolvers = new Vector(resolvers.size()); for (int i=0; i<resolvers.size(); i++)
     * result.resolvers.addElement(resolvers.elementAt(i)); #MIDP_INCLUDE_END
     */
  ...
  /**
   * Equality operation. This method compares an <code>AID</code> object with another or with a Java
   ...
   *         <ul>
   *         <li>The argument <code>o</code> is an <code>AID</code> object with the same
   *         <em>GUID</em> in its name slot (apart from differences in case).
   *         <li>The argument <code>o</code> is a <code>String</code> that is equal to the
   *         <em>GUID</em> contained in the name slot of this Agent ID (apart from differences in
   *         case).
   *         </ul>
   */
   ...
   * 
   * @param o Another <code>AID</code> object, that will be compared with the current
   *        <code>AID</code>.
   * @return -1, 0 or 1 according to the lexicographical order of the <em>GUID</em> of the two agent
   *         IDs, apart from differences in case.
   */
  ...
  /**
   * Hash code. This method returns an hash code in such a way that two <code>AID</code> objects
   * with equal names or with names differing only in case have the same hash code.
   * 
  ...
  /**
   * Returns the local name of the agent (without the HAP). If the agent is not local, then the
   * method returns its GUID.
   */
  ...
  /**
   * Returns the HAP of the agent or null if the GUID of this <code>AID</code> is not of the form
