...
public class NotificationService extends BaseService {
  ...
  private static final String[] OWNED_COMMANDS = new String[] {NotificationSlice.SNIFF_ON,
      ...
      NotificationSlice.NOTIFY_CHANGED_AGENT_PRINCIPAL, NotificationSlice.NOTIFY_BEHAVIOUR_ADDED,
      NotificationSlice.NOTIFY_BEHAVIOUR_REMOVED, NotificationSlice.NOTIFY_CHANGED_BEHAVIOUR_STATE};
  ...
  private class NotificationSourceSink implements Sink {
    ...
    private void handleSniffOn(VerticalCommand cmd)
    ...
    private void handleSniffOff(VerticalCommand cmd)
        throws IMTPException, ServiceException, NotFoundException {
      Object[] params = cmd.getParams();
      AID sniffer = (AID) params[0];
      List targets = (List) params[1];
    ...
    private void handleDebugOn(VerticalCommand cmd)
        throws IMTPException, ServiceException, NotFoundException {
      Object[] params = cmd.getParams();
    ...
    private void handleDebugOff(VerticalCommand cmd)
        throws IMTPException, ServiceException, NotFoundException {
      Object[] params = cmd.getParams();
      AID introspector = (AID) params[0];
      List targets = (List) params[1];
  ...
  private class ServiceComponent implements Service.Slice {
    ...
    private void sniffOn(AID snifferName, AID targetName) throws IMTPException {
      ...
      if (tn == null) { // Need a new notifier
        ...
        try {
          ...
          myContainer.initAgent(id, tn, null, null); // FIXME: Modify to use a proper owner
                                                     // Principal
    ...
    private void debugOn(AID introspectorName, AID targetName) throws IMTPException {
      ...
      while (itReady.hasNext()) {
        ...
        AgentEvent ev =
            new AgentEvent(AgentEvent.ADDED_BEHAVIOUR, targetName, bid, myContainer.getID());
      ...
      while (itBlocked.hasNext()) {
        ...
        AgentEvent ev =
            new AgentEvent(AgentEvent.ADDED_BEHAVIOUR, targetName, bid, myContainer.getID());
        tn.addedBehaviour(ev);
        ev = new AgentEvent(AgentEvent.CHANGED_BEHAVIOUR_STATE, targetName, bid,
            Behaviour.STATE_READY, Behaviour.STATE_BLOCKED, myContainer.getID());
  ...
  private void firePostedMessage(ACLMessage msg, AID receiver) {
    ...
    if (l != null) {
      MessageEvent ev =
          new MessageEvent(MessageEvent.POSTED_MESSAGE, msg, sender, receiver, myID());
      Iterator it = l.iterator();
      while (it.hasNext()) {
        MessageListener ml = (MessageListener) it.next();
        ml.postedMessage(ev);
  ...
  private void fireReceivedMessage(ACLMessage msg, AID receiver) {
    ...
    if (l != null) {
      MessageEvent ev =
          new MessageEvent(MessageEvent.RECEIVED_MESSAGE, msg, sender, receiver, myID());
      Iterator it = l.iterator();
      while (it.hasNext()) {
        MessageListener ml = (MessageListener) it.next();
  ...
  private void fireRemovedBehaviour(AID agentID, Behaviour b) {
    ...
    if (l != null) {
      AgentEvent ev =
          new AgentEvent(AgentEvent.REMOVED_BEHAVIOUR, agentID, new BehaviourID(b), myID());
      Iterator it = l.iterator();
      while (it.hasNext()) {
        AgentListener al = (AgentListener) it.next();
  ...
  private void fireChangedBehaviourState(AID agentID, Behaviour b, String from, String to) {
    ...
    List l = agentListeners.startScanning();
    if (l != null) {
      AgentEvent ev = new AgentEvent(AgentEvent.CHANGED_BEHAVIOUR_STATE, agentID,
          new BehaviourID(b), from, to, myID());
      Iterator it = l.iterator();
      while (it.hasNext()) {
        AgentListener al = (AgentListener) it.next();
