...
class FullResourceManager implements ResourceManager {
  ...
  public static final String DISABLE_THREAD_GROUP_INTERRUPT =
      "jade_core_FullResourceManager_disablethreadgroupinterrupt";
  private static final boolean DEFAULT_DISABLE_THREAD_GROUP_INTERRUPT = false;
  public static final String THREAD_GROUP_INTERRUPT_TIMEOUT =
  ...
  public void initialize(Profile p) {
    ...
    disableThreadGroupInterrupt = myProfile.getBooleanProperty(DISABLE_THREAD_GROUP_INTERRUPT,
        DEFAULT_DISABLE_THREAD_GROUP_INTERRUPT);

    String tmp = myProfile.getParameter(THREAD_GROUP_INTERRUPT_TIMEOUT,
        DEFAULT_THREAD_GROUP_INTERRUPT_TIMEOUT);
    threadGroupInterruptTimeout = Integer.parseInt(tmp);

    if (!myProfile.getBooleanProperty(Profile.NO_DISPLAY, false)) {
      // Start the AWT-Toolkit outside the JADE Thread Group to avoid annoying
      // InterruptedException-s on termination
