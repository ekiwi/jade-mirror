...
/**
 * 
 * A generic implementation of the <code>Command</code> interface, operating at the meta-level to
 * provide a generic transformation of method invocations. Generic commands can be used both as
 * vertical and horizontal commands. Objects of this class can also be recycled to build <i>command
...
public class GenericCommand implements HorizontalCommand, VerticalCommand {
  ...
  /**
   * Creates a new generic command, with the given name and belonging to the given service and
   * interaction.
   * 
   ...
   * 
   * @param obj The parameter to add. The actual type of <code>obj</code> must be such that can be
   *        delivered over the network by the concrete platform IMTP.
   * 
