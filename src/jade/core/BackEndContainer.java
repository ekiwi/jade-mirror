...
public class BackEndContainer extends AgentContainerImpl implements BackEnd {
  ...
  public boolean connect() {
    try {
      ...
      myLogger.log(Logger.FINE,
          "BackEnd container " + myProfile.getParameter(Profile.CONTAINER_NAME, null)
      ...
      if (connected) {
        ...
        if ("true".equals(myProfile.getParameter(RESYNCH, "false"))) {
          myLogger.log(Logger.INFO, "BackEnd container "
              + myProfile.getParameter(Profile.CONTAINER_NAME, null) + " activating re-synch ...");
          resynch();
        } else {
  ...
  /**
   * A new agent has just started on the FrontEnd. Adjust the agent name taking into account
   * wild-cards. Issue an INFORM_CREATED vertical command.
   * 
  ...
  public void messageOut(ACLMessage msg, String sender) throws NotFoundException, IMTPException {
    ...
    myLogger.log(Logger.FINE, getID() + " - Delivering OUT message "
        + ACLMessage.getPerformative(msg.getPerformative()) + ", size=" + size);
    handleSend(msg, id, false);
  }

  public Object serviceInvokation(String actor, String serviceName, String methodName,
      Object[] methodParams) throws NotFoundException, ServiceException, IMTPException {
      ...
      if (m != null) {
        if (!m.isAccessible()) {
          try {
            ...
            throw new NoSuchMethodException("Method " + methodName + "() of class "
                + getClass().getName() + " not accessible.");
          }
        }
  ...
  private boolean isCompatible(Method method, Object[] params) {
    // FIXME: At present we just check the number of parameters. Implement types
    // compatibility
    ...
    if (found) {
      ...
      if (image != null) {
        ...
        try {
          ...
          myLogger.log(Logger.FINE, getID() + " - Delivering IN message "
              + ACLMessage.getPerformative(msg.getPerformative()) + ", size=" + size);
  ...
  public boolean postMessagesBlockToLocalAgent(ACLMessage[] mm, AID receiverID) {
    // FIXME: To optimize performances we should support messages-block transfer
    // between BE and FE
    boolean ret = false;
  ...
  public void releaseLocalAgent(AID id) {
    // If the aquired agent was an image there is nothing to release (see FIXME
    // above)
  ...
  /**
   * This method is re-defined to avoid NullPointerException. In fact a search in the LADT would be
   * done for the agent to be debugged, but the LADT is obviously empty.
   */
  ...
  public List removePendingMessages(MessageTemplate template, boolean notifyFailure) {
    ...
    if (pendingMsg.size() > 0) {
      myLogger.log(Logger.INFO,
          "Removed " + pendingMsg.size() + " pending messages from BackEnd queue.");
    }
    if (notifyFailure) {
  ...
  /**
   * Start the front-end synchronization process. NOTE that when this method is called it may not be
   * possible to send commands to the FE. If a bi-connection dispatcher is used in fact, the INP
   * connection is not yet ready at this time. However we can't just send a SYNCH command and rely
   * on the store-and-forward mechanism of the FrontEndStub since we need to know when the SYNCH
   * command is completely served. --> We start a separated Thread that tries sending the SYNCH
      ...
      public void run() {
        while (!terminating) {
          try {
            ...
            myLogger.log(Logger.INFO,
                "BackEnd container " + here().getName() + " - Sending SYNCH command to FE ...");
            myFrontEnd.synch();
            notifySynchronized();
            myLogger.log(Logger.INFO,
                "BackEnd container " + here().getName() + " - Resynch completed");
            break;
          } catch (IMTPException imtpe) {
            ...
            myLogger.log(Logger.WARNING, "BackEnd container " + here().getName()
                + " - IMTP Exception in resynch. Wait a bit and retry...");
          }
        }
      }
    };
