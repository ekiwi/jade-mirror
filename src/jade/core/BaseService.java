...
/**
 * 
 * The <code>BaseService</code> abstract class partially implements the <code>Service</code>
 * interface, providing a simple and uniform mechanism for slice management and service discovery.
 * Developers interested in creating JADE kernel level services should extend
 * <code>BaseService</code> instead of directly implementing the <code>Service</code> interface
 * 
...
public abstract class BaseService implements Service {
  ...
  /**
   * The <code>getSlice()</code> implementation of this class works as follows:
   * <ol>
   * <li><i>First, the name alias table is used to convert the given slice name into another name,
   * if any</i></li>
   * 
   * <li><i>Then, the new name (which may or may not be different from the original one) is used to
   * look up an internal table keeping the service slices</i></li>
   * 
   * <li><i>If no slice was found, the</i> <code>ServiceFinder</code> <i>is asked to provide the
   * slice, which is then put into the local table.</i></li>
   * </ol>
   */
  ...
  /**
   * This method returns the current number of slices known to this service <b>on this node</b>. Due
   * to the distributed nature of many JADE services, there is no guaranteed that calling this
   * method for the same service on different nodes will actually result on the same number.
   * 
  ...
  /**
   * The <code>getAllSlices()</code> implementation of this class directly retrieves the current
   * list of slices from the Service Manager. Note that slices are retrieved directly from the Main
  ...
  public void broadcast(HorizontalCommand cmd, boolean includeMyself)
  ...
  /**
   * This protected method allows subclasses to define their own naming schemes, by adding aliases
   * for existing slice names.
   * 
  ...
  /**
   * This protected method is used by <code>getSlice()</code> to dereference aliases for slice
   * names. Subclasses can override this method to build their own service-specific naming schema.
   * 
  ...
  /**
   * This should be properly implemented by the services that have filters. Note that when called
   * multiple times with the same value of the <code>direction</code> parameter this method MUST
  ...
  /**
   * This should be properly implemented by the services that require a service specific Behaviour
   * running in the AMS.
   */
  ...
  public Object submit(VerticalCommand cmd) throws ServiceException {
    ...
    throw new ServiceException(
        "Command <" + cmdName + "> does not belong to service <" + getName() + ">");
  ...
  protected void clearCachedSlice(String name) {
    // slices may be null when calling this method on the Main Container, but on a
    // service not active on the Main Container
  ...
  /**
   * This method can be redefined to support service internal data inspection by means of the
   * ContainerMonitorAgent included in the misc add-on. The default implementation just dumps the
   * map of cached slices
   * 
  ...
  public String dump(String key) {
    if (key == null || key.equals(ALL_DUMP_KEY)) {
      ...
      while (it.hasNext()) {
        ...
        sb.append("- ").append(name).append(" --> " + stringifySlice((Slice) slices.get(name)))
            .append("\n");
      }
