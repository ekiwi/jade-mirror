...
/*
 * #MIDP_INCLUDE_BEGIN import javax.microedition.midlet.*; #MIDP_INCLUDE_END
 */

/**
 * The <code>Agent</code> class is the common superclass for user defined software agents. It
 * provides methods to perform basic agent tasks, such as:
 * <ul>
 * <li><b> Message passing using <code>ACLMessage</code> objects, both unicast and multicast with
 * optional pattern matching. </b></li>
 * <li><b> Complete Agent Platform life cycle support, including starting, suspending and killing an
 * agent. </b></li>
 * <li><b> Scheduling and execution of multiple concurrent activities. </b></li>
 * </ul>
 * 
 * Application programmers must write their own agents as <code>Agent</code> subclasses, adding
 * specific behaviours as needed and exploiting <code>Agent</code> class capabilities.
 * 
...
// #APIDOC_EXCLUDE_END
{
  ...
  /**
   * Inner class Interrupted. This class is used to handle change state requests that occur in
   * particular situations such as when the agent thread is blocked in the doWait() method.
   */
  ...
  /**
   * Inner class AssociationTB. This class manages bidirectional associations between Timer and
   * Behaviour objects, using hash tables. This class is synchronized with the operations carried
   * out by the TimerDispatcher. It allows also to avoid a deadlock when: 1) A behaviour blocks for
   * a very short time --> A Timer is added to the TimerDispatcher 2) The Timer immediately expires
   * and the TimerDispatcher try to restart the behaviour before the pair (b, t) is added to the
  ...
  /**
   * Restarts the behaviour associated with t. This method runs within the time-critical Timer
   * Dispatcher thread and is not intended to be called by users. It is defined public only because
  ...
  /**
   * Notifies this agent that one of its behaviours has been restarted for some reason. This method
   * clears any timer associated with behaviour object <code>b</code>, and it is unneeded by
   * application level code. To explicitly schedule behaviours, use <code>block()</code> and
  ...
  /**
   * This flag is used to distinguish the normal AP_ACTIVE state from the particular case in which
   * the agent state is set to AP_ACTIVE during agent termination (takeDown()) to allow it to
   * clean-up properly. In this case in fact a call to <code>doDelete()</code>,
   * <code>doMove()</code>, <code>doClone()</code> and <code>doSuspend()</code> should have no
   * effect.
   */
  ...
  // #MIDP_EXCLUDE_BEGIN
  /**
   * When set to false (default) all behaviour-related events (such as ADDED_BEHAVIOUR or
   * CHANGED_BEHAVIOUR_STATE) are not generated in order to improve performances. These events in
   * facts are very frequent.
   */
   ...
   * private boolean isInterrupted = false; #MIDP_INCLUDE_END
   */
   ...
   * 
   * @return The MessageQueue to be used by this agent or null if the internal message queue must be
   *         used
   */
   ...
   * <p>
   * Take care that the arguments are transient and they do not migrate with the agent neither are
   * cloned with the agent!
   * 
   * @return the array of arguments passed to this agent.
   * @see <a href=../../../tutorials/ArgsAndPropsPassing.htm>How to use arguments or properties to
   *      configure your agent.</a>
   **/
  ...
  /**
   * This method returns <code>true</code> when this agent is restarting after a crash. The
   * restarting indication is automatically reset as soon as the <code>setup()</code> method of this
   * agent terminates.
   * 
   * @return <code>true</code> when this agent is restarting after a crash. <code>false</code>
   *         otherwise.
   */
   ...
   * 
   * @return An <code>AID</code> object, that can be used to contact the AMS of this platform.
   */
   ...
   * 
   * @return An <code>AID</code> object, that can be used to contact the default DF of this
   *         platform.
   */
   ...
   * 
   * @return A <code>String</code> containing the local agent name (e.g. <em>peter</em>).
   */
  ...
  /**
   * Method to query the Home Agent Platform. This is the name of the platform where the agent has
   * been created, therefore it will never change during the entire lifetime of the agent. In JADE
   * the name of an agent by default is composed by the concatenation (using '@') of the agent local
   * name and the Home Agent Platform name
   *
   * @return A <code>String</code> containing the name of the home agent platform (e.g.
   *         <em>myComputerName:1099/JADE</em>).
   */
   ...
   * that is registered with the platform AMS.
   * 
   * @return An <code>Agent ID</code> object, containing the complete agent GUID, addresses and
   *         resolvers.
   */
  ...
  /**
   * This method adds a new platform address to the AID of this Agent. It is called by the container
   * when a new MTP is activated in the platform (in the local container - installMTP() - or in a
   * remote container - updateRoutingTable()) to keep the Agent AID updated.
   */
  ...
  /**
   * This method removes an old platform address from the AID of this Agent. It is called by the
   * container when a new MTP is deactivated in the platform (in the local container -
   * uninstallMTP() - or in a remote container - updateRoutingTable()) to keep the Agent AID
   * updated.
   */
   ...
   * 
   * @return A <code>Location</code> object, describing the location where this agent is currently
   *         running.
   */
  ...
  // #APIDOC_EXCLUDE_BEGIN
  /**
   * This method is used internally by the framework and should NOT be used by programmers. This is
   * used by the agent container to wait for agent termination. We have already called doDelete on
   * the thread which would have issued an interrupt on it. However, it still may decide not to
   * exit. So we will wait no longer than 5 seconds for it to exit and we do not care of this zombie
   * agent. FIXME: we must further isolate container and agents, for instance by using custom class
   * loader and dynamic proxies and JDK 1.3. FIXME: the timeout value should be got by Profile
   */
  public void join() {
    // #MIDP_EXCLUDE_BEGIN
    try {
      ...
      if (myThread.isAlive()) {
        System.out.println(
            "*** Warning: Agent " + myName + " did not terminate when requested to do so.");
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN if (myThread != null && myThread.isAlive()) { try { myThread.join(); }
     * catch (InterruptedException ie) { ie.printStackTrace(); } } #MIDP_INCLUDE_END
     */
  }
  // #APIDOC_EXCLUDE_END

  /**
   * Set message queue size. This method allows to change the number of ACL messages that can be
   * buffered before being actually read by the agent or discarded.
   * 
   * @param newSize A non negative integer value to set message queue size to. Passing 0 means
   *        unlimited message queue. When the number of buffered messages exceeds this value, older
   *        messages are discarded according to a <b><em>FIFO</em></b> replacement policy.
   ...
   * 
   * @return The number of messages that are currently stored into the message queue.
   **/
  ...
  /**
   * Reads message queue size. A zero value means that the message queue is unbounded (its size is
   * limited only by amount of available memory).
   * 
   * @return The actual size of the message queue (i.e. the max number of messages that can be
   *         stored into the queue)
  ...
  /**
   * The ActiveLifeCycle handles different internal states (INITIATED, ACTIVE, WAITING, IDLE). This
   * method switches between them.
   */
  ...
  // #APIDOC_EXCLUDE_BEGIN
  /**
   * Read current agent state. This method can be used to query an agent for its state from the
   * outside.
   * 
  ...
  /**
   * This is only called by the NotificationService to provide the Introspector agent with a
   * snapshot of the behaviours currently loaded in the agent
   */
  ...
  /**
   * Make this agent move to a remote location. This method is intended to support agent mobility
   * and is called either by the Agent Platform or by the agent itself to start a migration process.
   * It should be noted that this method just changes the agent state to <code>AP_TRANSIT</code>.
   * The actual migration takes place asynchronously. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
  ...
  /**
   * Make this agent be cloned on another location. This method is intended to support agent
   * mobility and is called either by the Agent Platform or by the agent itself to start a clonation
   * process. It should be noted that this method just changes the agent state to
   * <code>AP_COPY</code>. The actual clonation takes place asynchronously. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
  ...
  /**
   * Make a state transition from <em>active</em> or <em>waiting</em> to <em>suspended</em> within
   * Agent Platform Life Cycle; the original agent state is saved and will be restored by a
   * <code>doActivate()</code> call. This method can be called from the Agent Platform or from the
   * agent itself and stops all agent activities. Incoming messages for a suspended agent are
   * buffered by the Agent Platform and are delivered as soon as the agent resumes. Calling
   * <code>doSuspend()</code> on a suspended agent has no effect. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
  ...
  /**
   * Make a state transition from <em>suspended</em> to <em>active</em> or <em>waiting</em>
   * (whichever state the agent was in when <code>doSuspend()</code> was called) within Agent
   * Platform Life Cycle. This method is called from the Agent Platform and resumes agent execution.
   * Calling <code>doActivate()</code> when the agent is not suspended has no effect. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
  ...
  /**
   * Make a state transition from <em>active</em> to <em>waiting</em> within Agent Platform Life
   * Cycle. This method has only effect when called by the agent thread and causes the agent to
   * block, stopping all its activities until a message arrives or the <code>doWake()</code> method
   * is called.
   * 
  ...
  /**
   * Make a state transition from <em>active</em> to <em>waiting</em> within Agent Platform Life
   * Cycle. This method adds a timeout to the other <code>doWait()</code> version.
   * 
  ...
  /**
   * Make a state transition from <em>waiting</em> to <em>active</em> within Agent Platform Life
   * Cycle. This method is called from Agent Platform and resumes agent execution. Calling
  ...
  /**
   * Make a state transition from <em>active</em>, <em>suspended</em> or <em>waiting</em> to
   * <em>deleted</em> state within Agent Platform Life Cycle, thereby destroying the agent. This
   * method can be called either from the Agent Platform or from the agent itself. Calling
  ...
  // #MIDP_EXCLUDE_BEGIN
  /**
   * Write this agent to an output stream; this method can be used to record a snapshot of the agent
   * state on a file or to send it through a network connection. Of course, the whole agent must be
   * serializable in order to be written successfully. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   * @param s The stream this agent will be sent to. The stream is <em>not</em> closed on exit.
  ...
  /**
   * Read a previously saved agent from an input stream and restarts it under its former name. This
   * method can realize some sort of mobility through time, where an agent is saved, then destroyed
   * and then restarted from the saved copy. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   * @param s The stream the agent will be read from. The stream is <em>not</em> closed on exit.
   * @exception IOException Thrown if some I/O error occurs during stream reading.
   * @see jade.core.Agent#write(OutputStream s)
   *
   *      public static void read(InputStream s) throws IOException { try { ObjectInput in = new
   *      ObjectInputStream(s); String name = in.readUTF(); Agent a = (Agent)in.readObject();
   *      a.doStart(name); } catch(ClassNotFoundException cnfe) { cnfe.printStackTrace(); } }
   */

       /**
        * Read a previously saved agent from an input stream and restarts it under a different name.
        * This method can realize agent cloning through streams, where an agent is saved, then an
        * exact copy of it is restarted as a completely separated agent, with the same state but
        * with different identity and address. <br>
        * <b>NOT available in MIDP</b> <br>
        * 
        * @param s The stream the agent will be read from. The stream is <em>not</em> closed on
        *        exit.
        * @param agentName The name of the new agent, copy of the saved original one.
        * @exception IOException Thrown if some I/O error occurs during stream reading.
        * @see jade.core.Agent#write(OutputStream s)
        *
        *      public static void read(InputStream s, String agentName) throws IOException { try {
        *      ObjectInput in = new ObjectInputStream(s); String oldName = in.readUTF(); Agent a =
        *      (Agent)in.readObject(); a.doStart(agentName); } catch(ClassNotFoundException cnfe) {
        *      cnfe.printStackTrace(); } }
        */

       /**
        * This method reads a previously saved agent, replacing the current state of this agent with
        * the one previously saved. The stream must contain the saved state of <b>the same agent</b>
        * that it is trying to restore itself; that is, <em>both</em> the Java object <em>and</em>
        * the agent name must be the same. <br>
        * <b>NOT available in MIDP</b> <br>
        * 
        * @param s The input stream the agent state will be read from.
        * @exception IOException Thrown if some I/O error occurs during stream reading. <em>Note:
        *            This method is currently not implemented</em>
        */
  ...
  /**
   * This method should not be used by application code. Use the same-named method of
   * <code>jade.wrapper.Agent</code> instead. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   ...
   * In order for this method to work, the agent must have declared its will to accept objects from
   * other software components running within its JVM. This can be achieved by calling the
   * <code>jade.core.Agent.setEnabledO2ACommunication()</code> method. If the retrieved object was
   * originally inserted by an external component using a blocking call, that call will return
   * during the execution of this method. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   * @return the first object in the queue, or <code>null</code> if the queue is empty.
   * @see jade.wrapper.AgentController#putO2AObject(Object o, boolean blocking)
  ...
  /**
   * This method declares this agent attitude towards object-to-agent communication, that is,
   * whether the agent accepts to communicate with other non-JADE components living within the same
   * JVM. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   * @param enabled Tells whether Java objects inserted with <code>putO2AObject()</code> will be
   *        accepted.
   * @param queueSize If the object-to-agent communication is enabled, this parameter specifiies the
   *        maximum number of Java objects that will be queued. If the passed value is 0, no maximum
   *        limit is set up for the queue.
   * 
   * @see jade.wrapper.AgentController#putO2AObject(Object o, boolean blocking)
   * @see getO2AObject()
   * 
  ...
  /**
   * Sets the behaviour responsible for managing objects passed to the agent by means of the
   * Object-To-Agent (O2A) communication mechanism. If the O2A manager behaviour is set, whenever an
   * object is inserted in the O2A queue by means of the <code>putO2AObject()</code> method, only
   * the manager is waken up. This improves the efficiency since all behaviours not interested in
   * O2A communication remain sleeping. <br>
   * NOTE that this method only declares a behaviour as being responsible for managing objects
   * received by the agent by means of the O2A communication channel; in order to correctly run, the
   * behaviour must be added to the agent by means of the <code>addBehaviour()<code> method as
   * usual. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
  ...
  /**
   * Registers an implementation for a given O2A interface. All invocations to methods of the O2A
   * interface will be redirected to the registered implementation object.
   * 
  ...
  /**
   * This method is the main body of every agent. It provides startup and cleanup hooks for
   * application programmers to put their specific code into.
   * 
  ...
  private class ActiveLifeCycle extends LifeCycle {
    ...
    public void execute()
        throws JADESecurityException, InterruptedException, InterruptedIOException {
  ...
  /**
   * This protected method is an empty placeholder for application specific startup code. Agent
   * developers can override it to provide necessary behaviour. When this method is called the agent
   * has been already registered with the Agent Platform <b>AMS</b> and is able to send and receive
   * messages. However, the agent execution model is still sequential and no behaviour scheduling is
   * active yet.
   * 
   * This method can be used for ordinary startup tasks such as <b>DF</b> registration, but is
   * essential to add at least a <code>Behaviour</code> object to the agent, in order for it to be
   * able to do anything.
   * 
  ...
  /**
   * This protected method is an empty placeholder for application specific cleanup code. Agent
   * developers can override it to provide necessary behaviour. When this method is called the agent
   * has not deregistered itself with the Agent Platform <b>AMS</b> and is still able to exchange
   * messages with other agents. However, no behaviour scheduling is active anymore and the Agent
   * Platform Life Cycle state is already set to <em>deleted</em>.
   * 
   * This method can be used for ordinary cleanup tasks such as <b>DF</b> deregistration, but
   * explicit removal of all agent behaviours is not needed.
   */
  protected void takeDown() {}

  // #MIDP_EXCLUDE_BEGIN
  /**
   * This empty placeholder shall be overridden by user defined agents to execute some actions
   * before the original agent instance on the source container is stopped (e.g. releasing local
   * resources such as a GUI).<br>
   * <b>IMPORTANT:</b> At this point, it is ensured that the move process is successful and that a
   * moved agent instance has been created on the destination container Therefore setting the value
   * of a class field in this method will have no impact on the moved agent instance. Such
   * parameters must indeed be set <b>before</b> the <code>doMove()</code> method is called. <br>
   * <b>NOT available in MIDP</b> <br>
   */
  protected void beforeMove() {}

  /**
   * Actions to perform after moving. This empty placeholder method can be overridden by user
   * defined agents to execute some actions just after arriving to the destination agent container
   * for a migration. <br>
   * <b>NOT available in MIDP</b> <br>
   */
   ...
   * actions before copying an agent to another agent container. <br>
   * <b>NOT available in MIDP</b> <br>
   * 
   * @see beforeMove()
   * @see afterClone()
  ...
  /**
   * Actions to perform after cloning. This empty placeholder method can be overridden by user
   * defined agents to execute some actions just after creating an agent copy to the destination
   * agent container. <br>
   * <b>NOT available in MIDP</b> <br>
   */
  ...
  /**
   * This method is executed when blockingReceive() is called from a separate Thread. It does not
   * affect the agent state.
   */
  ...
  /**
   * This method adds a new behaviour to the agent. This behaviour will be executed concurrently
   * with all the others, using a cooperative round robin scheduling. This method is typically
   * called from an agent <code>setup()</code> to fire off some initial behaviour, but can also be
  ...
  /**
   * This method removes a given behaviour from the agent. This method is called automatically when
   * a top level behaviour terminates, but can also be called from a behaviour to terminate itself
  ...
  /**
   * Send an <b>ACL</b> message to another agent. This methods sends a message to the agent
   * specified in <code>:receiver</code> message field (more than one agent can be specified as
   * message receiver).
   * 
  ...
  /**
   * Receives an <b>ACL</b> message from the agent message queue. This method is non-blocking and
   * returns the first message in the queue, if any. Therefore, polling and busy waiting is required
   * to wait for the next message sent using this method.
   * 
  ...
  /**
   * Receives an <b>ACL</b> message matching a given template. This method is non-blocking and
   * returns the first matching message in the queue, if any. Therefore, polling and busy waiting is
   ...
   * @return A new ACL message matching the given template, or <code>null</code> if no such message
   *         is present.
   * @see jade.lang.acl.ACLMessage
  ...
  /**
   * Receives an <b>ACL</b> message from the agent message queue. This method is blocking and
   * suspends the whole agent until a message is available in the queue.
   * 
  ...
  /**
   * Receives an <b>ACL</b> message from the agent message queue, waiting at most a specified amount
   * of time.
   * 
   * @param millis The maximum amount of time to wait for the message.
   * @return A new ACL message, or <code>null</code> if the specified amount of time passes without
   *         any message reception.
   */
  ...
  /**
   * Receives an <b>ACL</b> message matching a given message template. This method is blocking and
   * suspends the whole agent until a message is available in the queue.
   * 
   * @param pattern A message template to match received messages against.
   * @return A new ACL message matching the given template, blocking until such a message is
   *         available.
  ...
  /**
   * Receives an <b>ACL</b> message matching a given message template, waiting at most a specified
   * time.
   * 
   * @param pattern A message template to match received messages against.
   * @param millis The amount of time to wait for the message, in milliseconds.
   * @return A new ACL message matching the given template, or <code>null</code> if no suitable
   *         message was received within <code>millis</code> milliseconds.
  ...
  /**
   * Puts a received <b>ACL</b> message back into the message queue. This method can be used from an
   * agent behaviour when it realizes it read a message of interest for some other behaviour. The
   * message is put in front of the message queue, so it will be the first returned by a
  ...
  // #APIDOC_EXCLUDE_BEGIN
  /**
   * This method blocks until the agent has finished its start-up phase (i.e. until just before its
   * setup() method is called. When this method returns, the target agent is registered with the AMS
   * and the JADE platform is aware of it.
   */
  ...
  /**
   * Put a received message into the agent message queue. The message is put at the back end of the
   * queue. This method is called by JADE runtime system when a message arrives, but can also be
   * used by an agent, and is just the same as sending a message to oneself (though slightly
   * faster).
   * 
  ...
  /**
   * Retrieve a configuration property set in the <code>Profile</code> of the local container
   * (first) or as a System property.
   * 
   * @param key the key that maps to the property that has to be retrieved.
   * @param aDefault a default value to be returned if there is no mapping for <code>key</code>
   */
  ...
  // #MIDP_EXCLUDE_BEGIN
  /**
   * Return the configuration properties exactly as they were passed to the Profile before starting
   * the local JADE container.
   */
  ...
  /**
   * This method is used to interrupt the agent's thread. In J2SE/PJAVA it just calls
   * myThread.interrupt(). In MIDP, where interrupt() is not supported the thread interruption is
   * simulated as described below. The agent thread can be in one of the following three states: 1)
   * Running a behaviour. 2) Sleeping on msgQueue due to a doWait() 3) Sleeping on myScheduler due
   * to a schedule() with no active behaviours Note that in MIDP the suspended state is not
   * supported The idea is: set the 'isInterrupted' flag, then wake up the thread wherever it may be
   */
  private void interruptThread() {
     ...
     * 
     * // case 1: Nothing to do. // case 2: Signal on msgQueue. synchronized (msgQueue)
     * {msgQueue.notifyAll();} // case 3: Signal on the Scheduler synchronized (myScheduler)
     * {myScheduler.notifyAll();} } #MIDP_INCLUDE_END
     */
  }

  /**
   * Since in MIDP Thread.interrupt() does not exist and a simulated interruption is used to
   * "interrupt" the agent's thread, we must check whether the simulated interruption happened just
   * before and after going to sleep.
   */
  void waitOn(Object lock, long millis) throws InterruptedException {
    /*
     * #MIDP_INCLUDE_BEGIN synchronized (this) { if (isInterrupted) { isInterrupted = false; throw
     * new InterruptedException(); } } #MIDP_INCLUDE_END
     */
    lock.wait(millis);
    /*
     * #MIDP_INCLUDE_BEGIN synchronized (this) { if (isInterrupted) { isInterrupted = false; throw
