...
public class ProfileImpl extends Profile {
  ...
  /**
   * Creates a Profile implementation using the given properties to configure the platform startup
   * process.
   * 
  ...
  /**
   * Creates a Profile implementation with the following default configuration: <br>
   * if isMain is true, then the profile is configured to launch a main-container on the localhost,
   * RMI internal Message Transport Protocol, port number 1099, HTTP MTP. <br>
   * if isMain is false, then the profile is configured to launch a remote container on the
   * localhost, connecting to the main-container on the localhost through RMI internal Message
   * Transport Protocol, port number 1099.
   */
  public ProfileImpl(boolean isMain) {
     ...
     */
    props.setProperty(Profile.MAIN, (new Boolean(isMain)).toString()); // set to a main/non-main
                                                                       // container
  }
  ...
  /**
   * This constructor creates a default Profile for launching a platform (<b>i.e. a main
   * container!!</b>).
   * 
   * @param host is the name of the host where the main-container should be listen to. A null value
   *        means use the default (i.e. localhost)
   * @param port is the port number where the main-container should be listen for other containers.
   *        A negative value should be used for using the default port number.
   ...
   * @param platformID is the symbolic name of the platform, if different from default. A null value
   *        means use the default (i.e. localhost)
   * @param isMain if isMain is false, then the profile is configured to launch a remote container,
   *        if true a main-container
   **/
  ...
  void init() {
    ...
    if (props.getProperty(JVM) == null) {
       ...
       */
      /*
       * #MIDP_INCLUDE_BEGIN props.setProperty(JVM, MIDP); props.setProperty(MAIN, "false");
       * #MIDP_INCLUDE_END
       */
    }
   ...
   * specified key is found, <code>aDefault</code> is returned.
   * 
   ...
   * 
   * @return The platform service manager, either the real implementation or a remote proxy object.
   * @throws ProfileException If some needed information is wrong or missing from the profile.
   */
   ...
   * 
   * @return The platform service finder, either the real implementation or a remote proxy object.
   * @throws ProfileException If some needed information is wrong or missing from the profile.
   */
