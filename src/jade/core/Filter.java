 ...
 * process commands coming from the upper JADE layers. Filters process commands when their
 * <code>accept()</code> method and <code>postProcess</code> methods are invoked; Filters in the
 * filter chain are nested. The <code>accept()</code> method is called before the command is
 * processed by successive filters in the chain. It should be used to take proper actions before the
 * command consumption. On the other hand the <code>postProcess()</code> method is called after the
 * command has been processed by successive filters in the chain. It should be used to take proper
 * actions after the command consumption.
 * 
...
public abstract class Filter {
   ...
   * 
   * @param cmd A <code>VerticalCommand</code> describing what operation has been requested from
   *        previous layers (that can be the actual prime source of the command or previous filters
   *        in the chain).
   */
   ...
   *        previous layers (that can be the actual prime source of the command or previous filters
   *        in the chain).
   * @return A boolean value, telling whether the filtered command has been accepted or not. A
   *         filter can veto a command by returning <code>false</code> from this method. A vetoed
   *         command is not propagated in the filter chain.
   */
  ...
  /**
   * Post-process a command object after it has been processed by the successive filters in the
   * filter chain.
   * 
   * @param cmd A <code>VerticalCommand</code> describing what operation has been requested from
   *        previous layers (that can be the actual prime source of the command or previous filters
  ...
  /**
   * Sets the blocking state of this filter. A blocked filter does not process commands, and also
   * prevents subsequent filters to process them.
   * 
  ...
  /**
   * Inquires the blocking state of this filter. A blocked fliter does not process commands, and
   * also prevents subsequent filters to process them.
   * 
  ...
  /**
   * Sets the skipping state of this filter. A skipped filter does not process commands, but passes
   * them directly to subsequent filters.
   * 
  ...
  /**
   * Inquires the skipping state of this filter. A skipped filter does not process commands, but
   * passes them directly to subsequent filters.
   * 
