...
/*
 * #DOTNET_INCLUDE_BEGIN import jade.core.Profile; import System.Net.*; import System.Net.Sockets.*;
 * #DOTNET_INCLUDE_END
 */
 ...
 * 
 * The server is only running if there are nodes to supervise. By default it waits at port 28000 for
 * incomming UDP datagrams. The maximum time between two ping messages is by default 3 second. After
 * 5 seconds a node is marked for removing from the platform.
 * <p>
 * 
...
class UDPMonitorServer {
  ...
  /*
   * #DOTNET_INCLUDE_BEGIN private Socket server; #DOTNET_INCLUDE_END
   */
      ...
      if (mon != null) {
        synchronized (mon) { // Mutual exclusion with pingReceived()
          if (mon.getDeadlineID() == id) {
            ...
            logger.log(Logger.WARNING, "expired Deadline " + id + " for node " + nodeID
                + " is not the same as monitor Deadline " + mon.getDeadlineID());
          }
        }
      }
    }
  ...
  private class PingHandler implements Runnable {
    ...
    private void handlePing() throws IOException {
      ...
      /*
       * #DOTNET_INCLUDE_BEGIN ubyte[] recData = new ubyte[datagramBuffer.getUByte().length];
       * 
       * if ( server != null) { try { if (server.get_Available() <= 0) return; } catch
       * (System.ObjectDisposedException ode) { return; } } else return;
       * 
       * try { server.Receive(recData, 0, server.get_Available(), SocketFlags.None); } catch
       * (SocketException se) { int socketError = se.get_ErrorCode(); return; } IPEndPoint IPendPt =
       * (IPEndPoint) server.get_LocalEndPoint(); IPAddress address = IPendPt.get_Address();
       * datagramBuffer.copyUByte(recData); #DOTNET_INCLUDE_END
       */
   ...
   */
  UDPMonitorServer(UDPNodeMonitoringService s, String h, boolean alho, int p, int pd, int pdl,
      int ul, int onpc, int mtup, NetworkChecker ch) {
    myService = s;
    host = h;
    ...
    try {
      ...
      /*
       * #DOTNET_INCLUDE_BEGIN server = new Socket(AddressFamily.InterNetwork, SocketType.Dgram,
       * ProtocolType.Udp); #DOTNET_INCLUDE_END
       */
    } catch (Exception e) { // .net requires I catch Exception instead of IOException
  ...
  synchronized void start() throws Exception {
    ...
    try {
      ...
      if (!acceptLocalHostOnly || Profile.isLocalHost(host)) {
        ...
        logger.log(Logger.WARNING, "Cannot bind UDP Server Socket on port " + port
            + ". Let the system select a free one.");
        if (acceptLocalHostOnly) {
    ...
    /*
     * #DOTNET_INCLUDE_BEGIN server = new Socket(AddressFamily.InterNetwork, SocketType.Dgram,
     * ProtocolType.Udp); server.set_Blocking( false );
     * server.SetSocketOption(SocketOptionLevel.Socket, SocketOptionName.ReuseAddress, 1); String
     * defaultNetworkName = Profile.getDefaultNetworkName(); System.Net.IPHostEntry hostEntry =
     * System.Net.Dns.GetHostByName( defaultNetworkName ); System.Net.IPAddress[] ipAddresses =
     * hostEntry.get_AddressList(); long ipAddressLong = ipAddresses[0].get_Address(); if (
     * !server.get_Connected() ) { IPEndPoint ipPoint = new IPEndPoint(IPAddress.Any, port); try {
     * server.Bind( ipPoint ); } catch(SocketException se) { int socketError = se.get_ErrorCode(); }
     * } #DOTNET_INCLUDE_END
     */
  ...
  synchronized void stop() {
    try {
      ...
      /*
       * #DOTNET_INCLUDE_BEGIN server.Shutdown(SocketShutdown.Both); server.Close();
       * #DOTNET_INCLUDE_END
       */
  ...
  /**
   * Registers a <code>UDPNodeFailureMonitor</code>. All nodes targeted by this monitor are now
   * supervised by the <code>UDPMonitorServer</code>. Its listener gets informed about any state
   * changes.
   */
  ...
  /**
   * Deregisters a <code>UDPNodeFailureMonitor</code> and all its targeted nodes from monitoring.
   */
  ...
  /**
   * This method is invoked by a PingHandler thread when a new ping message has been received
   * 
  ...
  private void handleUnknownPing(String nodeID) {
    ...
    if (cnt.getValue() < maxTracedUnknownPings) {
      logger.log(Logger.WARNING,
          "UDP ping message with the unknown node ID '" + nodeID + "' received");
      cnt.increment();
      if (cnt.getValue() == orphanNodePingsCnt) {
        // A node is considered orphan only once after the reception of 10 "unknown
        // pings"
  ...
  protected void timeout(String nodeID, UDPNodeFailureMonitor mon) {
    ...
    if (oldState == UDPNodeFailureMonitor.STATE_CONNECTED) {
      // Try to ping the monitored node explicitly to be sure it is actually
      // disconnected
      try {
        myService.pingNode(nodeID);
        // For some reason we are not receiving PING packets, but the node is alive and
        // reachable.
      ...
      if (checker == null || checker.isNetworkUp()) {
        // Unreachable-limit Expired! If no NetworkChecker is specified or the
        // NetworkChecker says
        // that the network is properly working, consider the monitored node DEAD.
        newState = UDPNodeFailureMonitor.STATE_FINAL;
      } else {
        // Network down --> do not consider the node dead, and add another timer to
        // check again later
        logger.log(Logger.WARNING, "Unreachable limit exceeded for node " + nodeID
            + ", however the network appears to be down --> Give the node another chance");
        addDeadline(nodeID, unreachLimit);
      }
    }
