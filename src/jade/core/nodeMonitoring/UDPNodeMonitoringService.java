...
// Take care that the DOTNET build file (dotnet.xml) uses this file (it is copied just after the
// preprocessor excluded it)
// #J2ME_EXCLUDE_FILE
// #APIDOC_EXCLUDE_FILE
...
public class UDPNodeMonitoringService extends NodeMonitoringService {
  ...
  /**
   * This constant is the name of the property whose value contains the hostname where the Main
   * Container is listening for UDP pings. If not specified the value of the Profile.LOCAL_HOST
   * option is used. If this is null too the default network name is used.
   */
  public static final String HOST = PREFIX + "host";

  /**
   * This constant is the name of the property whose value contains a boolean indication that
   * specifies whether or not UDP pings must be accepted on the indicated local-host only (default =
   * false).
   */
   ...
   * number where the Main Container is listening for UDP pings.
   */
  public static final String PORT = PREFIX + "port";

  /**
   * This constant is the name of the property whose value contains an integer representing the time
   * interval (in milliseconds) in which a peripheral container sends UDP ping messages to the Main
   * Container.<br>
  ...
  /**
   * This constant is the name of the property whose value contains an integer representing the
   * maximum time (in milliseconds) the main container waits for a ping message before considering
   * the peripheral container unreachable.<br>
  ...
  /**
   * This constant is the name of the property whose value contains an integer representing the
   * maximum time a node can stay unreachable after it gets removed from the platform.<br>
   * This property is only meaningful on a main container.
   */
  ...
  /**
   * This constant is the name of the property whose value contains an integer representing the
   * number of UDP ping packets that must be received from an un-monitored node before considering
   * it an orphan node and issuing an Orphan-Node Vertical command.<br>
  ...
  /**
   * This constant is the name of the property whose value contains an integer representing the
   * maximum number of UDP ping packets received from an un-monitored node that are traced.
   * Successive packets from the same un-monitored node will be completely ignored The default for
   * this property is 100.<br>
   * This property is only meaningful on a main container.
   */
  public static final String MAX_TRACED_UNKNOWN_PINGS = PREFIX + "maxtracedunknownpings";

  /**
   * This constants is the name of the property whose value contains the fully qualified class name
   * of a concrete implementation of the <code>NetworkChecker</code> interface. If this property is
   * set, before considering dead a monitored node that remains unreachable for more than the
   * <code>unreachable-limit</code>, the specified <code>NetworkChecker</code> is activated to check
   * if the lack of UDP ping packets depends on a network disconnection. If this is the case the
   * node is kept in the <code>UNREACHABLE</code> state and is not removed.<br>
   * This property is only meaningful on a main container.
   * 
   * @see NetworkChecker
   */
  ...
  /**
   * Vertical command issued on the Main Container when a given number of ping packets are received
   * from an unknown node
   */
  public static final String ORPHAN_NODE = "Orphan-Node";

  private static final String[] OWNED_COMMANDS =
      new String[] {NODE_UNREACHABLE, NODE_REACHABLE, ORPHAN_NODE};
  ...
  public void init(AgentContainer ac, Profile p) throws ProfileException {
    ...
    if (mainContainer != null) {
      ...
      String host = p.getParameter(HOST, p.getParameter(Profile.LOCAL_HOST, Profile
          .getDefaultNetworkName(p.getBooleanProperty(Profile.PRIVILEDGE_LOGICAL_NAME, false))));
      ...
      try {
        myServer = new UDPMonitorServer(this, host, acceptLocalHostOnly, port, pingDelay,
            pingDelayLimit, unreachLimit, orphanNodePingsCnt, maxTracedUnknownPings, checker);
        myServer.start();
        // Port may have changed
        port = myServer.getPort();
        myLogger.log(Logger.INFO,
            "UDPMonitorServer successfully started. Host = " + host + ", port = " + port
                + " pingdelaylimit = " + pingDelayLimit + " unreachablelimit = " + unreachLimit);
      } catch (Exception e) {
  ...
  private NetworkChecker initNetworkChecker(Profile p) {
    ...
    if (networkCheckerClass != null) {
      try {
        ...
        myLogger.log(Logger.WARNING, "NetworkChecker " + networkCheckerClass
            + " cannot be created, instantiated or initialized.", e);
      }
    }
    return checker;
  }
  ...
  /**
   * Extracts an integer value from a given profile. If the value is less than zero it returns the
  ...
  void activateUDP(Node n, long key) {
    if (myServer != null) {
      ...
      try {
        ...
        try {
          slice.activateUDP(myServiceManager.getLocalAddress(), myServer.getHost(),
              myServer.getPort(), myServer.getPingDelay(), key);
        ...
        // Slice is null --> The UDPNodeMonitoringService is not installed on the node
        // to be monitored
        myLogger.log(Logger.WARNING,
            "Can't monitor node " + n.getName() + ". UDPNodeMonitoringService not installed.");
      } catch (ServiceException se) {
        myLogger.log(Logger.WARNING, "Can't monitor node " + n.getName()
            + ". Service error on remote node: " + se.getMessage());
  ...
  void deactivateUDP(Node n, long key) {
    try {
      // Ping the node first to avoid using a stale network connection (in case the
      // remote node is dead).
      // In that case in fact we could wait for a long time before getting the socket
      // exception
      n.ping(false);
      UDPNodeMonitoringSlice slice = (UDPNodeMonitoringSlice) getSlice(n.getName());
      // Note that there can't be caching problems in this case since they were (if
      // present) already solved in activateUDP()
      if (slice != null) {
  ...
  private void startUDPClient(String label, String host, int port, int pingDelay, long key)
    ...
    try {
      ...
      client.start();
      myLogger.log(Logger.INFO, "UDP Monitor Client for " + label + " successfully started. Host = "
          + host + " port = " + port + " pingdelay = " + pingDelay);
    } catch (Exception e) {
      // Rollback
  ...
  void pingNode(String nodeID) throws IMTPException {
    try {
      // This method is invoked by the UDPMonitorServer --> it can only be invoked on
      // a Main Container
  ...
  /**
   * Inner class ServiceComponent A slice is needed to process the H_ACTIVATEUDP and H_DEACTIVATEUDP
   * horizontal commands that deal with UPD Monitor clients activation
   */
  ...
  /**
   * Inner class UDPMonitorIncomingFilter An incoming filter is needed to react to failures of main
   * containers that are monitoring this node by stopping the related UDP Monitor clients. This main
   * container is either the main container a node is attached to or a main container replica if the
   * Main Replication Service is active
   */
