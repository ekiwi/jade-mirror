...
/**
 * The <code>BlockingNodeFailureMonitor</code> class detects node failures and notifies its
 * registered listener using blocking calls to the ping() remote method of the monitored Node.
 *
  ...
  public void run() {
    ...
    while (!nodeExited && !stopped) {
      try {
        ...
        if (myLogger.isLoggable(Logger.INFO)) {
          myLogger.log(Logger.INFO, "PING from node " + target.getName()
              + " exited with exception. " + imtpe1.getMessage());
        }
