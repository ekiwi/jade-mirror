...
/**
 * The <code>UDPNodeFailureMonitor</code> class detects node failures and notifies its registered
 * listener using the UDP protocol.
 * 
...
class UDPNodeFailureMonitor extends NodeFailureMonitor {
  ...
  /**
   * The target is connected. That means that it is sending ping messages regulary.
   */
  public static final int STATE_CONNECTED = 0;

  /**
   * The target is unreachable. The target doesn't send ping messages any more.
   */
  ...
  /**
   * Returns the time when the last ping message has been received from the targeted node
   * 
  ...
  void setState(int newState) {
    ...
    if (logger.isLoggable(Logger.FINEST)) {
      logger.log(Logger.FINEST,
          "Transition to state " + newState + " for node '" + target.getName() + "'");
    }
  ...
  /**
   * Sets the time when the last ping message has been received from the targeted node
   * 
   * @param time the difference, measured in milliseconds, between the current time and midnight,
   *        January 1, 1970 UTC.
   */
