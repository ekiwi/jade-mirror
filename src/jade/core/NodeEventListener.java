...
/**
 * This interface is used to notify the JADE kernel of important events related to the platform
 * nodes.
 * <ul>
 * <li>Node addition.</li>
 * <li>Node removal.</li>
...
public interface NodeEventListener {
  ...
  /**
   * This method is invoked when the local node discovers that a platform node is not reachable
   * anymore. The discovery is due to some IMTP-specific network mechanism, and <em>does not</em>
   * mean that the remote node has failed. A subsequent <code>nodeReachable()</code> call might be
   * issued when the IMTP connection becomes working again.
   * 
  ...
  /**
   * This method is invoked when the local node discovers that a platform node, that was previously
   * unreachable, has become reachable again.
   * 
