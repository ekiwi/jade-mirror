...
/**
 * This class provides a partial implementation of the <code>Node</code> interface. Concrete IMTPs
 * will have to provide a full implementation of the <code>Node</code> interface, possibly by
...
public abstract class BaseNode implements Node, Serializable {
  ...
  /**
   * Provides an IMTP independent implementation for the horizontal command serving mechanism. IMTP
   * dependent implementations of the accept() method should invoke this method.
   * 
   * @param cmd The horizontal command to process.
   * @return The object that is the result of processing the command.
   * @throws ServiceException If the service the command belongs to is not present on this node.
   */
  public Object serveHorizontalCommand(HorizontalCommand cmd) throws ServiceException {
    ...
    if (myLogger.isLoggable(Logger.FINE)) {
      myLogger.log(Logger.FINE, "Node " + myName + " serving incoming H-Command " + commandName
    ...
    if (slice != null) {
      ...
      if (vCmd != null) {
        ...
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE, "Node " + myName + " issuing incoming V-Command "
              + vCmd.getName() + " of Service " + vCmd.getService());
        }
      ...
      if (ret != null) {
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE, "Node " + myName + " return value for incoming H-Command "
              + commandName + " of Service " + serviceName + " = " + ret);
        }
      }
      return ret;
    } else {
      String s =
          "Node " + myName + ": Service " + serviceName + " Unknown. Command = " + commandName;
  ...
  /**
   * Serves an incoming vertical command, locally. This method is invoked if a new
   * <code>VerticalCommand</code> object is generated by a slice targetted by a former
   * <code>HorizontalCommand</code>, which happens if the <code>Slice.serve()</code> yields a
   * non-null result.
   * 
   * This method makes it so that the newly created vertical command is handed to the command
   * processor to first pass through all incoming filters and then to be dispatched to its proper
