...
class MainDetectionManager {
  ...
  private final static String OPT_MCAST_FIRST_TIMEOUT =
      "jade_core_MainDetectionManager_mcastfirsttimeout";
  ...
  private static class MainAddr {
    ...
    public MainAddr(String address) {
      StringTokenizer ast;
      /*
       * Address must be in form "proto:host:port" If address has a bad format, the following code
  ...
  static class MulticastParams {
    ...
    private void checkTrue(boolean condition, String paramName, String paramValue)
    ...
    private int parseInt(String paramName, String paramValue) throws ProfileException {
      try {
        ...
        throw new ProfileException(
            "Bad value \"" + paramValue + "\" for option " + paramName + ": integer value required",
            nfe);
      }
    }
  ...
  private static MainAddr extractAddress(String response, String proto) {
    logger.log(Logger.FINER, "MainDetectionManager::extractAddress(response=\"" + response
        + "\", proto=\"" + proto + "\")");
    MainAddr result = null;

    // parse a list of addresses in form
    // proto_1:hostname_1:port_1;...;proto_n:hostname_n:port_n
    ...
    while (st.hasMoreTokens()) {
      ...
      try {
        ...
        result = ma;
        /*
         * If either no protocol specified or main has an address with desired protocol, then we are
         * done, otherwise, go on searching for the right protocol.
         */
  ...
  private static MainAddr manageGetMainResponses(List responses, String proto) {
    logger.log(Logger.FINER, "MainDetectionManager::manageGetMainResponses(responses.size()="
        + responses.size() + ", proto=\"" + proto + "\")");
    ...
    while (iter.hasNext()) {
      ...
      try {
        ...
        logger.log(Logger.WARNING, "Error managing response \"" + response + "\" from " + senderHost
            + ":" + senderPort + "; response discarded", e);
      }
    }
  ...
  private static List multicAsk(String request, Profile p) throws ProfileException {
    ...
    List result = null;
    /*
     * usually we expect only one Main, but when more are active, we collect all responses and
     * choose the one we like more
     */
    ...
    try {
      ...
      do {
        try {
          logger.log(Logger.FINER, "MainDetectionManager::multicAsk(): sending msg =\"" + request
              + "\" to " + mcast.address + ":" + mcast.port);
          packet = new DatagramPacket(request.getBytes(PROTO_ENCODING), request.length(),
              mcastGroupAddress, mcast.port);
          socket.send(packet);
          ...
          do {
            try {
              ...
              logger.log(Logger.FINER,
                  "MainDetectionManager::multicAsk(): received " + recv.getLength() + " bytes");
              responses.add(recv);
          ...
          --retries;
          logger.log(Logger.FINER,
              "MainDetectionManager::multicAsk(): timeout, " + retries + " retries left");
        }
      } while (retries > 0);
      if (responses.size() > 0) {
  ...
  private static MainAddr getMainAddress(Profile profile) throws ProfileException {
    ...
    if (platformName == null) {
      throw new ProfileException(
          "platform id is mandatory when using automatic main detection; use \""
              + MATCH_ALL_PLATFORMS + "\" to match all");
    }
  ...
  public static MulticastMainDetectionListener createListener(ProfileImpl profile, IMTPManager m)
      throws ProfileException {
