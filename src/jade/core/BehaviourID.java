...
public class BehaviourID implements Concept {
  ...
  /**
   * This constructor builds a new behaviour ID, describing the given behaviour object. The various
   * attributes of the behaviour ID (behaviour name, behaviour class, etc.) are set accordingly.
   * 
   * @param b The <code>Behaviour</code> object that is to be described with this ID.
   */
  ...
  /**
   * Set the class name for this behaviour ID. This is the name of the Java class implementing the
   * described agent behaviour.
   * 
   ...
   * 
   * @return A string describing the kind of behaviour, or <code>null</code> if no kind was set.
   */
  ...
  /**
   * Adds a new behaviour ID as a child of this one. The parent-child relationship between behaviour
   * IDs reflects the one between their described behaviours.
   * 
   ...
   * 
   * @return If the children collection is empty, <code>true</code> is returned, and
  ...
  /**
   * Equality test on two behaviour IDs. They are considered to be equal if and only if their
   * <i>name</i>, <i>className</i> and <i>kind</i> attributed are the same.
   * 
   * @param o The right hand side of the equality test (the left hand one being the current object).
   * @return If the <code>o</code> parameter is a behaviour ID with the same name, class name and
   *         kind of the current object, <code>true</code>. Otherwise, <code>false</code> is
   *         returned.
   */
  public boolean equals(Object o) {
    if (o != null && o instanceof BehaviourID) {
      BehaviourID b = (BehaviourID) o;
      return (checkEquals(name, b.name) && checkEquals(className, b.className)
          && checkEquals(kind, b.kind));
    } else {
      return false;
    }
  }
  ...
  /**
   * Hash code operation, compliant with identity-by-name. This method returns an hash code for a
   * behaviour ID, so that two behaviour IDs with the same name, class name and kind have the same
   * hash code.
   */
  public int hashCode() {
    int result = 0;
  ...
  /**
   * Create a string representation for this behaviour ID. The actual representation is simply the
   * name of the behaviour ID.
   * 
