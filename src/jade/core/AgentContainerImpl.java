...
/**
 * This class is a concrete implementation of the JADE agent container, providing runtime support to
 * JADE agents.
 * 
...
class AgentContainerImpl implements AgentContainer, AgentToolkit {
  ...
  /**
   * Profile option that specifies whether or not all log messages should be produced during the
   * shutdown procedure
   */
  ...
  jade.wrapper.AgentContainer getContainerController() {
    return getContainerController(myNodeDescriptor.getOwnerPrincipal(),
        myNodeDescriptor.getOwnerCredentials());
  }

  public jade.wrapper.AgentContainer getContainerController(JADEPrincipal principal,
      Credentials credentials) {
    return new jade.wrapper.AgentContainer(getContainerProxy(principal, credentials), this,
        getPlatformID());
  }

  /**
   * Return a proxy that allows making requests to the local container as if they were received from
   * the main. This allows dealing uniformly with local and remote requests. Local requests occurs
   * at bootstrap and following calls to the in-process interface.
   */
  private jade.wrapper.ContainerProxy getContainerProxy(final JADEPrincipal principal,
    ...
    return new jade.wrapper.ContainerProxy() {
      ...
      public void createAgent(AID id, String className, Object[] args) throws Throwable {
        ...
        AgentManagementSlice target =
            (AgentManagementSlice) getProxyToLocalSlice(AgentManagementSlice.NAME);
        target.createAgent(id, className, args, principal, null, AgentManagementSlice.CREATE_ONLY,
            dummyCmd);
      }
      ...
      public MTPDescriptor installMTP(String address, String className) throws Throwable {
        // Do as if it was a remote call from the main to allow
        // security checks to take place if needed
        jade.core.messaging.MessagingSlice target =
            (jade.core.messaging.MessagingSlice) getProxyToLocalSlice(
      ...
      public void suspendAgent(AID id) throws Throwable {
        ...
        jade.core.management.AgentManagementSlice target =
            (jade.core.management.AgentManagementSlice) getProxyToLocalSlice(
      ...
      public void moveAgent(AID id, Location where) throws Throwable {
        // Do as if it was a remote call from the main to allow
        // security checks to take place if needed
        jade.core.mobility.AgentMobilitySlice target =
            (jade.core.mobility.AgentMobilitySlice) getProxyToLocalSlice(
      ...
      private SliceProxy getProxyToLocalSlice(String serviceName) throws Throwable {
        ...
        return (SliceProxy) myIMTPManager.createSliceProxy(serviceName,
            svc.getHorizontalInterface(), myIMTPManager.getLocalNode());
      }
    };
  }
  ////////////////////////////////////////////////////////
  ...
  /**
   * Issue an INFORM_CREATED vertical command. Note that the Principal, if any, is that of the owner
   * of the newly born agent, while the Credentials, if any, are a set additional initial
   * credentials to be attached to the newly born agent. The SecurityService, if active, will create
   * a new Principal for the newly born agent and will initialize its credentials as the union of
   * the initial credential and the ownership certificate.
   */
  public void initAgent(AID agentID, Agent instance, JADEPrincipal ownerPrincipal,
      Credentials initialCredentials)
      throws NameClashException, IMTPException, NotFoundException, JADESecurityException {

    // Replaces wildcards
    agentID.setName(
        JADEManagementOntology.adjustAgentName(agentID.getName(), new String[] {myID.getName()}));
    ...
    GenericCommand cmd =
        new GenericCommand(jade.core.management.AgentManagementSlice.INFORM_CREATED,
    ...
    if (ret != null) {
      if (ret instanceof NameClashException) {
        ...
        throw new IMTPException("Unexpected error initializing agent " + agentID.getName(),
            (Throwable) ret);
      }
    }
  }
  ...
  protected void init() throws IMTPException, ProfileException {
    ...
    try {
      ...
      if (myProfile.getBooleanProperty(Profile.DETECT_MAIN, true)
          && ((ProfileImpl) myProfile).isMain()) {
        try {
          mainDetectionListener =
              MainDetectionManager.createListener((ProfileImpl) myProfile, myIMTPManager);
        } catch (ProfileException pe) {
          if ("true"
              .equalsIgnoreCase(myProfile.getBootProperties().getProperty(Profile.DETECT_MAIN))) {
            ...
            myLogger.log(Logger.WARNING,
                "Automatic main-detection mechanism initialization failed (" + pe.getMessage()
                    + "). Mechanism disabled!");
          }
        }
      }
    ...
    myID = new ContainerID(myProfile.getParameter(Profile.CONTAINER_NAME, PlatformManager.NO_NAME),
        addr);
   ...
   */
  protected void startNode() throws IMTPException, ProfileException, ServiceException,
      JADESecurityException, NotFoundException {
  ...
  void initMandatoryServices(List services) throws ServiceException {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN dsc = startService("jade.core.messaging.LightMessagingService", false);
     * #MIDP_INCLUDE_END
     */
    dsc.setMandatory(true);
    services.add(dsc);
  ...
  boolean joinPlatform() {
    ...
    try {
      ...
      myLogger.log(Logger.SEVERE,
          "Communication failure while joining agent platform: " + imtpe.getMessage());
      ...
      myLogger.log(Logger.SEVERE,
          "Authentication or authorization failure while joining agent platform.");
      ae.printStackTrace();
      endContainer();
      cleanIMTPManager();
      return false;
  ...
  private void checkLocalHostAddress() {
    ...
    if (address.equals(Profile.LOCALHOST_CONSTANT)
        || address.equals(Profile.LOOPBACK_ADDRESS_CONSTANT)) {
      myLogger.log(Logger.WARNING,
    ...
    try {
      ...
      while (agentSpecifiers.hasNext()) {
        ...
        if (s.getName() != null) {
          ...
          try {
            // #MIDP_EXCLUDE_BEGING
            getContainerProxy(myNodeDescriptor.getOwnerPrincipal(),
                myNodeDescriptor.getOwnerCredentials()).createAgent(agentID, s.getClassName(),
                    s.getArgs());
            // #MIDP_EXCLUDE_END
            /*
             * #MIDP_INCLUDE_BEGIN String serviceName =
             * jade.core.management.AgentManagementSlice.NAME; Service svc =
             * myServiceFinder.findService(serviceName); jade.core.management.AgentManagementSlice
             * target = (jade.core.management.AgentManagementSlice)
             * myIMTPManager.createSliceProxy(serviceName, svc.getHorizontalInterface(),
             * myIMTPManager.getLocalNode()); GenericCommand dummyCmd = new GenericCommand(null,
             * null, null); dummyCmd.setPrincipal(myNodeDescriptor.getOwnerPrincipal());
            ...
            myLogger.log(Logger.SEVERE,
                "Cannot create agent " + s.getName() + ": " + t.getMessage());
          }
        } else {
          myLogger.log(Logger.WARNING,
              "Cannot create an agent with no name. Class was " + s.getClassName());
        }
      }
      ...
      if (myProfile.getBooleanProperty(ENABLE_MONITOR, true)) {
        ...
        try {
          getContainerProxy(myNodeDescriptor.getOwnerPrincipal(),
              myNodeDescriptor.getOwnerCredentials()).createAgent(monitorId, MONITOR_AGENT_CLASS,
                  new Object[] {this, localAgents});
  ...
  public void handleBehaviourAdded(AID agentID, Behaviour b) {
    GenericCommand cmd =
        new GenericCommand(jade.core.event.NotificationSlice.NOTIFY_BEHAVIOUR_ADDED,
            jade.core.event.NotificationSlice.NAME, null);
    cmd.addParam(agentID);
    cmd.addParam(b);
    // No security check is meaningful on this action --> don't even set the
  ...
  public void handleBehaviourRemoved(AID agentID, Behaviour b) {
    GenericCommand cmd =
        new GenericCommand(jade.core.event.NotificationSlice.NOTIFY_BEHAVIOUR_REMOVED,
            jade.core.event.NotificationSlice.NAME, null);
    cmd.addParam(agentID);
    cmd.addParam(b);
    // No security check is meaningful on this action --> don't even set the
  ...
  public void handleChangeBehaviourState(AID agentID, Behaviour b, String from, String to) {
    GenericCommand cmd =
        new GenericCommand(jade.core.event.NotificationSlice.NOTIFY_CHANGED_BEHAVIOUR_STATE,
            jade.core.event.NotificationSlice.NAME, null);
    cmd.addParam(agentID);
    cmd.addParam(b);
    cmd.addParam(from);
  ...
  // #MIDP_EXCLUDE_BEGIN
  // FIXME: to be removed
  public void handleChangedAgentPrincipal(AID agentID, JADEPrincipal oldPrincipal,
      Credentials creds) {

    /***
     * 
     * myNotificationManager.fireEvent(NotificationManager.CHANGED_AGENT_PRINCIPAL, new
     * Object[]{agentID, oldPrincipal,
     ...
     * re.printStackTrace(); } catch (NotFoundException nfe) { nfe.printStackTrace(); }
     * 
     ***/
  }
  // #MIDP_EXCLUDE_END
  ...
  public void handleChangedAgentState(AID agentID, int oldState, int newState) {
    ...
    GenericCommand cmd =
        new GenericCommand(jade.core.management.AgentManagementSlice.INFORM_STATE_CHANGED,
            jade.core.management.AgentManagementSlice.NAME, null);
    cmd.addParam(agentID);
    cmd.addParam(from);
    cmd.addParam(to);
  ...
  public void setPlatformAddresses(AID id) {
    GenericCommand cmd =
        new GenericCommand(jade.core.messaging.MessagingSlice.SET_PLATFORM_ADDRESSES,
            jade.core.messaging.MessagingSlice.NAME, null);
    cmd.addParam(id);
    // No security check is meaningful on this action --> don't even set the
    // Credentials
  ...
  public void powerUpLocalAgent(AID agentID) throws NotFoundException {
    ...
    int type =
        (agentID.equals(theAMS) || agentID.equals(theDefaultDF) ? ResourceManager.SYSTEM_AGENTS
  ...
  protected ServiceDescriptor startService(String name, boolean activateIt)
