 ...
 * that are used to monitor the availability of nodes and detect node failures. In addition to that
 * it provides static methods to initialize and create instances of monitors depending on the
 * current settings in the profile.
 * 
 * An instance of a subclass of the <code>NodeFailureMonitor</code> can only supervise a single
 * node. If there are additional nodes in the same JVM, you can add these as child nodes. A child
 * node is not supervised directly. Instead it has always the same state than its parent node. So if
 * the parent node gets unreachable automatically all its child nodes will turn to the state
 * unreachable.
 * 
 * <p>
 * Since JADE 3.3 there are two different types of failure monitoring available:
 * <ul>
 * <li>Blocking failure monitoring based on RMI (Default)</li>
 * <li>UDP based failure monitoring</li>
 * </ul>
 * <p>
 * 
 ...
 * <td>If <code>true</code>, indicates that the UDP based failure monitoring has to be used. If
 * <code>false</code> or not specified the default RMI based failure monitoring is activated.</td>
 * <td>X</td>
 * <td>X</td>
 * </tr>
 * <tr>
 * <td><code>jade_core_NodeFailureMonitor_udp-port</code></td>
 * <td>Specifies the port number where the main container will listen for UDP pings. The default
 * value is <code>28000</code>. (This parameter is only used in combination with (*)</td>
 * <td>X</td>
 * <td>X</td>
 * </tr>
 * <tr>
 * <td><code>jade_core_NodeFailureMonitor_udp-ping-delay-limit</code></td>
 * <td>Defines the maximum time (in milliseconds) the main container will wait for incoming ping
 * messages. The default value is <code>3000</code>. (This parameter is only used in combination
 * with (*)</td>
 * <td>&nbsp;</td>
 * <td>X</td>
 * </tr>
 ...
 * <td>Defines the maximum time (in milliseconds) a node can be temporarily unreachable until it
 * gets removed from the platform. The default value is <code>10.000</code>. (This parameter is only
 * used in combination with (*)</td>
 * <td>&nbsp;</td>
 * <td>X</td>
 * </tr>
 * <tr>
 * <td><code>jade_core_NodeFailureMonitor_udp-ping-delay</code></td>
 * <td>Defines the time interval (in milliseconds) in which a peripheral container sends UDP ping
 * messages to the main container. The default value is <code>1.000</code>. (This parameter is only
 * used in combination with (*)</td>
 * <td>X</td>
 * <td>&nbsp;</td>
 * </tr>
...
public abstract class NodeFailureMonitor {
  ...
  private void issueNodeReachable(Node n) {
    if (theMonitoringService != null) {
      GenericCommand cmd = new GenericCommand(NodeMonitoringService.NODE_REACHABLE,
          theMonitoringService.getName(), null);
      cmd.addParam(n);
      try {
  ...
  private void issueNodeUnreachable(Node n) {
    if (theMonitoringService != null) {
      GenericCommand cmd = new GenericCommand(NodeMonitoringService.NODE_UNREACHABLE,
          theMonitoringService.getName(), null);
      cmd.addParam(n);
      try {
        theMonitoringService.submit(cmd);
  ...
  public static NodeFailureMonitor getDefaultFailureMonitor() {
    try {
      return (NodeFailureMonitor) Class
          .forName("jade.core.nodeMonitoring.BlockingNodeFailureMonitor").newInstance();
    } catch (Throwable t) {
      // Should never happen
      t.printStackTrace();
      return null;
