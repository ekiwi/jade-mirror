...
public class AgentReplicationHandle {
  ...
  /**
   * Utility method to be called at the beginning of each replicated method as exemplified below<br>
   * <code>
  ...
  public static void replicate(Agent a, String methodName, Object[] arguments) {
    if (isReplicatedCall()) {
      ...
      // AgentReplicationService.ServiceComponent.invokeAgentMethod()) --> just do
      // nothing
      return;
    }
    try {
      AgentReplicationHelper helper =
          (AgentReplicationHelper) a.getHelper(AgentReplicationHelper.SERVICE_NAME);
      helper.invokeReplicatedMethod(methodName, arguments);
    } catch (ServiceNotActiveException snae) {
      // AgentReplicationService not installed --> For sure this agent is not
      // replicated --> do nothing
    } catch (Exception e) {
      myLogger.log(Logger.WARNING,
          "Error accessing replication helper of agent " + a.getLocalName(), e);
    }
  }
