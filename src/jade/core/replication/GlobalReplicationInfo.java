...
class GlobalReplicationInfo implements Serializable {
  ...
  /**
   * Remove the current master replica Select a new one and return it or null if no replica is
   * available
   */
  synchronized AID masterReplicaDead() {
    ...
    if (allReplicas.size() == 0) {
      ...
      myLogger.log(Logger.INFO, "New master replica " + masterAid.getLocalName()
          + " selected for virtual agent " + virtualAid.getLocalName());
    }
    return masterAid;
  }
  ...
  synchronized void masterReplicaChanged(AID newMasterAid) {
    ...
    myLogger.log(Logger.INFO, "New master replica " + masterAid.getLocalName()
        + " set for virtual agent " + virtualAid.getLocalName());
  }
}
