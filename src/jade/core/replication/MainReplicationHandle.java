...
/**
 * This class allows other services to exploit the MainReplicationService to keep local information
 * in synch among slices on replicated Main Containers
 */
public class MainReplicationHandle {
  ...
  public void invokeReplicatedMethod(String methodName, Object[] params) {
    ...
    if (replicationService != null) {
      GenericCommand cmd = new GenericCommand(MainReplicationSlice.H_INVOKESERVICEMETHOD,
      ...
      try {
        ...
        // Should never happen as real exceptions are logged inside
        // broadcastToReplicas()
        myLogger.log(Logger.WARNING, "Error propagating H-command " + cmd.getName()
            + " to replicas. Method to invoke on replicas was " + methodName, e);
      }
    }
    // #J2ME_EXCLUDE_END
  }
