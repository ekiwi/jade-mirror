...
public interface AgentReplicationSlice extends Slice {
  ...
  // NOTE: These HCommands are always broadcasted --> no need to a dedicated
  // method
  ...
  void invokeAgentMethod(AID aid, String methodName, Object[] arguments)
  ...
  // This is used to notify a slice that a replica of a given virtual agent is
  // going to
  // be created there. This is necessary since the newly created replica agent may
  // access its AgentReplicationHelper right in the afterClone() method and, at
  // that time,
  // the slice of the new replica may not have been notified yet about the
  // new-replica --> virtual-agent correspondence. This in facts is done by the
  // master
  // replica slice only after the successful completion of the master cloning
  // process.
