...
public class AgentReplicationService extends BaseService {
  ...
  private Map<AID, GlobalReplicationInfo> globalReplications =
      new Hashtable<AID, GlobalReplicationInfo>();
  ...
  private Map<AID, List<ReplicaInfo>> pendingReplicaCreationRequests =
      new Hashtable<AID, List<ReplicaInfo>>();
  ...
  public void boot(Profile p) throws ServiceException {
    super.boot(p);
    try {
      // Initialize the MessagingService. No need to check if it is != null since it
      // is mandatory
      theMessagingService =
          (MessagingService) myContainer.getServiceFinder().findService(MessagingService.NAME);
    ...
    try {
      ...
      if (myContainer.getServiceFinder().findService(AgentMobilityService.NAME) == null) {
        throw new ServiceException(
            "AgentMobilityService not installed. AgentReplicationService cannot work properly");
      }
    } catch (IMTPException imtpe) {
      // Should never happen since this is a local call
      throw new ServiceException("Cannot retrieve the local MessagingService.", imtpe);
  ...
  private class AgentReplicationHelperImpl implements AgentReplicationHelper {
    ...
    public void init(Agent a) {
      ...
      // If the agent retrieving this helper is already a replica, initialize
      // virtualAid
          ...
          for (AID replica : currentReplicas) {
            ...
            if (!replica.equals(myAid)) {
              try {
                ...
                myLogger.log(Logger.WARNING, "Replica " + replica.getLocalName()
                    + " not found. Likely it died in the meanwhile");
              } catch (Exception e) {
                myLogger.log(Logger.SEVERE,
                    "Error retrieving location for agent " + replica.getLocalName(), e);
              }
            }
          }
        } else {
          myLogger.log(Logger.SEVERE, "Virtual agent " + virtualAid.getLocalName()
              + " for replica agent " + myAid.getLocalName() + " not found");
        }
      }
    }
    ...
    public AID makeVirtual(String virtualName, int replicationMode) throws ServiceException {
      if (virtualAid == null) {
        ...
        // Immediately release the agent: AMSService.register() requires that the agent
        // receives the AMS reply
        ...
        throw new ServiceException(
            "Agent " + myAid.getLocalName() + " has already been made virtual");
      }
    }
      ...
      if (virtualAid != null) {
        if (isMaster()) {
          ...
          if (slice != null) {
            ...
            AID replicaAid =
                new AID(AID.createGUID(replicaName, myContainer.getPlatformID()), AID.ISGUID);
            try {
            ...
            // If there are no other ongoing replica creation processes --> directly clone
            // the (master) agent.
            if (rr.size() == 1) {
            ...
            throw new ServiceException(
                "AgentReplicationService not installed in the destination container "
                    + where.getName() + " for replica " + replicaName);
          }
        } else {
          throw new ServiceException(
              "Agent " + myAid.getLocalName() + " is not the master replica");
        }
      } else {
    ...
    public void invokeReplicatedMethod(String methodName, Object[] arguments) {
      ...
      myLogger.log(Logger.FINE,
          "Invoking method " + methodName + " on " + tmp.length + " replica(s)");
      for (ReplicaInfo r : tmp) {
        try {
          ...
          myLogger.log(Logger.SEVERE, "Error propagating call to method " + methodName
              + " to agent " + r.replicaAid.getLocalName(), e);
        }
      }
    }

    private boolean invokeOnReplica(String methodName, Object[] arguments, ReplicaInfo r)
        throws Exception {
      myLogger.log(Logger.FINER,
          "Invoking method " + methodName + " on replica " + r.replicaAid.getLocalName());
      ...
      // If not found in Main Container --> Ignore: replica has terminated in the
      // meanwhile
      do {
        ...
        try {
          myLogger.log(Logger.CONFIG,
              "Updating location of replica " + r.replicaAid.getLocalName());
          Location l = getLocation(r.replicaAid);
          // The replica agent is alive: update its location and retry
          r.where = l;
        } catch (NotFoundException nfe1) {
          // The replica agent does not exist anymore in the whole platform --> silently
          // remove it
          return false;
        }
      } while (true);
    ...
    private synchronized void addPeerReplica(ReplicaInfo r) {
      if (!peerReplicas.contains(r)) {
        myLogger.log(Logger.CONFIG, "Adding replica " + r.replicaAid.getLocalName()
            + " to Helper of agent " + myAid.getLocalName());
        peerReplicas.add(r);
    ...
    private synchronized void removePeerReplica(ReplicaInfo r) {
      if (peerReplicas.remove(r)) {
        myLogger.log(Logger.CONFIG, "Removing replica " + r.replicaAid.getLocalName()
            + " from Helper of agent " + myAid.getLocalName());
        peerReplicasArray = peerReplicas.toArray(new ReplicaInfo[0]);
      }
    }
  ...
  private class CommandOutgoingFilter extends Filter {
    public CommandOutgoingFilter() {
      super();
      setPreferredPosition(2); // Before the Messaging (encoding) filter and the security related
                               // ones
    }
    ...
    public final boolean accept(VerticalCommand cmd) {
      ...
      if (name.equals(MessagingSlice.SEND_MESSAGE)) {
        ...
        if (info != null) {
          // Receiver is a virtual AID --> Redirect the SEND_MESSAGE command to one of the
          // implementation replicas
          ...
          // In case the selected replica is no longer valid, the message will have to be
          // delivered
          // again to another replica --> instruct JADE not to clear the message content
          // (see
          // jade.core.messaging.OutBox.addLast())
          gMsg.setModifiable(false);
          ACLMessage msg = gMsg.getACLMessage();
          if (msg != null) {
            msg.addUserDefinedParameter(AgentReplicationHelper.VIRTUAL_RECEIVER,
        ...
        if (msg != null) {
          ...
          if (virtualName != null) {
            ...
            AID virtualAid =
                new AID(AID.createGUID(virtualName, myContainer.getPlatformID()), AID.ISGUID);
            ...
            if (info != null) {
              ...
              myLogger.log(Logger.FINE,
                  "Redirecting message " + ACLMessage.getPerformative(msg.getPerformative()) + "["
                      + msg.getContent() + "] from dirty replica " + receiver.getLocalName()
                      + " to new replica " + newReplica.getLocalName());
      ...
      if (cmd.getName().equals(AgentMobilityHelper.INFORM_CLONED)) {
        ...
        if (rr != null) {
          ...
          if (r.where.equals(where) && r.replicaAid.getLocalName().equals(newName)) {
            ...
            // Finally, if there are other pending replica creation requests, serve the next
            // one
            if (rr.size() > 0) {
              ReplicaInfo nextR = rr.get(0);
              // In this very moment the agent state is already AP_COPY --> Cloning the agent
  ...
  private class CommandIncomingFilter extends Filter {
    ...
    public final boolean accept(VerticalCommand cmd) {
      ...
      if (myContainer.getMain() != null) {
        if (name.equals(AgentManagementSlice.INFORM_KILLED)) {
          // If the dead agent is a master replica of a virtual agent, select a new master
          // replica and broadcast the information
          AID deadAgent = (AID) cmd.getParam(0);
          handleInformKilled(deadAgent);
        } else if (name.equals(Service.NEW_SLICE)) {
          // If the new slice is an AgentReplicationSlice, notify it about the current
          // virtual agents
    ...
    private void handleNewSlice(String newSliceName) {
      try {
        ...
        GlobalReplicationInfo[] allInfos =
            globalReplications.values().toArray(new GlobalReplicationInfo[0]);
        ...
        myLogger.log(Logger.WARNING,
            "Error notifying new slice " + newSliceName + " about current replication information",
            t);
      }
    }
    ...
    private void handleReattached() {
      try {
        ...
        // Notify the new Main Slice all replication information related to virtual
        // agents
        // for which an implementation replica lives in the local container.
        // NOTE that the same replication information can be notified to the new Main
        // Slice
            ...
            if (virtualAid != null && !vv.contains(virtualAid)) {
              ...
              if (info != null) {
                try {
                  ...
                  myLogger.log(Logger.WARNING,
                      "Error notifying main slice about current local replication information", e);
                }
              }
              vv.add(virtualAid);
            }
  ...
  private void localNotifyReplicaAddedToMaster(AID masterAid, ReplicaInfo r) {
    ...
    if (agent != null) {
      ...
      try {
        ...
        myLogger.log(Logger.WARNING, "Unexpected exception notifying master agent (replicaAdded())",
            e);
      }
    }
  }
  ...
  private void notifyReplicaRemovedToMaster(AID masterAid, AID removedReplica, Location where) {
    try {
      ...
      // Should never happen as this masterAid has just been selected (and checked) as
      // new master
      myLogger.log(Logger.WARNING, "Error notifying master replica " + masterAid.getLocalName()
          + " that replica " + removedReplica.getLocalName() + " has been removed", e);
    }
  }

  private void localNotifyReplicaRemovedToMaster(AID masterAid, AID removedReplica,
      Location where) {
    Agent agent = myContainer.acquireLocalAgent(masterAid);
    if (agent != null) {
      myContainer.releaseLocalAgent(masterAid);
      try {
        ...
        myLogger.log(Logger.WARNING, "Unexpected exception notifying master agent (becomeMaster())",
            e);
      }
    }
  }
  ...
  private void notifyBecomeMasterToMaster(AID masterAid) {
    try {
      ...
      // Should never happen as this masterAid has just been selected (and checked) as
      // new master
      myLogger.log(Logger.WARNING, "Error notifying new master replica " + masterAid.getLocalName()
          + " it just took the leadership", e);
    }
  }
  ...
  private AID getVirtualAid(AID aid) {
    ...
    if (agent != null) {
      try {
        ...
        myLogger.log(Logger.WARNING,
            "Unexpected error retrieving AgentReplicationHelper for agent " + aid.getName());
        return null;
      } finally {
  ...
  private void broadcastNewVirtualAgent(AID virtualAid, AID masterAid, int replicationMode) {
    ...
    try {
      ...
      myLogger.log(Logger.SEVERE,
          "Error broadcasting new virtual agent " + virtualAid.getLocalName(), e);
    }
  }
  ...
  private void broadcastAddReplica(AID masterAid, ReplicaInfo r) {
    ...
    if (virtualAid != null) {
      myLogger.log(Logger.CONFIG, "Broadcasting new replica " + r.replicaAid.getLocalName()
          + " of virtual agent " + virtualAid.getLocalName());
      ...
      try {
        ...
        myLogger.log(Logger.SEVERE, "Error broadcasting new replica " + r.replicaAid.getLocalName()
            + " of virtual agent " + virtualAid.getLocalName(), e);
      }
    } else {
      myLogger.log(Logger.WARNING,
          "Cannot find virtual agent for master replica " + masterAid.getLocalName());
    }
  }

  private void broadcastMasterReplicaChanged(AID virtualAid, AID newMasterAid) {
    myLogger.log(Logger.CONFIG, "Broadcasting master replica changed for virtual agent "
        + virtualAid.getLocalName() + ". New master replica = " + newMasterAid.getLocalName());
    GenericCommand cmd =
        new GenericCommand(AgentReplicationSlice.H_MASTERREPLICACHANGED, NAME, null);
    cmd.addParam(virtualAid);
    cmd.addParam(newMasterAid);

    try {
      broadcast(cmd, false); // Do NOT include myself since the change in the local container has
                             // already
                             // been done
    } catch (Exception e) {
      myLogger.log(Logger.WARNING, "Error broadcasting master replica changed for virtual agent "
          + virtualAid.getLocalName(), e);
    }
  }

  private void broadcastVirtualAgentDead(AID virtualAid) {
    myLogger.log(Logger.CONFIG,
        "Broadcasting virtual agent " + virtualAid.getLocalName() + " dead");
  ...
  private void cloneReplica(AID aid, String replicaName, Location where) {
    Agent agent = myContainer.acquireLocalAgent(aid);
    if (agent != null) {
      myLogger.log(Logger.CONFIG, "Cloning agent " + aid.getLocalName() + " to create replica "
          + replicaName + " on container " + where.getName());
  ...
  private void asynchCloneReplica(AID aid, final String replicaName, final Location where) {
    ...
    if (agent != null) {
      agent.addBehaviour(new OneShotBehaviour(agent) {
        @Override
        public void action() {
          myLogger.log(Logger.CONFIG, "Cloning agent " + myAgent.getLocalName()
              + " to create replica " + replicaName + " on container " + where.getName());
  ...
  private final void sendMessage(AID sender, GenericMessage gMsg, AID receiver) {
    GenericCommand cmd =
        new GenericCommand(MessagingSlice.SEND_MESSAGE, MessagingService.NAME, null);
    cmd.addParam(sender);
    cmd.addParam(gMsg);
  ...
  private void invokeAgentMethod(AID aid, String methodName, Object[] arguments)
    ...
    if (agent != null) {
      ...
      try {
        ...
        if (myLogger.isLoggable(Logger.FINE)) {
          myLogger.log(Logger.FINE,
              "Invoking replicated method " + methodName + " on agent " + aid.getLocalName());
        }
        AgentReplicationHandle.enterReplicatedCall();
        m.invoke(agent, arguments);
      } catch (NoSuchMethodException nsme) {
        throw new ServiceException("Method " + methodName + " not found in class "
            + agent.getClass().getName() + " of agent " + agent.getLocalName());
      } catch (IllegalAccessException iae) {
        throw new ServiceException(
            "Method " + methodName + " of class " + agent.getClass().getName() + " of agent "
                + agent.getLocalName() + " cannot be accessed");
      } catch (InvocationTargetException ite) {
        throw new ServiceException(
            "Exception excecuting method " + methodName + " of agent " + aid.getLocalName(),
            ite.getCause());
      } finally {
  ...
  private void addReplica(AID virtualAid, AID replicaAid, Location where) throws Exception {
    myLogger.log(Logger.CONFIG,
        "Received new replica information: virtual=" + virtualAid.getLocalName() + ", replica="
            + replicaAid.getLocalName() + ", location=" + where.getName());

    addReplicaVirtualMapping(replicaAid, virtualAid);

    GlobalReplicationInfo info = globalReplications.get(virtualAid);
    if (info != null) {
      // If some of the current replicas live in the local container, update the
      // related helper
      AID[] currentReplicas = info.getAllReplicas();
      for (int i = 0; i < currentReplicas.length; ++i) {
        ...
        if (agent != null) {
          try {
            ...
            // Should never happen since the AgentReplicationService is certainly installed
            // in this container
            myLogger.log(Logger.WARNING,
                "Unexpected error retrieving AgentReplicationHelper for agent " + aid.getName(),
                se);
      ...
      // Then add the new replica. In this way the helper of the new replica will
      // never
      // be updated with the information of the new replica itself
      info.addReplica(replicaAid);
    } else {
      myLogger.log(Logger.WARNING, "Global Replication information for virtual agent "
          + virtualAid.getLocalName() + " not found in container " + myContainer.getID().getName());
    }
  }
  ...
  private void removeReplica(AID virtualAid, AID replicaAid) {
    ...
    if (info != null) {
      ...
      // If some of the remaining replicas live in the local container, update the
      // related helper
      AID[] currentReplicas = info.getAllReplicas();
      for (int i = 0; i < currentReplicas.length; ++i) {
        AID aid = currentReplicas[i];
        ...
        if (agent != null) {
          try {
            ...
            // in this container
            myLogger.log(Logger.WARNING,
                "Unexpected error retrieving AgentReplicationHelper for agent "
                    + aid.getLocalName(),
                se);
          } finally {
            myContainer.releaseLocalAgent(aid);
  ...
  private void addReplicaVirtualMapping(AID replicaAid, AID virtualAid) {
    ...
    if (oldVirtualAid == null || !oldVirtualAid.equals(virtualAid)) {
      myLogger.log(Logger.CONFIG, "Added replica-to-virtual mapping: " + replicaAid.getLocalName()
  ...
  private void removeReplicaVirtualMapping(AID replicaAid) {
    ...
    if (virtualAid != null) {
      myLogger.log(Logger.CONFIG, "Removed replica-to-virtual mapping: " + replicaAid.getLocalName()
          + "-->" + virtualAid.getLocalName());
    }
  }

  private GlobalReplicationInfo newVirtualAgent(AID virtualAid, AID masterAid, int replicationMode)
      throws Exception {
    GlobalReplicationInfo info = null;
    // In the reattach procedure this method can be called in parallel for the same
    // virtual agent
    synchronized (globalReplications) {
      info = globalReplications.get(virtualAid);
      if (info == null) {
        myLogger.log(Logger.CONFIG, "New virtual agent: virtual=" + virtualAid.getLocalName()
            + ", master=" + masterAid.getLocalName());
        ...
        if (!masterAid.equals(info.getMaster())) {
          throw new ServiceException("Inconsistent replication information for virtual agent "
              + virtualAid.getLocalName() + ": current-master = " + info.getMaster().getLocalName()
              + ", new-master = " + masterAid.getLocalName());
        }
      }
    }
  ...
  private void handleMasterReplicaDead(GlobalReplicationInfo info) {
    // No need to check if impl is null since this can only be executed on a Main
    // Container
    MainContainer impl = myContainer.getMain();
    do {
      AID newMasterAid = info.masterReplicaDead();
      if (newMasterAid != null) {
        // New master replica selected. Check if it is actually alive. Otherwise try
        // again
  ...
  private void checkAllReplications() {
    ...
    for (GlobalReplicationInfo info : gg) {
      ...
      if (impl.acquireAgentDescriptor(masterAid) != null) {
        ...
        myLogger.log(Logger.INFO, "Master replica " + masterAid.getLocalName()
            + " of virtual agent " + info.getVirtual().getLocalName() + " ALIVE");
      } else {
      ...
      for (AID replicaAid : allReplicas) {
        if (!replicaAid.equals(masterAid)) {
          if (impl.acquireAgentDescriptor(replicaAid) != null) {
            ...
            myLogger.log(Logger.INFO, "Replica " + replicaAid.getLocalName() + " of virtual agent "
                + info.getVirtual().getLocalName() + " ALIVE");
          } else {
  ...
  private Method getMethod(Agent agent, String methodName) throws NoSuchMethodException {
    ...
    if (m == null) {
      // NOTE: We cannot use Class.getMethod() since we have parameter values, but not
      // parameter types
