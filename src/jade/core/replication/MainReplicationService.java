...
/**
 * A kernel-level service to manage a ring of Main Containers, keeping the various replicas in sync
 * and providing failure detection and recovery to make JADE tolerate Main Container crashes.
 * 
...
public class MainReplicationService extends BaseService {
  ...
  public static final String SNAPSHOT_ON_FAILURE =
      "jade_core_replication_MainReplicationService_snapshotonfailure";
  ...
  private static final String[] OWNED_COMMANDS =
      new String[] {MainReplicationSlice.LEADERSHIP_ACQUIRED};
  ...
  public void boot(Profile p) throws ServiceException {
    try {
      ...
      for (int i = 0; i < slices.length; i++) {
        try {
          ...
          if (!sliceName.equals(localNodeName)) {
            slice.addReplica(localNodeName, myPlatformManager.getLocalAddress(), myLabel, dsc,
                localServices);
          }
      ...
      if (myLabel > 0) {
        myLogger.log(Logger.INFO, "Main container ring re-arranged: label = " + myLabel
            + " monitored label = " + localSlice.monitoredLabel);
      }
    } catch (IMTPException imtpe) {
  ...
  /**
   * This method is invoked when the monitored main node remains unreachable for too long and is
   * therefore considered dead. The default implementation simply returns REMOVE_NODE so that the
   * node is removed. Subclasses may redefine this method to implement application-specific checks
   * aimed at detecting whether the unreachability depends on a real death or on temporary network
   * problems and returns different actions according to the situation.
   * 
  ...
  /**
   * Inner class CommandOutgoingFilter Keep tool agents information in synch among replicas
   */
  private class CommandOutgoingFilter extends Filter {
    ...
    private void handleNewTool(VerticalCommand cmd) throws IMTPException, ServiceException {
      ...
      GenericCommand hCmd =
          new GenericCommand(MainReplicationSlice.H_NEWTOOL, MainReplicationSlice.NAME, null);
      hCmd.addParam(tool);
    ...
    private void handleDeadTool(VerticalCommand cmd) throws IMTPException, ServiceException {
      ...
      GenericCommand hCmd =
          new GenericCommand(MainReplicationSlice.H_DEADTOOL, MainReplicationSlice.NAME, null);
      hCmd.addParam(tool);
  ...
  /**
   * Inner class CommandIncomingFilter Keep agents and MTPs information in synch among replicas
   */
  private class CommandIncomingFilter extends Filter {
    ...
    private void handleInformCreated(VerticalCommand cmd) throws IMTPException, NotFoundException,
        NameClashException, JADESecurityException, ServiceException {
      ...
      if (!(ret != null && ret instanceof NameClashException)) {
        ...
        GenericCommand hCmd =
            new GenericCommand(MainReplicationSlice.H_BORNAGENT, MainReplicationSlice.NAME, null);
    ...
    private void handleInformKilled(VerticalCommand cmd)
      ...
      GenericCommand hCmd =
          new GenericCommand(MainReplicationSlice.H_DEADAGENT, MainReplicationSlice.NAME, null);
      hCmd.addParam(agentID);
    ...
    private void handleInformStateChanged(VerticalCommand cmd)
      ...
      if (newState.equals(jade.domain.FIPAAgentManagement.AMSAgentDescription.SUSPENDED)) {
        ...
        GenericCommand hCmd = new GenericCommand(MainReplicationSlice.H_RESUMEDAGENT,
            MainReplicationSlice.NAME, null);
        hCmd.addParam(agentID);
    ...
    private void handleNewMTP(VerticalCommand cmd) throws IMTPException, ServiceException {
      ...
      GenericCommand hCmd =
          new GenericCommand(MainReplicationSlice.H_NEWMTP, MainReplicationSlice.NAME, null);
      hCmd.addParam(mtp);
      hCmd.addParam(cid);
    ...
    private void handleDeadMTP(VerticalCommand cmd) throws IMTPException, ServiceException {
      ...
      GenericCommand hCmd =
          new GenericCommand(MainReplicationSlice.H_DEADMTP, MainReplicationSlice.NAME, null);
      hCmd.addParam(mtp);
      hCmd.addParam(cid);
  ...
  private class ServiceComponent implements Service.Slice, NodeEventListener {
      ...
      if (nodeMonitor != null) {
        if (myLogger.isLoggable(Logger.CONFIG))
          myLogger.log(Logger.CONFIG,
              "Stop monitoring node <" + nodeMonitor.getNode().getName() + ">");
        nodeMonitor.stop();
      }
    }

    private void attachTo(int label, MainReplicationSlice slice)
    ...
    private void invokeServiceMethod(String serviceName, String methodName, Object[] args)
        throws Throwable {
      ...
      try {
        myLogger.log(Logger.INFO,
            "Invoking replicated method " + methodName + " on service " + serviceName);
        m.invoke(svc, args);
    ...
    private Method getMethod(Service svc, String methodName) throws Exception {
      ...
      if (m == null) {
        throw new NoSuchMethodException(
            "Method " + methodName + " not found is service " + svc.getName());
      }
      return m;
    }
    ...
    private void addReplica(String sliceName, String smAddr, int sliceIndex, NodeDescriptor dsc,
        Vector services) throws IMTPException, ServiceException {
      ...
      if (myLabel == 0) {
        ...
        try {
          ...
          myLogger.log(Logger.WARNING,
              "Unauthorized Main Container node " + dsc.getNode().getName(), jse);
        }

      }
      myLogger.log(Logger.INFO, "Main container ring re-arranged: label = " + myLabel
          + " monitored label = " + monitoredLabel);
    }
    ...
    private void bornAgent(AID name, ContainerID cid, JADEPrincipal principal,
        Credentials credentials) throws NameClashException, NotFoundException {
    ...
    public void nodeRemoved(Node n) {
      ...
      switch (action) {
        ...
        case WAIT:
          myLogger.log(Logger.WARNING,
              "Network problems are preventing the monitoring of main node <" + n.getName()
                  + ">. Stop monitor it");
          toBeMonitored = n;
          break;
        case SUICIDE:
    ...
    private void removeTerminatedNode(Node n) {
      ...
      try {
        ...
        if ((oldLabel != 0) && (myLabel == 0)) {
          ...
          VerticalCommand cmd =
              new GenericCommand(MainReplicationSlice.LEADERSHIP_ACQUIRED, NAME, null);
          submit(cmd);
        } else {
          if (feeder != null) {
  ...
  void broadcastToReplicas(HorizontalCommand cmd, boolean includeSelf)
      throws IMTPException, ServiceException {
    Object[] slices = replicas.toArray();
      ...
      if (includeSelf || !sliceName.equals(localNodeName)) {
        ...
        if (ret instanceof Throwable) {
          ...
          myLogger.log(Logger.SEVERE,
              "Error propagating H-command " + cmd.getName() + " to slice " + sliceName,
              (Throwable) ret);
        }
      }
    }
  ...
  public String dump(String key) {
    ...
    sb.append("- Monitored PlatformManager replica = ").append(localSlice.monitoredSvcMgr)
        .append("\n");
