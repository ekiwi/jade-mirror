...
/**
 * This class is a concrete implementation of the JADE main container, providing runtime support to
 * JADE agents, and the special, front end container where the AMS and the Default DF can run. This
 * class cannot be instantiated from applications. Instead, the
...
public class MainContainerImpl implements MainContainer, AgentManager {
  ...
  /**
   * Profile option that specifies the classes of agents that must be restarted in case of fault of
   * the master Main Contaier
   */
  private static final String REPLICATED_AGENTS = "jade_core_MainContainerImpl_replicatedagents";

  /**
   * Profile option that specifies whether or not child nodes such as BackEnd containers managed by
   * the BEManagementService must be considered when shutting down the platform. Considering that
   * there can be thousands of such nodes, explicitly killing them all may lead to very long shut
   * down time. Possible values: true, false (default)
   */
  private static final String IGNORE_CHILD_NODES_ON_SHUTDOWN =
      "jade_core_MainContainerImpl_ignorechildnodes";
  ...
  /**
   * Profile option that specifies the implementation class of the DF agent. Such class must extend
   * the jade.domain.df standard class.
   */
  ...
  public MainContainerImpl(Profile p, PlatformManagerImpl pm) throws ProfileException {
    ...
    ignoreChildNodesOnShutdown =
        "true".equals(p.getParameter(IGNORE_CHILD_NODES_ON_SHUTDOWN, "false"));
  ...
  /**
   * Store the AID of a newly born agent if it has started on a remote Main Container and its class
   * is one of those to be replicated
   */
   ...
   */
  public void bornAgent(AID aid, ContainerID cid, JADEPrincipal principal, String ownership,
        ...
        if (forceReplacement) {
          ...
          if (!old.getDescription().getState().equals(AMSAgentDescription.LATENT)) {
            ...
            throw new NameClashException(
                "Agent " + aid.getName() + " already present in the platform ");
          }
        }
      } else {
   ...
   */
  public void movedAgent(AID agentID, ContainerID srcID, ContainerID destID)
   ...
   * 
   * @see AgentManager#create(String agentName, String className, String arguments[], ContainerID
   *      cid, String ownership, CertificateFolder certs) throws UnreachableException,
   *      JADESecurityException, NotFoundException
   */
  public void create(String name, String className, Object args[], ContainerID cid,
      JADEPrincipal owner, Credentials initialCredentials, JADEPrincipal requesterPrincipal,
    ...
    GenericCommand cmd =
        new GenericCommand(jade.core.management.AgentManagementSlice.REQUEST_CREATE,
            jade.core.management.AgentManagementSlice.NAME, null);
   ...
   */
  public void suspend(final AID agentID)
      throws NotFoundException, UnreachableException, JADESecurityException {
    GenericCommand cmd =
        new GenericCommand(jade.core.management.AgentManagementSlice.REQUEST_STATE_CHANGE,
            jade.core.management.AgentManagementSlice.NAME, null);
    cmd.addParam(agentID);
   ...
   */
  public void activate(final AID agentID)
      throws NotFoundException, UnreachableException, JADESecurityException {
    GenericCommand cmd =
        new GenericCommand(jade.core.management.AgentManagementSlice.REQUEST_STATE_CHANGE,
            jade.core.management.AgentManagementSlice.NAME, null);
   ...
   */
  public void killContainer(ContainerID cid, JADEPrincipal requesterPrincipal,
      Credentials requesterCredentials)
      throws NotFoundException, UnreachableException, JADESecurityException {
    GenericCommand cmd =
        new GenericCommand(jade.core.management.AgentManagementSlice.KILL_CONTAINER,
            jade.core.management.AgentManagementSlice.NAME, null);
    cmd.addParam(cid);
    cmd.setPrincipal(requesterPrincipal);
    cmd.setCredentials(requesterCredentials);
  ...
  public void shutdownPlatform(JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
    ...
    GenericCommand cmd =
        new GenericCommand(jade.core.management.AgentManagementSlice.SHUTDOWN_PLATFORM,
            jade.core.management.AgentManagementSlice.NAME, null);
    cmd.setPrincipal(requesterPrincipal);
    cmd.setCredentials(requesterCredentials);
    Object ret = myCommandProcessor.processOutgoing(cmd);
      ...
      for (int i = 0; i < allContainers.length; i++) {
        ...
        if (dsc != null) {
          if (dsc.getParentNode() != null) {
            shutdownContainer(targetID, "Child-Container", requesterPrincipal,
    ...
    try {
      ...
      if (verboseShutdown) {
        myLogger.log(Logger.INFO,
            "Local node " + localContainerID.getName() + " does not exist. Ignoring...");
      }
  ...
  private void shutdownContainer(ContainerID targetID, String type,
      JADEPrincipal requesterPrincipal, Credentials requesterCredentials)
      throws JADESecurityException {
    try {
      if (verboseShutdown) {
      ...
      if (removed) {
        ...
        myLogger.log(Logger.WARNING, type + " " + targetID.getName()
            + " did not terminate when requested to do so. Ignoring...");
      }
    } catch (NotFoundException nfe) {
      // Ignore the exception as we are removing a non-existing container
      if (verboseShutdown) {
      ...
      myLogger.log(Logger.WARNING,
          type + " " + targetID.getName() + " unreachable [" + ue + "]. Ignoring...");
    } catch (JADESecurityException se) {
      // Let it through
      throw se;
    } catch (Throwable t) {
  ...
  public MTPDescriptor installMTP(String address, ContainerID cid, String className)
    ...
    /***
     * System.out.println("--- New MTP ---"); System.out.println("Name: " + dsc.getName());
     * System.out.println("Addresses: "); for(int i = 0; i < dsc.getAddresses().length; i++) {
     * System.out.println("[" + dsc.getAddresses()[i] + "]"); } System.out.println("Supported
     * Protocols: "); for(int i = 0; i < dsc.getSupportedProtocols().length; i++) {
   ...
   */
  public void sniffOn(AID snifferName, List toBeSniffed)
      throws NotFoundException, UnreachableException {
   ...
   */
  public void sniffOff(AID snifferName, List notToBeSniffed)
      throws NotFoundException, UnreachableException {
   ...
   */
  public void debugOn(AID debuggerName, List toBeDebugged)
      throws NotFoundException, UnreachableException {
   ...
   */
  public void debugOff(AID debuggerName, List notToBeDebugged)
      throws NotFoundException, UnreachableException {
  ...
  /**
   * Modify the registration of an agent to the White Pages service of this platform. If the
   * modification implies a change in the agent state (and the agent lives in the platform) -->
   * force that change If the modification implies a change in the agent ownership (and the agent
