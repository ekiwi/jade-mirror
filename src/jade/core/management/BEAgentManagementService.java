...
/**
 * 
 * The JADE service to manage the basic agent life cycle: creation, destruction, suspension and
...
public class BEAgentManagementService extends BaseService {
  ...
  private class CommandSourceSink implements Sink {
    ...
    private void handleInformCreated(VerticalCommand cmd) throws IMTPException, NotFoundException,
      ...
      if ((params.length > 1) && (params[1] instanceof Agent)) {
        ...
        BackEndContainer.AgentImage image =
            (BackEndContainer.AgentImage) pendingImages.remove(agentID);
        if (image == null) {
          // The agent spontaneously born on the FrontEnd --> its image still has to be
          // created
    ...
    private void handleInformKilled(VerticalCommand cmd)
      ...
      BackEndContainer.AgentImage image =
          (BackEndContainer.AgentImage) myContainer.removeAgentImage(target);
    }
    ...
    /*
     * private void createAgentOnBE(AID target, Agent instance, VerticalCommand cmd) throws
     * IMTPException, JADESecurityException, NameClashException, NotFoundException, ServiceException
     * { // Connect the new instance to the local container Agent old =
     * myContainer.addLocalAgent(target, instance);
     * 
     * try { // Notify the main container through its slice AgentManagementSlice mainSlice =
     * (AgentManagementSlice)getSlice(MAIN_SLICE);
     * 
     * try { mainSlice.bornAgent(target, myContainer.getID(), cmd); } catch(IMTPException imtpe) {
     * // Try to get a newer slice and repeat... mainSlice =
     * (AgentManagementSlice)getFreshSlice(MAIN_SLICE); mainSlice.bornAgent(target,
     * myContainer.getID(), cmd); } } catch(NameClashException nce) {
     * myContainer.removeLocalAgent(target); if(old != null) { myContainer.addLocalAgent(target,
     * old); } throw nce; } catch(IMTPException imtpe) { myContainer.removeLocalAgent(target); throw
     * imtpe; } catch(NotFoundException nfe) { myContainer.removeLocalAgent(target); throw nfe; }
     * catch(JADESecurityException ae) { myContainer.removeLocalAgent(target); throw ae; } }
     */
  ...
  private class CommandTargetSink implements Sink {
    ...
    private void handleRequestCreate(VerticalCommand cmd) throws IMTPException,
        JADESecurityException, NotFoundException, NameClashException, ServiceException {
    ...
    /**
     * Force the creation of an agent on the FrontEnd. Note that the agent to create can have a
     * different owner with respect to the owner of this "container" --> Its image holding the
     * agent's ownership information must be created now and not in the bornAgent() method. This
     * image is stored in the pendingImages map for later retrieval (see bornAgent()).
     */
    private void createAgent(AID agentID, String className, Object[] args, JADEPrincipal ownership,
        Credentials creds) throws IMTPException {
      ...
      BackEndContainer.AgentImage previous =
          (BackEndContainer.AgentImage) pendingImages.put(agentID, image);
    ...
    private void changeAgentState(AID agentID, int newState)
     ...
     * 
     * // "Kill" all agent images AID[] targets = myContainer.getAgentImages(); for(int i = 0; i <
     * targets.length; i++) { AID target = targets[i]; try { // Remove the dead agent from the agent
     * images BackEndContainer.AgentImage image = myContainer.removeAgentImage(target);
     * 
     * // Notify the main container through its slice AgentManagementSlice mainSlice =
     * (AgentManagementSlice)getSlice(MAIN_SLICE);
     * 
     * try { mainSlice.deadAgent(target); } catch(IMTPException imtpe) { // Try to get a newer slice
     * and repeat... mainSlice = (AgentManagementSlice)getFreshSlice(MAIN_SLICE);
     * mainSlice.deadAgent(target); } } catch (Exception ex) { ex.printStackTrace(); } }
     * 
     * myContainer.shutDown(); }
     */
  ...
  /**
   * Inner mix-in class for this service: this class receives commands from the service
   * <code>Sink</code> and serves them, coordinating with remote parts of this service through the
