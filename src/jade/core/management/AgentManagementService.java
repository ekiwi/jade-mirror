...
public class AgentManagementService extends BaseService {
  ...
  private class CommandSourceSink implements Sink {
    ...
    private void handleRequestCreate(VerticalCommand cmd) throws IMTPException,
      ...
      if (myLogger.isLoggable(Logger.CONFIG))
        myLogger.log(Logger.CONFIG,
            "Source Sink consuming command REQUEST_CREATE. Name is " + name);
    ...
    private void handleInformCreated(VerticalCommand cmd) throws IMTPException, NotFoundException,
      ...
      if (myLogger.isLoggable(Logger.CONFIG)) {
        ...
        myLogger.log(Logger.CONFIG, "Source Sink consuming command INFORM_CREATED. Name is "
            + target.getName() + ownerInfo);
      }
  ...
  private class CommandTargetSink implements Sink {
    ...
    private void handleRequestCreate(VerticalCommand cmd) throws IMTPException,
      ...
      if (myLogger.isLoggable(Logger.FINE)) {
        String ownerInfo = owner != null ? ", Owner = " + owner : "";
        myLogger.log(Logger.FINE, "Target sink consuming command REQUEST_CREATE: Name is "
    ...
    private void createAgent(AID agentID, String className, Object arguments[], JADEPrincipal owner,
      ...
      try {
        ...
        throw new IMTPException("Class " + className + " for agent " + agentID + " not found",
            cnfe);
      } catch (InstantiationException ie) {
        throw new IMTPException(
            "Class " + className + " for agent " + agentID + " cannot be instantiated", ie);
    ...
    private void bornAgent(AID name, ContainerID cid, JADEPrincipal principal,
      ...
      if (impl != null) {
        ...
        try {
          ...
          /*
           * #CUSTOMJ2SE_INCLUDE_BEGIN try {
           * //System.out.println("Replacing old agent "+name.getName());
           * if(myLogger.isLoggable(Logger.INFO))
           * myLogger.log(Logger.INFO,"Replacing old agent "+name.getName()); dyingAgents.add(name);
           * ((jade.core.AgentManager) impl).kill(name, principal, credentials);
           * waitUntilDead(name); impl.bornAgent(name, cid, principal, ownership, false); } catch
           * (Exception e) { dyingAgents.remove(name); impl.bornAgent(name, cid, principal,
           * ownership, true); } #CUSTOMJ2SE_INCLUDE_END
           */
        }
      }
    }
    ...
    /*
     * #CUSTOMJ2SE_INCLUDE_BEGIN private jade.util.leap.List dyingAgents = new
     * jade.util.leap.ArrayList();
     * 
     * private void waitUntilDead(AID id) { synchronized (dyingAgents) { while
     * (dyingAgents.contains(id)) { try { dyingAgents.wait(); } catch (Exception e) {} } } }
     * 
     * private void notifyDead(AID id) { synchronized (dyingAgents) { dyingAgents.remove(id);
     * dyingAgents.notifyAll(); } } #CUSTOMJ2SE_INCLUDE_END
     */
    ...
    private void deadAgent(AID name) throws NotFoundException {
      ...
      if (impl != null) {
        ...
        /*
         * #CUSTOMJ2SE_INCLUDE_BEGIN notifyDead(name); #CUSTOMJ2SE_INCLUDE_END
         */
      }
    }
  ...
  private void initAgent(AID target, Agent instance, VerticalCommand vCmd) throws IMTPException,
      JADESecurityException, NameClashException, NotFoundException, ServiceException {
    // #J2ME_EXCLUDE_BEGIN
