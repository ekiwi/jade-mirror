...
/**
 * This ClassLoader is intended to be used to load agent classes packed within a jar file. If the
 * specified jar does not exist this ClassLoader will attempt to load classes from the system
 * ClassLoader.
 * 
...
public class JarClassLoader extends ClassLoader {
  ...
  public void close() {
    ...
    // _jarFile.close();
    // } catch (IOException ioe) {
    // System.out.println("JarClassLoader: Error closing Jar file: " +
    // _jarFile.getName());
    // }
  ...
  protected Class findClass(String className) throws ClassNotFoundException {
    ...
    if (is != null) {
      ...
      throw new ClassNotFoundException(
          "Class " + className + " not found in JAR file: " + getJarFileName());
    }
  }

  /*
   * protected URL findResource(String name) { try { String fileUrl = _file.toURL().toString();
   * System.out.println("%%%%%%%%%%%% FILE URL is "+fileUrl); return new
   * URL("jar:"+fileUrl+"!/"+name); } catch (MalformedURLException mue) { // Should never happen
   * mue.printStackTrace(); return null; } }
   */

  /**
   * Get a resource from within this classloader.
   *
   * public InputStream getResourceAsStream(String name) { if (_jarFile != null) {
   * 
   * ZipEntry zEntry = _jarFile.getEntry(name);
   * 
   * try { if (zEntry != null) { return _jarFile.getInputStream(zEntry); } else return null; } catch
  ...
  /**
   * Clean up the JarClassLoader. This means closing the JAR file if it has not explicitly done
   * before by using the provided close() method.
   */
