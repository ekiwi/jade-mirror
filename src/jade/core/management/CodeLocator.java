 ...
 * 
 * The access to the methods should be made inside a synchronized block against the CodeLocator
 * instance.
 * 
...
public class CodeLocator {
   ...
   * @param localCode Indicates the code is local (i.e. has not get to by any migration service) and
   *        cannot be modified.
   * @throws Exception
   */
  ...
  public synchronized void cloneAgent(AID oldName, AID newName) {
    ...
    if (_agents.containsKey(oldName)) {
      ...
      while (subs.hasMoreElements()) {
        clNewTemp =
            ((CodeLocatorListener) subs.nextElement()).handleCloneAgent(oldName, newName, cl);
        if (clNewTemp != null)
          clNew = clNewTemp;
      }
   ...
   * 
   * @param name Agent name.
   * @return True - The agent is registered. False - The agent is not registered.
   */
   ...
   * 
   * @param cle Class with a method per event which should be overloaded by users.
   */
