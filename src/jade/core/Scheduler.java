 ...
 * 
 * + Manages the resources needed to synchronize and execute agent behaviours, such as thread pools,
 * locks, etc.
 * 
 ****************************************************************/
class Scheduler implements Serializable {
  ...
  // #MIDP_EXCLUDE_END
  /*
   * #MIDP_INCLUDE_BEGIN protected Vector readyBehaviours = new Vector(); protected Vector
   * blockedBehaviours = new Vector(); #MIDP_INCLUDE_END
   */
  ...
  public synchronized void add(Behaviour b) {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN readyBehaviours.addElement(b); #MIDP_INCLUDE_END
     */
    notify();
    // #MIDP_EXCLUDE_BEGIN
  ...
  public synchronized void block(Behaviour b) {
    if (removeFromReady(b)) {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN blockedBehaviours.addElement(b); #MIDP_INCLUDE_END
       */
      // #MIDP_EXCLUDE_BEGIN
  ...
  /**
   * Restarts all behaviours. This method simply calls Behaviour.restart() on every behaviour. The
   * Behaviour.restart() method then notifies the agent (with the Agent.notifyRestarted() method),
   * causing Scheduler.restart() to be called (this also moves behaviours from the blocked queue to
   * the ready queue --> we must copy all behaviours into a temporary buffer to avoid concurrent
   * modification exceptions). Why not restarting only blocked behaviours? Some ready behaviour can
   * be a ParallelBehaviour with some of its children blocked. These children must be restarted too.
   */
  public synchronized void restartAll() {
    ...
    for (Iterator it = readyBehaviours.iterator(); it.hasNext();)
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN for(Iterator it = new EnumIterator(readyBehaviours.elements());
       * it.hasNext();) #MIDP_INCLUDE_END
       */
    ...
    for (Iterator it = blockedBehaviours.iterator(); it.hasNext();) {
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN for(Iterator it = new EnumIterator(blockedBehaviours.elements());
       * it.hasNext();) { #MIDP_INCLUDE_END
       */
      ...
      // #DOTNET_EXCLUDE_END
      /*
       * #DOTNET_INCLUDE_BEGIN Object tmpB = null; try // Hack: sometimes .NET inserts into this
       * array a non-Behaviour object { tmpB = it.next(); behaviours[counter++] = (Behaviour)tmpB; }
       * catch(ClassCastException cce) {
       * System.out.println("Found an object of type "+tmpB.getClass().getName()
       * +" instead of Behaviour"); cce.printStackTrace(); } #DOTNET_INCLUDE_END
       */
    }
    ...
    for (int i = 0; i < behaviours.length; i++) {
      Behaviour b = behaviours[i];
      /*
       * #DOTNET_INCLUDE_BEGIN if (b != null) #DOTNET_INCLUDE_END
       */
      b.restart();
  ...
  /**
   * Selects the appropriate behaviour for execution, with a trivial round-robin algorithm.
   */
  public synchronized Behaviour schedule() throws InterruptedException {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN Behaviour b = (Behaviour)readyBehaviours.elementAt(currentIndex);
     * #MIDP_INCLUDE_END
     */
  ...
  private boolean removeFromBlocked(Behaviour b) {
    ...
    // #MIDP_EXCLUDE_END
    /*
     * #MIDP_INCLUDE_BEGIN return blockedBehaviours.removeElement(b); #MIDP_INCLUDE_END
     */
  }
  ...
  private boolean removeFromReady(Behaviour b) {
    ...
    if (index != -1) {
      ...
      // #MIDP_EXCLUDE_END
      /*
       * #MIDP_INCLUDE_BEGIN readyBehaviours.removeElement(b); #MIDP_INCLUDE_END
       */
