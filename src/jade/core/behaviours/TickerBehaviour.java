...
/**
 * This abstract class implements a <code>Behaviour</code> that periodically executes a user-defined
 * piece of code. The user is expected to extend this class re-defining the method
 * <code>onTick()</code> and including the piece of code that must be periodically executed into it.
 * 
...
public abstract class TickerBehaviour extends SimpleBehaviour {
  ...
  /**
   * Construct a <code>TickerBehaviour</code> that call its <code>onTick()</code> method every
   * <code>period</code> ms.
   * 
  ...
  /**
   * This method is invoked periodically with the period defined in the constructor. Subclasses are
   * expected to define this method specifying the action that must be performed at every tick.
   */
  protected abstract void onTick();

  /**
   * Turn on/off the "fixed period" mode. Given a period P, when fixed period mode is off (default),
   * this behaviour will wait for P milliseconds from the end of the n-th execution of the onTick()
   * method to the beginning of the n+1-th execution. When fixed period is on, this behaviour will
   * execute the onTick() method exactly every P milliseconds.
   * 
   * @param fixedPeriod A boolean value indicating whether the fixed period mode must be turned on
   *        or off.
   */
  ...
  /**
   * Make this <code>TickerBehaviour</code> terminate. Calling stop() has the same effect as
   * removing this TickerBehaviour, but is Thread safe
   */
  public void stop() {
    finished = true;
    restart();
  ...
  /**
   * Retrieve how many ticks were done (i.e. how many times this behaviour was executed) since the
   * last reset.
   * 
