...
/**
 * Base class for all composite behaviour whose children run serially, i.e. the composite behaviour
 * is blocked if and only if its current child is blocked.
 * 
...
public abstract class SerialBehaviour extends CompositeBehaviour {
  ...
  /**
   * Create a new <code>SerialBehaviour</code> object, without setting the owner agent.
   */
  ...
  /**
   * Handle block/restart notifications. A <code>SerialBehaviour</code> is blocked <em>only</em>
   * when its currently active child is blocked, and becomes ready again when its current child is
   * ready. This method takes care of the various possibilities.
   * 
