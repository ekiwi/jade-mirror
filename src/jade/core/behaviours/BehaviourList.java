...
class BehaviourList extends LinkedList implements Serializable {
  ...
  /**
   * Add a Behaviour to the tail of the list. This does not require adjusting the current index
   */
  ...
  /**
   * Remove b from the list. If b was in the list, return true otherwise return false. This requires
   * adjusting the current index in the following cases: - the index of the removed Behaviour is <
   * than the current index - the removed Behaviour is the current one and it is also the last
   * element of the list. In this case the current index must be set to 0
   */
  ...
  /**
   * Advance the current index (taking into account wrap around) and return the new current
   * Behaviour
   */
