...
/**
 * Composite behaviour with Finite State Machine based children scheduling. It is a
 * <code>CompositeBehaviour</code> that executes its children behaviours according to a FSM defined
 * by the user. More specifically each child represents a state in the FSM. The class provides
 * methods to register states (sub-behaviours) and transitions that defines how sub-behaviours will
 * be scheduled.
 * <p>
 * At a minimum, the following steps are needed in order to properly define a
 * <code>FSMBehaviour</code>:
 * <ul>
 * <li>register a single Behaviour as the initial state of the FSM by calling the method
 * <code>registerFirstState</code>;
 * <li>register one or more Behaviours as the final states of the FSM by calling the method
 * <code>registerLastState</code>;
 * <li>register one or more Behaviours as the intermediate states of the FSM by calling the method
 * <code>registerState</code>;
 * <li>for each state of the FSM, register the transitions to the other states by calling the method
 * <code>registerTransition</code>;
 * <li>the method <code>registerDefaultTransition</code> is also useful in order to register a
 * default transition from a state to another state independently on the termination event of the
 * source state.
 * </ul>
 * A number of other methods are available in this class for generic tasks, such as getting the
 * current state or the name of a state, ...
 * 
...
public class FSMBehaviour extends SerialBehaviour {
  ...
  /**
   * Register a <code>Behaviour</code> as a state of this <code>FSMBehaviour</code>. When the FSM
   * reaches this state the registered <code>Behaviour</code> will be executed.
   * 
  ...
  /**
   * Register a <code>Behaviour</code> as the initial state of this <code>FSMBehaviour</code>.
   * 
   * @param state The <code>Behaviour</code> representing the state
   * @param name The name identifying the state.
   */
  ...
  /**
   * Register a <code>Behaviour</code> as a final state of this <code>FSMBehaviour</code>. When the
   * FSM reaches this state the registered <code>Behaviour</code> will be executed and, when
   * completed, the <code>FSMBehaviour</code> will terminate too.
   * 
   * @param state The <code>Behaviour</code> representing the state
   * @param name The name identifying the state.
   */
   ...
   * @param event The termination event that fires this transition as returned by the
   *        <code>onEnd()</code> method of the <code>Behaviour</code> representing state s1.
  ...
  /**
   * Register a transition in the FSM defining the policy for children scheduling of this
   * <code>FSMBehaviour</code>. When this transition is fired the states indicated in the
   * <code>toBeReset</code> parameter are reset. This is particularly useful for transitions that
   * lead to states that have already been visited.
   * 
   * @param s1 The name of the state this transition starts from
   * @param s2 The name of the state this transition leads to
   * @param event The termination event that fires this transition as returned by the
   *        <code>onEnd()</code> method of the <code>Behaviour</code> representing state s1.
   * @param toBeReset An array of strings including the names of the states to be reset.
   * @see jade.core.behaviours.Behaviour#onEnd()
   */
   ...
   * <code>FSMBehaviour</code>. This transition will be fired when state s1 terminates with an event
   * that is not explicitly associated to any transition.
   * 
   * @param s1 The name of the state this transition starts from
   * @param s2 The name of the state this transition leads to
   */
   ...
   * Register a default transition in the FSM defining the policy for children scheduling of this
   * <code>FSMBehaviour</code>. This transition will be fired when state s1 terminates with an event
   * that is not explicitly associated to any transition. When this transition is fired the states
   * indicated in the <code>toBeReset</code> parameter are reset. This is particularly useful for
   ...
   * 
   * @param source The name of the source state
   * @return <code>true</code> if a default transition exits from the given source state.
   ...
   * 
   * @return the <code>Behaviour</code> representing the state whose name is <code>name</code>, or
   ...
   * 
   * @return the name of the state represented by <code>Behaviour</code> state, or <code>null</code>
   *         if the given behaviour is not a child of this FSM behaviour.
   */
  ...
  /**
   * Retrieve the exit value of the most recently executed child. This is also the trigger value
   * that selects the next FSM transition.
   * 
  ...
  /**
   * Prepare the first child for execution. The first child is the <code>Behaviour</code> registered
   * as the first state of this <code>FSMBehaviour</code>
   * 
  ...
  /**
   * This method schedules the next child to be executed. It checks whether the current child is
   * completed and, in this case, fires a suitable transition (according to the termination event of
   * the current child) and schedules the child representing the new state.
   * 
   * @param currentDone a flag indicating whether the just executed child has completed or not.
   * @param currentResult the termination value (as returned by <code>onEnd()</code>) of the just
   *        executed child in the case this child has completed (otherwise this parameter is
   *        meaningless)
   ...
   * 
   * @return true when the last child has terminated and it represents a final state. false
   *         otherwise
  ...
  protected boolean checkTermination(boolean currentDone, int currentResult) {
    ...
    if (myLogger.isLoggable(Logger.FINE)) {
      myLogger.log(Logger.FINE,
          "FSM-Behaviour " + getBehaviourName() + ": checkTermination() returning " + ret);
    }
    // #J2ME_EXCLUDE_END
    return ret;
  }
  ...
  /**
   * Return a Collection view of the children of this <code>SequentialBehaviour</code>
   * 
  ...
  /**
   * Temporarily disregards the FSM structure, and jumps to the given state. This method acts as a
   * sort of <code>GOTO</code> statement between states, and replaces the currently active state
   * without considering the trigger event or whether a transition was registered. It should be used
   * only to handle exceptional conditions, if default transitions are not effective enough.
   * 
   * @param next The name of the state to jump to at the next FSM cheduling quantum. If the FSM has
   *        no state with the given name, this method does nothing.
   */
  ...
  /**
   * Reset the children behaviours registered in the states indicated in the <code>states</code>
   * parameter.
   * 
  ...
  static class Transition implements Serializable {
    ...
    public String toString() {
      ...
      sb.append("(TRANSITION trigger=").append(trigger).append(", source=").append(src)
