...
/**
 * An abstract superclass for behaviours composed by many parts. This class holds inside a number of
 * <b><em>children behaviours</em></b>. When a <code>CompositeBehaviour</code> receives it execution
 * quantum from the agent scheduler, it executes one of its children according to some policy. This
 * class must be extended to provide the actual scheduling policy to apply when running children
 * behaviours.
 * 
...
public abstract class CompositeBehaviour extends Behaviour {
   ...
   * scheduling policy defined by concrete subclasses that implements the
   * <code>scheduleFirst()</code> and <code>scheduleNext()</code> methods.
   */
   ...
   * 
   * @return <code>true</code> if this <code>CompositeBehaviour</code> has finished executing,
   ...
   * <code>CompositeBehaviour</code> should terminate.
   * 
   * @param currentDone a flag indicating whether the just executed child has completed or not.
   * @param currentResult the termination value (as returned by <code>onEnd()</code>) of the just
   *        executed child in the case this child has completed (otherwise this parameter is
   *        meaningless)
   * @return true if the <code>CompositeBehaviour</code> should terminate. false otherwise.
   */
  ...
  /**
   * This method returns a Collection view of the children of this <code>CompositeBehaviour</code>
   */
