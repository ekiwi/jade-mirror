...
/**
 * This abstract class implements a OneShot task that must be executed only one just after a given
 * timeout is elapsed.
 * <p>
 * The task is simply the call to the method <code>handleElapsedTimeout()</code> that must be
 * implemented by all subclasses. Notice that the best practice in JADE is when this method just
 * adds a behaviour to the agent class.
 * <p>
 * All subclasses have available the protected variable <code>myAgent</code> that points to the
 * agent class.
 * <p>
 * The programmer must use this abstract class in this simple way:
 * <ul>
 * <li>implements a subclass that extends WakerBehaviour. This subclass must implement the method
 * <code>handleElapsedTimeout</code>.
 * <li>add the subclass to the list of behaviour of this agent by using <code>addBehaviour()</code>
 * method.
 * <li>the method <code>handleElapsedTimeout</code> must implement the task that will be executed
...
public abstract class WakerBehaviour extends SimpleBehaviour {
   ...
   * 
   * @param a is the pointer to the agent
   * @param timeout indicates the number of milliseconds after which the task must be executed
   */
  ...
  /**
   * This method is invoked when the deadline defined in the constructor is reached (or when the
   * timeout specified in the constructor expires). Subclasses are expected to define this method
   * specifying the action that must be performed at that time.
   */
   ...
   * This method must be called to reset the behaviour and starts again
   * 
   * @param timeout indicates in how many milliseconds from now the behaviour must be waken up
   *        again.
   */
  ...
  /**
   * Make this WakerBehaviour terminate without calling the onWake() method. Calling stop() has the
   * same effect as removing this WakerBehaviour, but is Thread safe
   */
  public void stop() {
    finished = true;
    restart();
