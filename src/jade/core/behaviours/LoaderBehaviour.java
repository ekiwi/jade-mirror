 ...
 * When an agent runs an instance of this behaviour it becomes able to load and execute completely
 * new behaviours, i.e. behaviours whose code is not included in the classpath of the JVM where the
 * agent lives. Loading behaviour requests must have the <code>ACLMessage.REQUEST</code>
 * performative and must use the BehaviourLoading ontology and the LEAP language.
 * 
 * <br>
...
public class LoaderBehaviour extends Behaviour {
  ...
  /**
   * Construct a LoaderBehaviour to be executed by a given agent and that will use a given class
   * loader to load behaviours whose code is not embedded in the LoadBehaviour request.
   */
  ...
  /**
   * The done() method is redefined to make this behaviour terminate when its <code>stop()</code>
   * method is called.
   */
  public boolean done() {
    return finished;
  ...
  /**
   * Add a loaded behaviour to the agent. Subclasses may redefine this method to handle the
   * behaviour addition operation in an application specific way.
   * 
   * @param b The <code>Behaviour</code> to be added.
   * @param request The <code>ACLMessage</code> carrying the <code>LoadBehaviour</code> request.
   */
  ...
  /**
   * Suclasses may redefine this method to prevent the behaviour loading operation under specific
   * conditions. This default implementation always returns <code>true</code>
   */
  ...
  private Behaviour load(String className, Hashtable classes)
      throws ClassNotFoundException, InstantiationException, IllegalAccessException {
  ...
  private class HashClassLoader extends ClassLoader {
    ...
    /*
     * #PJAVA_INCLUDE_BEGIN In PersonalJava loadClass(String, boolean) is abstract --> we must
     * implement it protected Class loadClass(String name, boolean resolve) throws
     * ClassNotFoundException { // 1) Try to see if the class has already been loaded Class c =
     * findLoadedClass(name);
     * 
     * // 2) Try to load the class using the system class loader if(c == null) { try { c =
     * findSystemClass(name); } catch (ClassNotFoundException cnfe) { } }
     * 
     * // 3) If still not found, try to load the class from the code if(c == null) { c =
     * findClass(name); }
     * 
  ...
  /**
   * Inner class ResultCollector. This behaviour restores the output parameters at the end of the
   * execution of the loaded behaviour and sends back the result notification to the requester.
   */
