...
/**
 * This class provides support for executing JADE Behaviours in dedicated Java Threads. In order to
 * do that it is sufficient to add to an agent a normal JADE Behaviour "wrapped" into a "threaded
 * behaviour" as returned by the <code>wrap()</code> method of this class (see the example below).
 * 
 * <pr>
 * <hr>
 * <blockquote>
 ...
 * <hr>
 * 
 * This class also provides methods to control the termination of the threads dedicated to the
 * execution of wrapped behaviours
 * 
 * <br>
...
public class ThreadedBehaviourFactory {
  ...
  /**
   * Wraps a normal JADE Behaviour <code>b</code> into a "threaded behaviour". Adding the wrapper
   * behaviour to an agent results in executing <code>b</code> in a dedicated Java Therad.
   */
  ...
  /**
   * @return The number of active threads dedicated to the execution of wrapped behaviours.
   */
  public int size() {
   ...
   * 
   * @param timeout The maximum timeout to wait for threaded behaviour termination.
   * @return <code>true</code> if all threaded behaviour have actually completed, <code>false</code>
   *         otherwise.
   */
   ...
   * instead of getThread().interrupt() because i) the latter may have no effect if called just
   * after the threaded behaviour suspended itself and ii) the threaded behaviour may be suspended
   ...
   * itself and has the effect of releasing its dedicated Java Thread. This can later be restored by
   * means of the <code>resume()</code> method.
   */
  ...
  /**
   * Resume a threaded behaviour. Assign a new Java Thread to a threaded behaviour that is currently
   * suspended.
   */
  ...
  public class ThreadedBehaviourWrapper extends Behaviour implements Runnable {
    ...
    /**
     * Propagate the parent to the wrapped behaviour. NOTE that the <code>parent</code> member
     * variable of the wrapped behaviour must point to the DummyParentBehaviour --> From the wrapped
     * behaviour accessing the actual parent must always be retrieved through the getParent()
     * method.
     */
    ...
    /**
     * Propagate a restart() call (typically this happens when this ThreadedBehaviourWrapped is
     * directly added to the agent Scheduler and a message is received) to the wrapped threaded
     * behaviour.
     */
    public void restart() {
      myBehaviour.restart();
    ...
    /**
     * Propagate a DOWNWARDS event (typically this happens when this ThreadedBehaviourWrapper is
     * added as a child of a CompositeBehaviour and the latter, or an ancestor, is
     * blocked/restarted) to the wrapped threaded behaviour. If the event is a restart, also notify
    ...
    public void run() {
      ...
      try {
        ...
        myLogger.log(Logger.WARNING, "Threaded behaviour " + myBehaviour.getBehaviourName()
            + " interrupted before termination");
    ...
    /**
     * Interrupt a threaded behaviour. This method should be used instead of getThread().interrupt()
     * because the latter may have no effect if called just after the threaded behaviour suspended
     * itself
     */
  ...
  /**
   * Inner class DummyParentBehaviour. This class has the only purpose of propagating restart events
