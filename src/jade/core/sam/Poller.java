...
class Poller extends Thread {
  ...
  /**
   * This is the method that produces a new record for each monitored entity/counter. It is invoked
   * periodically by the poller Thread. At each invocation it retrieves the relevant information
   * from each SAM Service slice.
   */
  void poll() {
      ...
      for (int i = 0; i < slices.length; i++) {
        ...
        try {
          ...
          myLogger.log(Logger.FINER,
              "SAM information successfully retrieved from node " + s.getNode().getName());
        } catch (Exception imtpe) {
          // Note that getAllSlices() always retrieves "fresh" slices --> no need for any
          // retry
          myLogger.log(Logger.WARNING,
              "Error retrieving SAM information from node " + s.getNode().getName(), imtpe);
        } finally {
          stopWatchDog();
        }
      }
