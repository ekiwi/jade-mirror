...
/**
 * An instance of this class is passed to all configured <code>SAMInfoHandler<code>-s at each
 * polling time and groups together all information collected by the SAM Service at that polling
 * time.
 */
public class SAMInfo implements Serializable {
  ...
  /**
   * If there are entities/counters of the form a#b, a#c... produce an aggregated entity a. Since a
   * itself may have the form a1#a2, iterate until there are no more aggregations
   */
  void computeAggregatedValues() {
    // Aggregate measures
    Map<String, AverageMeasure> aggregatedMeasures =
        oneShotComputeAggregatedMeasures(entityMeasures);
  ...
  private static Map<String, AverageMeasure> oneShotComputeAggregatedMeasures(
      Map<String, AverageMeasure> measures) {
    ...
    for (String entityName : measures.keySet()) {
      ...
      if (k > 0) {
        // This is an "aggregated measure component" (aaa#bbb) --> accumulate component
        // contribution
  ...
  private static void addAllMeasures(Map<String, AverageMeasure> mm1,
      Map<String, AverageMeasure> mm2) {
  ...
  private static Map<String, Long> oneShotComputeAggregatedCounters(Map<String, Long> counters) {
    ...
    for (String counterName : counters.keySet()) {
      ...
      if (k > 0) {
        // This is an "aggregated counter component" (aaa#bbb) --> accumulate component
        // contribution
