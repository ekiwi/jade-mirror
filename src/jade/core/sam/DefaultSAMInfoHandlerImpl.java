...
class DefaultSAMInfoHandlerImpl implements SAMInfoHandler {
  ...
  // For counters we need to keep the total value together with the Stream used to
  // write the CSV file
  ...
  public void initialize(Profile p) throws Exception {
    ...
    if (!samInfoDirectory.exists()) {
      myLogger.log(Logger.CONFIG,
          "SAM csv directory " + samInfoDirectory + " does not exists. Creating it ...");
  ...
  public void handle(Date timeStamp, SAMInfo info) {
    ...
    for (String entityName : entityMeasures.keySet()) {
      ...
      try {
        ...
        if (stream == null) {
          // This is the first time we get a measure for this entity --> Initialize the
          // file
          ...
          stream
              .println("Time-stamp" + csvSeparator + "Average-value" + csvSeparator + "N-samples");
        ...
        // Likely someone removed the CSV file in the meanwhile. Reset everything so
        // that at next round the file will be re-created
    ...
    for (String counterName : counterValues.keySet()) {
      ...
      try {
        ...
        if (ci == null) {
          // This is the first time we get a value for this counter --> Initialize its csv
          // file
  ...
  /**
   * If the entity name has the form a/b/c then create file SAM_c.csv in sub-directory b of
   * directory a under the indicated csv-directory. Create directories if missing
   */
