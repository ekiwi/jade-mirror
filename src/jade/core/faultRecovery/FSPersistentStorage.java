...
/**
 * Default implementation of the PersistentStorage interface saving persistent information in the
 * local file system.
 * 
...
class FSPersistentStorage implements PersistentStorage {
  ...
  public void init(Profile p) throws Exception {
    ...
    if (!locationDir.exists()) {
      myLogger.log(Logger.CONFIG,
          "FSPS location directory " + locationName + " does not exists. Creating it ...");
    ...
    File[] ff = locationDir.listFiles(new FilenameFilter() {
      public boolean accept(File dir, String name) {
        ...
        return (name.endsWith(EXTENSION) || name.endsWith(CHILD_EXTENSION)
            || name.endsWith(UNREACHABLE_EXTENSION));
      }
    });
    // #DOTNET_EXCLUDE_END

    /*
     * #DOTNET_INCLUDE_BEGIN String[] ss = locationDir.list(new FilenameFilter() { public boolean
     * accept(File dir, String name) { if (!clearPlatformInfo &&
     * name.equals(PLATFORM_FILE_NAME+EXTENSION)) { return false; } return (name.endsWith(EXTENSION)
     * || name.endsWith(CHILD_EXTENSION) || name.endsWith(UNREACHABLE_EXTENSION)); } } );
     * 
     * File[] ff = new File[ss.length]; for(int i=0; i < ss.length; i++) { ff[i] = new File(
     * locationDir.getPath(), ss[i] ); } #DOTNET_INCLUDE_END
     */
  ...
  public void storePlatformInfo(String platformName, String address) throws Exception {
    ...
    if (myLogger.isLoggable(Logger.FINE)) {
      myLogger.log(Logger.FINE, "Platform information (name = " + platformName + ", address = "
          + address + " saved in persistent storage");
    }
  }
  ...
  public void storeNode(String name, boolean isChild, byte[] nn) throws Exception {
    ...
    // If the newly stored node was already in the persistent storage as an
    // unreachable node --> remove it
  ...
  public Map getAllNodes(boolean children) throws Exception {
    ...
    /*
     * #DOTNET_INCLUDE_BEGIN String[] ss = locationDir.list(new FilenameFilter() { public boolean
     * accept(File dir, String name) { return (name.endsWith(end)); } } );
     * 
     * File[] ff = new File[ss.length]; for(int i=0; i < ss.length; i++) { ff[i] = new File(
     * locationDir.getPath(), ss[i] ); } #DOTNET_INCLUDE_END
     */
  ...
  private File getUnreachableFile(File f) {
    String unreachableName =
        locationDir.getPath() + fileSeparator + f.getName() + UNREACHABLE_EXTENSION;
