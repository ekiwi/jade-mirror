...
/**
 * The FaultRecovery service allows recovering a platform after a fault and a successive restart of
 * the main container.
 * 
 * On a Main container this service keeps track of platform nodes in a persistent storage. When the
 * platform shuts down the persistent storage is cleared. At bootstrap time the service gets all
 * nodes from the persistent storage (there are nodes only if the main container is restarting after
 * a crash) and notifies them (if still alive) about the recovery.
 * 
 * On peripheral containers, the node, when notified about a main recovery, re-adds itself to the
 * recovered main (actually this is done by the ServiceManager) and issues a Service.REATTACHED
 * incoming V-Command. The FaultRecovery service filter intercepts this command and re-adds all
 * agents living in the container.
 * 
...
public class FaultRecoveryService extends BaseService {
  ...
  public static final String CLEAN_STORAGE =
      "jade_core_faultRecovery_FaultRecoveryService_cleanstorage";
  public static final String PERSISTENT_STORAGE_CLASS =
      "jade_core_faultRecovery_FaultRecoveryService_persistentstorage";
  public static final String ORPHAN_NODE_POLICY =
      "jade_core_faultRecovery_FaultRecoveryService_orphannodepolicy";

  public static final String PERSISTENT_STORAGE_CLASS_DEFAULT =
      "jade.core.faultRecovery.FSPersistentStorage";
  ...
  // Horizontal command to force the termination of an orphan node when the KILL
  // Orphan Node Policy is set
    ...
    if (myMain != null) {
      try {
        ...
        if (currentPlatformName.equals(oldPlatformName)) {
          MainReplicationService replService = (MainReplicationService) myContainer
              .getServiceFinder().findService(MainReplicationService.NAME);
          // Do not activate the fault recovery procedure if the Main Container is
          // replicated: in that case all containers were already adopted
          ...
          // This is a different platform and we don't want to reattach agents with a
  ...
  private void checkNode(String name, byte[] nn, String oldAddress, String currentAddress) {
    ...
    try {
      ...
      myLogger.log(Logger.INFO,
          "Node " + name + " unreachable. It has likely been killed in the meanwhile");
    } catch (Exception e) {
      myLogger.log(Logger.WARNING, "Error deserializing node " + name + ". ", e);
    }
    // If we get here the node either has been killed in the meanwhile or cannot be
    // deserialized -->
    ...
    try {
      ...
      myLogger.log(Logger.WARNING,
          "Cannot remove node " + node.getName() + " from persistent storage. ", ex);
    }
  }
  ...
  private void killNode(String name, byte[] nn) {
    ...
    try {
      ...
      myLogger.log(Logger.INFO, "Node " + name
          + " unreachable (it has likely been killed in the meanwhile) or does not support fault recovery.");
    } catch (Exception e) {
      myLogger.log(Logger.WARNING, "Error deserializing node " + name + ". ", e);
    }
  ...
  /**
   * Inner class MainCommandIncomingFilter. This filter is installed on a Main Container and
   * intercepts the NEW_NODE and DEAD_NODE V-Commands
   */
  ...
  /**
   * Inner class MainCommandOutgoingFilter. This filter is installed on a Main Container and
   * intercepts the NODE_UNREACHABLE and NODE_REACHABLE VCommands of the NodeMonitoringService and
  ...
  /**
   * Inner class ContainerCommandIncomingFilter. This filter is installed on a peripheral Container
   * and intercepts the REATTACHED V-Commands
   */
  ...
  private void handleOrphanNode(String nodeName) {
    if (bootComplete) {
      ...
      try {
        ...
        myLogger.log(Logger.WARNING,
            "Error searching for unreachable node " + nodeName + " in the persistent storage", e);
      }
    }
  }

  /**
   * The container reattached to a recovered Main. Inform the new Main about all local agents.
   */
    ...
    for (int i = 0; i < ids.length; ++i) {
      ...
      if (agent != null) {
        ...
        try {
          ...
          // Note also that we use agent.getAID() instead of id since the latter may have
