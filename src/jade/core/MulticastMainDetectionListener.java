...
class MulticastMainDetectionListener implements Runnable {
  ...
  private final static Logger logger =
      Logger.getMyLogger(MulticastMainDetectionListener.class.getName());
  ...
  public MulticastMainDetectionListener(ProfileImpl profile, IMTPManager manager)
  ...
  private String serveGetMain(String request) throws IMTPException {
    logger.log(Logger.FINER,
        "MulticastMainDetectionListener::serveGetMain(request=\"" + request + "\")");
    ...
    if (i > 0) {
      ...
      request = request.substring(0, i);
      logger.log(Logger.FINER,
          "MulticastMainDetectionListener::serveGetMain(): request is for platform \""
              + platformName + "\"");
    }
    ...
    if (platformName != null && !platformName.equals(myPlatform)) {
      ...
      logger.log(Logger.FINER,
          "MulticastMainDetectionListener::serveGetMain(): my platform is \"" + myPlatform
    ...
    if (responseAddresses.size() < 1) {
      response = errorResponse(MainDetectionManager.PROTO_RESP_NOTFOUND,
          "Cannot manage protocol " + proto);
    } else {
  ...
  private String executeRequest(String request) throws Exception {
    logger.log(Logger.FINER,
        "MulticastMainDetectionListener::executeRequest(request=\"" + request + "\")");
    String result = null;
  ...
  public void run() {
    ...
    try {
      ...
      while (true) {
        try {
          ...
          try {
            ...
            /*
             * when stop() is called, we get here with a SocketException: Socket closed and active
             * == false
             */
            if (active) {
              /*
          ...
          try {
            ...
            logger.log(Logger.WARNING,
                "MulticastMainDetectionListener::run(): error managing request", e);
          ...
          if (response != null) {
            ...
            logger.log(Logger.FINER, "MulticastMainDetectionListener::run(): sending response \""
                + response + "\" to " + clientAddr + ":" + responsePort);
            socket.send(packet);
          } else {
      ...
      logger.log(Logger.SEVERE,
          "Error in listener thread, MulticastMainDetectionListener is no more active", e);
    }
  }
