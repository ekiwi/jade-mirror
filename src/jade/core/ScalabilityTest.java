...
public class ScalabilityTest {
  ...
  // - Ready-go mode: All agents starts and, when they are all ready, the user is
  // prompted
  ...
  // - Step-by-step mode: Prompt the user after each sender-receiver couple
  // initialization
  ...
  // If specified makes agents begin the test scattered randomly within a time
  // interval.
  ...
  private static void initCouple(Properties basePP, String prefix, int index) {
    ...
    /*
     * Properties pp = new Properties(); if (host != null) { pp.setProperty(MicroRuntime.HOST_KEY,
     * host); } if (port != null) { pp.setProperty(MicroRuntime.PORT_KEY, port); } if (proto !=
     * null) { pp.setProperty(MicroRuntime.PROTO_KEY, proto); } if (maxDiscTime != null) {
    ...
    /*
     * pp = new Properties(); if (host != null) { pp.setProperty(MicroRuntime.HOST_KEY, host); } if
     * (port != null) { pp.setProperty(MicroRuntime.PORT_KEY, port); } if (proto != null) {
     * pp.setProperty(MicroRuntime.PROTO_KEY, proto); } if (maxDiscTime != null) {
     * pp.setProperty(JICPProtocol.MAX_DISCONNECTION_TIME_KEY, maxDiscTime); }
     */
  ...
  /**
   * Inner class BitrateSenderAgent Send messages to the corresponding receiver. When the receiver
   * gets message 0 takes the START time. When the receiver gets message N (number of iterations)
   * takes the END time and completes
   */
  public static class BitrateSenderAgent extends Agent {
    ...
    protected void setup() {
      ...
      System.out.println(
          "BitRate-Sender " + getName() + " ready: my receiver is " + myReceiver.getName());
      notifyReady();
          ...
          if (msg != null) {
            ...
            if (!terminated) {
              if (nIterations > 0 && cnt >= nIterations) {
                ...
                // Unless we are in step-by-step mode, take time interval between iterations
                // into account
                if (timeInterval != STEPBYSTEP_TIME_INTERVAL) {
  ...
  /**
   * Inner class RTTSenderAgent Send messages to the corresponding receiver. For each message waits
   * for the response and compute the round trip time.
   */
  public static class RTTSenderAgent extends Agent {
    ...
    protected void setup() {
      ...
      System.out
          .println("RTT-Sender " + getName() + " ready: my receiver is " + myReceiver.getName());
      notifyReady();
