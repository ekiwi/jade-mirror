...
public class ResourceManagementService extends BaseService {
  ...
  private class ServiceComponent implements Service.Slice {
    ...
    private byte[] getResource(String name, int fetchMode) throws IOException, NotFoundException {
      ...
      myLogger.log(Logger.WARNING,
          "Received getResource " + name + " with unsupported fetchMode " + fetchMode);
      throw new IllegalArgumentException(
          "getResource " + name + " with unsupported fetchMode " + fetchMode);
    }

    private byte[] getResourceFromShared(String name) throws IOException, NotFoundException {
      // Get shared folder
      String sharedFolder =
          System.getProperty(ResourceManagementHelper.SHARED_RESOURCES_FOLDER_KEY);
  ...
  private class ResourceManagementHelperImpl implements ResourceManagementHelper {
    ...
    public byte[] getResource(String name, int fetchMode)
        throws ServiceException, NotFoundException {
      return getResource(name, fetchMode, null);
    }

    public byte[] getResource(String name, String containerName)
        throws ServiceException, NotFoundException {
      return getResource(name, ResourceManagementHelper.ALL_RESOURCES, containerName);
    }

    public byte[] getResource(String name, int fetchMode, String containerName)
        throws ServiceException, NotFoundException {
      ...
      if (slice == null) {
        throw new ServiceException(
            "getResource called with a wrong container name (" + containerName + ")");
      }
      ...
      try {
        ...
        throw new ServiceException("Error accessing resource " + name + " from "
            + slice.getNode().getName() + " with fetch-mode " + fetchMode, t);
      }
