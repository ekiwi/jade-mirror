...
public class DynamicJadeGateway {
   ...
   **/
  public final void execute(Object command)
   ...
   * @see jade.wrapper.AgentController#putO2AObject(Object, boolean)
   **/
  public final void execute(Object command, long timeout)
      throws StaleProxyException, ControllerException, InterruptedException {
    Event e = null;
    synchronized (this) {
      checkJADE();
    ...
    if (myAgent == null) {
      try {
        ...
        if (a instanceof GatewayAgent) {
          ...
          // We are able to detect the GatewayAgent state only if the internal agent is a
          // GatewayAgent instance
        ...
        if (gatewayAgentState == NOT_ACTIVE) {
          // Set the ACTIVE state synchronously so that when checkJADE() completes
          // isGatewayActive() certainly returns true
   ...
   * 
   * @param agentName is the name of the JadeGateway internal agent. If null is passed the default
   *        name will be used.
  ...
  public final void init(String agentName, String agentClassName, Object[] agentArgs,
    ...
    if (jadeProps != null) {
      // Since we will create a non-main container --> force the "main" property to be
      // false
  ...
  final void initProfile() {
    // to initialize the profile every restart, otherwise an exception would be
    // thrown by JADE
  ...
  public boolean isGatewayActive() {
    if (gatewayAgentState != UNKNOWN) {
      ...
      // If we are not able to monitor the actual gatewayAgentState, just check if
      // myContainer and myAgent are not null
  ...
  class GatewayListenerImpl implements GatewayListener {
    public void handleGatewayConnected() {
      // This is executed by the GatewayAgent Thread --> Notify listeners by means of
      // an ad-hoc
      // Thread to avoid deadlocks with other threads waiting for the execute() method
      // to complete
      Thread t = new Thread() {
        public void run() {
