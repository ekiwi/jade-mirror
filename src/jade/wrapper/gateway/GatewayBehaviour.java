 ...
 * <code>JadeGateway</code> enables two alternative ways to implement a gateway that allows non-JADE
 * code to communicate with JADE agents. <br>
 * The first one is to extend the <code>GatewayAgent</code> (see its javadoc for reference). <br>
 * The second one is to extend this <code>GatewayBehaviour</code> and add an instance of this
 * Behaviour to your own agent that will have to function as a gateway.
 * 
...
public abstract class GatewayBehaviour extends CyclicBehaviour {
  ...
  /**
   * Queue of all pending commands that have not yet been released (see method releaseCommand) In
   * this Hash, the key is the Object while the value is the Event
   */
  ...
  public void action() {
    ...
    if (myLogger.isLoggable(Logger.INFO)) {
      myLogger.log(Logger.INFO,
          myAgent.getLocalName() + " started execution of command " + e.getSource());
    }
  ...
  /**
   * subclasses must implement this method. The method is called each time a request to process a
   * command is received from the JSP Gateway.
   * <p>
   ...
   * <code>releaseCommand</code>. Sometimes, you might prefer launching a new Behaviour that
   * asynchronously processes this command and release the command just when the Behaviour
   * terminates, i.e. in its <code>onEnd()</code> method.
   **/
  ...
  final public void releaseCommand(Object command) {
    ...
    if (e != null) {
      if (myLogger.isLoggable(Logger.INFO)) {
        myLogger.log(Logger.INFO,
            myAgent.getLocalName() + " terminated execution of command " + command);
      }
