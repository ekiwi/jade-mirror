 ...
 * Behaviour to your own agent that will have to function as a gateway (see its javadoc for
 * reference).
 * 
 * @see JadeGateway
...
public class GatewayAgent extends Agent {
  ...
  /**
   * subclasses may implement this method. The method is called each time a request to process a
   ...
   * Sometimes, you might prefer launching a new Behaviour that processes this command and release
   * the command just when the Behaviour terminates, i.e. in its <code>onEnd()</code> method.
   **/
  ...
  /*
   * Those classes that extends this setup method of the GatewayAgent MUST absolutely call
   * <code>super.setup()</code> otherwise this method is not executed and the system would not work.
   * 
