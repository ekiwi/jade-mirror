...
public class SplitJadeGateway extends DynamicJadeGateway {
    ...
    MicroRuntime.startJADE(properties, new Runnable() {
      public void run() {
        myLogger.log(Logger.INFO,
            "Disconnected from the platform at " + properties.getProperty(Profile.MAIN_HOST) + ":"
                + properties.getProperty(Profile.MAIN_PORT));
      }
    });

    if (MicroRuntime.isRunning()) {
      myLogger.log(Logger.INFO,
          "Connected to the platform at " + properties.getProperty(Profile.MAIN_HOST) + ":"
              + properties.getProperty(Profile.MAIN_PORT));
      ...
      myLogger.log(Logger.WARNING,
          "Cannot connect to the platform at " + properties.getProperty(Profile.MAIN_HOST) + ":"
              + properties.getProperty(Profile.MAIN_PORT));
      shutdown();
    }
  }
  ...
  public boolean isGatewayActive() {
    if (gatewayAgentState != UNKNOWN) {
      ...
      // MicroRuntime is running and myAgent is not null
