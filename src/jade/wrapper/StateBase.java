...
/**
 * Provides an abstract base class for implementations of state. <br>
...
public abstract class StateBase implements State, Comparable, Serializable {
  ...
  /**
   * Creates a state object for a given code. To avoid coding errors, the integer codes used to
   * create instances of this class should come from the concrete extensions of this class.
   * <p>
  ...
  StateBase(int code, int aBaseCode, String[] names) {
    ...
    if ((code >= baseCode) && (code <= (baseCode + stateNames.length - 1))) {
      m_code = code;
    } else {
      throw new IllegalArgumentException("State code out of range(" + baseCode + "-"
  ...
  /**
   * Converts a descriptive state name into a state code. The name comparison is not case sensitive.
   * If the name cannot be located for any reason a {@link java.lang.IllegalArgumentException
   ...
   * @return The descriptive name of the state or null if the state code is "out of range" (i.e.
   *         invalid).
   */
  ...
  /**
   * Determines if an object is equal to this object. In order for the the passed object to be equal
   * to this one the passed object must be an instance if State and contain the same state code.
   * 
  ...
  /**
   * Compares a given object to this state object for the purpose of collating. This method is used
   * for the sorting of states. A positive value means the receiver (this object) is "greater than"
   * the passed object. A negative value means the receiver is "less than" the passed object. A
   * value of zero means the receiver and the passed object are equal to each other.
   *
   ...
   *         <ul>
   *         <li>the passed object is null (the receiver is considered "greater than" null
   *         objects)</li>
   *         <li>the state code held by the receiver (this object) is greater than the code of the
   *         passed object</li>
   *         </ul>
   *         <p>
   ...
   *         The value -1 is returned if:
   *         <ul>
   *         <li>the passed object is not a AgentStateImpl object (the receiver is considered "less
   *         than" objects of other types)</li>
   *         <li>the state code held by the receiver (this object) is less than the code of the
   *         passed object</li>
   *         <ul>
   */
  ...
  /**
   * Return a string containing all descriptions and their codes. Usefull for debugging.
   * 
