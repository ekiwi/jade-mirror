...
/**
 * Provides a concrete implementation of the State interface for agent platforms. <br>
...
public class PlatformState extends StateBase {
  ...
  /**
   * An array of string names, one for each of the valid state this object can represent. This array
   * MUST be initialized before the other static constructors.
   */
  static final String stateNames[] = {"Void", // 0
      "Initializing", // 1
      "Initialized", // 2
      "Starting", // 3
      "Ready", // 4
  ...
  /**
   * A constant that holds an object to represent the state FAILED. A component will be placed in
   * this state if a failure occurs during most of the transition states. These states are
   * INITIALIZING, STARTING, and SUSPENDING. If a failure occurs during the processing associated
   * with these transitional states the component will be placed into the FAILED state. A component
   * may then only be transitioned into a KILLING and then KILLED state at some later time. No other
   * state transitions are allowed from the FAILED state.
   */
